#!/usr/bin/env bash
# OpenClaw 2026.5.3-1 compatibility patches for the local plur1bus deployment.
#
# The 2026.4.29 hotfix patched broad tool-allowlist behavior that is now
# upstream in 2026.5.3-1. This script keeps only the local runtime fixes that
...
patch_openclaw_20260503_runtime() {
...
def backup(path):
    if path in backed:
        return
    bak = f"{path}.bak-openclaw-20260503-{stamp}"
...
try:
    ...
    pass
if pkg_version and pkg_version != "2026.5.3-1":
    print(f"[patch] OpenClaw 2026.5.3-1 compat: package version is {pkg_version}, skipping dist patch")
    raise SystemExit(0)
...
if os.path.exists(active_memory):
    replace_once(
        active_memory,
        "plur1bus-openclaw-20260503-active-empty-fallback",
...
\t\tif (raceResult === TIMEOUT_SENTINEL) {""",
    ...
    replace_once(
        active_memory,
        "plur1bus-openclaw-20260503-no-empty-cache",
        """function shouldCacheResult(result) {
\treturn result.status === "ok" || result.status === "empty";
}""",
        """function shouldCacheResult(result) {
\treturn result.status === "ok"; /* plur1bus-openclaw-20260503-no-empty-cache */
}""",
        "active-memory no empty-result cache",
    )
    replace_once(
        active_memory,
        "plur1bus-openclaw-20260503-embedded-timeout-cap",
        "\t\tconst embeddedTimeoutMs = params.config.timeoutMs + params.config.setupGraceTimeoutMs;",
        "\t\tconst embeddedTimeoutMs = params.config.timeoutMs; /* plur1bus-openclaw-20260503-embedded-timeout-cap */",
        "active-memory embedded timeout cap",
    )
    replace_once(
        active_memory,
        "plur1bus-openclaw-20260503-watchdog-cap",
        "\tconst watchdogTimeoutMs = params.config.timeoutMs + params.config.setupGraceTimeoutMs;",
        "\tconst watchdogTimeoutMs = params.config.timeoutMs; /* plur1bus-openclaw-20260503-watchdog-cap */",
        "active-memory watchdog cap",
    )
    replace_once(
        active_memory,
        "plur1bus-openclaw-20260503-hook-budget",
        "\t\tconst beforePromptBuildTimeoutMs = config.timeoutMs + config.setupGraceTimeoutMs;",
        "\t\tconst beforePromptBuildTimeoutMs = Math.min(config.timeoutMs, 3e3); /* plur1bus-openclaw-20260503-hook-budget */",
        "active-memory before_prompt_build budget",
    )
    replace_once(
        active_memory,
        "plur1bus-openclaw-20260503-active-memory-lane",
        "\t\t\tagentId: params.agentId,\n\t\t\tmessageChannel,",
        "\t\t\tagentId: params.agentId,\n\t\t\tlane: \"active-memory\", /* plur1bus-openclaw-20260503-active-memory-lane */\n\t\t\tmessageChannel,",
...
if subagent_announce:
    replace_once(
        subagent_announce,
        "plur1bus-openclaw-20260503-subagent-announce-timeout-cap",
        "const DEFAULT_SUBAGENT_ANNOUNCE_TIMEOUT_MS = 12e4;",
        "const DEFAULT_SUBAGENT_ANNOUNCE_TIMEOUT_MS = 3e4; /* plur1bus-openclaw-20260503-subagent-announce-timeout-cap */",
        "subagent announce timeout cap",
    )
    replace_once(
        subagent_announce,
        "plur1bus-openclaw-20260503-session-only-announce-short-wait",
        "\t\tlet directAnnounceResponse;\n\t\ttry {",
        "\t\tconst directAnnounceTimeoutMs = params.expectsCompletionMessage && !deliveryTarget.deliver ? Math.min(announceTimeoutMs, 15000) : announceTimeoutMs; /* plur1bus-openclaw-20260503-session-only-announce-short-wait */\n\t\tlet directAnnounceResponse;\n\t\ttry {",
        "subagent session-only completion announce short wait",
    )
    replace_once(
        subagent_announce,
        "plur1bus-openclaw-20260503-session-only-announce-no-final-wait",
        "\t\t\t\t\texpectFinal: true,\n\t\t\t\t\ttimeoutMs: announceTimeoutMs",
        "\t\t\t\t\texpectFinal: deliveryTarget.deliver ? true : false, /* plur1bus-openclaw-20260503-session-only-announce-no-final-wait */\n\t\t\t\t\ttimeoutMs: directAnnounceTimeoutMs",
...
for pattern, label in [
    ...
    target = find_one(
        pattern,
        lambda c: "lane: AGENT_LANE_SUBAGENT" in c or "plur1bus-openclaw-20260503-subagent-session-lane" in c,
        label,
        required=False,
    )
    if not target:
    ...
    replace_once(
        target,
        "plur1bus-openclaw-20260503-subagent-child-lane-import",
        'import { t as AGENT_LANE_SUBAGENT } from "./lanes-BvSfSAfS.js";',
        'import { i as resolveNestedAgentLaneForSession, t as AGENT_LANE_SUBAGENT } from "./lanes-BvSfSAfS.js"; /* plur1bus-openclaw-20260503-subagent-child-lane-import */',
        f"{label} import",
    )
    session_expr = "childSessionKey" if pattern.startswith("subagent") else "sessionKey"
    replace_once(
        target,
        "plur1bus-openclaw-20260503-subagent-session-lane",
        "\t\t\t\tlane: AGENT_LANE_SUBAGENT,",
        f"\t\t\t\tlane: resolveNestedAgentLaneForSession({session_expr}), /* plur1bus-openclaw-20260503-subagent-session-lane */",
        label,
    )
...
subagent_control = find_one(
    ...
    lambda c: (
        ...
        or "plur1bus-openclaw-20260503-subagent-steer-child-lane" in c
        or "plur1bus-openclaw-20260503-subagent-send-child-lane" in c
...
if subagent_control:
    replace_once(
        subagent_control,
        "plur1bus-openclaw-20260503-subagent-control-child-lane-import",
        'import { t as AGENT_LANE_SUBAGENT } from "./lanes-BvSfSAfS.js";',
        'import { i as resolveNestedAgentLaneForSession, t as AGENT_LANE_SUBAGENT } from "./lanes-BvSfSAfS.js"; /* plur1bus-openclaw-20260503-subagent-control-child-lane-import */',
        "subagent control per-child lane import",
    )
    replace_once(
        subagent_control,
        "plur1bus-openclaw-20260503-subagent-steer-child-lane",
        "\t\t\t\tlane: AGENT_LANE_SUBAGENT,\n\t\t\t\ttimeout: 0",
        "\t\t\t\tlane: resolveNestedAgentLaneForSession(params.entry.childSessionKey), /* plur1bus-openclaw-20260503-subagent-steer-child-lane */\n\t\t\t\ttimeout: 0",
        "subagent steer per-child lane",
    )
    replace_once(
        subagent_control,
        "plur1bus-openclaw-20260503-subagent-send-child-lane",
        "\t\t\t\tlane: AGENT_LANE_SUBAGENT,\n\t\t\t\ttimeout: 0",
        "\t\t\t\tlane: resolveNestedAgentLaneForSession(targetSessionKey), /* plur1bus-openclaw-20260503-subagent-send-child-lane */\n\t\t\t\ttimeout: 0",
...
heartbeat_runner = find_one(
    ...
    lambda c: (
        ...
        or "plur1bus-openclaw-20260503-no-startup-heartbeat-storm" in c
        or "plur1bus-openclaw-20260503-heartbeat-break-agent-after-run" in c
...
if heartbeat_runner:
    replace_once(
        heartbeat_runner,
        "plur1bus-openclaw-20260503-no-startup-heartbeat-storm",
        '\t\tconst isInterval = reason === "interval";',
        '\t\tconst isInterval = reason === "interval" || reason === "startup"; /* plur1bus-openclaw-20260503-no-startup-heartbeat-storm */',
        "heartbeat startup due schedule",
    )
    replace_once(
        heartbeat_runner,
        "plur1bus-openclaw-20260503-heartbeat-break-agent-after-run",
        "\t\t\t}\n\t\t\tif (ran) return {",
        "\t\t\t\tif ((reason === \"interval\" || reason === \"startup\") && ran) {\n"
        "\t\t\t\t\tlet deferIndex = 0; /* plur1bus-openclaw-20260503-heartbeat-break-agent-after-run */\n"
...
if os.path.exists(boot_md):
    replace_once(
        boot_md,
        "plur1bus-openclaw-20260503-boot-md-nonblocking",
        "\tawait runStartupTasks({",
        "\tsetImmediate(() => {\n\t\trunStartupTasks({ /* plur1bus-openclaw-20260503-boot-md-nonblocking */",
...
agent_runner = find_one(
    ...
    lambda c: ('transcriptPrompt: ""' in c or "plur1bus-openclaw-20260503-nonempty-flush-prompt" in c) and "memoryFlushWritePath" in c,
...
if agent_runner:
    replace_once(
        agent_runner,
        "plur1bus-openclaw-20260503-nonempty-flush-prompt",
        '\t\t\t\t\ttranscriptPrompt: "",',
        '\t\t\t\t\ttranscriptPrompt: "Run the hidden pre-compaction memory flush now. Do not send a visible user-facing reply.", /* plur1bus-openclaw-20260503-nonempty-flush-prompt */',
        "memory flush transcript prompt",
    )
else:
...
for path in sorted(backed):
    print(f"[patch] backup: {path}.bak-openclaw-20260503-{stamp}")
PYEOF
}

patch_openclaw_20260503_runtime || rc=1
