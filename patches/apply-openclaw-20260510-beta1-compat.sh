#!/usr/bin/env bash
# OpenClaw 2026.5.10-beta.1 compatibility patches for the local plur1bus deployment.
#
# This is intentionally separate from the 2026.5.4-2026.5.7 patch because
# active-memory changed shape in the beta. Keep beta anchor drift isolated.
...
patch_openclaw_20260510_beta1_runtime() {
...
def backup(path):
    if path in backed:
        return
    bak = f"{path}.bak-openclaw-20260510-beta1-{stamp}"
...
try:
    ...
    pass
supported_versions = {"2026.5.10-beta.1"}
if pkg_version and pkg_version not in supported_versions:
    print(f"[patch] OpenClaw 2026.5.10-beta.1 compat: package version is {pkg_version}, skipping dist patch")
    raise SystemExit(0)
...
\treturn result.status === "ok"; /* plur1bus-openclaw-20260504-no-empty-cache */
    ...
    replace_once(
        active_memory,
        "plur1bus-openclaw-20260510-beta1-no-empty-cache-upstream",
        """function shouldCacheResult(result) {
\treturn result.status === "ok" && result.summary.length > 0;
}""",
        """function shouldCacheResult(result) {
\treturn result.status === "ok" && result.summary.length > 0; /* plur1bus-openclaw-20260510-beta1-no-empty-cache-upstream */
}""",
        "active-memory no empty-result cache upstream-compatible",
        required=False,
    )
    replace_once(
        active_memory,
...
for path in sorted(backed):
    print(f"[patch] backup: {path}.bak-openclaw-20260510-beta1-{stamp}")
PYEOF
}

patch_openclaw_20260510_beta1_runtime || rc=1
