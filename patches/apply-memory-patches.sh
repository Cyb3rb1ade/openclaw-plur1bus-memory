...
patch_plur1bus_openclaw_compat() {
  ...
  case "$version" in
    2026.5.10-beta.1)
      bash "$script_dir/apply-openclaw-20260510-beta1-compat.sh"
      ;;
...
patch_plugin_deploy_sync() {
  ...
  local DST="/root/index.js"
  if [ ! -f "$SRC" ]; then
    echo "[patch] plugin-deploy-sync: canonical source not found ($SRC) — skipping"
    return 0
  fi
  if diff -q "$SRC" "$DST" > /dev/null 2>&1; then
    echo "[patch] plugin-deploy-sync: already in sync"
    return 0
  fi
  cp "$SRC" "$DST"
  echo "[patch] plugin-deploy-sync: updated /root/index.js from repo ($(wc -c < "$SRC") bytes)"
}
patch_plugin_deploy_sync || rc=1
