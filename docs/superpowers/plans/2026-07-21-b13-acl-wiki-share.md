...
An authenticated user principal is the collision-free opaque value `user:v1:<sha256(JSON.stringify([channel, accountId, userId]))>`, with each input component validated at its real field limit before canonical JSON serialization. `channel`/provider and `accountId` are mandatory for user-scoped sharing; absence fails closed instead of collapsing accounts. `ctx.chatId` remains the validated raw host `conversationId` so existing `allowedChatIds` configuration keeps its exact meaning. A separate `conversationPrincipal` for confirmation is an opaque hash of the host-resolved channel, account, conversation, parent, and thread binding, with the stable host `sessionKey` as a bounded fallback only when no conversation binding is exposed. The same user principal is persisted in `ownerUserId`, compared by ACL, and hashed to `u-${sha256(principal).slice(0, 62)}` for its physical route. Raw workspace/user identities never become path segments or shared-row provenance, and the raw `userId`/`chatId` remain available only for authorization; confirmation passes `conversationPrincipal` as its bound chat value.

Official host contexts are resolved explicitly rather than assumed to contain synthetic fields. Registered command handlers call async `resolveHostCommandMemoryContext(commandCtx, ...)`: it uses `commandCtx.senderId/channel/accountId`, read-only `getCurrentConversationBinding()` when available, the injected `api.runtime.agent.resolveAgentWorkspaceDir(commandCtx.config, agentId)`, and `resolveChatKind(commandCtx)` to build the frozen security context before authorization or any store/provider work. `chatKind` is accepted only as `private | group | unknown`; missing/ambiguous stays unknown, and `senderIsOwner` alone never implies private. The adapter never calls `requestConversationBinding()` and never mutates host bindings. Tool contexts use trusted `messageChannel`, `agentAccountId`, `deliveryContext`, and `requesterSenderId`; prompt hooks use their canonical `workspaceDir` and a bounded registration-local session identity registry populated by `message_received` (`ctx.channelId/accountId/conversationId/sessionKey` plus `event.senderId`). A missing or conflicting account/conversation/session binding omits user-shared recall and fails closed for user sharing or confirmation; it never defaults an account to `default`.
...
| File | Responsibility |
|---|---|
| `lib/memory-request-context.js` | Build/freeze canonical agent/workspace/user/chat principals, adapt official host contexts, maintain bounded session identity hints, reject conflicts, and produce <=64-char route keys. |
...
Also test a mismatching `workspaceDir` against an explicit trusted alias, non-existent `workspaceDir`, missing channel/account for `requireUser`, over-limit `INPUT_LIMITS.USER_ID`/`CHAT_ID`/`AGENT_ID`, and object/boolean/non-finite values. Transport IDs may be strings or finite safe integers (converted once to canonical decimal strings because channel SDKs expose numeric IDs); all other types fail before `validateInput`. Retain positive controls for matching `agent-private`, canonical `workspace`, and `userPrincipal`-bound `user`, plus denial of internal alias `workspace_shared` as `acl.unknown_scope`.
...
- a `PluginCommandContext` fixture has `senderId`, `channel`, `accountId`, `agentId`, `sessionKey`, `config`, `getCurrentConversationBinding()`, and injected `resolveAgentWorkspaceDir()` but no `workspaceId`, `workspaceKey`, `workspaceDir`, `userId`, or `chatId`; assert it resolves the configured canonical workspace, hashed user principal, and stable conversation principal without calling `requestConversationBinding()`;
- a tool fixture uses `messageChannel`, `agentAccountId`, `deliveryContext`, `requesterSenderId`, `agentId`, `sessionKey`, and `workspaceDir`;
- a `message_received` fixture seeds the bounded registry, and a following `before_prompt_build` fixture with only `sessionKey`, `agentId`, `workspaceDir`, and provider/chat fields recovers the exact same user/conversation principals;
- a different/missing account or session never reuses a principal, registry eviction is deterministic at the cap, and cron/background contexts never inherit a user.
...
- a registered command fixture proves original `args`, locale fields, and `runtimeContext.llm` reach parsing/render/LLM code unchanged while all auth/ACL spies receive only the separate frozen `memoryCtx`.
...
```bash
node --test --test-concurrency=1 tests/user-scope-acl.test.js tests/memory-store-input-validation.test.js tests/smoke-migration.test.js tests/b13-memory-request-context.test.js
```
...
export function resolveMemoryRequestContext(commandCtx, {
  ...
  const channel = validatedIdentity(commandCtx?.channel ?? commandCtx?.provider, 64, "channel");
  const accountId = validatedIdentity(commandCtx?.accountId ?? commandCtx?.account_id, INPUT_LIMITS.AGENT_ID, "accountId");
  ...
  return Object.freeze({
    ...
    conversationPrincipal: validatedIdentity(commandCtx?.conversationPrincipal, 128, "conversationPrincipal"),
...
Also export `buildMemoryWorkspaceAliases(cfg, precomputedNeoAliases)`, async `resolveHostCommandMemoryContext(commandCtx, { resolveAgentWorkspaceDir, workspaceAliases, requireWorkspace, requireUser, requireConversation })`, `resolveToolMemoryRequestContext(toolCtx, options)`, and `createMemorySessionIdentityRegistry({ maxEntries: 1000 })`. The command adapter calls the host workspace resolver and `getCurrentConversationBinding()` exactly once, validates the returned binding, keeps its `conversationId` as `chatId`, derives the thread-aware `conversationPrincipal` (or validated stable session fallback), and then delegates to the synchronous resolver. A session fallback may satisfy confirmation binding but never fabricates a raw `chatId` for `allowedChatIds`. The registry stores only bounded validated identity components/principals keyed by a validated session key, is registration-local/in-memory, uses explicit LRU eviction, and exposes separate `observeMessageReceived(event, ctx)` and `resolveHookContext(ctx)` operations. Log rejected host/binding data with `safeWarn` without raw identifiers.

`validatedIdentity()` inspects raw adapter fields before any `resolveIdentity()` coercion, accepts only strings or finite safe integers, trims/canonicalizes them, calls `validateInput` with the named real limit, and throws `new Error(result.error)` when `!result.ok`. Host/tool adapters extract the documented trusted variants themselves; they do not pass objects through `String()`. `resolveCanonicalWorkspacePrincipal(bindings, workspaceAliases)` consumes only the already normalized deeply frozen snapshot and applies the exact mapping rules in the fixed contract above; it never reads aliases from `commandCtx` itself. Without a trusted mapping, multiple unlike principals conflict. A sole ID/key becomes `workspace:v1:<value>` and a sole directory becomes `workspace-dir:v1:<realpath>`; never silently prefer one. `index.js` builds one trusted snapshot from `cfg` plus the precomputed Neo defaults and passes it into every context resolution, including commands, manual recall, auto-recall, and sharing; tests pass an explicit frozen fixture. In `acl-middleware.js`, export `resolveOwnershipBindings(memory)` and `validateOwnershipTuple(memory)`; reject different non-empty agent aliases and workspace aliases, and require the binding relevant to the row's scope. Irrelevant legacy metadata is validated/preserved but never grants access: private authorizes only the agent pair, workspace only the canonical workspace pair, and user only the principal owner. New writers still emit the minimal tuples defined above. `checkAccess()` compares workspace rows to `ctx.workspaceIdentity` and user rows to `ctx.userPrincipal` only after validation, with stable reasons such as `acl.agent_private.missing_owner`, `acl.workspace.missing_workspace`, `acl.workspace.conflicting_binding`, and `acl.user.missing_principal`.
...
Replace `resolveStoreScopeAccess()` and both ordinary store paths with the frozen context contract before embedding/duplicate/merge/write work. Every new/replacement row sets `agentId` and `storedBy` to the safe agent. `workspace` rows require the canonical workspace principal and set both aliases to it with empty owner; `user` rows require the hashed user principal, set `ownerUserId` to it, and leave both workspace aliases empty; `agent-private` rows require the agent binding and leave public bindings empty. Duplicate/merge ACL uses the same frozen context. Trusted mapped legacy workspace aliases may be canonicalized during reads through `resolveOwnershipBindings(memory, ctx.workspaceAliases)`; unmapped/conflicting workspace rows and legacy raw `ownerUserId` values fail closed and are documented for explicit operator repair rather than requester-relative adoption. Add store/merge regressions that round-trip each scope and remain visible only to the originating canonical context. Later integration tasks route the same resolver through each remaining call site.
...
```bash
git add lib/memory-request-context.js lib/acl-middleware.js index.js tests/user-scope-acl.test.js tests/memory-store-input-validation.test.js tests/smoke-migration.test.js tests/b13-memory-request-context.test.js
...
```js
function isActiveKindRow(row, kind) {
...
function visibleResults(ctx, results, kind) {
  return (Array.isArray(results) ? results : []).filter((result) => {
    const row = result?.entry || result;
    return isActiveKindRow(row, kind) && checkAccess(ctx, row).allowed;
  });
}
```

At the registered handler boundary, run `validateCommandArgs(commandCtx?.args)`, resolve `memoryCtx` asynchronously, and call `checkWikiAuth(memoryCtx, cfg, { chatKind: memoryCtx.chatKind, localeCtx: commandCtx })` before constructing dependencies, resolving tone, opening a pool, embedding, or calling an LLM; destructive add/delete use the same memory context with destructive auth. Change the helper signature to `checkWikiAuth(memoryCtx, cfg, { destructive = false, chatKind = memoryCtx.chatKind, localeCtx = null } = {})`, call `isAuthorized(memoryCtx, cfg, { destructive, chatKind })` without spreading `localeCtx`, and use `localeCtx` only to localize a denial. Help/invalid input returns a localized usage/error without data work. Call `runWikiCommand(commandCtx, { ...deps, ctx: memoryCtx })`: the original invocation supplies only validated args/locale/runtime capability, while selection and writes use `deps.ctx`. `searchByKind()` fallback must include the same active/null predicate as its SQL branch and log caught search failures through `safeDebug(logger, "wiki search failed", error)` without record text. Every caught fallback, LLM, `findSimilar`, or `getById` error must be rethrown, returned as an error result, or safely logged; no silent catches. Curated search uses `visibleResults(ctx, ..., "wiki")`; the legitimate normal-memory fallback uses `visibleResults(ctx, ..., "memory")` and must remain reachable.

All Wiki embedding calls preserve method purpose and agent cache scope: query/search uses `embeddings.embedQuery(text, { agentId: ctx.agentId })` when available (falling back to `embed(text, { agentId })` only for providers without `embedQuery`), while stored/add content uses `embeddings.embed(text, { agentId: ctx.agentId })`. `wikiAdd()` must use the real current duplicate API `db.findSimilar(vector, fullText, 0.92)`, then call `visibleResults(ctx, existing, "wiki")` before rendering a preview; a foreign or normal-memory hit does not suppress the store. Its row uses the exact canonical workspace tuple described by the registered-handler test, never a raw/default workspace key. Both `wikiDelete()` branches call `visibleResults(ctx, candidates, "wiki")` before kind response, ambiguity rendering, archive, or delete. ACL-denied UUID/query rows return exactly `wiki.delete_not_found`; never return a distinct denial. Pass `ctx`, `workspaceDir`, and `logger` through the helpers. After a successful delete, call:
...
`shareCard()` must `safeUuid(sourceId)` inside the export, receive the initialized `MultiNamespacePool`, lease exactly `privatePool.withWriteDb(agent, ...)`, initialize and fetch the source there, require active/null status, require `expiresAt` to be absent/zero or strictly later than the captured request time, require a public `agent-private`/`workspace`/`user` scope (never `workspace_shared`), and enforce `checkAccess(opts.ctx, card)`. Classify sensitivity before provider work with independent checks for both `category` and `type` (neither may mask the other), plus `memoryClass`, `neverForget`, and importance; an expired or unapproved source returns before embedding or target lease. Then call the request-bound existing provider `embeddings.embed(card.text || card.summary, { agentId: agent })` while the source lease is alive, lease exactly one requested `sharedPool.withWorkspaceDb/withUserDb`, and call `storeSharedMemory(targetDb, ...)` without a nested/double lease. Re-read and re-authorize the source, including the expiry check against a fresh completion time, in the same authoritative writer during confirmation completion. Remove the old zero-vector and `workspace_shared` path. Add invalid UUID, already-expired and expires-during-confirmation, benign-category+sensitive-type, sensitive-category+benign-type, and no-embed/no-target-lease regressions.
...
Add these access-composition cases: private-only context (no workspace and no user principal) still recalls private DBs; workspace-only adds workspace; user-only adds user without requiring workspace; all-bound adds both. A missing shared binding is a skipped optional source, never a reason to drop private recall. Add `/memory` topic and time-mode parity, active lifecycle filtering, `userPrincipal` propagation, canonical search only once, Semantic Lens/CRR ordering unchanged, and spies proving initial/refined/graph and `/memory` topic embeddings all receive `{ agentId: requestingAgent }` through the existing provider without key/model/endpoint overrides.
...
Add an expired legacy row and a boundary-time row. Capture one migration timestamp and assert any positive `expiresAt <= migrationNow` is counted as `expiredSkipped` and receives zero embed, target lease/store/readback, source marker, or source update work in both dry-run and apply; non-expiring zero/null and future rows retain normal behavior.

Add registered `/plur1bus migrate-legacy-shared [--report report.json] [--apply]` tests through the real plugin harness and the official host-shaped command fixture from Task 1. Resolve host context before auth; a destructively authorized operator uses initialized pools/providers and `resolvedCtx.agentId/workspaceDir/workspaceAliases`, while an unauthorized chat performs zero pool/embed/report work. Assert a cron identity does not bypass the destructive operator gate; this is an operator command, not a cron-internal job. Add a source inspection assertion that no standalone script, alternate `MemoryDB`/`AgentDbPool`, embedding-provider constructor, config loader, API-key option, or base-path option is introduced.
...
Call `safeAgentId(agentId)`, require the deeply frozen trusted `workspaceAliases` snapshot, and `safeUuid(row.id)` before any provider/target/source write. Invalid IDs become bounded repair entries with no provider/write work. Dry-run scans only through `privatePool.withAuthoritativeReadDb(agentId, ...)` and never leases/initializes the writer. Apply first acquires the exact `privatePool.withWriteDb(agentId, ...)` lease, initializes it, completes/verifies the marker-column migration, and only then nests the separate authoritative read-only lease/snapshot while holding the writer for per-row marker updates; this prevents schema mutation after the read stream starts. Neither mode enumerates named recall namespaces, shared pools, or a requester-relative DB, and neither uses `scanActive()`. If the authoritative table does not exist, return a zero-count report. Inspect its schema, require core `id/text/scope/status`, and select the intersection of the full ownership/content/sensitivity/provenance/version/copy columns so pre-migration schemas receive explicit code defaults rather than an invalid missing-column SELECT. Execute one continuous read-only query/snapshot with the static predicate `scope = 'workspace_shared' AND (status IS NULL OR status = '' OR status = 'active')` via the runtime JS API `query.execute({ maxBatchLength: 200 })`, and consume its `AsyncIterable<RecordBatch>`; never use repeated unordered offset queries. Process one hard-bounded batch at a time. If the installed LanceDB runtime does not expose this bounded execution API, fail closed before apply rather than materializing the full table. A multi-batch DB-free fixture proves no skip/duplicate while the separate writer marks rows. Bound repair output separately while maintaining aggregate counts. Resolve workspace binding from equal non-empty `workspaceId`/`workspaceKey`, or the sole non-empty alias; canonicalize it with the explicit trusted snapshot to the same Task 1 workspace principal and never use current requester context. For valid rows, call the existing initialized provider as `embeddings.embed(row.text || row.summary, { agentId })`, call Task 7's idempotent workspace store with `action: "legacy_workspace_shared_migration"` and `allowSensitiveShare: true`, read back the exact shared ID/key, then update only `legacyShareMigrationMarker` on the source. Never change legacy scope/status/text/shareProvenance or delete it.

Capture one immutable `migrationNow` before scanning and include `expiresAt` in the selected-column intersection when present. Before any workspace resolution, embedding, target lease/write/readback, or source marker work, classify a row with finite positive `expiresAt <= migrationNow` as `expiredSkipped`, increment only bounded aggregate/report counts, and continue. This code gate is mandatory even if the storage query also includes an expiry predicate, so pre-migration schemas and coerced values fail closed rather than extending retention.
...
The operator parser accepts only `--report <basename>` and `--apply`, rejects duplicates/unknowns, and defaults to dry-run plus an immutable generated report name. It requires `resolvedCtx.workspaceDir` for the pinned private report directory and obtains the agent exclusively from the authenticated/resolved OpenClaw command context; it never accepts agent, DB base, model, endpoint, key, or credential options. Add it to the explicit destructive-action classification from Task 5.
...
Document command grammar, strict scopes, <=64-character physical routes, conflict-rejecting workspace canonicalization (no hidden precedence), channel+account+user principal, confirmation policy, optional shared recall sources, canonical-origin dedup, and the destructively authorized initialized-runtime legacy dry-run/apply workflow. State explicitly that no standalone DB/config/credential bootstrap exists and that Multi-Namespace, Neo/Obsidian aliases, Semantic Lens, CRR, default OpenClaw LLM, and per-agent auth/API keys do not change.
