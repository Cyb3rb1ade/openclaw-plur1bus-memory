...
- a `PluginCommandContext` fixture has `senderId`, `channel`, `accountId`, `agentId`, `sessionKey`, `from`, `to`, `messageThreadId`, `threadParentId`, `config`, a null-returning `getCurrentConversationBinding()`, and injected `resolveAgentWorkspaceDir()` but no `workspaceId`, `workspaceKey`, `workspaceDir`, `userId`, `chatId`, or `chatType`; assert the closed host-route decoder resolves the configured canonical workspace, raw parent chat ID, private/group kind, hashed user principal, and thread-aware conversation principal without calling `requestConversationBinding()`. Cover real Telegram, Discord, Slack, and Mattermost direct/group target shapes, canonical main-DM session keys, thread routes, mismatching session-vs-target facts, and existing `allowedChatIds` plus no-whitelist private/group authorization;
- a tool fixture uses `messageChannel`, `agentAccountId`, `deliveryContext`, `requesterSenderId`, `agentId`, `sessionKey`, and `workspaceDir`;
- a `message_received` fixture seeds the bounded registry with validated `runId + sessionKey`, and a following `before_prompt_build` fixture with the exact same run/session, `agentId`, `workspaceDir`, and provider/chat fields consumes and recovers the exact same user/conversation principals;
- two simultaneous users in one group/session with different `runId` values cannot overwrite, swap, or reuse principals even when hooks interleave; missing/conflicting/replayed run IDs, different account/session, second consumption, deterministic cap eviction, and cron/background contexts all return no user principal.
- registry keys are the validated composite `runId + agentId + sessionKey + provider + accountId`, never `sessionKey` alone. The message adapter obtains provider/account only from the documented `ctx.channelId`/`ctx.accountId` plus `event.senderId`; the agent hook obtains provider from `messageProvider` and must never reinterpret its conversation-target `ctx.channel` as a provider. Any missing or mismatching component is a cache miss, including equal session keys across concurrent runs, agents, accounts, or providers.
...
Also export `buildMemoryWorkspaceAliases(cfg, precomputedNeoAliases)`, async `resolveHostCommandMemoryContext(commandCtx, { resolveAgentWorkspaceDir, workspaceAliases, requireWorkspace, requireUser, requireConversation })`, `resolveHostCommandRouteFacts(commandCtx)`, `resolveToolMemoryRequestContext(toolCtx, options)`, and `createMemorySessionIdentityRegistry({ maxEntries: 1000 })`. The command adapter calls the host workspace resolver, decodes and cross-checks the current route from public session-key helpers plus the closed supported-channel `from/to` forms, and calls `getCurrentConversationBinding()` at most once only as optional corroboration. A null binding is the normal fresh-install case, not an identity failure. The adapter derives raw parent `chatId`, authoritative `chatKind`, and a thread-aware `conversationPrincipal` from the verified current route before delegating to the synchronous resolver; a bare session string never fabricates any of those fields. The registry stores only bounded validated identity components/principals keyed by the validated composite tuple `[runId, agentId, sessionKey, provider, accountId]`, is registration-local/in-memory, uses explicit LRU eviction, and exposes separate transport-specific `observeMessageReceived(event, ctx)` and consuming `resolveHookContext(ctx)` operations. The message adapter requires equal non-empty event/context run IDs when both exist, reads provider/account from documented message fields, and refuses to overwrite a different identity for the same run tuple. The hook adapter requires the exact run tuple, reads provider from `messageProvider`, and treats `ctx.channel` only as a conversation target. Missing/mismatching/replayed tuple components never fall back to a session-only lookup; two concurrent runs in one group session remain isolated. Log rejected host/binding data with `safeWarn` without raw identifiers.
...
Import the two session helpers only from the documented `openclaw/plugin-sdk/routing` surface; never import hashed `dist/` artifacts or OpenClaw test-only exports. The local route decoder mirrors only the closed public delivery grammar and rejects unknown peer kinds, extra/missing identity components, unsupported channels, and any disagreement between session, `from/to`, parent, and thread facts.
...
```bash
git add lib/memory-request-context.js lib/acl-middleware.js lib/db-adapter.js lib/wiki-command.js lib/telegram-commands/memory-edit.js lib/telegram-commands/memory-query.js lib/recall-pipeline.js index.js tests/user-scope-acl.test.js tests/memory-store-input-validation.test.js tests/smoke-migration.test.js tests/memory-db-lifecycle-atomic.test.js tests/smoke-wiki-command.test.js tests/forget-correct-confirm.test.js tests/recall-golden-set-pipeline.test.js tests/b13-memory-request-context.test.js tests/b13-acl-callsite-adapters.test.js
