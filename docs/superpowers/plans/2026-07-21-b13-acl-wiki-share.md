...
Capture one immutable `migrationNow` before scanning and include `expiresAt` in the selected-column intersection when present. Before any workspace resolution, embedding, target lease/write/readback, or source marker work, normalize expiry with a strict type-aware parser: only null/empty/zero (non-expiring) or a finite positive numeric value strictly greater than `migrationNow` may continue. A finite positive value `<= migrationNow` increments `expiredSkipped`; negative, non-finite, boolean/object, or non-numeric values produce a bounded `invalid_expiry` repair/skip. Both skip paths perform zero provider/target/marker work. This code gate is mandatory even if the storage query also includes an expiry predicate.
