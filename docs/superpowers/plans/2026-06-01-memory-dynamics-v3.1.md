# Phase 7: Memory Dynamics — Implementation Plan (v3.1)
...
| `lib/jobs/memory-dynamics-maintenance.js` | Daily job: processes ledger per agent/workspace, aggregates retrievals, applies reinforcement + decay |
...
| `lib/recall-pipeline.js` | Multiply `strengthFactor` into final score, emit ledger events after final selection via callback |
...
- [ ] **Step 2.2: Add existing-table migration in `openTable` branch**

In `MemoryDB.init()`, after `this.table = await this.db.openTable(TABLE_NAME)`:

```javascript
// Migrate: add missing dynamics columns
...
- [ ] **Step 2.3: Add dynamics fields to MemoryDB.search() result mapping**
...
export function applyFlashbulbEncoding(row, now = Date.now()) {
...
export function applyDynamicsDefaults(entry, now = Date.now()) {
  const base = {
    retrievalCount: 0,
    lastRetrievedAt: 0,
    memoryStrength: 1.0,
  ...
  const flashbulb = applyFlashbulbEncoding(enriched, now);
...
describe("memory-dynamics", () => {
  ...
  it("applyDynamicsDefaults applies flashbulb when eligible", () => {
    const entry = { text: "test", emotionalIntensity: 0.9, importance: 0.9, category: "decision", memoryStrength: 0.6 };
    const result = applyDynamicsDefaults(entry);
    assert.ok(result.memoryStrength > 0.6);
    assert.ok(result.halfLifeDays > 30);
  });

  it("createRetrievalLedgerEntry creates valid UUID hash", () => {
...
- [ ] **Step 4.3: Add maintenance tests**

Add these 4 tests to `tests/memory-dynamics.test.js`:

```javascript
import { processRetrievalLedger, applyDailyDecayToAll } from "../lib/jobs/memory-dynamics-maintenance.js";

describe("memory-dynamics-maintenance", () => {
  it("processRetrievalLedger filters by agentId/workspaceKey", async () => {
    const mockDb = {
      table: {
        query: () => ({ where: () => ({ limit: () => ({ toArray: async () => [{ id: "m1", retrievalCount: 0, memoryStrength: 0.5, createdAt: Date.now() }] }) }) }),
        update: async () => {},
      },
    };
    const mockStore = {
      ...
      writeRunState: () => {},
      readRetrievalLedger: () => [
        { memoryId: "m1", agentId: "agentA", workspaceKey: "wk1", createdAt: new Date().toISOString() },
        { memoryId: "m1", agentId: "agentB", workspaceKey: "wk1", createdAt: new Date().toISOString() },
      ],
    };
    const result = await processRetrievalLedger(mockDb, mockStore, { agentId: "agentA", workspaceKey: "wk1" });
    assert.strictEqual(result.reinforced, 1);
  });

  it("watermark does not advance on DB failure", async () => {
    ...
    const mockStore = {
      readRunState: () => ({}),
      writeRunState: () => { stateWritten = true; },
      readRetrievalLedger: () => [
        { memoryId: "m1", agentId: "agentA", workspaceKey: "wk1", createdAt: new Date().toISOString() },
      ],
    };
    const result = await processRetrievalLedger(mockDb, mockStore, { agentId: "agentA" });
    assert.strictEqual(result.failures, 1);
    assert.strictEqual(stateWritten, false);
  });

  it("missing memory does not block watermark", async () => {
    let stateWritten = false;
    const mockDb = {
      table: {
    ...
    const mockStore = {
      ...
      writeRunState: () => { stateWritten = true; },
      readRetrievalLedger: () => [
        { memoryId: "missing", agentId: "agentA", workspaceKey: "wk1", createdAt: new Date().toISOString() },
      ],
    };
    const result = await processRetrievalLedger(mockDb, mockStore, { agentId: "agentA" });
    assert.strictEqual(result.reinforced, 0);
    assert.strictEqual(stateWritten, true);
  });

  it("dry-run does not write DB or state", async () => {
    let dbUpdated = false;
    let stateWritten = false;
    const mockDb = {
      table: {
        query: () => ({ where: () => ({ limit: () => ({ toArray: async () => [{ id: "m1", retrievalCount: 0, memoryStrength: 0.5, createdAt: Date.now() }] }) }) }),
        update: async () => { dbUpdated = true; },
      },
    };
    const mockStore = {
      readRunState: () => ({}),
    ...
    const result = await processRetrievalLedger(mockDb, mockStore, { agentId: "agentA", dryRun: true });
    assert.strictEqual(dbUpdated, false);
    assert.strictEqual(stateWritten, false);
  });
});
```
...
git commit -m "test(memory-dynamics): 13 tests (9 pure + 4 maintenance)"
```
...
- [ ] **Step 6.2: Apply strength factor after vector search**

After vector search results, before importance boost:
```javascript
// Guard: memoryStrength must be preserved through all pipeline stages
...
- [ ] **Step 6.3: Emit ledger events AFTER final selection**

After the pipeline has fully processed (dedup, canonical, rerank, etc.), before returning:

```javascript
// Emit retrieval ledger events for memories that were actually returned
if (retrievalLogger && ordered.length > 0) {
...
git commit -m "feat(recall-pipeline): strength factor + retrievalLogger after final selection"
```
...
- [ ] **Step 7.1: Write processRetrievalLedger with agent/workspace filter**
...
export async function processRetrievalLedger(db, neoStore, opts = {}) {
  ...
  const agentState = dynamicsState[agentId] || {};
  const lastProcessedAt = agentState.lastRetrievalLedgerProcessedAt || 0;

  const allTail = neoStore.readRetrievalLedger(50_000);
  const entries = allTail
    .filter(e => e.agentId === agentId)
    .filter(e => !workspaceKey || e.workspaceKey === workspaceKey)
    .filter(e => Number(new Date(e.createdAt || 0)) > lastProcessedAt);

  // Guard: Warn if tail may not include all unprocessed entries
  const tailTimes = allTail.map(e => Number(new Date(e.createdAt || 0))).filter(Number.isFinite);
  const oldestTailAt = tailTimes.length ? Math.min(...tailTimes) : 0;
  if (allTail.length >= 50_000 && oldestTailAt > lastProcessedAt) {
    logger.warn?.("retrieval-ledger tail may not include all unprocessed entries; increase limit or process more often");
  }
  ...
  for (const [memoryId, events] of byMemoryId) {
    ...
    try {
      const rows = await db.table.query()
        ...
        .toArray();
      if (rows.length === 0) {
        // Guard: Missing memory should not block watermark forever
        logger.warn?.(`dynamics-maintenance: memory ${memoryId} missing; marking ${events.length} ledger events stale`);
        for (const event of events) {
          const eventTime = Number(new Date(event.createdAt || now));
          if (Number.isFinite(eventTime) && eventTime > 0) {
            processedEntryTimes.push(eventTime);
          }
        }
        continue;
      }
  ...
  // Only advance watermark if ALL entries were processed successfully
  if (!dryRun && failures === 0 && neoStore.writeRunState && processedEntryTimes.length > 0) {
    const maxCreatedAt = processedEntryTimes.reduce((a, b) => Math.max(a, b), lastProcessedAt);
    const newState = {
      ...state,
      memoryDynamics: {
  ...
  logger.info?.(`dynamics-maintenance: ${reinforced} memories reinforced from ${entries.length} ledger entries (${failures} failures)`);
  return { reinforced, entriesProcessed: entries.length, failures, dryRun };
}
```

- [ ] **Step 7.2: Write applyDailyDecayToAll**
...
git commit -m "feat(memory-dynamics-maintenance): agent-safe ledger processing + daily decay"
```
...
- [ ] **Step 8.2: Add maintenance step BEFORE neo-store pruning**

```javascript
// ── 1.5 Memory Dynamics: Ledger Processing + Decay ──────────────────────
// MUST run BEFORE neoStore.pruneAll() so ledger events aren't lost
let dynamics = null;
if (opts.rawDb?.table && opts.neoStore) {
  try {
    const ledgerResult = await processRetrievalLedger(opts.rawDb, opts.neoStore, {
      logger,
...
At both pipeline call sites, first get neoStore:
```javascript
const recallNeoStore = getNeoStore(ctx, event);
```
...
retrievalLogger: (items) => {
  try { recallNeoStore.appendRetrievalLedger(items); } catch (err) {
...
const result = await runDailyConsolidation(memoryDbAdapter, internalAgent, {
  ...
  workspaceKey: commandCtx.workspaceKey || workspaceKeyFromContext(ctx, event) || "default",
});
```
...
Expected: 53 passing (40 existing + 9 pure-function + 4 maintenance tests)
...
- [ ] **Ledger processed idempotently:** Per-agent `lastRetrievalLedgerProcessedAt`
- [ ] **Agent/workspace filter:** `processRetrievalLedger` filters by `agentId` and `workspaceKey`
- [ ] **Watermark only on success:** No advance if `failures > 0`
- [ ] **Backlog warning:** Warn if ledger tail is near limit
- [ ] **Ledger events after final selection:** Not after intermediate boost stage
- [ ] **No workspaceDir in pipeline:** Only `workspaceKey`/`agentId`/`retrievalLogger`
- [ ] **Existing tables migrated:** `addColumns` in `MemoryDB.init()` openTable branch
- [ ] **`firstValidTimestamp` skips 0:** Proper fallback chain
- [ ] **`__schema__` skipped in decay and reinforcement**
- [ ] **SQL IDs safely escaped:** `replaceAll("'", "''")`
...
- [ ] **neoStore variable explicit at pipeline call sites**

---

## Execution Handoff

**Plan complete and saved to `docs/superpowers/plans/2026-06-01-memory-dynamics-v3.1.md`.**
