...
| Compaction memory flush | Durable capture before compaction | PLUR1BUS supplies no file-memory flush plan because conversation capture is handled by typed hooks. This avoids a second file-memory write lane. | n/a (harness owns this) — `Engine.checkpoint(agentId, "compaction")` replaces the read of `event.compactedAt` (PR-15). |
