...
| Operator-Dashboard | Ja, eigene Hermes-Erweiterung | Serverseitig gewähltes Profil/Agent/Scope, redigierter Status sowie eng begrenzte Workshop-Listen/Reviews und revision-gebundenes Approve/Publish. Mutationen verlangen eine kurzlebige Einmal-Nonce, verifizierte Sessionbindung, Origin- und Confirm-Header; kein generischer Command-/Pfad-/Config-Endpunkt. Publish warnt über profilweite, nicht ACL-geschützte Sichtbarkeit. Kein visueller Browser-Abnahmetest. |
...
| Re-Embedding produktiv umschalten | Teilweise, nativ begrenzt | Operator-CLI kann eine vollständig validierte Staging-Generation explizit aktivieren oder recovern. Pointer/Journale sind atomar und die Runtime hält kooperierende Shared-Leases bis I/O drainiert. Nur der einzelne private Standard-Writer ist zulässig; Named Namespaces, nicht-kooperierende Prozesse, automatische Umschaltung und eine Live-Gateway-Abnahme bleiben ausgeschlossen. |
...
| Obsidian | Ja, teilweise | Bestehende native Mirror-/Sync-/Graph-Funktionen bleiben erhalten. Die lokale Operator-CLI plant eine explizite Quelle und importiert nur nach exakter revisionsgebundener Zustimmung in die kanonische Route. Browser-Zielerkennung, Consent-UI, Watch- und Konfliktworkflow bleiben offen. |
