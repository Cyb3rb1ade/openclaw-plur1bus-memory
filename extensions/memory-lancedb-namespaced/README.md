...
`Vault maintenance only - no memory import`; fuer sie ist keine Memory-Freigabe
noetig. Nutzer sollen Details pruefen, `/plur1bus_review explain` verwenden
oder erwartete Befunde mit `/plur1bus_review reject all` schliessen.
...
Diese Commands sind OpenClaw Slash-/Plugin-Commands. Es gibt kein benoetigtes
standalone `plur1bus` Shell-Binary und kein `openclaw plur1bus` CLI-Namespace.

Fuer Telegram-Clients registriert PLUR1BUS zusaetzlich native Shortcuts wie
`/plur1bus_morning`, `/plur1bus_evening`, `/plur1bus_status`,
`/plur1bus_review`, `/plur1bus_dashboards`, `/plur1bus_conflicts` und
`/plur1bus_cron`. Telegram akzeptiert nur 100 Commands im Bot-Menue. Wenn
native Skill-Commands diese Liste vor den Plugin-Commands fuellen, reduziere
die nativen Skill-Commands fuer den betroffenen Telegram-Account oder setze
`channels.telegram.accounts.<account>.commands.nativeSkills=false`. Der
generische `/skill` Dispatcher bleibt verfuegbar.
