...
```json
{
  ...
  "runtime": {
    "recallTimeoutMs": 45000,  // Timeout für Recall-Worker (Standard: 45s)
    "captureTimeoutMs": 60000  // Timeout für Capture-Worker (Standard: 60s)
  }
}
```

**Lange Queries (>20 000 Zeichen, seit 5.1.x):** Statt Hard-Truncation wird der Prompt via LLM zu Key-Topics zusammengefasst bevor er ans Embedding geht. Erfordert `merging.model` (Standard: `kimi-for-coding`). Fallback auf Tail-Truncation wenn LLM nicht verfügbar.
...
3. **Überlange Texte (>captureMaxChars)** werden via LLM zusammengefasst, wenn `merging.enabled=true` und ein explizites `merging.model` konfiguriert ist. Ohne LLM oder bei Fehler: Fallback auf Truncation
4. Origin wird automatisch erkannt: Gruppenkontext-Signale → `"group"`, sonst `"dm"`
5. **Embed + Duplikat-Check laufen parallel** (alle 8 Items gleichzeitig, seit 5.1.x). Writes bleiben seriell (LanceDB-Versioning). Vorher: sequentiell, 8 × (embed + scan) = oft >30s Timeout.
...
**ANN-Index (seit 5.1.x):** MemoryDB baut nach je 500 Writes automatisch einen HNSW-PQ-Vektorindex (`m=16, numSubVectors=96`). Ohne Index → O(n)-Flat-Scan → Timeout. Mit Index → ~50ms statt 3-8s pro Suche. Index-Status prüfen: `(await table.listIndices())` — leer = kein Index, manuell bauen via `createIndex('vector', { config: lancedb.Index.hnswPq({…}), replace: true })`.
...
`scope` bleibt per Standard `agent-private`; `workspace` erlaubt workspace-gebundenes Teilen, und `user` ist owner-bound: Zugriff ist nur für denselben `userId` erlaubt.
