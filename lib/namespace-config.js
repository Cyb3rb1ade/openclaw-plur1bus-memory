...
/**
 * Recognize the private flat-path shim emitted by index.js until Task 2 removes it.
 * This exact shape is accepted only by compatibility wrappers, never by explicit layouts.
 */
function isInternalLegacyFlatSentinel(config) {
  if (!isPlainObject(config)) return false;
  const keys = Reflect.ownKeys(config);
  return keys.length === 2
    && keys.includes("activeWriteNamespace")
    && keys.includes("activeRecallNamespaces")
    && config.activeWriteNamespace === "."
    && Array.isArray(config.activeRecallNamespaces)
    && config.activeRecallNamespaces.length === 1
    && config.activeRecallNamespaces[0] === ".";
}
...
function normalizeNamespaceConfig(
  ...
  { allowInternalLegacyFlatSentinel = false } = {},
) {
  if (allowInternalLegacyFlatSentinel && isInternalLegacyFlatSentinel(config)) {
    return {
      activeWriteNamespace: ".",
      activeRecallNamespaces: ["."],
      legacyReadOnlyNamespaces: [],
...
export function resolveWriteNamespace(nsCfg = {}) {
  return normalizeNamespaceConfig(nsCfg, NAMESPACE_CONFIG_PATH, {
    allowInternalLegacyFlatSentinel: true,
  }).activeWriteNamespace;
}
...
export function isLegacyReadOnly(namespaceName, nsCfg = {}) {
  return normalizeNamespaceConfig(nsCfg, NAMESPACE_CONFIG_PATH, {
    allowInternalLegacyFlatSentinel: true,
  }).legacyReadOnlyNamespaces.includes(namespaceName);
}
