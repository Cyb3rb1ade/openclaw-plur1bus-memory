...
import { projectFeatureModels } from "./featureModels.js";
import { projectChatModels, projectSubagentModels } from "./chat-model.js";
import { activeCaptureChunkingId } from "./setup/control-ui-write.js";
import { projectDashboardSettings, projectPendingChanges } from "./dashboard-settings.js";
import { projectOperations } from "./dashboard-operations.js";

import { FEATURE_DEFINITIONS } from "./feature-definitions.js";
export { FEATURE_DEFINITIONS };

export const FEATURE_CARD_DEFINITIONS = Object.freeze([
  ...
  { id: "rem", feature: "neo", label: "REM", dependencies: ["merging_route"], purpose: "Background consolidation of memories into patterns." },
  ...
  { id: "reactivation", feature: "conversationReactivationRecall", label: "Reactivation Recall", dependencies: [], purpose: "Brings open threads back after a long pause." },
  { id: "reaction-nudge", feature: "reactionNudge", label: "Reaction Nudge", dependencies: [], purpose: "Suggests a reaction emoji where the channel supports it." },
  { id: "morning-review", feature: "morningReview", label: "Morning Review", dependencies: [], purpose: "Daily review of yesterday's memories, run in the morning." },
  { id: "evening-review", feature: "eveningReview", label: "Evening Review", dependencies: [], purpose: "Daily review of the day's memories, run in the evening." },
  // No feature switch of its own: the card only carries its three settings.
  { id: "style-directive", feature: null, label: "Style Directive", dependencies: [], purpose: "Shapes how a reply reads: time of day, opinion, asking back." },
]);
...
// Each capability lists every place its key may legitimately live. `apiKey`
// holds a literal or a SecretRef, `apiKeyEnv` names an environment variable
// instead. Reading only `apiKey` reported "missing" for a working Cohere
// reranker configured through `apiKeyEnv`, so both are checked.
//
// The rest mirrors the runtime's own resolution, because the table is only
// useful if it agrees with what actually runs:
// - `envDefault`: the variable the runtime falls back to when neither config
//   path is set (`resolveConfiguredApiKey(cfg, "${OPENAI_API_KEY}")`). A
//   default install with the key in the environment used to read "missing"
//   while embeddings were demonstrably working.
// - `keyless`: providers that never need a key (local models, disabled).
// - `optional`: a capability that is simply not switched on, as opposed to one
//   that is switched on and lacks its key.
// - `hostRouted`: falls back to OpenClaw's own model route when it has no key.
const CREDENTIAL_DEFINITIONS = Object.freeze([
  ["embedding", Object.freeze({
    paths: [["embedding", "apiKey"], ["embedding", "apiKeyEnv"]],
    envDefault: "OPENAI_API_KEY",
    keyless: (config) => getPath(config, ["embedding", "provider"]) === "local-transformers",
  })],
  ["embeddingFallback", Object.freeze({
    paths: [["embedding", "fallback", "apiKey"], ["embedding", "fallback", "apiKeyEnv"]],
    envDefault: "OPENAI_API_KEY_FALLBACK",
    optional: (config) => !isPlainRecord(getPath(config, ["embedding", "fallback"])),
  })],
  ["reranker", Object.freeze({
    paths: [["reranker", "apiKey"], ["reranker", "apiKeyEnv"]],
    keyless: (config) => {
      const provider = getPath(config, ["reranker", "provider"]);
      return provider === "local-transformers" || provider === "disabled" || getPath(config, ["reranker", "enabled"]) === false;
    },
  })],
  ["merging", Object.freeze({ paths: [["merging", "apiKey"], ["merging", "apiKeyEnv"]], hostRouted: true })],
  ["knowledgePromotion", Object.freeze({ paths: [["schicht15", "apiKey"], ["schicht15", "apiKeyEnv"]], hostRouted: true })],
  ["skillMiner", Object.freeze({ paths: [["skillMiner", "apiKey"], ["skillMiner", "apiKeyEnv"]], hostRouted: true })],
  ["criticalPush", Object.freeze({ paths: [["criticalPush", "apiKey"], ["criticalPush", "apiKeyEnv"]], hostRouted: true })],
  ["emotionTier3", Object.freeze({ paths: [["emotion", "t3", "apiKey"], ["emotion", "t3", "apiKeyEnv"]], hostRouted: true })],
]);
...
// Presence only. The value itself is never read into the projection.
function envHasValue(env, name) {
  return isPlainRecord(env) && typeof env[name] === "string" && env[name].trim() !== "";
}
...
function configuredCredential(value, isEnvName = false) {
  if (value === undefined || value === null || value === "") return { status: "missing", source: null };
  if (typeof value === "string") return { status: "configured", source: isEnvName ? "env" : "plaintext" };
...
// Takes the first path that carries a value, so a capability configured through
// `apiKeyEnv` is not reported as missing just because `apiKey` is unset. The
// resolved path travels with the result: the dashboard used to print a fixed
// `<section>.apiKey` label even when the key actually sat in `apiKeyEnv`, which
// pointed the reader at the wrong config line.
function resolveCredential(config, definition, env) {
  const { paths } = definition;
  const defaultPath = paths[0].join(".");
  if (definition.keyless?.(config)) return { status: "not_required", source: null, path: defaultPath };
  if (definition.optional?.(config)) return { status: "optional", source: null, path: defaultPath };
  for (const path of paths) {
    const resolved = configuredCredential(getPath(config, path), path.at(-1) === "apiKeyEnv");
    if (resolved.status !== "missing") return { ...resolved, path: path.join(".") };
  }
  if (definition.envDefault && envHasValue(env, definition.envDefault)) {
    return { status: "configured", source: "env_default", path: definition.envDefault };
  }
  return { status: "missing", source: definition.hostRouted ? "host_route" : null, path: defaultPath };
}
...
function projectFeatureCards(features) {
  return FEATURE_CARD_DEFINITIONS.map((definition) => {
    const feature = definition.feature === null
      ? { configured: true, effective: true, reason: null } // a settings-only card is always "on"
      : features[definition.feature] || { configured: false, effective: false, reason: "feature_unavailable" };
    return {
      id: definition.id,
...
function projectHealthCounts(value, name, { allowUnknown = false } = {}) {
  ...
  const counts = value.map((entry) => {
    ...
    const validCount = (allowUnknown && entry.cards === null)
      || (Number.isSafeInteger(entry.cards) && entry.cards >= 0);
    if (!validCount || seen.has(entry.id)) {
...
function projectMemoryHealth(value) {
  ...
  try {
    ...
    return {
      ...
      namespaces,
      cards: {
        // Fail closed, like the health module's own normalizer.
        largestAgentCards: Number.isSafeInteger(value.cards.largestAgentCards) && value.cards.largestAgentCards >= 0
          ? value.cards.largestAgentCards
          : null,
        ...
        byPrimaryAgent: projectHealthCounts(value.cards.byPrimaryAgent ?? [], "primary agent", { allowUnknown: true }),
      },
...
function stateForReembeddingStep(state, step) {
  // Idle: nothing runs. "next" names the first step to take; "current" read
  // as "in progress" right above the line "No active re-embedding migration".
  if (!state) return step === "dry-run" ? "next" : "pending";
 ...
 */
// 7.12.48: geminte Skills fuer das Dashboard. Alles auf den Karten ist
// LLM-Ausgabe aus Gedaechtnis-Auszuegen: Whitelist fuer Status, feste
// Laengen fuer Text, Zahlen nur als endliche Werte.
const SKILL_PROPOSAL_STATUS = new Set(["pending_review", "activation_partial", "active"]);
const SKILL_WORKSHOP_MODES = new Set(["host", "on", "off"]);
const SKILL_WORKSHOP_HOST_MODES = new Set(["auto", "propose", "off"]);
...
function projectSkillProposal(value) {
  if (!value || typeof value !== "object") return null;
  const id = typeof value.id === "string" && /^[0-9a-f-]{36}$/i.test(value.id) ? value.id : "";
  if (!id || !SKILL_PROPOSAL_STATUS.has(value.status)) return null;
  const evidence = value.evidence && typeof value.evidence === "object" ? value.evidence : {};
  const memories = Array.isArray(evidence.memoryIds) ? evidence.memoryIds.length : 0;
  const skillPath = typeof value.activation?.skillPath === "string" ? value.activation.skillPath : "";
  const discoveredAt = typeof value.proposedAt === "string" && !Number.isNaN(Date.parse(value.proposedAt))
    ? new Date(value.proposedAt).toISOString()
    : "";
  return {
    id,
    skillName: boundedText(value.skillName, 80),
    title: boundedText(value.skillTitle, 160) || boundedText(value.skillName, 80) || id,
    status: value.status,
    bound: typeof value.openClawWorkshop?.proposalId === "string" && value.openClawWorkshop.proposalId.length > 0,
    location: /[\\/]workshop-skills[\\/]/.test(skillPath) ? "workshop" : skillPath ? "workspace" : "",
    category: boundedText(value.category, 40),
    grade: boundedText(evidence.grade, 80),
    confidence: finiteOrNull(evidence.llmConfidence),
    score: finiteOrNull(evidence.score),
    memories,
    discoveredAt,
    description: boundedText(value.description, 600),
    benefit: boundedText(value.benefit, 400),
    instructions: boundedText(value.instructions, 4000),
    examples: (Array.isArray(value.examples) ? value.examples : [])
      .map((example) => boundedText(example, 300))
      .filter(Boolean)
      .slice(0, 8),
  };
}

export function projectSkillWorkshop(value) {
  const source = value && typeof value === "object" ? value : {};
  const counts = { pending: 0, active: 0, rejected: 0 };
  const workspaces = [];
  for (const workspace of Array.isArray(source.workspaces) ? source.workspaces : []) {
    const id = boundedText(workspace?.id, 120) || "default";
    const agents = [];
    for (const agent of Array.isArray(workspace?.agents) ? workspace.agents : []) {
      const agentId = typeof agent?.agentId === "string" && /^[a-z0-9][a-z0-9_-]{0,63}$/i.test(agent.agentId) ? agent.agentId : "";
      if (!agentId) continue;
      const proposals = [];
      for (const raw of Array.isArray(agent.proposals) ? agent.proposals : []) {
        if (raw?.status === "rejected") { counts.rejected += 1; continue; }
        const proposal = projectSkillProposal(raw);
        if (!proposal) continue;
        if (proposal.status === "active") counts.active += 1; else counts.pending += 1;
        proposals.push(proposal);
      }
      // Offene zuerst, dann aktive; innerhalb der Gruppe die neuesten oben.
      proposals.sort((left, right) => (
        (left.status === "active") - (right.status === "active")
        || right.discoveredAt.localeCompare(left.discoveredAt)
      ));
      agents.push({ agentId, proposals });
    }
    if (agents.length > 0) workspaces.push({ id, agents });
  }
  return {
    available: source.available === true,
    mode: SKILL_WORKSHOP_MODES.has(source.mode) ? source.mode : "host",
    hostMode: SKILL_WORKSHOP_HOST_MODES.has(source.hostMode) ? source.hostMode : "auto",
...
function projectObsidianVault(value) {
  ...
  const configuredCount = Number.isSafeInteger(value.configuredCount) && value.configuredCount >= 0
    ? value.configuredCount
    : (value.configured === true ? 1 : 0);
  return {
    configured: value.configured === true,
    configuredCount,
    confirmed: value.confirmed === true,
    // `requireVaultPathConfirmation: false` means the bridge acts without a
    // receipt; the page must not call such a target "not yet confirmed".
 ...
 * @param {object} options Runtime configuration and aggregate observations.
 * @returns {object} Operator view; unknown primary-agent card counts remain null.
 */
export function buildControlPlaneProjection({
  ...
  obsidianVault = null,
  skillWorkshop = null,
  // Read by the runtime and handed in as plain values: the gc job's report
  // (main agent's workspace) and one process-pressure reading.
  gcReport = null,
  pressure = null,
  // The plugin's config subtree as it stands in openclaw.json (read by the
  // runtime), so the page can mark what is saved but not yet running.
  fileConfig = null,
  // Environment for the runtime's default key variables. Injected, and empty
  // by default, so the projection stays pure: the runtime passes process.env.
  env = {},
} = {}) {
  ...
  const credentials = {};
  for (const [name, definition] of CREDENTIAL_DEFINITIONS) {
    credentials[name] = resolveCredential(config, definition, env);
  }
  ...
  return {
    ...
    featureModels: projectFeatureModels(config, hostConfig),
    chatModels: projectChatModels(config, hostConfig),
    subagentModels: projectSubagentModels(config, hostConfig),
    captureChunking: { mode: activeCaptureChunkingId(config) },
    settings: projectDashboardSettings(config),
    pending: projectPendingChanges(config, fileConfig),
    operations: projectOperations({ config, health: projectMemoryHealth(health), gcReport, pressure }),
    credentials,
    providers: {
    ...
    skillWorkshop: projectSkillWorkshop(skillWorkshop),
  };
}
