...
const CREDENTIAL_DEFINITIONS = Object.freeze([
  ["embedding", [["embedding", "apiKey"], ["embedding", "apiKeyEnv"]]],
  ["embeddingFallback", [["embedding", "fallback", "apiKey"], ["embedding", "fallback", "apiKeyEnv"]]],
  ["reranker", [["reranker", "apiKey"], ["reranker", "apiKeyEnv"]]],
  ["merging", [["merging", "apiKey"], ["merging", "apiKeyEnv"]]],
  ["knowledgePromotion", [["schicht15", "apiKey"], ["schicht15", "apiKeyEnv"]]],
  ["skillMiner", [["skillMiner", "apiKey"], ["skillMiner", "apiKeyEnv"]]],
  ["criticalPush", [["criticalPush", "apiKey"], ["criticalPush", "apiKeyEnv"]]],
  ["emotionTier3", [["emotion", "t3", "apiKey"], ["emotion", "t3", "apiKeyEnv"]]],
]);

// Capabilities that fall back to the host's own model route when they have no
// key of their own. For these, an absent key means "uses the OpenClaw default
// route", not "switched off".
const HOST_ROUTED_CREDENTIALS = Object.freeze(new Set([
  "merging",
  "knowledgePromotion",
  "skillMiner",
  "criticalPush",
...
function resolveCredential(config, paths, hostRouted) {
  ...
  return { status: "missing", source: hostRouted ? "host_route" : null, path: paths[0].join(".") };
}
...
export function buildControlPlaneProjection({
  ...
  const credentials = {};
  for (const [name, paths] of CREDENTIAL_DEFINITIONS) {
    credentials[name] = resolveCredential(config, paths, HOST_ROUTED_CREDENTIALS.has(name));
  }
