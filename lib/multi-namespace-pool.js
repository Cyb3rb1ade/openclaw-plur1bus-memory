...
import { redactError, settleSafeWarning, trySafeWarn } from "./safe-logging.js";
...
function contextualizeNamespaceError(namespace, phase, error) {
  const contextual = new Error(
    `namespace=${namespace ?? "legacy-flat"} ${phase} failed: ${redactError(error).message}`,
    { cause: error },
  );
...
async function warnShutdownFailure(logger, namespace, error) {
  const warning = trySafeWarn(
    logger,
    `memory-lancedb-namespaced namespace=${namespace ?? "legacy-flat"} phase=shutdown`,
    error,
    { namespace: namespace ?? "legacy-flat", phase: "shutdown" },
  );
  const outcome = await settleSafeWarning(warning);
  return outcome.ok
    ? null
    : contextualizeNamespaceError(namespace, "shutdown-warning", outcome.error);
}
...
/** Coordinate same-agent reads and writes across a validated namespace layout. */
export class MultiNamespacePool {
  ...
  constructor(layout, vectorDim, AgentDbPoolClass, logger = null) {
    ...
    this.logger = logger;
    this._pools = new Map();
    this._authoritativeReadPools = new Map();
  ...
  _getAuthoritativeReadPool() {
    this._assertOpen();
    const namespace = this._writeNamespace();
    const key = this.layout.mode === "legacy-flat" ? LEGACY_FLAT_KEY : namespace;
    if (this.layout.mode === "named") this._validateNamedRoutes();
    if (!this._authoritativeReadPools.has(key)) {
      const routePath = this._routePaths.get(key);
      if (!routePath) throw new Error("authoritative read route is not configured");
      const pathGuard = this.layout.mode === "named"
        ? () => this._validateNamedRoutes()
        : null;
      const secureOptions = this.layout.mode === "named"
        ...
        : {};
      this._authoritativeReadPools.set(key, new this.AgentDbPool(
        routePath,
        this.vectorDim,
        this.logger,
        { ...secureOptions, readOnly: true, pathGuard },
      ));
    }
    return this._authoritativeReadPools.get(key);
  }
  ...
  /**
   * Describe the exact active writer route without enumerating recall routes.
   * @param {string} agentId Agent whose authoritative route is being pinned.
   * @returns {{mode: string, namespace: string|null, path: string}}
   */
  authoritativeRouteDescriptor(agentId) {
    ...
    return Object.freeze({
      ...
      namespace,
      path: this._routePaths.get(key),
    });
  }

  /**
   * Lease a separate read-only pool rooted at the exact active writer route.
   * @param {string} agentId Agent identity used for route isolation.
   * @param {Function} fn Lease-bound operation.
   * @returns {Promise<*>}
   */
  async withAuthoritativeReadDb(agentId, fn) {
    this._assertOpen();
    if (typeof fn !== "function") throw new TypeError("withAuthoritativeReadDb requires a callback");
    const id = this._agentId(agentId);
    const readPool = this._getAuthoritativeReadPool();
    return this._trackOperation(() => readPool.withDb(id, fn));
  }
  ...
  async shutdown() {
    ...
    const shutdownPromise = (async () => {
      ...
      const pools = [
        ...this._pools.entries(),
        ...this._authoritativeReadPools.entries(),
      ];
      const results = await Promise.all(pools.map(async ([key, pool]) => {
        if (typeof pool.shutdown !== "function") return [];
        try {
          await pool.shutdown();
          return [];
        } catch (error) {
          const namespace = key === LEGACY_FLAT_KEY ? null : key;
          const contextual = contextualizeNamespaceError(namespace, "shutdown", error);
          const errors = [contextual];
          const loggingError = await warnShutdownFailure(
            this.logger,
            namespace,
            contextual,
          );
      ...
      this._pools.clear();
      this._authoritativeReadPools.clear();
