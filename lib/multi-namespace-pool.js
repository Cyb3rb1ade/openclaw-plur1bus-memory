...
function warnShutdownFailure(logger, namespace, message) {
  try {
    logger?.warn?.(message);
    return null;
  } catch (error) {
    ...
    const contextual = new Error(
      `namespace=${namespace ?? "legacy-flat"} shutdown-warning failed: ${cause.message}`,
      { cause },
    );
    contextual.namespace = namespace;
    contextual.phase = "shutdown-warning";
    return contextual;
  }
}
    ...
    const shutdownPromise = (async () => {
      ...
      const results = await Promise.all(pools.map(async ([key, pool]) => {
        ...
        try {
          ...
          const loggingError = warnShutdownFailure(
            this.logger,
            namespace,
