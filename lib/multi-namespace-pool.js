...
function deriveExpectedCanonicalTarget(path) {
  ...
  while (!pathEntryExists(existingAncestor)) {
    ...
    if (parent === existingAncestor) throw new Error(`No existing ancestor for path: ${path}`);
...
function establishNamedBaseDir(baseDir) {
  const pin = deriveExpectedCanonicalTarget(baseDir);
  if (!existsSync(pin.absolutePath)) {
    const beforeCreate = deriveExpectedCanonicalTarget(baseDir);
    if (beforeCreate.expectedTarget !== pin.expectedTarget) {
      throw new Error("named namespace root ancestor canonical target changed before creation");
    }
    // Create through the pinned canonical ancestor, so a deterministic swap of
    // a lexical ancestor cannot redirect mkdir. The post-check binds the
    // configured spelling to that exact target.
    mkdirSync(pin.expectedTarget, { recursive: true });
  }
  const canonicalTarget = realpathSync(pin.absolutePath);
  if (canonicalTarget !== pin.expectedTarget) {
    throw new Error("named namespace root canonical target changed during creation");
  }
...
export class MultiNamespacePool {
  ...
  constructor(layout, vectorDim, AgentDbPoolClass, logger = null) {
    ...
    if (this.layout.mode === "legacy-flat") {
      ...
      this._canonicalBaseDir = establishNamedBaseDir(this.layout.baseDir);
  ...
  _validateNamedRoutes() {
    const currentBaseDir = realpathSync(this.layout.baseDir);
    if (currentBaseDir !== this._canonicalBaseDir) {
  ...
  _getPool(namespace) {
    ...
    if (!this._pools.has(key)) {
      ...
      const child = readOnly
        ? new this.AgentDbPool(routePath, this.vectorDim, this.logger, { readOnly: true, pathGuard })
        : new this.AgentDbPool(routePath, this.vectorDim, this.logger, { pathGuard });
