...
export class Tier3LLMClassifier {
   ...
   * @param {string} [options.model] — model name to use (default: "kimi-for-coding")
   * @param {Function} [options.callLlm] — plugin-internal LLM fn (messages) => string; preferred over apiKey
   */
  constructor(options = {}) {
  ...
  async classify(text, source = "user", tier1Result = null) {
    ...
    if (this._callLlm) {
      try {
        responseText = (await this._callLlm(messages)) || "";
      } catch (err) {
