...
const DEFAULT_MAX_SOUL_BACKUPS = 5;

export function soulRuntimeRules(version = "4.2.18") {
  return [
...
function renderSoulBlock(version = "4.2.18") {
...
export function patchSoulMd(path, options = {}) {
  if (!mutationAllowed(options.mutationPolicy, "soul_write")) {
    return {
      ok: true,
      changed: false,
      applied: false,
      reason: "mutation_policy_denied",
      plannedActions: [{ capability: "soul_write", path }],
    };
  }
  ...
  const version = options.version || "4.2.18";
  ...
  if (existing && options.backup !== false) writeFileSync(`${target}.bak-plur1bus-soul-${Date.now()}-${process.pid}`, existing, "utf8");
  const tmp = `${target}.tmp-${process.pid}-${Date.now()}`;
  writeFileSync(tmp, next, "utf8");
  renameSync(tmp, target);
  if (existing && options.backup !== false) pruneSoulBackups(target, options.maxBackups);
  return { ok: true, changed: true, path: target };
}

function pruneSoulBackups(target, maxBackups = DEFAULT_MAX_SOUL_BACKUPS) {
  const limit = Number.isFinite(Number(maxBackups))
    ? Math.max(0, Math.floor(Number(maxBackups)))
    : DEFAULT_MAX_SOUL_BACKUPS;
  const dir = dirname(target);
  const prefix = `${basename(target)}.bak-plur1bus-soul-`;
