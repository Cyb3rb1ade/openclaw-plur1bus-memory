 ...
 */
export function buildMoodStyleDirective(mood) {
  try {
    if (!mood || typeof mood !== "object") return null;
    const dominant = mood.dominant;
    if (!dominant || !KNOWN_DIMS.has(dominant)) return null;

    const intensity = KNOWN_INTENSITIES.has(mood.intensity) ? mood.intensity : "mittel";
    const toneSentence = TONE_BY_DIM[dominant][intensity];
    if (!toneSentence) return null;

    const parts = [toneSentence];

    const energySentence = ENERGY_BY_TREND[mood.trend];
    if (energySentence) parts.push(energySentence);

    parts.push(NO_LABEL_INSTRUCTION);
