...
/**
 * Return the largest complete nonnegative card count, or unknown (never coerce
 * null to zero). A present row that is unreadable makes the whole answer
 * unknown — and so does a missing row, which is why the health scan has to
 * vouch for the inventory with `agentCountsComplete`: a partition whose count
 * failed leaves no row behind to inspect. Anything but an explicit true is
 * unknown, so a layer that drops the flag refuses rather than permits.
 */
export function largestKnownAgentCount(snapshot) {
  if (snapshot?.cards?.agentCountsComplete !== true) return null;
  const rows = snapshot?.cards?.byAgent;
  if (!Array.isArray(rows) || rows.length === 0
      || rows.some(row => !Number.isSafeInteger(row?.cards) || row.cards < 0)) return null;
  return Math.max(...rows.map(row => row.cards));
}
