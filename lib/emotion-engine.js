...
export class EmotionEngine {
  ...
  constructor(config = {}) {
    ...
    const hasInjectedCompletion = typeof config.tier3?.callLlm === "function";
    const hasDirectProvider = Boolean(
      typeof config.tier3?.model === "string"
      && config.tier3.model.trim()
      && (config.tier3?.apiKey || config.tier3?.openaiClient),
    );
    /** @type {boolean} — Budget-Gate: t3 darf nur laufen wenn explizit enabled + mindestens ein vollständiger Provider */
    this._t3Enabled = config.tier3?.enabled !== false && (hasInjectedCompletion || hasDirectProvider);
  ...
  get _t3() {
    if (!this._tier3) {
      const injectedCallLlm = this._config.tier3?.callLlm || null;
      this._tier3 = new Tier3LLMClassifier({
        ...(typeof injectedCallLlm !== "function"
          && typeof this._config.tier3?.model === "string"
          && this._config.tier3.model.trim()
          ? { model: this._config.tier3.model.trim() }
          : {}),
   ...
   *   - Default ("beim kleinsten Zweifel Tier 3"):
   *     1. Try tier1.
   *     2. If tier1 returns null → tier2 → if tier2 confidence < escalationConfidence → tier3.
   *     3. If tier1 is ambivalent (|valence| < 0.2) or confidence < 0.5 → tier2.
   *        If T1/T2 emotions disagree → eskaliere immer zu Tier 3.
   *        Else: wenn bestes Ergebnis < escalationConfidence → tier3.
   *     4. Wenn tier1 confidence >= escalationConfidence → tier1 direkt.
   *     5. Sonst → tier3 (mit Timeout-Schutz und Fallback auf lokales Ergebnis).
   *
   * @param {string} text
  ...
  async analyze(text, source = "user", forceTier = null, context = {}) {
    throwIfAborted(context?.signal, "emotion analysis aborted");
    ...
    if (forceTier === 1) {
      ...
      result = await this._tier3Only(text, source, context);
    } else {
      result = await this._defaultRouting(text, source, context);
    }
  ...
  async _tier3Only(text, source, context = {}) {
    ...
    return this._t3.classify(text, source, null, context);
  }
  ...
  async _defaultRouting(text, source, context = {}) {
    ...
    if (!t1) {
      ...
      return this._maybeT3(text, source, t2, context);
    }
    ...
    if (t1Ambivalent || t1Weak) {
      if (!this._t2Enabled) {
        return this._maybeT3(text, source, t1, context);
      }
      const t2 = this._t2.classify(text, source);
  ...
  async _maybeT3(text, source, fallback, context = {}) {
    ...
    // 7.12.22: Der Capture-Pfad laesst Tier 3 hier aus und markiert die Zeile
    // stattdessen fuer den Cron `emotion-refine`. Das lokale Ergebnis samt
    // Konfidenz ist dasselbe, das sonst die Eskalation ausgeloest haette.
    if (context?.skipTier3) return fallback || this._fallbackScore(source);
    let timeout = null;
    try {
      const result = await Promise.race([
        this._t3.classify(text, source, fallback, context),
        new Promise((resolve) => {
          timeout = setTimeout(() => resolve(null), this._t3TimeoutMs);
        }),
      ]);
