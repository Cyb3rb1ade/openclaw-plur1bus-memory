...
export function parseObsidianCommandPlan(rawTokens = [], context = {}) {
  ...
  return deepFreeze({
    ...
    // Fingerprint-and-booleans explanation of the vault receipt check, when the
    // caller resolved one. Carried so a denial can say which binding failed.
    vaultConfirmation: context.vaultConfirmation && typeof context.vaultConfirmation === "object"
      ? context.vaultConfirmation
      : null,
    mutationPolicy,
  });
}
...
/**
 * Name the gates that are closed on a policy.
 *
 * A denial used to surface as a bare "mutation_policy_denied" although the
 * policy object carries every gate value. Four independent preconditions can
 * close it, so the operator -- and anyone debugging -- had no way to tell an
 * unconfirmed vault from a dry run. All returned values are booleans and a mode
 * name; nothing here exposes a path, an identity or a secret.
 *
 * @param {object} policy Canonical Obsidian mutation policy.
 * @returns {string[]} Closed gate names, empty when the policy permits writes.
 */
export function closedMutationGates(policy) {
  if (!policy || policy.kind !== POLICY_KIND) return ["policy_missing"];
  const closed = [];
  if (policy.applyMode !== true) closed.push(`mode_not_apply(${String(policy.mode)})`);
  if (policy.dryRun !== false) closed.push("dry_run");
  if (policy.allowWrite !== true) closed.push("allow_write_disabled");
  if (policy.vaultConfirmed !== true) closed.push("vault_not_confirmed");
  if (policy.actionConfirmed !== true) closed.push("action_not_confirmed");
  return closed;
}
