...
export function createMemoryTurnRouteRegistry({
  ...
  const prune = () => {
    ...
    const expiredClaimSessions = new Set([...claimed.values()].filter((ticket) => !(ticket.expiresAt > current)).map((ticket) => ticket.sessionKey));
    for (const sessionKey of expiredClaimSessions) taintAndClearSession(sessionKey);
  };
  const evictPending = () => {
  ...
  const evictClaimed = () => {
    while (claimed.size > maxClaimed) taintAndClearSession(claimed.values().next().value.sessionKey);
  };
  ...
  function observeReplyDispatch(event = {}) {
    ...
    try {
      ...
      const sender = assertSame("senderId", [
        rawSender,
        rawString(raw?.ChannelContext, ["senderId"], INPUT_LIMITS.USER_ID, "senderId"),
      ...
      const rawChat = assertSame("chatId", [
        ...
        rawString(raw?.ChannelContext, ["chatId"], INPUT_LIMITS.CHAT_ID, "chatId"),
        rawString(raw?.ChannelContext?.chat, ["id"], INPUT_LIMITS.CHAT_ID, "chatId"),
      ]);
      if (!rawChat) throw new Error("incomplete dispatch chat");
  ...
  return Object.freeze({
    ...
    clear() { pending.length = 0; runIndex.clear(); claimed.clear(); tainted.clear(); globalTaintUntil = 0; },
    pendingCount() { prune(); return pending.length; },
    stateCounts() { prune(); return Object.freeze({ pending: pending.length, runIndex: runIndex.size, claimed: claimed.size, tainted: tainted.size, globalTaint: globalTaintUntil > now() }); },
  });
}
...
export async function resolveHostHookMemoryContext(hookCtx, {
  ...
  try {
    ...
    const nestedSender = assertSame("hook sender", [
      validatedIdentity(hookCtx?.channelContext?.senderId, INPUT_LIMITS.USER_ID, "senderId", { allowNumber: true }),
      validatedIdentity(hookCtx?.channelContext?.sender?.id, INPUT_LIMITS.USER_ID, "senderId", { allowNumber: true }),
    ]);
    if (!nestedSender) throw new Error("sender_duplicate_missing");
    ...
    const nestedChat = assertSame("hook chat", [
      validatedIdentity(hookCtx?.channelContext?.chatId, INPUT_LIMITS.CHAT_ID, "chatId", { allowNumber: true }),
      validatedIdentity(hookCtx?.channelContext?.chat?.id, INPUT_LIMITS.CHAT_ID, "chatId", { allowNumber: true }),
    ]);
    if (!nestedChat) throw new Error("chat_duplicate_missing");
    assertSame("hook chat", [chatId, nestedChat]);
    const explicitHookThread = assertSame("hook thread", [
      validatedIdentity(hookCtx?.messageThreadId, INPUT_LIMITS.THREAD_ID, "threadId", { allowNumber: true }),
      validatedIdentity(hookCtx?.channelContext?.threadId, INPUT_LIMITS.THREAD_ID, "threadId", { allowNumber: true }),
    ]);
