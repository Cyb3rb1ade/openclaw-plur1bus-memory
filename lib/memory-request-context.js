...
export function resolveCanonicalWorkspacePrincipal(bindings, workspaceAliases = EMPTY_WORKSPACE_ALIASES) {
  ...
  for (const raw of [bindings?.explicitId, bindings?.explicitKey]) {
    if (!raw) continue;
    principals.push(lookupWorkspaceAlias(maps.aliases, raw) || normalizeWorkspaceTarget(raw, "workspace identity"));
  }
...
export function createMemoryTurnRouteRegistry({
  ...
  const retireClaimedRun = (runId, ticket) => {
    ...
    retiredRuns.set(runId, Object.freeze({ sessionKey: ticket.sessionKey, expiresAt: now() + ttlMs }));
    while (retiredRuns.size > maxClaimed) retiredRuns.delete(retiredRuns.keys().next().value);
  };
  const prune = () => {
    const current = now();
    ...
    for (const [runId, ticket] of [...claimed]) if (!(ticket.expiresAt > current)) retireClaimedRun(runId, ticket);
  };
  const evictPending = () => {
