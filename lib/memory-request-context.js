 ...
 * Host routing support is injected by the host (HostServices.routing).
 */
...
import { getProcessSingleton } from "./process-singleton.js";
...
// Grammar words no route provider may claim (fix round 1): the peer-kind
// vocabulary the session-key/target parser reserves (:511, :579), plus the
// other route-shaped keywords elsewhere in this module — "main" (the default
// workspace alias, WORKSPACE_KEY_PREFIX), "cron" (the non-user trigger and
// isCronCommandContext's channel value) and "webchat" (the headless-route
// marker in resolveHostHookPrincipal). A provider named one of these would be
// indistinguishable from that grammar at a call site that switches on it.
const RESERVED_ROUTE_PROVIDER_NAMES = new Set([...SUPPORTED_PEER_KINDS, "main", "cron", "webchat"]);
...
export function registerRouteProvider(name) {
  ...
  if (RESERVED_ROUTE_PROVIDER_NAMES.has(normalized)) {
    throw new Error(`"${normalized}" is a reserved name and cannot be registered as a channel`);
  }
...
export function normalizeChatKind(value) {
...
/**
 * Readable, path-free labels for the shared workspace pools an alias snapshot
 * describes. The dashboard counts shared cards per pool key (`w-<hash>`),
 * which tells an operator nothing; this maps each key to the alias that names
 * it (`main`) and each pre-alias directory identity of the same workspace to
 * `<alias>.dir`. Labels satisfy the public id contract and never carry a path.
 *
 * @param {object} snapshot Normalized workspace alias snapshot.
 * @returns {Array<{poolKey: string, label: string, identity: string}>}
 */
export function describeWorkspacePoolLabels(snapshot = EMPTY_WORKSPACE_ALIASES) {
  const normalized = normalizeAndFreezeWorkspaceAliases(snapshot);
  const labelByKey = new Map();
  for (const entry of normalized.aliases) {
    const current = labelByKey.get(entry.workspaceKey);
    if (!current || entry.alias.length < current.length || (entry.alias.length === current.length && entry.alias < current)) {
      labelByKey.set(entry.workspaceKey, entry.alias);
    }
  }
  const out = [];
  const seen = new Set();
  const push = (identity, label) => {
    if (!/^[A-Za-z][A-Za-z0-9._:-]{0,127}$/.test(label)) return;
    const poolKey = workspacePoolKey(identity);
    if (seen.has(poolKey)) return;
    seen.add(poolKey);
    out.push(Object.freeze({ poolKey, label, identity }));
  };
  for (const [identity, label] of labelByKey) push(identity, label);
  for (const entry of normalized.paths) {
    const label = labelByKey.get(entry.workspaceKey);
    if (label) push(`${WORKSPACE_DIR_PREFIX}${entry.path}`, `${label}.dir`);
  }
  return Object.freeze(out);
}

/**
 * Readable, id-free labels for the shared user pools the host configuration
 * implies. A user principal hashes channel, account and user id; the config
 * names the allowed direct-message users per channel account (`allowFrom`)
 * and binds accounts to agents, so each such user has a predictable pool.
 * The label names the agent, channel and account (`main.telegram.default`),
 * numbered when an account admits several users; the user id itself never
 * appears. Ambiguous or wildcard bindings yield the channel and account only.
 *
 * @param {object} hostConfig Host configuration with `channels` and `bindings`.
 * @returns {Array<{poolKey: string, label: string}>}
 */
/**
 * Agents with their own channel binding are the primary agents (Bernd,
 * Bernhardine, Heisenberg); subagents never carry a binding. Sorted, unique.
 */
export function describePrimaryAgentIds(hostConfig = {}) {
  const ids = new Set();
  for (const binding of Array.isArray(hostConfig?.bindings) ? hostConfig.bindings : []) {
    const agentId = binding?.agentId;
    if (typeof agentId === "string" && /^[A-Za-z][A-Za-z0-9._:-]{0,127}$/.test(agentId)) ids.add(agentId);
  }
  return Object.freeze([...ids].toSorted((left, right) => left.localeCompare(right)));
}

export function describeUserPoolLabels(hostConfig = {}) {
  const out = [];
  const seen = new Set();
  const agentFor = (channel, accountId) => {
    const agents = new Set();
    for (const binding of Array.isArray(hostConfig?.bindings) ? hostConfig.bindings : []) {
      if (binding?.match?.channel !== channel) continue;
      const account = binding?.match?.accountId;
      if (account === accountId && typeof binding?.agentId === "string" && binding.agentId) agents.add(binding.agentId);
    }
    return agents.size === 1 ? [...agents][0] : "";
  };
  for (const [channel, channelCfg] of Object.entries(hostConfig?.channels || {})) {
    if (!channelCfg || typeof channelCfg !== "object") continue;
    for (const [accountId, accountCfg] of Object.entries(channelCfg.accounts || {})) {
      const allowed = Array.isArray(accountCfg?.allowFrom) ? accountCfg.allowFrom : [];
      const users = allowed
        .map((value) => (typeof value === "number" && Number.isSafeInteger(value) ? String(value) : value))
        .filter((value) => typeof value === "string" && /^[A-Za-z0-9_.:@-]{1,128}$/.test(value));
      if (users.length === 0) continue;
      const agentId = agentFor(channel, accountId);
      const base = [agentId, channel, accountId].filter(Boolean).join(".");
      users.forEach((userId, index) => {
        let ctx;
        try {
          ctx = resolveMemoryRequestContext({ agentId: agentId || "default", channel, accountId, userId, chatId: userId });
        } catch {
          return;
        }
        if (!ctx.userPrincipal) return;
        const label = users.length > 1 ? `${base}.${index + 1}` : base;
        if (!/^[A-Za-z][A-Za-z0-9._:-]{0,127}$/.test(label)) return;
        const poolKey = userPoolKey(ctx.userPrincipal);
        if (seen.has(poolKey)) return;
        seen.add(poolKey);
        out.push(Object.freeze({ poolKey, label }));
      });
    }
  }
  return Object.freeze(out);
...
export function createHostRoutingLoader({ importRouting = missingHostRouting, logger = null } = {}) {
...
export function createHostIncognitoSessionClassifier({
  importRouting = missingHostRouting,
  logger = null,
} = {}) {
  let loadPromise = null;
  return async function classifyHostIncognitoSession(sessionKey) {
    if (!loadPromise) {
      // Do not cache the rejection: a failed load would otherwise keep every
      // later turn unclassifiable, which fails closed and disables durable
      // capture for the whole gateway lifetime. Clear it so a retry can heal.
...
export async function resolveHostCommandMemoryContext(commandCtx, {
  ...
  // Only a sessionId the host has persisted is conversation identity. The
  // Telegram command path hands over `entry?.sessionId || randomUUID()`: while
  // a chat has no persisted session, every command carries a fresh random id.
  // A two-step confirmation is two commands, so binding that value made the
  // conversation principal differ between prepare and confirm and confirm
  // answered binding_mismatch. When a session lookup is wired, use what the
  // store holds and treat an absent entry as "no session"; without a lookup,
  // keep trusting the call, which is what hosts and suites without a store
  // rely on -- and what keeps a genuinely wrong session refused.
  // The lookup answers { available, entry }. "Capability missing" and "entry
  // missing" are different situations: a host without a session store keeps
  // today's behaviour, a host whose store has no entry for this chat is exactly
  // the case where the call's sessionId is a per-command random.
  let sessionId = validatedIdentity(commandCtx?.sessionId, INPUT_LIMITS.SESSION_ID, "sessionId");
  if (typeof resolveSessionEntry === "function") {
    const lookup = await resolveSessionEntry({ agentId: facts.agentId, sessionKey: facts.sessionKey });
    if (lookup?.available === true) {
      sessionId = validatedIdentity(lookup.entry?.sessionId, INPUT_LIMITS.SESSION_ID, "persisted sessionId");
    }
  }
...
/**
 * Describe the direct conversation a canonical session key names, or null
 * when the key is not a provider-bound direct chat (main, heartbeat, cron,
 * group). Only public routing functions are used; nothing is guessed.
 */
export function describeDirectSessionRoute(sessionKey, routingCapability) {
  const capability = validateRoutingCapability(routingCapability);
  let facts;
  try {
    facts = parseSessionFacts(sessionKey, capability);
  } catch {
    return null;
  }
  const route = facts.route;
  if (!route || route.peerKind !== "direct" || !route.provider || !facts.accountId || !route.peerId) return null;
  return Object.freeze({
    agentId: facts.agentId,
    sessionKey: facts.sessionKey,
...
/**
 * Resolve the memory context an operator names by session key. A direct chat
 * session yields the same identity-bound context the chat command path
 * builds for that conversation (user, channel, account, conversation
 * principal), so identity-bound steps such as the vault confirmation can be
 * driven from the CLI for exactly that conversation. Any other session kind
 * yields the plain agent/workspace context it always did.
 */
export async function resolveSessionOwnerMemoryContext({
  sessionKey,
  ...
  const routingCapability = await routingLoader();
  const direct = describeDirectSessionRoute(sessionKey, routingCapability);
  if (direct) {
  ...
  if (typeof resolveAgentWorkspaceDir !== "function") throw new Error("resolveAgentWorkspaceDir capability is required");
  const workspaceDir = await resolveAgentWorkspaceDir(config, agentId);
  return resolveMemoryRequestContext({ agentId, sessionKey, workspaceDir }, { requireWorkspace, workspaceAliases });
}
...
function dispatchBodyForCommandCheck(raw) {
  for (const key of ["CommandBody", "BodyForCommands", "BodyForAgent", "Body", "RawBody", "Transcript"]) {
    const value = raw?.[key];
    if (typeof value === "string" && value.trim()) return value.slice(0, COMMAND_BODY_LIMIT).trimStart();
  }
  return "";
}

export const SHARED_TURN_ROUTES_KEY = "plur1bus.memoryTurnRoutes.v1";

/**
 * 7.12.36: Ein Register je Prozess statt je Plugin-Instanz. Der Host legt je
 * Agenten-Lauf eine neue Instanz an; Dispatch-Beobachtung (Gateway-Instanz)
 * und Prompt-Hook (Lauf-Instanz) muessen dasselbe Register sehen.
 * @param {object} options wie createMemoryTurnRouteRegistry; nur der erste Aufruf im Prozess wirkt.
 */
export function getSharedMemoryTurnRouteRegistry(options = {}) {
  return getProcessSingleton(SHARED_TURN_ROUTES_KEY, () => createMemoryTurnRouteRegistry(options));
}
...
export function createMemoryTurnRouteRegistry({
  ...
  const prune = () => {
    ...
    // 7.12.39: Ein abgelaufenes, nie beanspruchtes Ticket wird still verworfen
    // statt die Session zu sperren. Entsteht regulaer, wenn eine zweite
    // Nachricht waehrend eines laufenden Turns eintrifft: der Host baut dafuer
    // keinen eigenen Prompt, das Ticket bleibt liegen. Der Taint bei Ablauf
    // schuetzte nichts (ein abgelaufenes Ticket ist ohnehin nie beanspruchbar,
    // `ticket_expired`), sperrte aber die naechsten Turns fuer weitere ttlMs
    // ohne user-Scope (Bernd 10.09.2026 19:32/19:33). Reihenfolge- und
    // Identitaetskonflikte tainten weiterhin.
    for (let index = pending.length - 1; index >= 0; index--) {
      const ticket = pending[index];
      if (ticket.expiresAt > current) continue;
      pending.splice(index, 1);
      if (ticket.runId) runIndex.delete(ticket.runId);
      noteOutcome(ticket.sessionKey, "observe", "expired_unclaimed");
    }
  ...
  // 7.12.33: letzter Ausgang je Session (Beobachtung und Anspruch), fuer die
  // Fallback-Warnung des Prompt-Hooks. Begrenzt wie die anderen Register.
  // 7.12.34: Beobachtung und Anspruch getrennt merken — der Anspruch
  // ueberschrieb sonst den Grund der Beobachtung (Bernd 10.09.2026 14:27:
  // `claim:no_ticket … pending=0`, ohne dass sichtbar war, warum kein Ticket
  // entstand).
  const outcomes = new Map();
  function noteOutcome(sessionKey, stage, reason) {
    const key = typeof sessionKey === "string" && sessionKey ? sessionKey : "?";
    const entry = outcomes.get(key) || {};
    entry[stage] = { reason: String(reason).slice(0, 80), at: now() };
    outcomes.delete(key);
    outcomes.set(key, entry);
    if (outcomes.size > maxPending) outcomes.delete(outcomes.keys().next().value);
  }
  function explain(sessionKey) {
    const entry = outcomes.get(String(sessionKey || ""));
    if (!entry) return "none";
    const parts = [];
    for (const stage of ["observe", "claim"]) {
      if (entry[stage]) parts.push(`${stage}:${entry[stage].reason}`);
    }
    return parts.join("|") || "none";
  }
  function lastObserve(sessionKey) {
    return outcomes.get(String(sessionKey || ""))?.observe?.reason || "";
  }

  function observeReplyDispatch(event = {}) {
    prune();
    const raw = event?.ctx && typeof event.ctx === "object" ? event.ctx : event;
    // 7.12.33: jeder stille Ausstieg hinterlaesst einen Grund (explain()), den
    // der Prompt-Hook in seine Fallback-Warnung schreibt. Bis dahin war
    // `reason=ticket` nicht weiter aufloesbar (Bernd, 10.09.2026 13:30).
    const noteKey = [event?.sessionKey, raw?.SessionKey, raw?.sessionKey].find((value) => typeof value === "string" && value.trim()) || "";
    const skip = (reason) => { noteOutcome(noteKey, "observe", reason); return undefined; };
    if (event.isTailDispatch === true || raw.isTailDispatch === true || event.isTail === true || raw.tail === true) return skip("tail_dispatch");
    if (raw.CommandTurn !== undefined) {
      if (!raw.CommandTurn || typeof raw.CommandTurn !== "object") return skip("command_turn_invalid");
      if (raw.CommandTurn.kind !== "normal" || raw.CommandTurn.source !== "message") return skip(`command_turn:${raw.CommandTurn.kind}/${raw.CommandTurn.source}`);
      if (raw.CommandTurn.body !== undefined && raw.CommandTurn.body !== raw.CommandBody) return skip("command_turn_body_mismatch");
      if (raw.CommandSource !== undefined) return skip("command_source_present");
    } else if (raw.CommandSource === "native" || raw.CommandSource === "text") {
      return skip(`command_source:${raw.CommandSource}`);
    }
    if (event.isCommand === true || raw.isCommand === true || event.nativeCommand === true) return skip("is_command");
    // 7.12.32: Der Text dient nur der Frage "ist das ein /Kommando?". Bis 7.12.31
    // musste CommandBody vorhanden und kuerzer als 4000 Zeichen sein — Foto
    // ohne Bildunterschrift (leerer CommandBody) und lange Sprachnachrichten
    // (Transkript) bekamen deshalb kein Ticket, der Prompt-Hook fiel mit
    // reason=ticket auf den unauthentifizierten Kontext zurueck (Eva,
    // 09./10.09.2026, jede Sprach- und Bildnachricht). Jetzt gilt der erste
    // vorhandene Text, ohne Laengenpflicht. Ganz ohne Text (auch kein Body,
    // kein Transkript) bleibt es wie bisher kein beobachteter Nutzer-Turn.
    const body = dispatchBodyForCommandCheck(raw);
    if (!body) {
      safeWarn(logger, "memory-turn-routes.command-body", "empty_dispatch_body", { reason: "empty" });
      return skip("empty_body");
    }
    if (body.startsWith("/")) return skip("slash_command");
    ...
    try {
      ...
      if (!provider || !accountId || !sender || !chatId) return skip("incomplete_identity");
      ...
      if (runId && runIndex.has(runId)) {
        ...
        return skip("run_already_pending");
      }
      if (runId && claimed.has(runId)) {
        ...
        return skip("run_already_claimed");
      }
      if (runId && retiredRuns.has(runId)) return skip("run_retired");
      if (globalTaintUntil > now() || tainted.has(ticket.sessionKey)) return skip("session_tainted");
      pending.push(ticket);
      if (runId) runIndex.set(runId, ticket);
      evictPending();
      noteOutcome(ticket.sessionKey, "observe", `registered:${runId ? "run" : "session"}`);
    } catch (error) {
      ...
      return skip(`invalid_identity:${String(error?.message || "invalid").slice(0, 48)}`);
    }
    return undefined;
  }
  ...
  function claimForPrompt(hookCtx, proofMode, verifyTicket) {
    ...
    const deny = (reason) => { noteOutcome(session.sessionKey, "claim", reason); return null; };
    if (globalTaintUntil > now()) return deny("global_taint");
    if (tainted.has(session.sessionKey)) return deny("session_tainted");
    if (retiredRuns.has(runId)) return deny("run_retired");
    if (claimed.has(runId)) {
      ...
      if (prior.sessionKey !== session.sessionKey || typeof verifyTicket !== "function" || verifyTicket(prior) !== true) {
        ...
        return deny("prior_claim_mismatch");
      }
      return prior;
    }
    const head = pending.find((item) => item.sessionKey === session.sessionKey);
    let ticket = proofMode === "turn-run" ? runIndex.get(runId) : head;
    if (!ticket) return deny(`no_ticket:${proofMode}:pending=${pending.length}`);
    if (!(ticket.expiresAt > now())) return deny("ticket_expired");
    if (ticket.sessionKey !== session.sessionKey || ticket !== head) {
      ...
      return deny(ticket.sessionKey !== session.sessionKey ? "ticket_session_mismatch" : "ticket_not_head");
    }
    if (typeof verifyTicket !== "function" || verifyTicket(ticket) !== true) {
      taintAndClearSession(session.sessionKey);
      return deny("ticket_verify_failed");
    }
    const index = pending.indexOf(ticket);
    if (index < 0) {
      taintAndClearSession(session.sessionKey);
      return deny("ticket_vanished");
    }
    pending.splice(index, 1);
    if (ticket.runId) runIndex.delete(ticket.runId);
    ...
    if (!evictClaimed()) return deny("claimed_overflow");
    noteOutcome(session.sessionKey, "claim", "claimed");
    return record;
  }
...
// 7.12.30: OpenClaw 2026.9 traegt Kanal, Konto, Ziel und Thread eines
// Session-Eintrags unter `entry.delivery` ({route, context, origin}); die
// Felder `deliveryContext`/`origin`/`lastChannel`/`lastAccountId`/`lastTo`/
// `lastThreadId` von 2026.7/8 fehlen dort. Der Hook-Kontext fiel deshalb in
// JEDEM DM-Turn mit `provider` auf den unauthentifizierten Basis-Kontext
// zurueck (kein User-Principal, user-Scope-Erinnerungen wie Traeume nie
// erinnert). Beide Formen werden gleichwertig gelesen; Widersprueche zwischen
// vorhandenen Aliassen bleiben ein Fehler.
function sessionEntryDeliveryView(entry) {
  ...
  const origin = asObject(delivery?.origin);
  const route = asObject(delivery?.route);
  const routeTarget = asObject(route?.target);
  const routeThread = asObject(route?.thread);
  return {
    providerAliases: [
      [asObject(entry?.deliveryContext), "channel"], [asObject(entry?.origin), "provider"], [asObject(entry), "lastChannel"],
      [context, "channel"], [origin, "provider"], [route, "channel"],
    ],
    accountAliases: [
      [asObject(entry?.deliveryContext), "accountId"], [asObject(entry?.origin), "accountId"], [asObject(entry), "lastAccountId"],
      [context, "accountId"], [origin, "accountId"], [route, "accountId"],
    ],
    targets: [entry?.deliveryContext?.to, entry?.origin?.to, entry?.lastTo, context?.to, origin?.to, routeTarget?.to],
    threads: [entry?.deliveryContext?.threadId, entry?.origin?.threadId, entry?.lastThreadId, context?.threadId, origin?.threadId, routeThread?.id],
  };
}

export async function resolveHostHookPrincipal(hookCtx, {
  getSessionEntry,
  workspaceAliases = EMPTY_WORKSPACE_ALIASES,
  ...
  // 7.12.30: Diagnose fuer den Fallback — bis dahin stand im Log nur
  // `{"reason":"provider"}`, ohne Werte und ohne Dauer.
  const startedAt = Date.now();
  const diag = { step: "start", provider: "", entryProvider: "", entryAccount: "", sessionEntryMs: 0, ticket: "" };
  try {
    ...
    if (!hasTransportIdentity) {
      ...
      return { trust: "inferred", memoryCtx: resolveMemoryRequestContext({
    ...
    diag.step = "session_entry";
    // Der Host liest den Eintrag synchron aus der Agenten-SQLite (gemessen
    // 10–90 ms; die 15-s-Timeouts vom 09./10.09.2026 kamen aus dem davor
    // laufenden Reply-Outcome-Handler). Die Dauer wird trotzdem gemessen und
    // ab 1 s gemeldet; ein Timeout laesst sich einem synchronen Aufruf nicht
    // ueberstuelpen.
    const entryStartedAt = Date.now();
    const entry = await getSessionEntry({ agentId: session.agentId, sessionKey: session.sessionKey, readConsistency: "latest" });
    diag.sessionEntryMs = Date.now() - entryStartedAt;
    if (diag.sessionEntryMs >= 1000) {
      try { logger?.warn?.(`memory-request-context: session entry read took ${diag.sessionEntryMs} ms (agent=${session.agentId})`); } catch (_) { /* best-effort */ }
    }
    diag.step = "entry";
    ...
    // 7.12.30: Liefert der Host kein messageProvider, gilt der Provider des
    // kanonischen Session-Keys (host-verifiziert; die Ticket-Pruefung unten
    // vergleicht ihn ohnehin gegen den Dispatch).
    const provider = normalizeProvider(base.channel, capability) || session.route?.provider || "";
    diag.provider = provider;
    const view = sessionEntryDeliveryView(entry);
    const entryProvider = strictPresentAliases(view.providerAliases, (value) => normalizeProvider(value, capability), "entry provider");
    diag.entryProvider = entryProvider;
    if (!provider || provider !== entryProvider) throw new Error("provider");
    const entryAccount = strictPresentAliases(view.accountAliases, (value) => normalizeAccount(value, capability), "entry account");
    diag.entryAccount = entryAccount;
    if (!entryAccount) throw new Error("account");
    diag.step = "target";
    const entryTargets = view.targets
    ...
    const entryThread = assertSame("entry thread", [
      ...view.threads.map((value) => validatedIdentity(value, INPUT_LIMITS.THREAD_ID, "threadId", { allowNumber: true })),
      targetThread,
    ]);
    diag.step = "ticket";
    ...
    if (!claimed) {
      diag.ticket = typeof turnRoutes?.explain === "function" ? turnRoutes.explain(session.sessionKey) : "n/a";
      throw new Error("ticket");
    }
    return { trust: "proved", memoryCtx: resolveMemoryRequestContext({
      agentId: base.agentId,
      workspaceDir: base.workspaceDir,
      sessionKey: base.sessionKey,
      sessionId: base.sessionId,
    ...
    safeWarn(logger, "memory-request-context.hook", error, {
      reason: String(error?.message || "invalid").slice(0, 64),
      step: diag.step,
      provider: diag.provider,
      entryProvider: diag.entryProvider,
      entryAccount: diag.entryAccount,
      ticket: diag.ticket,
      sessionEntryMs: diag.sessionEntryMs,
      ms: Date.now() - startedAt,
    });
    return { trust: "inferred", memoryCtx: base };
  }
}
