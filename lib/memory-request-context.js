...
export async function resolveHostHookMemoryContext(hookCtx, {
  ...
  try {
    ...
    if (!hasTransportIdentity) {
      ...
      const parsed = capability.parseAgentSessionKey(base.sessionKey);
      if (!parsed?.agentId || !parsed?.rest || safeAgentId(parsed.agentId) !== base.agentId) {
