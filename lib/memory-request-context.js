...
function conversationPrincipalFor(facts) {
  // A conversation is channel, account, chat and thread -- not the individual
  // call. sessionId used to be hashed in here, and it changes between two
  // consecutive messages from the same user in the same chat. That made every
  // two-step confirmation (vault-confirm, semantic-discovery) unreachable over
  // a chat channel: prepare and confirm are necessarily two messages, so the
  // principal always differed and confirm answered binding_mismatch.
  //
  // Removing it does not widen the check. Channel, account, peer kind, parent
  // peer, conversation and thread stay bound here, and the one-time
  // confirmation separately binds userId, userPrincipal, agentId and
  // workspaceIdentity. A different user, chat or thread still fails.
  const payload = [
    "plur1bus-confirmation", 2, facts.agentId, facts.sessionKey,
    facts.channel, facts.accountId,
...
export async function resolveHostCommandMemoryContext(commandCtx, {
  ...
  return resolveMemoryRequestContext({
    ...
    conversationPrincipal: facts.channel && facts.accountId
      ? conversationPrincipalFor(facts)
      : "",
