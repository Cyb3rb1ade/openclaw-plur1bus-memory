...
export async function resolveHostHookMemoryContext(hookCtx, {
  ...
  try {
    ...
    const transportIdentityPresent = [
      hookCtx?.messageProvider,
      hookCtx?.channel,
      hookCtx?.accountId,
      hookCtx?.senderId,
    ...
    const officialHeadlessRoute = !transportIdentityPresent
      && parsedAgentSession?.agentId
      && parsedAgentSession?.rest
      && derivedHeadlessRouteIds.every((value) => value === parsedAgentSession.rest);
    const hasTransportIdentity = transportIdentityPresent
      || (derivedHeadlessRouteIds.length > 0 && !officialHeadlessRoute);
