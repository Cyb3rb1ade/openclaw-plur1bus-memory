 ...
 * den Telegram-Outbound-Adapter des Hosts an dasselbe Ziel, das der Cron-Plan
 * aus der Channel-Konfiguration ableitet. Scheitert etwas, bekommt der Aufrufer
 * die noch nicht gesendeten Karten als Text zurück und liefert sie wie bisher
 * über die Cron-Zustellung aus.
 */

import { buildCriticalButtonMessages } from "./critical-buttons.js";
import { deriveDeliveryFromChannelConfig } from "./setup/feature-cron-plan.js";
 ...
 */
export async function deliverCriticalButtonPush({ agentId, result, config, loadAdapter, warning = "", logger }) {
  ...
  const delivery = deriveDeliveryFromChannelConfig(agentId, config);
  if (!delivery || delivery.channel !== "telegram") return fallback("no_telegram_target");
