...
function normalizeSelection(value, generation, { target = false } = {}) {
  const candidate = value ?? (target
    ? { mode: "generation", generation }
    : { mode: "generation", generation });
  if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) {
    throw new Error("invalid reembedding generation selection");
  }
  if (candidate.mode === "legacy" && !target && Object.keys(candidate).length === 1) {
    return Object.freeze({ mode: "legacy" });
  }
  if (
    candidate.mode !== "generation"
    || Object.keys(candidate).sort().join(",") !== "generation,mode"
    || migrationId(candidate.generation, "selection generation") !== generation
  ) throw new Error("invalid reembedding generation selection");
  return Object.freeze({ mode: "generation", generation });
}
...
export async function createReembeddingPlan(request = {}, dependencies = {}) {
  ...
  const plan = deepFreeze({
    ...
    source: {
      ...
      selection: normalizeSelection(sourceRaw.selection, sourceGeneration),
      ...
      ...(sourceRaw.secretRef ? { secretRef: normalizeSecretRef(sourceRaw.secretRef) } : {}),
      tables,
    },
    target: {
      generation: targetGeneration,
      selection: normalizeSelection(undefined, targetGeneration, { target: true }),
