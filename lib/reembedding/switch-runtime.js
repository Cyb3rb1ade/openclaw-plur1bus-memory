...
export function createReembeddingSwitchRuntime({
  ...
  const switchGeneration = ({ id, token } = {}) => exclusive(async () => {
    ...
    try {
      ...
      await mutateSelection(selectionFrom(record, "target"));
      targetSelected = true;
      const probe = validateProbe(await probeRuntime({
        expectedGeneration: record.target.generation,
        migrationId: id,
        rollback: false,
      }));
      ...
      if (targetSelected) {
        try {
          await mutateSelection(selectionFrom(record, "source"));
          validateProbe(await probeRuntime({
            expectedGeneration: record.source.generation,
            migrationId: id,
            rollback: true,
          }));
        } catch (rollbackError) {
  ...
  const planManualRollback = async ({ completedId, newMigrationId } = {}) => {
    ...
    return Object.freeze({
      ...
      target: Object.freeze({ fingerprint: record.source.fingerprint }),
