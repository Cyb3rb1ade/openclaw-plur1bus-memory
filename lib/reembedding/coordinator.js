...
import {
  ...
  verifyMigrationConfirmationBinding,
...
const DEFAULT_LIMITS = Object.freeze({
  ...
  batchSize: 8,
  concurrency: 1,
  // One real provider batch can consume most of the Gateway liveness window.
  // Every further durable batch therefore requires an explicit resume RPC.
  maxBatchesPerOperation: 1,
  maxCalls: 100_000,
...
export function createReembeddingCoordinator({
  ...
  const targetProvider = async (record) => {
    if (provider && providerPlanDigest !== record.planDigest) {
      const staleProvider = provider;
      await staleProvider.shutdown?.();
      provider = null;
      providerPlanDigest = null;
    }
    if (!provider) {
      const candidate = await createTargetProvider({
        fingerprint: record.target.fingerprint,
        secretRef: record.target.secretRef,
      });
      if (!candidate || (typeof candidate.embedBatch !== "function" && typeof candidate.embedPassage !== "function" && typeof candidate.embed !== "function")) {
        const validationError = new Error("target embedding provider is invalid");
        try {
          await candidate?.shutdown?.();
        } catch (cleanupError) {
          throw new AggregateError(
            [validationError, cleanupError],
            "invalid target embedding provider cleanup failed",
          );
        }
        throw validationError;
      }
      provider = candidate;
      providerPlanDigest = record.planDigest;
    }
    if (providerPlanDigest !== record.planDigest) {
  ...
  const runBatches = async (initialRecord, token) => {
    if (!verifyMigrationConfirmationBinding(token, initialRecord.confirmation)) {
  ...
  const apply = ({ id, token } = {}) => exclusive(async () => {
    ...
    const validConfirmation = record.state === "planned"
      ? verifyMigrationConfirmation(token, record.confirmation, now())
      : verifyMigrationConfirmationBinding(token, record.confirmation);
    if (!validConfirmation) throw new Error("invalid or expired reembedding confirmation");
  ...
  const resume = ({ id, token } = {}) => exclusive(async () => {
    ...
    if (!record) throw new Error("reembedding migration not found");
    if (!verifyMigrationConfirmationBinding(token, record.confirmation)) {
      throw new Error("invalid or expired reembedding confirmation");
    }
