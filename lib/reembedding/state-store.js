...
export function createMigrationStateStore({ stateRoot, now = Date.now } = {}) {
  ...
  const create = (input) => enqueue(async () => {
    ...
    if (Object.values(state.migrations).some((record) => !TERMINAL_STATES.has(record.state))) {
    ...
    const next = {
      ...
      migrations: { ...state.migrations, [input.id]: candidate },
    };
