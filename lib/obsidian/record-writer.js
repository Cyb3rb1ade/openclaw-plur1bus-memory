...
export function writeRecordNote(rawConfig, record, options = {}) {
  if (!mutationAllowed(options.mutationPolicy, "vault_write")) {
    return { ok: true, path: "", changed: false, applied: false, reason: "mutation_policy_denied" };
  }
  ...
  const generated = formatFrontmatter(
    ...
    buildManagedBlock({
      ...
      version: options.version || "4.2.18",
  ...
  if (existsSync(targetPath)) {
    ...
    if (replaced.changed === false) {
      // P1-1: Managed Block unverändert → keine Datei-Operation, mtime bleibt.
      return { ok: true, path: relPath, changed: false };
    }
