...
export function buildProjectHub(rawConfig, topic, options = {}) {
  if (!mutationAllowed(options.mutationPolicy, "vault_write")) {
    return { ok: true, topic, generated: [], applied: false, reason: "mutation_policy_denied" };
  }
  ...
  for (const [file, body] of Object.entries(files)) {
    ...
    if (existsSync(targetPath)) {
      ...
      const replaced = replaceManagedBlock(parsed.body, { id: blockId, body, version: "4.2.18" });
      ...
      const block = buildManagedBlock({ id: blockId, body, version: "4.2.18" });
