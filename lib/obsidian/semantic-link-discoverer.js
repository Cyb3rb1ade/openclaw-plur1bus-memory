  ...
  for (const record of queue) {
    ...
    try {
      ...
      if (status === 429) {
        ...
        if (dirty && canWrite) saveLinkIndex(vaultPath, existingIndex);
        return {
          processed,
          skipped,
          unchanged,
          ...
          batchAborted: true,
          ...(dirty && !canWrite ? { blocked: true, reason: "confirm_required" } : {}),
        };
      }
...
export async function runSemanticLinkIndexBatch(rawConfig = {}, options = {}) {
  ...
  const result = {
    ...
    wouldWrite: Boolean(options.confirm && complete && changed && records.length > 0),
    updated: 0,
    unchanged: 0,
    skipped: 0,
    conflicts: [],
  ...
  const manifestPath = writeSemanticLinkIndexManifest(rawConfig, plan, options);
  saveLinkIndex(vaultPath, { version: "1", entries: normalizedEntries });
