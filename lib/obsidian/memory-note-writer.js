...
export function writeMemoryNotes(rawConfig, records, options = {}) {
  ...
  const effectiveDryRun = dryRun || !mutationAllowed(options.mutationPolicy, "vault_write");

  const { reviewPath } = resolveReviewPath(rawConfig, ".");
  const memoriesDir = join(reviewPath, "memories");

  if (!effectiveDryRun && !existsSync(memoriesDir)) {
  ...
  for (const record of records) {
    ...
    // Check if unchanged
    if (!effectiveDryRun && existsSync(filePath)) {
