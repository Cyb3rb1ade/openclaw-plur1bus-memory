...
const GENERATED_DEEP_ANALYSIS_TYPES = new Set([
  "duplicate_candidate",
  "impact_analysis",
  "provenance",
]);

const GENERATED_DEEP_ANALYSIS_PATH_PARTS = new Set([
  "duplicate-candidates",
  "impact-analysis",
  "provenance",
]);

const GENERATED_DEEP_ANALYSIS_NAME_RE = /^(?:dup|impact|prov)(?:-|$)|^(?:impact-prov|prov-impact|dup-impact)(?:-|$)/;

export function isGeneratedDeepAnalysisRecord(record = {}) {
  const type = record.plur1bus_type || record.type || "";
  if (GENERATED_DEEP_ANALYSIS_TYPES.has(type)) return true;
  if (type === "source" && record.scope === "dashboard_only") return true;

  const rel = String(record.path || record.filePath || "");
  const parts = rel.split(/[\\/]+/).filter(Boolean);
  if (parts.some((part) => GENERATED_DEEP_ANALYSIS_PATH_PARTS.has(part))) return true;

  const name = parts.length ? parts[parts.length - 1].replace(/\.md$/i, "") : "";
  return GENERATED_DEEP_ANALYSIS_NAME_RE.test(name);
}
...
export function buildRecordIndex(rawConfig, options = {}) {
  ...
  const records = [...merged.values()].filter((record) => !options.deepReviewInput || !isGeneratedDeepAnalysisRecord(record));
  const byType = {};
  const byId = {};
  const byMemoryId = {};
