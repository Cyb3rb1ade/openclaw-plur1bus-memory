...
export function acquireJobLock(lockPath, opts = {}) {
  ...
  if (existsSync(lockPath)) {
    ...
    if (age > staleMs) {
      try { unlinkSync(lockPath); } catch (err) { console.warn(`[job-lock] stale cleanup failed: ${redactError(err).message}`); }
    } else {
  ...
  try {
    ...
    // Always close the descriptor, even if the lock-file write throws (e.g. disk
    // full) — otherwise the fd leaks for the process lifetime.
    closeSync(fd);
  }
  return lockPath;
}
...
export function releaseJobLock(lockPath) {
  ...
  } catch (err) { console.warn(`[job-lock] release failed: ${redactError(err).message}`); }
}
