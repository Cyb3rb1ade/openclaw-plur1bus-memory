...
import { safeUuid, safeUuidList, safeAgentId, safeStatus, safeTimestamp } from "./sql-safety.js";
...
export function createDbAdapter(opts = {}) {
        ...
        if (vector && vector.length > 0) {
          ...
          try {
            ...
            rows = typeof builder.where === "function"
              ? await builder.where("status = 'active' OR status IS NULL").limit(limit).toArray()
  ...
  async function updateCardType(agent, id, type) {
    ...
    const safeType = safeStatus(type);
    try {
      await table.update({
