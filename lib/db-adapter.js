...
import { isDreamCard } from "./critical-review.js";
...
// Gleiche Lesart wie findPendingCriticalReviews: nur status "active", leer
// oder fehlend zählt als lebende Karte. Tombstones (MemoryDB.tombstone setzt
// status="deleted") sind keine Kandidaten für Klassifikation oder Push.
function isActiveStatus(value) {
  return value == null || value === "" || String(value) === "active";
}
 ...
 *   - `{ from, to }` → explizite Epoch-ms-Grenzen, additiv zu den benannten
 *     Ranges (E1 Task 4 fix round 1: MemoryOps.list's `since`/`until` sind
 *     beliebige Epoch-ms-Werte, kein benanntes Vokabular). `to` fällt ohne
 *     endliche Zahl auf `now` zurück; ein nicht-endliches `from` fällt auf
 *     die bestehende Standard-Range (letzte 7 Tage) zurück.
 */
export function computeCutoff(range, now = Date.now()) {
  if (range && typeof range === "object") {
    if (Number.isFinite(range.from)) {
      return { from: range.from, to: Number.isFinite(range.to) ? range.to : now };
    }
...
export function createDbAdapter(opts = {}) {
  ...
  /**
   * Stellt sicher, dass die memories-Tabelle die Spalte `chunkGroupId` besitzt.
   *
   * Seit 7.12.70 wird eine Nachricht mit mehreren unabhaengigen Aussagen als
   * mehrere Zeilen gespeichert (lib/memory-chunking.js). Diese Zeilen haengen
   * ueber `chunkGroupId` zusammen; die Recall-Seite deckelt darueber, wie viele
   * Teile derselben Nachricht in eine Trefferliste duerfen.
   *
   * Leer = nicht aufgeteilt. Die Recall-Seite faellt dann auf `sourceTurnId`
   * zurueck, so wie vor der Aufteilung. Bestandszeilen werden nicht angefasst.
   *
   * Idempotent und non-destruktiv, wie die uebrigen ensureXColumns-Funktionen.
   * Laeuft beim Oeffnen der Tabelle, also fuer JEDEN Agenten einzeln — ein
   * selten benutzter Store wird migriert, sobald er das erste Mal gebraucht
   * wird, und vorher schreibt auch niemand hinein.
   */
  async function ensureChunkColumns(agent, table) {
    if (!table) return;
    const schemaKey = `${basePath}:${agent}:chunk`;
    if (schemaExtended.get(schemaKey) === true) return;
    try {
      const schema = await timedRead(table.schema(), `db-adapter.chunk-schema:${agent}`);
      const fieldNames = schema.fields.map((f) => f.name);
      const columns = [
      ...
      for (const col of columns) {
        if (!fieldNames.includes(col.name)) {
          await timedWrite(table.addColumns([col]), `db-adapter.chunk-addColumns:${agent}:${col.name}`);
        }
      }
      schemaExtended.set(schemaKey, true);
    } catch (err) {
      logger.warn?.(`db-adapter: chunk schema extension failed for '${agent}': ${err.message}`);
    }
  }
  ...
  const resolveRawTable = opts.getTable || (async (agent) => {
    ...
    try {
      ...
      // A miss (no directory, no table, open failure or timeout) is answered
      // for this call only and never cached: the agent's first capture
      // creates the table later in the same process, and a cached null would
      // make getCard/resolveCandidates answer not-found for it until the
      // process restarts (E1 final review I1).
      ...
      try {
        ...
        // Strong read consistency, as MemoryDB.connect has (engine/store/memory-db.js):
        // without an interval this cached table keeps the version it was
        // opened with, so a row the pooled MemoryDB writes afterwards — the
        // new version /correct's safeUpdate stores (E1 Task 8), a capture —
        // stays invisible to getCard/resolveCandidates for the process's life.
        const db = await timedWrite(lancedbModule.connect(dbDir, { readConsistencyInterval: 0 }), "lancedb.connect");
        const names = await timedRead(db.tableNames(), "lancedb.tableNames");
        if (!names.includes(TABLE_NAME)) {
          // Not cached (see above), so release the connection instead of
          // opening a new one per lookup until the first capture (E1 re-review N3).
        ...
        await ensureChunkColumns(agent, table);
  ...
  async function updateCard(agent, id, newContent) {
    ...
    try {
      const updated = {
        ...
        // LanceDB liefert int64-Spalten als BigInt; `1n + 1` wirft
        // "Cannot mix BigInt and other types". Genau daran scheiterte jedes
        // /correct — in 21 000 Zeilen entstand nie eine zweite Version, und
        // der Fehler wurde am 09.09.2026 durch ein getipptes /correct sichtbar.
        versionNumber: Number(existing.versionNumber ?? 1) + 1,
  ...
  async function optimizeTable(agent, optimizeOpts = {}) {
    ...
    // 7.12.31: Fragment-Statistik vor/nach dem Lauf (LanceDB ≥ 0.20 hat
    // table.stats(); aeltere Builds liefern null), Timeout konfigurierbar.
    const { timeoutMs: rawTimeoutMs, ...lanceOpts } = optimizeOpts || {};
    const timeoutMs = Number.isFinite(Number(rawTimeoutMs)) && Number(rawTimeoutMs) >= 10_000 ? Number(rawTimeoutMs) : 600_000;
    const snapshot = async () => {
      if (typeof table.stats !== "function") return null;
      try { return await withTimeout(table.stats(), 30_000, `db-adapter.stats:${agent}`); } catch (_) { return null; }
    };
    // 7.12.45: Ein Rewrite verliert gegen gleichzeitige Updates (Capture,
    // Outcome-Dynamik) mit "Retryable commit conflict … Please retry" —
    // 11.09.2026 bei bernhardine und heisenberg. LanceDB meint das woertlich:
    // bis zu drei Anlaeufe mit kurzer Pause, innerhalb des Gesamtbudgets.
    const isRetryableConflict = (err) => /retryable commit conflict|preempted by concurrent|please retry/i.test(String(err?.message || err));
    const retryDelayMs = Number.isFinite(Number(optimizeOpts?.retryDelayMs)) ? Math.max(0, Number(optimizeOpts.retryDelayMs)) : 3000;
    // 13.09.2026: drei Versuche im Abstand von drei Sekunden verloren jede
    // Nacht gegen Emotions-, Dynamik- und Zaehler-Updates ("preempted by
    // concurrent transaction Update"); Bernds Tabelle stand bei 1522
    // Fragmenten. Zwoelf Versuche mit wachsendem Abstand (3 s … 60 s) finden
    // eine Schreiblluecke; die Gesamtwartezeit bleibt unter dem Timeout.
    const maxAttempts = Number.isFinite(Number(optimizeOpts?.maxAttempts)) && Number(optimizeOpts.maxAttempts) >= 1
      ? Math.floor(Number(optimizeOpts.maxAttempts)) : 12;
    const retryDelayFor = (attempt) => Math.min(60_000, retryDelayMs * 2 ** Math.max(0, attempt - 1));
    ...
    try {
      ...
      for (;;) {
        ...
        try {
          ...
          throwIfTimeout(err);
          const nextDelayMs = retryDelayFor(attempts);
          if (!isRetryableConflict(err) || attempts >= maxAttempts || Date.now() + nextDelayMs >= deadline) throw err;
          logger.warn?.(`db-adapter: optimizeTable fuer '${agent}' kollidierte mit einem gleichzeitigen Update (Versuch ${attempts}/${maxAttempts}), neuer Anlauf in ${nextDelayMs} ms`);
          await new Promise((resolve) => setTimeout(resolve, nextDelayMs));
        }
      }
      const ms = Date.now() - startedAt;
      const after = await snapshot();
      return { ok: true, stats, before, after, ms, attempts };
    } catch (err) {
      throwIfTimeout(err);
  ...
  async function findRecentUnclassified(agent, { sinceMinutes } = {}) {
    ...
    try {
      ...
      // Träume ausschließen (25.09.2026): die Dream-Engine speichert ihre
      // Erzählungen als Karten mit type "memory"; der Cron klassifizierte einen
      // Traum über Eva als "beziehung" und pushte ihn als Critical.
      const dreamClause = [
        columnNames.includes("origin") ? " AND (origin IS NULL OR origin != 'dream')" : "",
        columnNames.includes("memoryClass") ? " AND (memoryClass IS NULL OR memoryClass != 'dream')" : "",
      ].join("");
      const unclassifiedClause = UNCLASSIFIED_TYPES
        .map((t) => (t === "" ? "type = ''" : `type = ${sqlString(t)}`))
        .concat("type IS NULL")
        .join(" OR ");
      // Tombstones ausschließen (05.09.2026): zwei per memory_forget gelöschte
      // Karten blieben unklassifiziert, wurden vom Cron klassifiziert und als
      // Critical gepusht — der Review-Pfad (status = 'active') sah sie nie,
      // der Push nannte Referenzen, die es zum Prüfen nicht gab.
      const activeClause = hasStatusColumn
        ? " AND (status = 'active' OR status IS NULL OR status = '')"
        : "";
      const whereClause = hasTypeColumn
        ? `createdAt >= ${cutoffMs} AND (${unclassifiedClause})${activeClause}${dreamClause}`
        : `createdAt >= ${cutoffMs}${activeClause}${dreamClause}`;
