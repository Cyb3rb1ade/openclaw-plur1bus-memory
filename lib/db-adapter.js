    ...
    try {
      ...
      for (;;) {
        ...
        try {
          ...
          throwIfTimeout(err);
          if (!isRetryableConflict(err) || attempts >= maxAttempts || Date.now() + retryDelayMs >= deadline) throw err;
          logger.warn?.(`db-adapter: optimizeTable fuer '${agent}' kollidierte mit einem gleichzeitigen Update (Versuch ${attempts}/${maxAttempts}), neuer Anlauf in ${retryDelayMs} ms`);
