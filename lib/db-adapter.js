...
// Read-Ops (query/vectorSearch/countRows) bekommen 10s,
// Write-Ops (add/delete/update) bekommen 15s.
...
function rowToCard(row) {
  if (!row) return null;
  const agentId = row.agentId || row.storedBy || "";
  const workspaceId = row.workspaceId || row.workspaceKey || "";
  ...
  return {
    ...
    agentId,
    workspaceId,
    storedBy: row.storedBy || agentId,
    workspaceKey: row.workspaceKey || workspaceId,
  };
}
