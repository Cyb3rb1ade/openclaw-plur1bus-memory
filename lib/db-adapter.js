...
export function createDbAdapter(opts = {}) {
  ...
  async function findUnconfirmedCritical(agent, { olderThan = 0, scanLimit } = {}) {
    ...
    try {
      ...
      // Pushdown: vorher deckte die where-Klausel nur die Zeitbedingung ab, der
      // selektive Typfilter lief erst in JS hinter dem Deckel. `createdAt <= X`
      // trifft auf großen Tabellen fast alles — der 200er-Präfix sind die
      // ältesten Zeilen, und eine Critical-Karte darunter ist unwahrscheinlich.
      const grenze = Number(scanLimit) > 0 ? Number(scanLimit) : PENDING_CRITICAL_LIMIT;
      const typeClause = CRITICAL.map((t) => `type = ${sqlString(t)}`).join(" OR ");
      const whereClause = `createdAt <= ${safeOlderThan} AND (${typeClause})`;

      let rows = null;
      if (typeof table.query().where === "function") {
        try {
          rows = await timedRead(
            table.query().where(whereClause).limit(grenze).toArray(),
      ...
      if (rows === null) {
        rows = await timedRead(
          table.query().where(`createdAt <= ${safeOlderThan}`).limit(Math.max(grenze, PENDING_CRITICAL_SCAN_LIMIT)).toArray(),
          `db-adapter.findUnconfirmedCritical.scan:${agent}`,
        );
      }
      return rows
  ...
  async function findPendingCriticalReviews(agent, listOpts = {}) {
    ...
    const isPending = (r) => CRITICAL.includes(r.type)
      && r.confirmed !== true && r.confirmed !== 1
    ...
    try {
      ...
      if (typeof table.query().where === "function") {
        try {
          let builder = table.query().where(whereClause).limit(PENDING_CRITICAL_LIMIT);
      ...
      if (rows === null) {
        // Fallback für Query-Builder ohne where(): breiter holen und in JS filtern.
        let builder = table.query().limit(PENDING_CRITICAL_SCAN_LIMIT);
        if (selectColumns.length > 0) builder = builder.select(selectColumns);
        rows = await timedRead(builder.toArray(), `db-adapter.findPendingCriticalReviews.scan:${agent}`);
      }
      return rows
        .filter(isPending)
        .map((r) => {
