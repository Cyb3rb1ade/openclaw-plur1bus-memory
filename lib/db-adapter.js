...
export function createDbAdapter(opts = {}) {
  ...
  async function ensureClassificationColumns(agent, table) {
    ...
    try {
      ...
      logger.warn?.(`db-adapter: schema-lesen für '${agent}' fehlgeschlagen: ${err.message}`);
      return;
    }
    ...
    for (const col of columnsToAdd) {
      ...
      try {
        ...
        logger.warn?.(`db-adapter: addColumns('${col.name}') für '${agent}' fehlgeschlagen: ${err.message}`);
        allOk = false;
      }
    }

    // Nur als "erledigt" markieren wenn ALLE Spalten erfolgreich waren.
    // Bei Fehlern wird beim nächsten Aufruf erneut migriert.
    if (allOk) {
