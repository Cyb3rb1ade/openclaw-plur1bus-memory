...
function saveBundleRecord(loaded) {
  ...
  writeReviewBundle(paths, record.bundle, record.items, record.hygieneItems || [], maintenance, null, {
...
export function expireStaleBundles(rawConfig = {}, options = {}) {
  if (!mutationAllowed(options.mutationPolicy, "review_write")) {
    return { expired: 0, expiredIds: [], applied: false, reason: "mutation_policy_denied" };
  }
...
export function writeCommandsMarkdown(rawConfig = {}, options = {}) {
  if (!mutationAllowed(options.mutationPolicy, "vault_write")) {
    return { written: false, applied: false, reason: "mutation_policy_denied" };
  }
  const paths = resolveObsidianBridgePaths(rawConfig, options);
...
export async function handleObsidianBridgeCommand(tokens = [], context = {}) {
  ...
  let plan;
  try {
    plan = parseObsidianCommandPlan(tokens, {
      memoryCtx,
      mode: cfg.mode,
  ...
  try {
    ...
    if (command === "impact" && sub === "analyze") return commandResult(analyzeImpact(commandConfig, tokens[2] || "all", { agentId, workspaceKey, records: context.records || [], mutationPolicy: plan.mutationPolicy }));
