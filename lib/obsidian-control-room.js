...
function writeReviewBundle(paths, bundle, items, hygieneItems, maintenance, mood = null) {
  ...
  atomicWriteText(target, renderReviewBundleMarkdown(bundle, [...items, ...safeHygieneItems], maintenance, mood));
...
export async function prepareReviewBundle(rawConfig = {}, options = {}) {
  ...
  if (paths.ok && paths.cfg.allowWrite !== false && paths.cfg.morningReview.writeReviewBundle !== false) {
    written = writeReviewBundle(paths, bundle, items, hygieneItems, maintenance, options.mood ?? null);
...
export function isObsidianCommandDestructive(tokens = []) {
  ...
  // A --dry-run flag makes an otherwise destructive command read-only.
  if (lower.includes("--dry-run")) return false;

  // Fail-safe: dangerous flags anywhere in the command line trigger auth.
  const dangerousFlags = ["--apply", "--write", "--delete", "--force-soul", "--migrate-soul-memory-rules", "--allow-delete"];
  if (lower.some((token) => dangerousFlags.includes(token))) return true;

  // Subcommands that are inherently memory-authoritative or config/system-affecting.
  const command = lower[0] || "";
  const sub = lower[1] || "";

  if (command === "init" && sub === "workspaces") return true;
  if (command === "review" && (sub === "apply" || sub === "quickapply")) return true;
  if (command === "soul" && sub === "patch") return true;
  if (command === "cron" && (sub.startsWith("install") || sub.startsWith("uninstall"))) return true;

  return false;
}

export async function handleObsidianBridgeCommand(tokens = [], context = {}) {
  // AUTH-003: destructive Obsidian subcommands require the same authorization as
  // /forget, /correct, /enable, /disable, and /plur1bus setup.
  if (isObsidianCommandDestructive(tokens)) {
  ...
  const explicitAgentId = context.commandCtx?.agentId || context.agentId || "";
  const explicitWorkspaceKey = context.commandCtx?.workspaceKey || context.workspaceKey || "";
  const agentId = explicitAgentId || "main";
  const workspaceKey = explicitWorkspaceKey || inferWorkspaceKeyForAgent(explicitAgentId) || "main";
  ...
  try {
    ...
    if (command === "evening-review" || command === "evening-deep-review") {
      ...
      const eveningWorkspaceKey = explicitWorkspaceKey || inferWorkspaceKeyForAgent(explicitAgentId);
      const result = runEveningDeepReview(commandConfig, { agentId: eveningAgentId, workspaceKey: eveningWorkspaceKey, workspaceDir: context.workspaceDir, records: context.records, items: context.items });
    ...
    if (command === "weekly") {
      ...
      return commandResult(reviewBundleSummary(await prepareReviewBundle(commandConfig, { agentId, workspaceKey, reviewProfiles: ["maintenance", "project_manager"] }), "PLUR1BUS Weekly Review"));
    }
    ...
    if (command === "rotate") {
      ...
      const dryRun = !tokens.includes("--apply");
      const allowDelete = tokens.includes("--allow-delete");
      const action = tokens.includes("--delete") ? "delete" : "move";
      const maxAgeDaysRaw = parseCommandOption(tokens, "--max-age-days", String(rawConfig.deepMaintenance?.archiveAfterDays ?? 30));
      const maxSizeMBRaw = parseCommandOption(tokens, "--max-size-mb", "");
