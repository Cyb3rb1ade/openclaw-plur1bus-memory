...
function routeDependencies(options) {
  return {
    ...
    ...(options?.agentModels && Object.keys(options.agentModels).length
      ? { agentModels: Object.freeze({ ...options.agentModels }) }
      : {}),
...
export function isLlmRouteAvailable(route) {
  return Boolean(route && (route.kind !== LLM_ROUTE_KINDS.UNAVAILABLE || Object.keys(route.agentModels || {}).length));
}
...
export async function completeFeatureLlm(
  ...
  // Selection is local to this call. Never mutate a route shared by agents,
  // or forward an agent override to OpenClaw's authority-bound LLM handle.
  const selectionAgent = callOptions?.agentId || route?.callContext?.agentId || callOptions?.resultCacheContext?.scopeId;
  const selectedModel = selectionAgent && Object.hasOwn(route?.agentModels || {}, selectionAgent)
    ? normalizeNonEmptyString(route.agentModels[selectionAgent]) : null;
  if (selectedModel) {
    route = resolveFeatureLlmRoute({ model: selectedModel, timeoutMs: route.timeoutMs, disableThinking: route.disableThinking }, {
      feature: route.feature, runtimeLlm: route.runtimeLlm, logger: route.logger,
      resultCache: route.resultCache, diagnosticsPath: route.diagnosticsPath,
    });
  }
