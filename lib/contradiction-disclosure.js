...
export function formatContradictionDisclosure(pairs, opts = {}) {
  try {
    ...
    const truncate = (str, max = MAX_CONTRADICTION_SNIPPET_CHARS) => {
      ...
      // sanitizeMemoryTextForPrompt truncates the RAW text before HTML-escaping,
      // so `s` may still exceed `max` (e.g. "&" -> "&amp;" grows 5x). Slicing the
      // already-escaped string at `max` can land mid-entity (dangling "&amp").
      // Strip any such trailing partial entity before appending the ellipsis.
      return s.slice(0, max).replace(/&[a-zA-Z]{0,5}$/, "") + "…";
    };
    ...
    if (result.length > maxBodyChars) {
      // Same entity-cut risk as the item-level truncate() above — strip a
      // dangling partial entity before trimming trailing whitespace.
      result = result.slice(0, maxBodyChars).replace(/&[a-zA-Z]{0,5}$/, "").trimEnd();
    }
