...
/**
 * Ist die Karte ein Traum der Dream-Engine (lib/dreaming/dream-narrative.js)?
 * Träume erzählen frei über Personen aus dem Gedächtnis und sind nie eine
 * Aussage des Benutzers — sie dürfen weder klassifiziert noch als Critical
 * gepusht werden (25.09.2026: Bernhardines Light-Dream über Eva kam als
 * „beziehung“ in Evas Critical-Push).
 *
 * @param {object} card
 * @returns {boolean}
 */
export function isDreamCard(card = {}) {
  return card?.origin === "dream" || card?.memoryClass === "dream";
}
...
/**
 * Typen, deren Inhalt in der Push-Karte nie gezeigt wird. Nur Geheimnisse:
 * Ein Passwort oder Token hat im Chatverlauf nichts verloren. Gesundheit und
 * Finanzen wurden bis 7.12.1 ebenfalls unterdrückt; der Push geht aber
 * ausschließlich in den Direktchat des Besitzers und zeigt dessen eigene
 * Aussage — ohne Vorschau kann niemand entscheiden, ob die Karte
 * hervorgehoben werden soll (Rückmeldung 07.09.2026). Wer die alte Politik
 * will, listet die Typen unter `criticalPush.hideTypes`.
 */
export const DEFAULT_HIDDEN_TYPES = Object.freeze(["zugang_passwort"]);
const ALWAYS_HIDDEN_TYPES = new Set(["zugang_passwort"]);

/**
 * Löst die wirksame Menge unterdrückter Typen auf: Zugangsdaten immer, dazu
 * jede konfigurierte Ergänzung.
 *
 * @param {Iterable<string>|undefined} hideTypes
 * @returns {Set<string>}
 */
export function resolveHiddenTypes(hideTypes) {
  const set = new Set(ALWAYS_HIDDEN_TYPES);
  if (hideTypes && typeof hideTypes[Symbol.iterator] === "function") {
    for (const entry of hideTypes) {
 ...
 *
 * @param {string} type
 * @param {Iterable<string>} [hideTypes] Konfigurierte Ergänzungen zu den Geheimnistypen.
 * @returns {boolean}
 */
export function isSuppressedType(type, hideTypes) {
  return resolveHiddenTypes(hideTypes).has(String(type || ""));
}
 ...
 * Zugangsdaten werden immer unterdrückt, weitere Typen nur, wenn sie in
 * `opts.hideTypes` stehen (Konfiguration `criticalPush.hideTypes`).
 *
 * @param {object} card
 * @param {object} [opts] — { lang, maxLen, hideTypes }
 * @returns {{suppressed: boolean, text: string, reason: string}}
 */
export function buildPreview(card = {}, opts = {}) {
  const lang = opts.lang || "de";
  const type = String(card?.type || "");
  if (isSuppressedType(type, opts.hideTypes)) {
    return {
      suppressed: true,
      text: "",
  ...
  // Klassifizierer-Karten tragen den Inhalt als `content` (db-adapter
  // findRecentUnclassified); der Titel ist nur der letzte Ausweg.
  const preview = sanitizePreview(card?.text || card?.summary || card?.content || card?.title || "", opts.maxLen);
...
export function buildCriticalMessage(card = {}, opts = {}) {
  ...
  const preview = buildPreview(card, { lang, hideTypes: opts.hideTypes });
