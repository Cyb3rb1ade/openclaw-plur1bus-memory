...
export function removeStopwords(query) {
  if (!query) return "";
  const raw = String(query).normalize("NFKD").replace(/[^\p{L}\p{M}\p{N}\s]/gu, " ").normalize("NFC");
...
function extractContextKeywords(topResult) {
  ...
  const raw = String(combined).normalize("NFKD").replace(/[^\p{L}\p{M}\p{N}\s]/gu, " ").normalize("NFC");
