...
export function createObsidianBridgeService(rawConfig = {}, options = {}) {
  ...
  async function rebuildDashboards({ lancedbRecords, shouldContinue } = {}) {
    ...
    if (pendingDashboardRebuild) {
      ...
      try {
        const pendingResult = await rebuildDashboards({ lancedbRecords, shouldContinue });
  ...
  function scheduleDashboardRebuild() {
    const trackTask = !dashboardRunning || !dashboardTask;
    const task = rebuildDashboards({ shouldContinue: () => running });
    if (!trackTask) return task;
    const settledTask = task.catch((err) => err).finally(() => {
      if (dashboardTask === settledTask) dashboardTask = null;
    });
    dashboardTask = settledTask;
    return task;
  }
  ...
  return {
    ...
    async start() {
      ...
      if (!cfg.watch) {
        ...
        scheduleDashboardRebuild().catch((err) => logger.warn?.(`plur1bus-obsidian-bridge: dashboard rebuild failed: ${String(err)}`));
        return;
      }
      timer = setInterval(() => {
        if (!running) return;
