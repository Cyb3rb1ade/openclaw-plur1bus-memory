...
export function scanWorkspace(workspace, options = {}) {
  ...
  return { workspace, files: files.sort((a, b) => a.relPath.localeCompare(b.relPath)), issues, skippedCount: skippedCount.value, totalBytesScanned };
}
...
export async function syncWorkspace(workspace, options = {}) {
  ...
  if (requireVaultPathConfirmation && !isVaultPathConfirmed(workspace)) {
    return {
      workspace,
...
export function createObsidianBridgeService(rawConfig = {}, options = {}) {
  ...
  async function syncOnce() {
    ...
    try {
      ...
      if (pendingSync) {
        ...
        // weil pendingSync vor dem Aufruf zurückgesetzt wird.
        return syncOnce();
      }
    }
  }
  ...
  async function rebuildDashboards({ lancedbRecords } = {}) {
    ...
    for (const workspace of workspaces) {
      try {
        ...
        if (memoryRecords.length > 0) {
          const mnResult = writeMemoryNotes(vaultCfg, memoryRecords, { logger, maxPerRun: memoryRecords.length });
        ...
        const cmdResult = writeCommandsMarkdown(vaultCfg, { logger });
        ...
        if (graphLinksCfg.enabled !== false) {
          ...
          const glResult = await writeGraphLinks(vaultCfg, mergedRecords, { logger, linkIndex });
    ...
    return { built, graphLinksUpdated: glUpdated, memoryMirror };
  }
