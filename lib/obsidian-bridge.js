  ...
  async function rebuildDashboards({ lancedbRecords } = {}) {
    ...
    for (const workspace of workspaces) {
      try {
        ...
        if (lancedbRecords?.length) {
          await writeMemoryNotes(vaultCfg, lancedbRecords, { logger });
        }
        ...
        const mergedRecords = [...allRecords, ...memoryNotes.filter(m => !allRecords.some(r => r.memory_id === m.memory_id))];

        if (mergedRecords.length === 0) continue;
