...
import { hostStateDir } from "./host-paths.js";
...
function materializableMemoryRecords(records) {
  ...
  const materializable = records.filter((record) => {
    ...
    const memoryKind = String(record.memoryKind || "").trim();
    if (memoryKind && memoryKind !== "memory") return false;
...
export function discoverLocalObsidianWorkspaceCandidates(rawConfig = {}, options = {}) {
  ...
  collectOpenClawHomeWorkspaceCandidates(options.openclawHome || hostStateDir(), candidateMap);
...
function isExplicitlyApproved(file, options = {}) {
  if (options.queueOnly === true) return false;
...
/**
 * Synchronize one configured Obsidian workspace with cooperative lifecycle cancellation.
 * @param {object} workspace Normalized workspace descriptor.
 * @param {object} options Sync, mutation-policy, and optional shouldContinue controls.
 * @returns {Promise<object>} Sync actions, issues, and scan evidence.
 */
export async function syncWorkspace(workspace, options = {}) {
  const canContinue = () => typeof options.shouldContinue !== "function" || options.shouldContinue();
  if (!canContinue()) {
    return {
      workspace,
      applied: false,
  ...
  for (const [relPath, previous] of Object.entries(state.files)) {
    ...
    if (tombstoneOnDelete && !previous.tombstonedAt) {
      const deletionApproved = options.queueOnly !== true
        && options.applyApproved === true
  ...
  if (canContinue() && manifestFiles.length > 0 && !dryRun) {
    writeBatchManifest(workspace, batchId, manifestFiles);
  }

  if (canContinue() && !dryRun) writeBridgeState(workspace, state);
  if (canContinue() && !dryRun && options.recordSyncMetrics !== false) {
    try {
...
/**
 * Create a lifecycle-owned Obsidian watcher and dashboard mirroring service.
 * @param {object} rawConfig Obsidian Bridge configuration.
 * @param {object} options Runtime loaders, mutation policies, and logger.
 * @returns {{id: string, start: Function, stop: Function, syncOnce: Function, rebuildDashboards: Function}} Host-managed service.
 */
export function createObsidianBridgeService(rawConfig = {}, options = {}) {
  ...
  const pendingSyncByGeneration = new Map();
  let stopping = false;
  const scheduledSyncTasks = new Map();
  let activeDashboardRequest = null;
  const pendingDashboardRequests = [];
  const pendingDashboardByGeneration = new Map();
  const scheduledDashboardTasks = new Map();
  let startPromise = null;
  let startPromiseIntent = null;
  ...
  const isLifecycleGenerationActive = (generation) => (
    running && !stopping && activeLifecycleGeneration === generation
  );

  async function executeSyncRequest({
    shouldContinue,
    recordSyncMetrics = true,
    queueOnly = false,
    generationToken,
  } = {}) {
    ...
    try {
      ...
      for (const workspace of workspaces) {
        ...
        results.push(await doSync(workspace, {
          ...
          recordSyncMetrics,
          queueOnly,
          memoryStore: queueOnly ? undefined : options.memoryStore,
          knowledgeUpdate: queueOnly ? undefined : options.knowledgeUpdate,
        }));
      }
      result = results;
    } catch (err) {
    ...
    if (generationToken === undefined || isLifecycleGenerationActive(generationToken)) syncFailCount = 0;
    return result;
  }

  function createSyncRequest(requestOptions, generation, manual) {
    let resolve;
    let reject;
    ...
    return { options: requestOptions, generation, manual, promise, resolve, reject };
  }

  function startNextSyncRequest() {
    activeSyncRequest = null;
    while (pendingSyncRequests.length > 0) {
      const next = pendingSyncRequests.shift();
      if (pendingSyncByGeneration.get(next.generation) === next) {
        pendingSyncByGeneration.delete(next.generation);
      }
      const canContinue = typeof next.options.shouldContinue !== "function"
        || next.options.shouldContinue();
      if (!canContinue) {
        next.resolve([]);
        continue;
      }
      if (!next.manual && syncFailCount >= MAX_CONSECUTIVE_FAILS) {
        next.resolve([]);
        continue;
      }
      startSyncRequest(next);
      break;
    }
  }

  function startSyncRequest(request) {
    activeSyncRequest = request;
    const task = executeSyncRequest(request.options);
    task.then(
      (result) => {
      ...
      (err) => {
        if (!request.manual) {
          if (isLifecycleGenerationActive(request.generation)) {
            syncFailCount += 1;
            ...
            logger.warn?.(`plur1bus-obsidian-bridge: stopping watch sync failed: ${String(err)}`);
          }
        }
        request.reject(err);
        startNextSyncRequest();
  ...
  function enqueueSyncRequest(requestOptions, { generation, manual }) {
    if (stopping) return Promise.resolve([]);
    if (activeSyncRequest) {
      if (!manual) {
        const existing = pendingSyncByGeneration.get(generation);
        if (existing) return existing.promise;
      }
      const pending = createSyncRequest(requestOptions, generation, manual);
      pendingSyncRequests.push(pending);
      if (!manual) pendingSyncByGeneration.set(generation, pending);
      return pending.promise;
    }
    const request = createSyncRequest(requestOptions, generation, manual);
    startSyncRequest(request);
    return request.promise;
  }

  function syncOnce(requestOptions = {}) {
    const manualOptions = { ...requestOptions };
    delete manualOptions.generationToken;
    delete manualOptions.shouldContinue;
    return enqueueSyncRequest(manualOptions, {
      generation: Symbol("manual-obsidian-sync-request"),
      manual: true,
    });
  }

  function trackScheduledSync(task, generation) {
    if (scheduledSyncTasks.has(task)) return task;
    scheduledSyncTasks.set(task, generation);
    task.then(
      () => scheduledSyncTasks.delete(task),
      () => scheduledSyncTasks.delete(task),
    );
    return task;
  }
  ...
  function scheduleSync(generation) {
    return trackScheduledSync(enqueueSyncRequest({
      shouldContinue: () => isLifecycleGenerationActive(generation),
      // atomicJsonUpdate has no cancellation primitive. Periodic host-owned
      // watcher runs must not enqueue telemetry that could write after stop.
      recordSyncMetrics: false,
      // Approval/apply remains an explicit command path. A periodic watcher
      // may only queue candidates and must never enter a durable memory store.
      queueOnly: true,
      generationToken: generation,
    }, { generation, manual: false }), generation);
  }

  async function executeDashboardRequest({ lancedbRecords, shouldContinue } = {}) {
    const canContinue = () => typeof shouldContinue !== "function" || shouldContinue();
    if (!serviceCanWrite) {
      return {
        applied: false,
    ...
    try {
        ...
        try {
          ...
          if (memoryRecords.length > 0) {
            ...
            if (mnResult.errors > 0) {
              ...
              workspaceErrors.push(new Error(
                `workspace ${workspace.workspaceId}: writeMemoryNotes reported ${mnResult.errors} error(s)`,
              ));
            }
          }
          ...
          workspaceErrors.push(new Error(`workspace ${workspace.workspaceId}: ${String(err)}`, { cause: err }));
        }
      }
      ...
      if (workspaceErrors.length > 0) {
        throw new AggregateError(
          workspaceErrors,
          `dashboard rebuild failed for ${workspaceErrors.length} workspace(s)`,
        );
      }
  ...
  function createDashboardRequest(requestOptions, generation, manual) {
    let resolve;
    let reject;
    const promise = new Promise((res, rej) => {
      resolve = res;
  ...
  function startNextDashboardRequest() {
    activeDashboardRequest = null;
    while (pendingDashboardRequests.length > 0) {
      const next = pendingDashboardRequests.shift();
      if (pendingDashboardByGeneration.get(next.generation) === next) {
        pendingDashboardByGeneration.delete(next.generation);
      }
      const canContinue = typeof next.options.shouldContinue !== "function"
        || next.options.shouldContinue();
      if (!canContinue) {
        next.resolve({ built: 0, graphLinksUpdated: 0, memoryMirror: null });
        continue;
      }
      if (!next.manual && dashboardFailCount >= MAX_CONSECUTIVE_FAILS) {
        next.resolve({ built: 0, graphLinksUpdated: 0, memoryMirror: null });
        continue;
      }
      startDashboardRequest(next);
      break;
    }
  }

  function startDashboardRequest(request) {
    activeDashboardRequest = request;
    const task = executeDashboardRequest(request.options);
    task.then(
      (result) => {
        if (
          result?.applied !== false
          && (request.manual || isLifecycleGenerationActive(request.generation))
        ) {
          dashboardFailCount = 0;
        }
        request.resolve(result);
        startNextDashboardRequest();
      },
      (err) => {
        if (!request.manual) {
          if (isLifecycleGenerationActive(request.generation)) {
            dashboardFailCount += 1;
            logger.warn?.(`plur1bus-obsidian-bridge: dashboard rebuild failed (${dashboardFailCount}/${MAX_CONSECUTIVE_FAILS}): ${String(err)}`);
          } else {
            logger.warn?.(`plur1bus-obsidian-bridge: stopping dashboard rebuild failed: ${String(err)}`);
          }
        }
        request.reject(err);
        startNextDashboardRequest();
  ...
  function enqueueDashboardRequest(requestOptions, { generation, manual }) {
    if (stopping) {
      return Promise.resolve({ built: 0, graphLinksUpdated: 0, memoryMirror: null });
    }
    if (activeDashboardRequest) {
      if (!manual) {
        const existing = pendingDashboardByGeneration.get(generation);
        if (existing) return existing.promise;
      }
      const pending = createDashboardRequest(requestOptions, generation, manual);
      pendingDashboardRequests.push(pending);
      if (!manual) pendingDashboardByGeneration.set(generation, pending);
      return pending.promise;
    }
    const request = createDashboardRequest(requestOptions, generation, manual);
    startDashboardRequest(request);
    return request.promise;
  }

  function rebuildDashboards(requestOptions = {}) {
    const manualOptions = { ...requestOptions };
    delete manualOptions.generationToken;
    delete manualOptions.shouldContinue;
    return enqueueDashboardRequest(manualOptions, {
      generation: Symbol("manual-obsidian-dashboard-request"),
      manual: true,
    });
  }

  function scheduleDashboardRebuild(generation) {
    const task = enqueueDashboardRequest({
      shouldContinue: () => isLifecycleGenerationActive(generation),
      generationToken: generation,
    }, { generation, manual: false });
    if (scheduledDashboardTasks.has(task)) return task;
    scheduledDashboardTasks.set(task, generation);
    task.then(
      () => scheduledDashboardTasks.delete(task),
      () => scheduledDashboardTasks.delete(task),
    );
    return task;
  }
  ...
  return {
    ...
    start() {
      ...
      if (startPromise && startPromiseIntent === lifecycleIntentVersion) return startPromise;
      if (running && !stopping) return Promise.resolve();
      const startIntent = ++lifecycleIntentVersion;
      const precedingStop = stopping && stopPromise ? stopPromise : Promise.resolve();
      const task = (async () => {
        // OpenClaw owns a strict replacement deadline. Do not publish a new
        // service owner while the old one can still mutate bridge state: if
        // the host times out this stop, it fails the replacement closed.
        await precedingStop;
        if (startIntent !== lifecycleIntentVersion) return;
        if (running) return;
        if (!serviceCanWrite) {
          return {
            applied: false,
        ...
        if (!cfg.watch) {
          const initialSyncTask = trackScheduledSync(enqueueSyncRequest({
            shouldContinue: () => isLifecycleGenerationActive(generation),
            generationToken: generation,
          }, { generation, manual: false }), generation);
          await initialSyncTask.catch((err) => logger.warn?.(`plur1bus-obsidian-bridge: sync failed: ${String(err)}`));
          if (!isLifecycleGenerationActive(generation)) return;
          scheduleDashboardRebuild(generation);
          return;
        }
        timer = setInterval(() => {
          if (!isLifecycleGenerationActive(generation)) return;
          if (syncFailCount >= MAX_CONSECUTIVE_FAILS) return;
          // The scheduled-task tracker owns rejection observation and the
          // request completion path updates failure state exactly once.
      ...
      startPromise = task;
      startPromiseIntent = startIntent;
      const clearStartPromise = () => {
        if (startPromise === task) {
          startPromise = null;
          startPromiseIntent = null;
        }
      };
      task.then(clearStartPromise, clearStartPromise);
      return task;
    },
    async stop() {
      ...
      stopPromise = (async () => {
        const stoppedGeneration = activeLifecycleGeneration;
        const activeManualTasks = [];
        if (activeSyncRequest?.manual) {
          activeManualTasks.push(activeSyncRequest.promise);
        }
        if (activeDashboardRequest?.manual) {
          activeManualTasks.push(activeDashboardRequest.promise);
        }
        // Attach rejection handlers before any host-task await. Otherwise a
        // manual request can reject during a slow host drain and briefly be an
        // unhandled rejection even though stop() observes it later.
        const activeManualDrain = Promise.allSettled(activeManualTasks);
        stopping = true;
        running = false;
        activeLifecycleGeneration = null;
        lifecycleGeneration += 1;
        for (let i = pendingSyncRequests.length - 1; i >= 0; i--) {
          const request = pendingSyncRequests[i];
          pendingSyncRequests.splice(i, 1);
          if (pendingSyncByGeneration.get(request.generation) === request) {
            pendingSyncByGeneration.delete(request.generation);
          }
          request.resolve([]);
        }
        for (let i = pendingDashboardRequests.length - 1; i >= 0; i--) {
          const request = pendingDashboardRequests[i];
          pendingDashboardRequests.splice(i, 1);
          if (pendingDashboardByGeneration.get(request.generation) === request) {
            pendingDashboardByGeneration.delete(request.generation);
          }
          request.resolve({ built: 0, graphLinksUpdated: 0, memoryMirror: null });
        }
        if (timer) clearInterval(timer);
        if (dashboardTimer) clearInterval(dashboardTimer);
        timer = null;
        dashboardTimer = null;
        const generationSyncTasks = [...scheduledSyncTasks.entries()]
          .filter(([, generation]) => generation === stoppedGeneration)
          .map(([task]) => task);
        const generationDashboardTasks = [...scheduledDashboardTasks.entries()]
          .filter(([, generation]) => generation === stoppedGeneration)
          .map(([task]) => task);
        // Host-owned sync and dashboard work may contain non-cancellable I/O.
        // Await it fail-closed rather than detaching a possible writer. Beta1
        // applies its shared 5s service-stop deadline outside this service and
        // refuses to start a replacement owner if that deadline is exceeded.
        await Promise.all([
          Promise.allSettled(generationSyncTasks),
          Promise.allSettled(generationDashboardTasks),
        ]);
        if (activeManualTasks.length > 0) {
          const manualResults = await activeManualDrain;
          for (const result of manualResults) {
            if (result.status === "rejected") {
              logger.warn?.(`plur1bus-obsidian-bridge: active manual task failed during stop: ${String(result.reason)}`);
            }
          }
        }
