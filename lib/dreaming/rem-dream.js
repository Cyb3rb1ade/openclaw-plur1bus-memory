...
import { appendDreamDiaryEntry, emitDreamCompletedEvent } from "./dream-diary.js";
...
/**
 * Kompakte Zusammenfassung eines Partitionslaufs für die Kommando-Antwort.
 *
 * Die Top-Level-Felder der Antwort stammen nur aus dem ersten Lauf mit Report
 * (sonst dem ersten Lauf überhaupt); Grund, Kandidatenzahl und Run-Key der
 * übrigen Partitionen gingen bisher verloren. Felder werden nur übernommen,
 * wenn sie den erwarteten Typ haben — nichts wird geraten.
 *
 * @param {{scope: string, result: object|null}} run Ergebnis eines Partitionslaufs
 * @returns {object} `{ scope, skipped, reason?, count?, runKey?, patternsFound? }`
 */
export function describeRemPartitionRun({ scope, result } = {}) {
  const report = result?.report;
  const summary = { scope, skipped: result?.skipped ?? false };
  if (typeof result?.reason === "string" && result.reason) summary.reason = result.reason;
  if (Number.isSafeInteger(result?.count)) summary.count = result.count;
  const runKey = report?.runKey ?? result?.runKey;
  if (typeof runKey === "string" && runKey) summary.runKey = runKey;
  if (Number.isSafeInteger(report?.patternsFound)) summary.patternsFound = report.patternsFound;
  return summary;
}
...
/**
 * Build a sparse, ACL-filtered neighbor graph from the authoritative table.
 *
 * A vector-search/query failure makes the candidate set incomplete. It is
 * therefore surfaced as `RemCandidateReadError`; callers must abort the run
 * instead of treating a partial graph as a valid zero-pattern result.
 *
 * @param {Array<object>} memories Authorized REM candidates.
 * @param {object} dbTable Authoritative table with vector-search support.
 * @param {object} [opts] Graph options.
 * @returns {Promise<Array<object>>} Sparse graph edges.
 * @throws {RemCandidateReadError} When a vector-search query cannot complete.
 */
export async function buildSparseNeighborGraph(memories, dbTable, opts = {}) {
  const { topK = 20, minSimilarity = 0.82, requestContext } = opts;
  const edges = [];
  ...
  for (const memory of memories) {
    ...
    try {
      if (typeof dbTable?.vectorSearch !== "function") {
        throw new TypeError("REM table does not provide vectorSearch");
      }
      const search = dbTable.vectorSearch(memory.vector);
      if (!search || typeof search.limit !== "function") {
        throw new TypeError("REM vectorSearch did not return a query builder");
      }
      const query = search.limit(topK);
      if (!query || typeof query.toArray !== "function") {
        throw new TypeError("REM vectorSearch query is incomplete");
      }
      const neighbors = await query.toArray();
      if (!Array.isArray(neighbors)) throw new TypeError("REM vectorSearch returned a non-array result");

      for (const neighbor of neighbors) {
        const candidate = memoryMap.get(neighbor.id);
        // Der Index liefert eine approximative Distanz, aus der sich keine
        // Ähnlichkeit rekonstruieren lässt: live ergab ein IDENTISCHER Vektor
        // `_distance = 0.6715` und damit `distanceToScore` = 0.598 — unter
        // jeder sinnvollen Schwelle. `minSimilarity` ist als Kosinus gemeint
        // (so wie `centroidMinSimilarity` in validateClusters), also wird hier
        // aus den ohnehin vorliegenden Vektoren echt gerechnet. Der Index
        // bleibt für die Vorauswahl der topK Nachbarn zuständig.
        const similarity = (memory.vector && candidate?.vector)
          ? cosineSimilarityVec(memory.vector, candidate.vector)
          : 0;
      ...
      throw err instanceof RemCandidateReadError ? err : new RemCandidateReadError(err);
    }
  }
 ...
 *     memoryStore: ownerBoundMemoryStore, // optional; no persistence fallback is allowed
 *     inputTarget: { aclBindings: partition, kind: partition.scope, workspaceDir }, // optional
 *     outputTarget: { aclBindings: partition, kind: partition.scope, workspaceDir }, // optional
 *   }
 *
 ...
 * `ownerBoundMemoryStore` is a bound DB adapter with a `store(entry)` method;
 * it is only needed when narrative `storeAsMemory` persistence is requested.
 * If it is absent, REM still performs candidate analysis and pattern writes,
 * and reports `narrativeMemoryPersistence.reason=missing_bound_memory_store`.
 * `outputTarget` is optional; private/user partitions must provide a bound
 * writer method and never use a workspace path fallback. Workspace paths are
 * accepted only for a target explicitly bound to the workspace partition.
...
/**
 * Assert that a sink or target carries the exact ACL partition for this run.
 * @param {object} target Candidate sink or output target.
 * @param {object} partition Normalized REM ACL partition.
 * @param {string} label Error-label prefix.
 * @returns {object} The validated ACL binding.
 */
export function assertBoundAcl(target, partition, label) {
...
/**
 * Validate the minimum owner-bound Neo pattern-store contract.
 * @param {object} neoStore Store with completion/pattern methods and binding.
 * @param {object} partition Normalized REM ACL partition.
 * @returns {object} The validated pattern store.
 */
export function assertBoundNeoStore(neoStore, partition) {
  ...
  assertBoundAcl(neoStore, partition, "REM pattern store");
  return neoStore;
}

/**
 * Validate the minimum owner-bound DB memory-write contract.
 * @param {object} memoryStore Adapter exposing `store(entry)` and ACL binding.
 * @param {object} partition Normalized REM ACL partition.
 * @returns {object} The validated memory store.
 */
export function assertBoundMemoryStore(memoryStore, partition) {
...
/**
 * Build a fresh owner-bound sink for one REM partition.
 *
 * The pattern store is mandatory for analytical REM state. Memory and output
 * sinks are optional and are never substituted with the primary DB or a
 * workspace path. Integrators should call this once per partition.
 *
 * @param {object} opts Sink components and exact ACL partition.
 * @returns {Readonly<object>} Validated partition sink.
 */
export function createRemPartitionSink({
  partition,
  neoStore,
  memoryStore = null,
  inputTarget = null,
  ...
  assertBoundAcl({ aclBindings: partition }, partition, "REM partition sink");
  const boundNeoStore = assertBoundNeoStore(neoStore, partition);
  const boundOutputTarget = outputTarget || null;
  const boundInputTarget = inputTarget || boundOutputTarget;
  if (boundOutputTarget) assertBoundAcl(boundOutputTarget, partition, "REM output target");
  if (boundInputTarget) assertBoundAcl(boundInputTarget, partition, "REM input target");
  const boundMemoryStore = memoryStore || null;
  if (boundMemoryStore) assertBoundMemoryStore(boundMemoryStore, partition);
  return Object.freeze({
    aclBindings: partition,
    neoStore: boundNeoStore,
    outputTarget: boundOutputTarget,
    inputTarget: boundInputTarget,
    memoryStore: boundMemoryStore,
  });
}
...
export async function resolveRemPartitionSink({
  ...
  return createRemPartitionSink({
    partition,
    neoStore: candidate.neoStore || candidate.store,
    outputTarget,
    inputTarget,
    memoryStore: candidate.memoryStore || null,
  });
}
...
export async function runRemDream({
  ...
  const { weekOf, startMs } = getPreviousWeekWindow(now ? new Date(now) : new Date());
  ...
  const narrativeMemoryRequested = Boolean(narrativeCfg?.enabled && narrativeCfg.storeAsMemory !== false);
  const narrativeMemoryPersistence = {
    requested: narrativeMemoryRequested,
    enabled: narrativeMemoryRequested && !dryRun && Boolean(sink.memoryStore),
    reason: !narrativeMemoryRequested
      ? "disabled_by_config"
      : dryRun
        ? "dry_run"
        : sink.memoryStore
          ? null
          : "missing_bound_memory_store",
  };
  if (narrativeMemoryRequested && !dryRun && !sink.memoryStore) {
    logger.warn?.(`rem-dream[${runKey}]: narrative memory persistence disabled: missing bound memoryStore; analytical patterns continue`);
  }
  ...
  let edges;
  try {
    edges = await buildSparseNeighborGraph(memories, db.table, { topK, minSimilarity: 0.82, requestContext });
  } catch (err) {
    const candidateError = err instanceof RemCandidateReadError ? err : new RemCandidateReadError(err);
    logger.debug?.(`rem-dream[${runKey}]: candidate vector query failed closed`);
    return {
      skipped: true,
  ...
  for (const cluster of validClusters) {
    ...
    patterns.push({
      ...
      visibility: {
        scope: normalizedPartition.scope || "agent_private",
        agentId,
        workspaceIdentity: normalizedPartition.workspaceIdentity || "",
        ownerUserId: normalizedPartition.ownerUserId || "",
      },
  ...
  const lastWeekPatterns = (await boundNeoStore.readPatterns(500, {
    requesterAgentId: agentId,
    requesterWorkspaceKey: normalizedPartition.workspaceIdentity,
    workspaceIdentity: normalizedPartition.workspaceIdentity,
    requesterOwnerId: normalizedPartition.ownerUserId,
  }) || []).filter((pattern) => {
  ...
  let narrative = null;
  let diaryOutcome = { written: false, reason: "not_attempted" };
  let mood = null;
  ...
  if (narrativeCfg?.enabled && clusterSampleSets.length > 0) {
    ...
    if (narrative) {
      ...
      if (narrativeMemoryPersistence.enabled) {
      ...
      // The diary is the agent's own file, so only agent-scoped material goes
      // there; a shared workspace or user partition never lands in it.
      if (narrativeCfg.diary !== false && !dryRun && normalizedPartition?.scope !== "workspace" && normalizedPartition?.scope !== "user" && ownerWorkspaceDir) {
        const diary = appendDreamDiaryEntry({
          workspaceDir: ownerWorkspaceDir,
          narrative,
          mode: "rem",
          timezone: narrativeCfg.timezone ?? null,
          logger,
        });
        diaryOutcome = { written: diary.written === true, ...(diary.reason ? { reason: diary.reason } : {}) };
        if (diary.written) {
          await emitDreamCompletedEvent({ workspaceDir: ownerWorkspaceDir, mode: "rem", narrative, logger });
        }
      }
    }
  ...
  // Ohne Narrativ ist die Woche NICHT erledigt. Bis 7.12.58 wurde der Lauf
  // auch dann als abgeschlossen markiert, wenn das Modell nichts geliefert
  // hatte — Bernds W36 (07.09.2026, 241 Erinnerungen, 59 s) steht so als
  // "completed" im Laufzustand, ohne dass je ein Traum in DREAMS.md kam, und
  // jede weitere Nacht meldete "already_processed". Jetzt bleibt der Lauf
  // offen und wird in der naechsten Nacht wiederholt; Muster werden erst mit
  // dem Narrativ festgeschrieben, damit die Wiederholung sie nicht doppelt
  // (appendPatterns haengt ungeprueft an). Ist das Narrativ per Konfiguration
  // abgeschaltet, wird nichts erwartet und der Lauf schliesst wie bisher.
  const narrativeExpected = narrativeCfg?.enabled !== false;
  if (!dryRun && narrativeExpected && !narrative) {
    logger.warn?.(`rem-dream[${runKey}]: kein Narrativ erzeugt — Lauf bleibt offen und wird wiederholt (memories=${memories.length}, patterns=${patterns.length})`);
  } else if (!dryRun) {
  ...
  logger.info?.(`rem-dream[${runKey}]: ${report.patternsFound} patterns (${report.new} new, ${report.stronger} stronger, ${report.disappeared} disappeared)`);
    ...
    return { report, trends, diary: diaryOutcome };
  } finally {
 ...
 */
/**
 * Where a REM partition's owner-bound output (vault note, DREAMS.md diary,
 * mood and soul snapshots) goes. The shared workspace partition writes into
 * the workspace it is bound to; the agent's OWN private partition writes into
 * that agent's workspace, which is its vault and the file the host's Dreams
 * page reads; every other partition (another agent's private data, user
 * pools) stays inside its Neo store directory. Until 7.12.2 the private
 * partition also went to the store directory, so REM dreams, their vault
 * notes and the mood context never reached the agent's workspace.
 *
 * @param {{partition: object, memoryCtx?: object|null, storeWorkspaceDir: string}} input
 * @returns {string}
 */
export function resolveRemOutputRoot({ partition, memoryCtx, storeWorkspaceDir }) {
  const ctxDir = typeof memoryCtx?.workspaceDir === "string" && memoryCtx.workspaceDir ? memoryCtx.workspaceDir : null;
  if (!ctxDir || !partition) return storeWorkspaceDir;
  if (partition.scope === "workspace" && partition.workspaceIdentity === memoryCtx.workspaceIdentity) return ctxDir;
  if (partition.scope === "agent-private" && partition.agentId && partition.agentId === memoryCtx.agentId) return ctxDir;
  return storeWorkspaceDir;
}
