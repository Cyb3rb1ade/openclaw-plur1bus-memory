 ...
 * `catch` darüber verwandelt den Fehler still in einen ungefilterten
 * Präfix-Scan. Genau so lief der Traum-Job auf den produktiven Tabellen
 * dauerhaft leer (`epistemicStatus` existiert dort nicht).
 *
 * `epistemicStatus` wird NULL-sicher formuliert: `!= 'invalidated'` allein ist
 * in SQL dreiwertig und würde Zeilen ohne gesetzten Wert still verwerfen.
 *
 * @param {object} table LanceDB-Tabelle.
 * @param {number} safeWeekStart
 * @returns {Promise<string>} where-Klausel; "true", wenn keine Spalte passt.
 */
async function buildCandidateWhere(table, safeWeekStart) {
  let felder = [];
  try {
  ...
  const zeit = [];
  if (hat("sourceTimestamp")) zeit.push(`sourceTimestamp >= ${safeWeekStart}`);
  if (hat("createdAt")) zeit.push(`createdAt >= ${safeWeekStart}`);

  const teile = [];
  if (zeit.length > 0) teile.push(`(${zeit.join(" OR ")})`);
...
export async function loadCandidateMemories(db, opts = {}) {
  const { weekStartMs, requestContext, aclPartition, maxMemories = 5000 } = opts;
  ...
  try {
    ...
    if (typeof query.where === "function") {
      rows = await query.where(await buildCandidateWhere(db.table, safeWeekStart)).limit(maxMemories).toArray();
    } else {
      rows = await query.limit(maxMemories).toArray();
    }
  } catch (_) {
    // Notnagel: der JS-Filter unten ist die konvergierende Sicherheitsgrenze für
    // ALLE Ladepfade, korrekt bleibt das Ergebnis also. Vollständig ist es dann
    // aber nicht mehr — ohne Pushdown ist dies ein beliebiger Präfix (LanceDB
    // liefert in Einfügereihenfolge, also die ältesten Zeilen), aus dem das
    // Wochenfenster nichts übriglässt. Genau das passierte live, weil die feste
    // where-Klausel `epistemicStatus` referenzierte und diese Spalte auf den
    // produktiven Tabellen fehlt. buildCandidateWhere verhindert das jetzt.
  ...
  return rows.filter(r => {
    ...
    // Applies to all three load paths above (where(), the no-where() branch,
    // and the catch fallback) — this single filter is the converging safety
    // boundary for all of them, same as the pre-existing status check.
    if (normalizeEpistemicStatus(r.epistemicStatus) === "invalidated") return false;
    ...
    if (ts < safeWeekStart) return false;
    return checkAccess(requestContext, r).allowed && sameRemBindings(remBindings(r, requestContext), aclPartition);
  }).map(r => ({
    id: r.id,
    text: r.text,
    summary: r.summary || "",
...
export function buildRemPartitions(requestContext) {
  ...
  if (requestContext?.userPrincipal) {
    kandidaten.push({ scope: "user", agentId, workspaceIdentity: "", ownerUserId: requestContext.userPrincipal });
  } else if (requestContext?.workspaceIdentity) {
    kandidaten.push({ scope: "workspace", agentId, workspaceIdentity: requestContext.workspaceIdentity, ownerUserId: "" });
  }
...
export async function runRemDream({
  ...
  const lockPath = neoStore?.paths?.workspaceDir
    ? join(neoStore.paths.workspaceDir, "locks", `rem-${weekOf}-${normalizedPartition.key}.lock`)
    : null;
  ...
  const memories = await loadCandidateMemories(db, { weekStartMs: startMs, requestContext, aclPartition: normalizedPartition, maxMemories });
  ...
  const lastWeekPatterns = neoStore.readPatterns(500).filter((pattern) => {
  ...
  if (narrativeCfg?.enabled && clusterSampleSets.length > 0) {
    ...
    if (workspaceDir && !dryRun) {
      try {
        ...
        if (echo) appendDreamEcho(workspaceDir, { ...echo, aclBindings: dreamAclBindings });
      } catch (err) {
...
export function writeRemDreamToVault(report, trends, workspaceDir) {
  try {
    ...
    const path = join(dir, `${weekOf}-${partition.key}-rem-dream.md`);
