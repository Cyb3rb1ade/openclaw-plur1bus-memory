...
export async function loadCandidateMemories(db, opts = {}) {
  ...
  const rows = await db.table.query().limit(maxMemories).toArray();
...
export async function buildSparseNeighborGraph(memories, dbTable, opts = {}) {
  const { topK = 20, minSimilarity = 0.82 } = opts;
  const edges = [];
  ...
  for (const memory of memories) {
    ...
    try {
      ...
      // Einzelne Memory darf nicht alles abbrechen
    }
  }
...
export async function summarizeClusterWithLlm(samples, llmCfg, callLlm, logger) {
...
export async function runRemDream({
  ...
  try {
    if (lockPath) lockAcquired = acquireJobLock(lockPath);
  } catch (lockErr) {
  ...
  const edges = await buildSparseNeighborGraph(memories, db.table, { topK, minSimilarity: 0.82 });
  ...
  for (const cluster of validClusters) {
    ...
    const summary = await summarizeClusterWithLlm(samples, llmCfg, callLlm, logger);
    patterns.push({
      id: randomUUID(),
      runKey,
      workspaceKey,
