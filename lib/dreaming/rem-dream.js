...
export async function loadCandidateMemories(db, opts = {}) {
  const { weekStartMs, requestContext, maxMemories = 5000 } = opts;
  ...
  return rows.filter(r => {
    ...
    return checkAccess(requestContext, r).allowed;
  }).map(r => ({
    id: r.id,
    text: r.text,
    summary: r.summary || "",
...
function sameRemPartition(left, right, requestContext) {
  const a = remBindings(left, requestContext);
  const b = remBindings(right, requestContext);
  return Boolean(a && b
    && a.scope === b.scope
    && a.agentId === b.agentId
...
export async function runRemDream({
  ...
  if (!requestContext || requestContext.agentId !== agentId) {
    return { skipped: true, reason: "acl_request_context_missing", runKey };
  }
  const memories = await loadCandidateMemories(db, { weekStartMs: startMs, requestContext, maxMemories });
  ...
  for (const cluster of validClusters) {
    ...
    if (!bindings || samples.some((sample) => !sameRemPartition(samples[0], sample, requestContext))) continue;
  ...
  const lastWeekPatterns = neoStore.readPatterns(500).filter((pattern) => {
    ...
    return bindings && patterns.some((current) => JSON.stringify(current.aclBindings) === JSON.stringify(bindings));
  });
  ...
  if (narrativeCfg?.enabled && clusterSampleSets.length > 0) {
    ...
    dreamAclBindings = remBindings(orderedClusters[0][0], requestContext);
