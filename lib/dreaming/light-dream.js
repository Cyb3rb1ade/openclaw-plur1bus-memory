...
import { appendDreamDiaryEntry, emitDreamCompletedEvent, diaryScopeAllowed } from "./dream-diary.js";
 ...
 */
/**
 * Delete+add a light-dream row. Forgotten rewritten text is refused before
 * delete so the source stays. Same-text replay is allowed.
 * @param {object} db
 * @param {object} row
 * @param {object} updated
 * @param {{signal?: AbortSignal, onDeleted?: Function}} [opts]
 * @returns {Promise<{added: boolean, blocked: boolean}>}
 */
export async function replaceLightDreamRow(db, row, updated, opts = {}) {
  const textChanged = String(updated?.text || "") !== String(row?.text || "")
    || String(updated?.summary || "") !== String(row?.summary || "");
  if (textChanged) {
    const { baseDbPath, agentId } = splitAgentDbPath(db?.dbPath || "");
    const guard = assertCardWriteAllowed({
      baseDbPath,
      agentId: row.agentId || row.storedBy || agentId,
      text: updated.text || updated.summary || "",
      scope: row.scope || "agent-private",
      workspaceIdentity: row.workspaceId || row.workspaceKey || "",
      ownerUserId: row.ownerUserId || "",
    });
    if (!guard.allowed) return { added: false, blocked: true };
  }
  const safeId = safeUuid(row.id);
  await db.table.delete(`id = "${safeId}"`);
  opts.onDeleted?.();
  const vector = updated.vector ?? row.vector;
  const vectorArray = Array.isArray(vector) ? vector : (vector ? Array.from(vector) : []);
  try {
    await db.table.add([{ ...updated, vector: vectorArray }]);
    ...
    try {
      ...
      throw new AggregateError(
        [addErr, rollbackErr],
        "light dream strengthen re-add and rollback both failed",
      );
    }
...
export async function strengthenMemory(db, memoryId, signal = null) {
  ...
  try {
    ...
    try {
      ...
      throwIfAborted(signal, "light dream aborted");
      const replaced = await replaceLightDreamRow(db, row, updated, { signal, onDeleted: () => { rowDeleted = true; } });
...
export async function lightDream({
  ...
  aclBindings = null,
  requestContext = null,
  // Durchgereicht an emitDreamCompletedEvent: undefined laesst dort den
  // Standardpfad (OpenClaw-SDK) greifen, ein Test kann eine Attrappe geben.
  importHostEvents = undefined,
}) {
  throwIfAborted(signal, "light dream aborted");
  const startTime = Date.now();
  const dreamId = randomUUID();
  const ownership = aclBindings ? validateOwnershipTuple({
    scope: aclBindings.scope,
    agentId: aclBindings.agentId,
    workspaceId: aclBindings.workspaceIdentity,
    workspaceKey: aclBindings.workspaceIdentity,
    ownerUserId: aclBindings.ownerUserId,
  }, requestContext?.workspaceAliases) : { ok: false };
  const validatedAclBindings = ownership.ok && checkAccess(requestContext, {
    scope: aclBindings?.scope,
    agentId: aclBindings?.agentId,
    workspaceId: aclBindings?.workspaceIdentity,
    workspaceKey: aclBindings?.workspaceIdentity,
  ...
  if (narrativeCfg?.enabled) {
    ...
    if (narrative) {
      ...
      if (narrativeCfg.storeAsMemory !== false) {
        dreamMemoryId = await storeDreamAsMemory({
          ...
          aclBindings: validatedAclBindings,
          logger,
          signal,
        });
        throwIfAborted(signal, "light dream aborted");
      }
      if (narrativeCfg.diary !== false && workspaceDir && diaryScopeAllowed(validatedAclBindings?.scope)) {
        ...
        if (diary.written) {
          await emitDreamCompletedEvent({ workspaceDir, mode: "light", narrative, logger, importHostEvents });
        }
        throwIfAborted(signal, "light dream aborted");
      }
    }
    ...
    if (workspaceDir) {
      try {
        ...
        if (echo) {
          throwIfAborted(signal, "light dream aborted");
          if (validatedAclBindings) appendDreamEcho(workspaceDir, { ...echo, aclBindings: validatedAclBindings });
        }
      } catch (err) {
        throwIfAborted(signal, "light dream aborted");
  ...
  const dreamEntry = {
    id: dreamId,
    type: "light_dream",
    agentId: turns?.[0]?.agentId || null,
    workspaceKey: turns?.[0]?.workspaceKey || "default",
    visibility: {
      scope: validatedAclBindings?.scope || "agent_private",
      agentId: turns?.[0]?.agentId || "",
      workspaceIdentity: validatedAclBindings?.workspaceIdentity || turns?.[0]?.workspaceKey || "",
      ownerUserId: validatedAclBindings?.ownerUserId || "",
    },
    createdAt: new Date().toISOString(),
