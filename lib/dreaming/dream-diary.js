...
import { loadHostSdk } from "../host-sdk-loader.js";
...
/**
 * The diary is the agent's own file: only the agent's private partition may
 * write it. ACL bindings spell that scope "agent-private"; older callers say
 * "agent" or nothing. Shared workspace and user partitions never qualify.
 * (Until 7.12.2 light dreams compared against "agent" only, so a binding
 * carrying "agent-private" never reached the diary.)
 *
 * @param {string|undefined|null} scope
 * @returns {boolean}
 */
export function diaryScopeAllowed(scope) {
  return scope == null || scope === "agent" || scope === "agent-private";
}
...
export async function emitDreamCompletedEvent({
  ...
  importHostEvents = () => loadHostSdk("memory-host-events"),
  logger = null,
} = {}) {
