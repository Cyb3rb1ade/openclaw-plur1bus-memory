 ...
 * Nutzt db.store() (normalizeEntryForTable) — nie direkt table.add().
 */
export async function saveReminder(db, opts) {
  ...
  const reminderKey = buildReminderKey(agentId, workspaceKey, text, remindAt);
  ...
  const raw = {
    ...
    remindAt: safeTimestamp(Number(remindAt)),
    remindedAt: 0,
    dispatchedAt: 0,
    acknowledgedAt: 0,
    cancelledAt: 0,
...
export async function listDueReminders(db, agentId, workspaceKey, now = Date.now()) {
  ...
  const results = await db.table
    .query()
    .where(`memoryKind = 'reminder' AND storedBy = ${sqlString(agentId)} AND workspaceKey = ${sqlString(workspaceKey)} AND remindAt <= ${safeNow} AND remindAt > 0 AND reminderStatus IN ('scheduled', 'due')`)
    .limit(50)
    .toArray();
  return results;
}
