...
export async function activateSkillProposal(workspaceDir, id, ctx = {}) {
  ...
  if (committedWorkshopEvent) {
    ...
    try {
      ...
      // 7.12.53: Diese Abkürzung fängt den Fall ab, dass der Lebenszyklus-Hook
      // während unseres apply-Aufrufs schon fertig synchronisiert hat. Sie galt
      // bis 7.12.52 auch für `activation_partial` — damit kehrte jeder zweite
      // Versuch zurück, bevor die Belegschleife lief, und „Freigabe abschließen"
      // im Dashboard bewirkte nichts. Nur ein vollständig aktiver Datensatz ist
      // wirklich fertig.
