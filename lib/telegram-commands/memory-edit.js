...
export async function shareCard(privatePool, sharedPool, embeddings, agent, id, opts = {}) {
  let sourceId;
  try { sourceId = safeUuid(id); } catch { return { ok: false, error: "share.invalid_id" }; }
  ...
  try {
    return await privatePool.withWriteDb(agent, async (sourceDb) => {
      ...
      const now = Date.now();
      if ((card.status != null && card.status !== "active") || !liveShareExpiry(card.expiresAt, now)) return { ok: false, error: "share.source_not_live" };
      ...
      if (reason && opts.allowSensitiveShare !== true) return { ok: false, error: `share.explicit approval required (${reason})` };
      const vector = await embeddings.embed(card.text || card.summary, { agentId: agent });
      ...
      if (!card || (card.status != null && card.status !== "active") || !liveShareExpiry(card.expiresAt, Date.now())) return { ok: false, error: "share.source_not_live" };
      if (!['agent-private', 'workspace', 'user'].includes(card.scope || 'agent-private')) return { ok: false, error: "share.source_scope_denied" };
      const completionAcl = denyIfAclBlocked(card, opts);
      if (completionAcl) return { ok: false, error: `access denied: ${completionAcl}` };
      const result = await lease.call(sharedPool, opts.ctx || {}, (targetDb) => storeSharedMemory(targetDb, card, opts.ctx || {}, { targetScope, vector, allowSensitiveShare: opts.allowSensitiveShare === true, logger: opts.logger }));
