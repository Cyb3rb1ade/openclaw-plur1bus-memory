...
export async function shareCard(privatePool, sharedPool, embeddings, agent, id, opts = {}) {
  ...
  try {
    return await privatePool.withWriteDb(agent, async (sourceDb) => {
      ...
      const result = await lease.call(sharedPool, opts.ctx || {}, (targetDb) => storeSharedMemory(targetDb, card, opts.ctx || {}, { targetScope, vector, sourceAgentId: sourceAgent, allowSensitiveShare: opts.allowSensitiveShare === true, logger: opts.logger }));
