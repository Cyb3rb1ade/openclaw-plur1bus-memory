...
import { hostHomeOverride } from "../host-paths.js";
...
import { SHARED_MEMORY_UNSUPPORTED } from "../shared-memory-pool.js";
...
/**
 * The archive directory /forget and /correct have always written to. Read per
 * call (OPENCLAW_HOME may change under tests); the OpenClaw adapter hands it to
 * the engine as host.capabilities.memoryArchiveDir (E1 Task 8).
 * @returns {string}
 */
export function resolveDefaultArchiveDir() {
  return join(hostHomeOverride() || homedir(), ".openclaw", "memory", "_archive");
}
...
export async function resolveCandidates(db, agent, query, opts = {}) {
  ...
  // Woertlicher Tie-Breaker. Die Vektorsuche bettet die Suchphrase als kurzes
  // Fragment ein; gegen ihren 1600-Zeichen-Ursprung erreicht das nur einen
  // maessigen Score, und kurze generische Zeilen ("Huhu, Diggi!", Stimmungs-
  // eintraege) liegen dann binnen 0,15 — am 09.09.2026 zweimal hintereinander
  // fuer ein Zitat, das nur in genau einer Erinnerung vorkommt. Wer eine
  // Erinnerung zitiert, meint sie: enthaelt genau eine Kandidatin die Phrase
  // woertlich, ist das eindeutig.
  const quoted = literalMatches(results, query);
  if (quoted.length === 1) {
    return { unique: true, card: quoted[0], candidates: results, matchedBy: "literal" };
  }
  // Abdeckungs-Tie-Breaker. Menschen zitieren selten woertlich; sie fassen
  // zusammen ("mein Mund ist voller Kreidestaub und Diggi haelt einen Kuchen
  // mit 78,1 Kerzen" ueber drei Saetze hinweg, 09.09.2026 nach dem woertlichen
  // Fix immer noch die Auswahlliste). Kommen alle markanten Suchwoerter in
  // genau einer Kandidatin vor und in keiner anderen annaehernd, ist die
  // Sache ebenso eindeutig wie ein Zitat.
  const covered = coverageMatches(results, query);
  if (covered.length === 1) {
    return { unique: true, card: covered[0], candidates: results, matchedBy: "coverage" };
  }
  return { unique: false, candidates: results.slice(0, 5) };
}

const COVERAGE_MIN_TOKENS = 3;
const COVERAGE_UNIQUE_MIN = 0.8;
const COVERAGE_RUNNER_UP_GAP = 0.3;

/**
 * Die Kandidatin, die (fast) alle markanten Suchwoerter enthaelt, waehrend
 * jede andere deutlich weniger davon traegt. Leer, wenn die Suche zu wenige
 * markante Woerter hat oder zwei Kandidatinnen aehnlich gut abdecken.
 */
export function coverageMatches(candidates, query) {
  const tokens = [...new Set(tokenizeForCoverage(query))];
  if (tokens.length < COVERAGE_MIN_TOKENS) return [];
  const scored = (Array.isArray(candidates) ? candidates : []).map((card) => {
    const hay = new Set(tokenizeForCoverage(`${card?.text || ""} ${card?.summary || ""}`));
    const hits = tokens.filter((token) => hay.has(token)).length;
    return { card, coverage: hits / tokens.length };
  }).sort((a, b) => b.coverage - a.coverage);
  const best = scored[0];
  if (!best || best.coverage < COVERAGE_UNIQUE_MIN) return [];
  const runnerUp = scored[1]?.coverage ?? 0;
  if (best.coverage - runnerUp < COVERAGE_RUNNER_UP_GAP) return [];
  return [best.card];
}

function tokenizeForCoverage(value) {
  return normalizeForLiteralMatch(value)
    .split(/[^\p{L}\p{N}]+/u)
    .filter((token) => token.length >= 4);
}

/** Kandidatinnen, deren Text die Suchphrase woertlich enthaelt (Gross/Klein und Leerraum tolerant). */
export function literalMatches(candidates, query) {
  const needle = normalizeForLiteralMatch(query);
  if (needle.length < 12) return [];
  return (Array.isArray(candidates) ? candidates : []).filter((card) => {
    const hay = normalizeForLiteralMatch(`${card?.text || ""} ${card?.summary || ""}`);
 ...
 *
 * @returns {{ok: boolean, archivePath?: string, error?: string, code?: string, alreadyTombstoned?: boolean, tombstoneId?: string|null}}
 */
export async function forgetCard(db, agent, id, opts = {}) {
  const {
    lang = "en",
    tone = "default",
    archiveDir = resolveDefaultArchiveDir(),
    workspaceDir,
    logger,
    baseDbPath,
    actor = "",
  ...
  try {
    ...
    return { ok: false, error: t("forget.db_read_error", { lang, tone, vars: { error } }), code: "storage" };
  }
  if (!card) return { ok: false, error: t("forget.card_not_found", { lang, tone, vars: { id } }), code: "not-found" };
  ...
  if (aclDenied) return { ok: false, error: `access denied: ${aclDenied}`, code: "not-found" };
  ...
  if (String(card.status || "") === "deleted") {
    ...
    if (baseDbPath) {
      try {
        ...
        if (!auditOk) {
          return { ok: false, error: t("forget.audit_failed", { lang, tone }), archivePath: "", code: "storage" };
        }
        backfillTombstoneId = backfill.tombstone.tombstoneId;
      } catch (err) {
        const error = logInternalError(logger, "memory-edit.forget.backfill", err, { agentId: agent, memoryId: id });
        return { ok: false, error: t("forget.delete_error", { lang, tone, vars: { error } }), code: "storage" };
      }
    }
    return { ok: true, alreadyTombstoned: true, archivePath: "", id, tombstoneId: backfillTombstoneId };
  }
  ...
  try {
    ...
    return { ok: false, error: t("forget.archive_failed", { lang, tone, vars: { error } }), code: "storage" };
  }
  ...
  try {
    ...
    return { ok: false, error: t("forget.delete_error", { lang, tone, vars: { error } }), archivePath, code: "storage" };
  }
  ...
  if (tombResult?.notFound) {
    ...
    return { ok: false, error: t("forget.card_not_found", { lang, tone, vars: { id } }), archivePath, code: "not-found" };
  }
  ...
  if (!committedAuditOk) {
    ...
    return { ok: false, error: t("forget.audit_failed", { lang, tone }), archivePath, id, code: "storage" };
  }
  return { ok: true, archivePath, id, alreadyTombstoned: Boolean(tombResult?.alreadyTombstoned), tombstoneId: tombstone.tombstoneId };
}

// ─── correctCard ─────────────────────────────────────────────────────────

/**
 * Updates a card. Archive-First, then updateCard or opts.updateMemory. A
 * non-throwing refusal from either (e.g. updateCard's own tombstone_blocked
 * guard) is never reported as `ok: true` and never writes the `memory.updated`
 * audit line (fix round 1, E1-R10).
 *
 * @returns {{ok: boolean, archivePath?: string, error?: string, code?: string, id?: string, newId?: string}}
 *   `newId` is additive (fix round 1, E1-R8): the id of the new, live version
 *   `updateCard`'s version-chain update produced. `id` stays the id passed in.
 */
export async function correctCard(db, agent, id, newContent, opts = {}) {
  const { lang = "en", tone = "default", archiveDir = resolveDefaultArchiveDir(), workspaceDir, logger } = opts;
  let card;
  try {
    card = await db.getCard(agent, id);
  } catch (err) {
    const error = logInternalError(logger, "memory-edit.correct.getCard", err, { agentId: agent, memoryId: id });
    return { ok: false, error: t("correct.db_read_error", { lang, tone, vars: { error } }), code: "storage" };
  }
  ...
  if (!card || String(card.status || "") === "deleted") {
    return { ok: false, error: t("correct.card_not_found", { lang, tone, vars: { id } }), code: "not-found" };
  }
  const aclDenied = denyIfAclBlocked(card, opts);
  if (aclDenied) return { ok: false, error: `access denied: ${aclDenied}`, code: "not-found" };
  ...
  try {
    ...
    return { ok: false, error: t("correct.archive_failed", { lang, tone, vars: { error } }), code: "storage" };
  }
  let updateResult;
  try {
    if (typeof opts.updateMemory === "function") {
      updateResult = await opts.updateMemory({ agent, id, newContent, card, archivePath });
    } else {
      updateResult = await db.updateCard(agent, id, newContent);
    }
  } catch (err) {
  ...
  // A non-throwing refusal (db.updateCard's own tombstone_blocked guard —
  // the corrected text matches a forgotten memory in the same scope) must
  // never be reported as success, and must not write a `memory.updated`
  // audit line (fix round 1, E1-R10): the mutation never happened.
  if (updateResult && updateResult.ok === false) {
    const conflict = updateResult.action === "tombstone_blocked" || updateResult.reason === "tombstone_blocked";
    const reasonText = String(updateResult.reason || updateResult.action || "update refused");
    return {
      ok: false,
      error: t("correct.update_error", { lang, tone, vars: { error: reasonText } }),
      archivePath,
      code: conflict ? "conflict" : "storage",
    };
  }
  ...
  // newId is additive (fix round 1, E1-R8): db.updateCard's version-chain
  // update returns the id of the new, live version; opts.updateMemory
  // (the adapter's own safeUpdate-backed hook) does not return one today,
  // so this falls back to the passed-in id exactly like the pre-existing
  // `id` field always has.
  const newId = updateResult?.id ?? updateResult?.newId ?? id;
  return { ok: true, archivePath, id, newId };
}
...
/**
 * Moved from adapter/openclaw/register-commands.js's `sourceDenied` (E1 Task
 * 6): a shareCard failure whose source memory is missing, not owned by the
 * caller, out of scope for sharing, or ACL-denied. The regex (and therefore
 * the adapter's behaviour) is unchanged; only its home moved so the typed
 * MemoryOps `share` (engine/memory-ops/write.js) can reuse the same
 * classification for its own "not-found" mapping.
 */
export function shareFailureCode(error) {
  return /^(?:share\.(?:card_not_found|source_not_live|source_scope_denied|source_owner_conflict|source_changed)|access denied:)/.test(String(error || ""));
}

/**
 * The machine-readable code for a shareCard failure string (E1 Task 6).
 * `share.source_changed` (content drift detected on re-embed) is carved out
 * of `shareFailureCode`'s broader "denied" bucket as "conflict", since a
 * caller can retry that one by re-reading and re-sharing; every other
 * `shareFailureCode` match is "not-found" (source missing/not
 * owned/ACL-denied — same anti-oracle answer as forget/correct).
 */
function codeForShareError(error) {
  const message = String(error || "");
  if (message === "share.invalid_id" || message === "share.invalid_target_scope") return "invalid-input";
  if (message.startsWith("share.explicit approval required")) return "approval-required";
  if (message === "share.source_changed") return "conflict";
  if (message.startsWith("shared memory is not supported")) return "unsupported";
  if (shareFailureCode(message)) return "not-found";
  return "storage";
}

/** Attaches a `code` to a shareCard failure result that doesn't already carry one. */
function withShareCode(result) {
  if (!result || result.ok !== false || result.code) return result;
  return { ...result, code: codeForShareError(result.error) };
}
 ...
 * @returns {{ok: boolean, sharedId?: string, error?: string, code?: string}}
 */
export async function shareCard(privatePool, sharedPool, embeddings, agent, id, opts = {}) {
  let sourceId; let sourceAgent;
  try { sourceId = safeUuid(id); sourceAgent = safeAgentId(agent); } catch { return { ok: false, error: "share.invalid_id", code: "invalid-input" }; }
  const targetScope = opts.targetScope || "workspace";
  if (!['workspace', 'user'].includes(targetScope)) return { ok: false, error: "share.invalid_target_scope", code: "invalid-input" };
  if (typeof privatePool?.withWriteDb !== "function" || typeof embeddings?.embed !== "function") return { ok: false, error: "share.db_read_error: internal error; details were logged", code: "storage" };
  try {
    const outcome = await privatePool.withWriteDb(agent, async (sourceDb) => {
      await sourceDb.init();
    ...
    return withShareCode(outcome);
  } catch (err) {
    if (isSafeSharePolicyError(err)) return { ok: false, error: `share.store_error: ${err.message}`, code: "storage" };
    if (err?.code === SHARED_MEMORY_UNSUPPORTED) return withShareCode({ ok: false, error: err.message });
    const error = logInternalError(opts.logger, "memory-edit.share.store", err, { agentId: agent, memoryId: sourceId });
    return { ok: false, error: `share.store_error: ${error}`, code: "storage" };
  }
}
...
function validateShareSource(card, agent, opts) {
  if ((card.status != null && card.status !== "active") || !liveShareExpiry(card.expiresAt, Date.now())) return { ok: false, error: "share.source_not_live", code: "not-found" };
  if (!['agent-private', 'workspace', 'user'].includes(card.scope || 'agent-private')) return { ok: false, error: "share.source_scope_denied", code: "not-found" };
  for (const alias of [card.agentId, card.storedBy]) {
    if (alias != null && alias !== "" && alias !== agent) return { ok: false, error: "share.source_owner_conflict", code: "not-found" };
  }
  if (!opts.ctx) return { ok: false, error: "access denied: acl.request.missing_context", code: "not-found" };
  const aclDenied = denyIfAclBlocked(card, opts);
  if (aclDenied) return { ok: false, error: `access denied: ${aclDenied}`, code: "not-found" };
  const reason = sensitiveShareReason(card);
  return reason && opts.allowSensitiveShare !== true ? { ok: false, error: `share.explicit approval required (${reason})`, code: "approval-required" } : null;
}
