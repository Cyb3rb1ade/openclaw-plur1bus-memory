...
import { hostConfigPath } from "../host-paths.js";
import { acquireJobLock, releaseJobLock } from "../job-lock.js";
import { t } from "../i18n.js";
import { validatePluginConfig } from "../setup/config-contract.js";
import { normalizePluginEntryCompatibility, PLUGIN_KEY } from "../setup/feature-profiles.js";
...
/**
 * Normalize, validate, and atomically persist a whitelisted feature toggle.
 ...
 * @returns {object} Structured success or no-write error result.
 */
export function toggleFeature(name, enable, opts = {}) {
  ...
  return withConfigLock(path, () => {
    ...
    try {
      const currentEntry = cfg?.plugins?.entries?.[PLUGIN_KEY] || {};
      const normalizedEntry = normalizePluginEntryCompatibility(currentEntry);
      setDeep(normalizedEntry.config, def.configPath.slice(4), !!enable);
      normalizedEntry.config = validatePluginConfig(normalizedEntry.config);
      setDeep(cfg, ["plugins", "entries", PLUGIN_KEY], normalizedEntry);
    } catch (err) {
      return { ok: false, error: t("toggle.config_read_error", { lang, tone, vars: { error: err.message } }), knownFeatures: known };
    }
    try {
