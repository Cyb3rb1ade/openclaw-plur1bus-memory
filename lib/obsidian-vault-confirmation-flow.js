...
export function confirmVaultConfirmation({
  ...
  if (canonicalJson(binding) !== canonicalJson(pending.binding)) {
    // Name the fields that differ, never their values. Five values bind a
    // one-time confirmation, and a bare "binding_mismatch" could not say which
    // one moved between prepare and confirm -- so a confirmation that fails for
    // the same actor in the same conversation was indistinguishable from one
    // correctly refused for a different actor.
    const mismatchedFields = Object.keys(binding)
      .filter((field) => binding[field] !== pending.binding?.[field])
      .sort();
    return { ok: false, reason: "binding_mismatch", mismatchedFields };
  }
