...
/** Pseudo task: the agent's default for every task without its own choice. */
export const AGENT_DEFAULT_KEY = "*";
...
/**
 * Every agent the host knows, in config order. OpenClaw keeps them under
 * `agents.entries` (an object keyed by id, or an array); `agents.list` is the
 * legacy array and is only consulted when `entries` is absent. Without
 * either, the host runs the implicit `main` agent.
 * @param {object} hostConfig OpenClaw config.
 * @returns {Array<object>} Raw agent records, each with a valid `id`.
 */
function knownAgents(hostConfig = {}) {
  const agents = record(hostConfig.agents);
  const entries = agents.entries;
  const listed = Array.isArray(entries) ? entries
    : entries && typeof entries === "object" ? Object.entries(entries).map(([id, value]) => ({ ...record(value), id }))
...
/**
 * Which agents get a column in the dashboard matrix. An explicit
 * `llmRouter.dashboardAgents` wins; otherwise the standing agents (those with
 * a heartbeat) plus any agent that already has an override, so a saved choice
 * never disappears from view. If that still yields nothing, every known agent.
 * A host with 32 entries — one primary agent and its sub-agents — would
 * otherwise render 32 columns nobody asked for.
 * @param {object} hostConfig OpenClaw config.
 * @param {object} config PLUR1BUS config.
 * @returns {Set<string>} Agent ids to show.
 */
export function dashboardAgentIds(hostConfig = {}, config = {}) {
  const known = knownAgents(hostConfig);
  const ids = new Set(known.map((agent) => agent.id));
  const explicit = Array.isArray(config?.llmRouter?.dashboardAgents)
    ? config.llmRouter.dashboardAgents.filter((id) => agentId(id) && ids.has(id)) : [];
  if (explicit.length) return new Set(explicit);
  const standing = known.filter((agent) => agent.heartbeat === true || (agent.heartbeat && typeof agent.heartbeat === "object")).map((agent) => agent.id);
  const overridden = Object.keys(record(config?.llmRouter?.agentModels)).filter((id) => ids.has(id));
  const chosen = new Set([...standing, ...overridden]);
  return chosen.size ? new Set(known.map((agent) => agent.id).filter((id) => chosen.has(id))) : ids;
}
...
export function configuredAgentModels(hostConfig = {}) {
  const defaults = record(record(hostConfig.agents).defaults);
  return knownAgents(hostConfig).map((agent) => {
...
export function featureModelOverrides(config, feature) {
  ...
  // A task choice beats the agent default; the agent default beats every
  // route the config file would otherwise pick, exactly like a task choice.
  return Object.freeze(Object.fromEntries(Object.entries(record(config?.llmRouter?.agentModels))
    .map(([id, models]) => [id, agentId(id) ? modelId(own(models, feature)) || modelId(own(models, AGENT_DEFAULT_KEY)) : null])
    .filter(([, model]) => model)));
}
...
export function projectFeatureModels(config = {}, hostConfig = {}) {
  ...
  const shown = dashboardAgentIds(hostConfig, config);
  const explicit = Array.isArray(config?.llmRouter?.dashboardAgents) && config.llmRouter.dashboardAgents.length > 0;
  return { selection: explicit ? "configured" : "standing", agents: configuredAgentModels(hostConfig).filter((agent) => shown.has(agent.id)).map((agent) => {
    const overrides = record(own(config.llmRouter?.agentModels, agent.id));
    const agentDefault = modelId(own(overrides, AGENT_DEFAULT_KEY)) || "";
    return {
      ...agent,
      agentDefault,
      ...
      features: FEATURE_MODEL_DEFINITIONS.map((definition) => {
        ...
        // What runs without a task choice: the agent default if there is one,
        // otherwise the route the config file would pick on its own.
        const configured = modelId(local.model) || (direct ? null : modelId(config.llmRouter?.defaultModel) || agent.defaultModel);
        return {
          id: definition.id, card: definition.card, label: definition.label,
          model: modelId(own(overrides, definition.id)) || "",
          inheritedModel: agentDefault || configured,
          direct,
        };
      }),
    };
...
export function validFeatureModelRequest(request = {}) {
  return [[request.agentId, 64, true], [request.feature, 64, true], [request.model, 256, false]]
    ...
    && Boolean(agentId(request.agentId)) && (FEATURE_IDS.has(request.feature) || request.feature === AGENT_DEFAULT_KEY)
    && (request.model === "" || Boolean(modelId(request.model)));
}

/**
 * Let the plugin use one more model: set allowModelOverride and append the
 * model to existing restrictive lists. Other trust bits are preserved, and a
 * wildcard list is left alone. Mutates the draft entry in place.
 * @param {object} entry Draft `plugins.entries.<id>`.
 * @param {string} model Concrete provider/model id.
 */
export function grantModelPermission(entry, model) {
...
/**
 * Every concrete model any known agent may use — the catalogue behind the
 * host-wide default model.
 * @param {object} hostConfig OpenClaw config.
 * @returns {Set<string>} Model ids.
 */
export function catalogModelIds(hostConfig = {}) {
  return new Set(configuredAgentModels(hostConfig).flatMap((agent) => agent.models.map((model) => model.id)));
}
...
export function createFeatureModelMutator({ api, pluginId = "memory-lancedb-namespaced" } = {}) {
  ...
  return async (request) => {
    ...
    return mutateConfigFile({ afterWrite: { mode: "auto" }, mutate(draft) {
      ...
      // config read, upgrade or reset.
      if (model) grantModelPermission(entry, model);
