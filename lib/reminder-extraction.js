...
const REMINDER_NUDGE_RE = /<reminder-nudge>[\s\S]*?<\/reminder-nudge>/gi;

export function stripReminderNudge(text) {
  return String(text ?? "").replace(REMINDER_NUDGE_RE, " ");
}

/**
 * Ein Reminder braucht mehr als die Zeitfloskel: mindestens ein weiteres Wort
 * mit Buchstaben ausserhalb der Floskel.
 */
export function hasReminderTopic(reminderText, evidence) {
  const rest = String(reminderText ?? "")
    .toLowerCase()
    .replace(String(evidence ?? "").toLowerCase(), " ")
    .replace(/[^\p{L}\p{N}]+/gu, " ")
    .trim();
  return /\p{L}{2,}/u.test(rest);
}
...
export function planReminderExtraction(item, opts = {}) {
  ...
  // 7.12.24: Eine eingebettete Reminder-Nudge zitiert fremde Zeitfloskeln
  // ("in 1 minute") und erzeugte daraus jede Runde einen neuen Reminder.
  const text = stripReminderNudge(item?.text);
  const parsed = parseReminderIntent(text, { now });
  if (!parsed.remindAt || parsed.timePrecision === "none") {
    return { skip: true, reason: "no_time" };
  }

  const reminderText = buildReminderText(text, parsed.evidence);
  // Ohne Thema ist der Reminder wertlos: der Agent erfindet beim Nudge eines
  // und die Floskel wird beim naechsten Capture erneut geparst.
  if (!hasReminderTopic(reminderText, parsed.evidence)) {
    return { skip: true, reason: "no_topic" };
  }

  return { skip: false, parsed, reminderText };
}
