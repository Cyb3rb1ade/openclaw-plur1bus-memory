...
/** Create the process-local fail-closed gate used for an atomic writer-generation switch. */
export function createMemoryMaintenanceGate({ now = Date.now } = {}) {
  ...
  const status = () => lease
    ? Object.freeze({ active: true, reason: lease.reason, since: lease.since })
