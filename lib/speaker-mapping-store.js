...
import { hostConfigPathOverride, hostStateDirOverride } from "./host-paths.js";
...
function resolveOpenClawConfigDir() {
  if (hostStateDirOverride()) {
    return hostStateDirOverride();
  }
  if (hostConfigPathOverride()) {
    return path.dirname(hostConfigPathOverride());
  }
...
export function getMergeResultByMediaOutputId(mediaOutputId) {
  ...
  try {
    ...
    row = select.get(mediaOutputId);
  } catch {
    // merge_results is created by the external diarization component and may not
    // exist in this shared db — degrade to null rather than throwing.
    return null;
  }
