...
function configuredDbPath(openclawHome, configuredPath, ...parts) {
  const homeRoot = resolvePath(openclawHome);
  const configuredRoot = resolvePath(configuredPath);
  const relativeToHome = relative(homeRoot, configuredRoot);
  if (relativeToHome === "" || (!relativeToHome.startsWith("..") && !isAbsolute(relativeToHome))) {
    return resolveInside(homeRoot, relativeToHome || ".", ...parts);
  }
  return resolveInside(configuredRoot, ...parts);
}
...
/**
 * Resolves the authoritative writer root from the effective plugin config.
 *
 * @param {object} pluginConfig
 * @param {string} openclawHome
 * @returns {string}
 */
export function resolvePromotionDbRoot(pluginConfig = {}, openclawHome) {
  const configuredBase = typeof pluginConfig?.baseDbPath === "string" && pluginConfig.baseDbPath.trim()
    ? pluginConfig.baseDbPath.trim()
    : join(openclawHome, "memory", "lancedb-namespaced");
  const baseDbPath = isAbsolute(configuredBase)
    ? resolvePath(configuredBase)
    : resolvePath(openclawHome, configuredBase);
  const namespacesExplicit = Object.prototype.hasOwnProperty.call(pluginConfig || {}, "namespaces");
  const layout = resolveNamespaceLayout(baseDbPath, pluginConfig?.namespaces || {}, {
    explicit: namespacesExplicit,
  });
  const writerRoot = layout.mode === "named"
    ? resolvePath(layout.baseDir, layout.activeWriteNamespace)
    : resolvePath(layout.baseDbPath);
  return configuredDbPath(openclawHome, writerRoot);
}

function loadPredecessorMarkers(openclawHome, agentId, readFile) {
  const statePath = resolveInside(openclawHome, ".embed-promotions-state", `${safeAgentId(agentId)}-promotions.json`);
  if (!existsSync(statePath)) return new Set();
  const state = JSON.parse(readFile(statePath, "utf8"));
  return new Set(Array.isArray(state?.embedded) ? state.embedded.filter((value) => typeof value === "string") : []);
}
 ...
 *
 * @param {{targets: Array<object>, openclawHome: string, dbRoot?: string, readFile?: Function}} options
 * @returns {{targets: Array<object>}}
 */
export function planPromotionReindex({ targets, openclawHome, dbRoot, readFile = readFileSync }) {
  const writerRoot = dbRoot || resolveInside(openclawHome, "memory", "lancedb-namespaced");
  const predecessorRoot = resolvePath(openclawHome, "memory", "lancedb-namespaced");
  const honorPredecessorState = resolvePath(writerRoot) === predecessorRoot;
  const plannedTargets = [];
  for (const target of targets || []) {
    const agentId = safeAgentId(target.agentId);
    const content = readFile(target.memoryPath, "utf8");
    const seen = new Set();
    const predecessorMarkers = honorPredecessorState
      ? loadPredecessorMarkers(openclawHome, agentId, readFile)
      : new Set();
    let alreadyEmbedded = 0;
    const promotions = [];
    for (const promotion of parsePromotionMarkers(content)) {
      if (predecessorMarkers.has(promotion.marker)) {
        alreadyEmbedded += 1;
        continue;
      }
    ...
    plannedTargets.push({
      ...
      dbPath: configuredDbPath(openclawHome, writerRoot, agentId),
      promotions,
      alreadyEmbedded,
    });
  }
...
function emptyCounts(planned = 0, skipped = 0) {
  return { planned, inserted: 0, skipped, failed: 0 };
}

function isExistingPredecessorPromotion(results, text) {
  return Array.isArray(results) && results.some((candidate) => {
    const entry = candidate?.entry || candidate;
    return entry?.origin === "dreaming-promotion" && entry?.text === text;
  });
}
...
export async function applyPromotionReindex(plan, dependencies = {}) {
  ...
  const predecessorSkipped = targets.reduce((sum, target) => sum + (target.alreadyEmbedded || 0), 0);
  const result = {
    ok: true,
    mode: dependencies.apply === true ? "apply" : "dry-run",
    counts: emptyCounts(planned, predecessorSkipped),
    agents: [],
    failures: [],
  };
  if (dependencies.apply !== true || planned === 0) {
    result.agents = targets.map((target) => ({
      ...
      counts: emptyCounts(target.promotions?.length || 0, target.alreadyEmbedded || 0),
    }));
    return result;
  }
  ...
  for (const target of targets) {
    const agentCounts = emptyCounts(target.promotions?.length || 0, target.alreadyEmbedded || 0);
    let db;
    try {
      ...
      for (const promotion of target.promotions || []) {
        try {
          ...
          if (typeof db.search === "function") {
            const predecessors = await db.search(vector, 5, 0.95);
            if (isExistingPredecessorPromotion(predecessors, promotion.text)) {
              result.counts.skipped += 1;
              agentCounts.skipped += 1;
              continue;
            }
