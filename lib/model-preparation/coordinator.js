...
export function createModelPreparationCoordinator({
  ...
  const run = async () => {
    ...
    try {
      ...
      // A shutdown aborts the preparation on purpose; the state below already
      // records `model_preparation_interrupted`. Warning about it made a clean
      // shutdown look like a failure in the operator log.
      if (interrupted) {
        logger?.info?.("[model-preparation] interrupted before the model was ready; no action needed");
      } else {
