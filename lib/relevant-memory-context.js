...
import { trimAtRecordBoundary, TRUNCATION_MARKER } from "./inject-budget.js";
 ...
 * (closing any of `<relevant-memories>`/`<memory-semantic-lens>`/
 * `<plur1bus-recall>` still open at that point) via the same
 * `trimAtRecordBoundary` helper `applyGlobalInjectBudget` uses
 * (lib/inject-budget.js) — an arbitrary character cut can otherwise leave a
 * half-open element or an unclosed wrapper, which is malformed XML the model
 * then has to parse. Falls back to a plain character slice only when no
 * complete record fits at all, so this function still always returns
 * `<= limit` characters.
 *
 * @param {string} output
