...
export function formatRelevantMemoriesContext(memories, {
  ...
  // Build overlay lookup: targetMemoryId → first overlay encountered in the input array
  ...
  const items = memories.map((m) => {
    ...
    if (ov && ov.status !== "provisional") {
      ...
      const overlayConfidence = typeof ov.confidence === "number" && ov.confidence >= 0 && ov.confidence <= 1
        ? ov.confidence
        : (typeof ov.confidenceDelta === "number" ? Math.max(0, Math.min(1, 0.6 + ov.confidenceDelta)) : 0.6);
      const confidenceStr = overlayConfidence.toFixed(2);
      overlayBlock = `\n    <interpretation-overlay shift-type="${safeShiftType}" confidence="${confidenceStr}" weeks-ago="${weeksAgo}" trigger-memory-ids="${triggerIdsStr}">${safeShiftDesc}</interpretation-overlay>`;
    }
