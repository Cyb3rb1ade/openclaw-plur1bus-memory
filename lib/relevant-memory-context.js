...
export function formatRelevantMemoriesContext(memories, {
  ...
  const renderMemoryItems = (input, { forceSource = null, trackOperationalWarning = false } = {}) => input.map((m) => {
    ...
    const epistemicAttr = ` epistemic="${sanitizeMemoryContextAttribute(normalizeEpistemicStatus(m.epistemicStatus), "untrusted")}"`;
    ...
    if (overlayBlock) {
      return `  <memory-record category="${category}" source="${safeSource}"${graphSourceAttr} id="${id}"${fadeAttr}${uncertainAttr}${dreamAttr}${depthAttr}${associationStrengthAttr}${statusAttr}${updateSourceAttr}${versionAttr}${epistemicAttr}${temporalAttrs}${traceAttrs}><quoted-evidence>${safeDisplay}</quoted-evidence>${overlayBlock}\n  </memory-record>`;
    }
    return `  <memory-record category="${category}" source="${safeSource}"${graphSourceAttr} id="${id}"${fadeAttr}${uncertainAttr}${dreamAttr}${depthAttr}${associationStrengthAttr}${statusAttr}${updateSourceAttr}${versionAttr}${epistemicAttr}${temporalAttrs}${traceAttrs}><quoted-evidence>${safeDisplay}</quoted-evidence></memory-record>`;
  }).join("\n");
