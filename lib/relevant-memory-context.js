...
import { trimAtRecordBoundary, TRUNCATION_MARKER, closeOpenElements } from "./inject-budget.js";
 ...
 *   onTruncate?: ({from, to}) => void — called once when the maxTotalChars cap shortens the block
 * }} options
 * @returns {string}
 */
 ...
 *
 * Cuts at the end of the last complete `<memory-record>` element that fits,
 * closing whatever element(s) are still open at that point — a real scan of
 * the tag stream, not a fixed list of "known wrapper" names (see
 * `trimAtRecordBoundary` / `closeOpenElements` in lib/inject-budget.js) — via
 * the same helper `applyGlobalInjectBudget` uses. An arbitrary character cut
 * can otherwise leave a half-open element or an unclosed wrapper, which is
 * malformed XML the model then has to parse.
 *
 * When no complete record fits at all (including when there are none),
 * falls back to cutting just before the first `<memory-record` — or at
 * `limit` if the budget doesn't even reach one, or none is present — rather
 * than slicing mid-record, then appends the marker and closes whatever is
 * still open at that cut point (again via `closeOpenElements`). Outside of
 * the operational-safety-warning branch below, this function returns
 * `<= limit` characters.
 *
 * The operational-safety-warning branch is an exception: when `limit` would
 * cut into that warning block, it is preserved whole regardless of `limit`
 * (deliberately — the warning matters more than the cap here), so the
 * result can run well past `limit` (observed: ~639 chars against a much
 * smaller limit) and can itself leave `<relevant-memories>` open, since it
 * returns a slice of `output` directly rather than going through
 * `trimAtRecordBoundary`/`closeOpenElements`. Pre-existing, tracked as a
 * follow-up (see `.superpowers/fix-report.md`), not fixed here.
 *
 * @param {string} output
...
function truncateMemoryContext(output, maxTotalChars) {
  ...
  const trimmed = trimAtRecordBoundary(output, limit);
  if (trimmed !== null) return trimmed;

  // No complete <memory-record> fits within `limit` (or there is none at all).
  // Cut before the first one rather than mid-record — or at `limit` when the
  // budget doesn't even reach the first record, or none exists — and close
  // whatever wrapper(s) that cut lands inside of, so this fallback is still
  // well-formed XML. Reserving room for those closings can itself push the
  // cut earlier, which can change what's open, so this converges the same
  // bounded way `trimAtRecordBoundary` does (non-increasing cut each pass).
  const recordIdx = output.indexOf("<memory-record");
  const firstCut = recordIdx !== -1 && recordIdx <= limit ? recordIdx : limit;
  let cut = firstCut;
  let reserve = 0;
  for (;;) {
    const budget = limit - marker.length - reserve;
    cut = Math.min(cut, Math.max(0, budget));
    const closings = closeOpenElements(output.slice(0, cut));
    if (cut + marker.length + closings.length <= limit) {
      return output.slice(0, cut) + marker + closings;
    }
    if (cut === 0) {
      // Pathological: even a bare marker plus closings doesn't fit `limit`.
      return (marker + closings).slice(0, limit);
    }
