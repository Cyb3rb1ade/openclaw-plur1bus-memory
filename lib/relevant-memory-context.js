...
export function formatRelevantMemoriesContext(memories, {
  ...
  const renderMemoryItems = (input, { forceSource = null } = {}) => input.map((m) => {
    ...
    return `  <memory-record category="${category}" source="${safeSource}"${graphSourceAttr} id="${id}"${fadeAttr}${depthAttr}${associationStrengthAttr}><quoted-evidence>${display}</quoted-evidence></memory-record>`;
  }).join("\n");
