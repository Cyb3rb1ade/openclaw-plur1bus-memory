...
export function formatRelevantMemoriesContext(memories, {
  ...
  const renderMemoryItems = (input, { forceSource = null, trackOperationalWarning = false } = {}) => input.map((m) => {
    ...
    if (overlayBlock) {
      return `  <memory-record category="${category}" source="${safeSource}"${graphSourceAttr} id="${id}"${fadeAttr}${dreamAttr}${depthAttr}${associationStrengthAttr}${statusAttr}${updateSourceAttr}${versionAttr}${temporalAttrs}${traceAttrs}><quoted-evidence>${safeDisplay}</quoted-evidence>${overlayBlock}\n  </memory-record>`;
    }
    return `  <memory-record category="${category}" source="${safeSource}"${graphSourceAttr} id="${id}"${fadeAttr}${dreamAttr}${depthAttr}${associationStrengthAttr}${statusAttr}${updateSourceAttr}${versionAttr}${temporalAttrs}${traceAttrs}><quoted-evidence>${safeDisplay}</quoted-evidence></memory-record>`;
  }).join("\n");
