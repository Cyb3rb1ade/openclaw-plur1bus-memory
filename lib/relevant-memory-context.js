  ...
  const items = memories.map((m) => {
    ...
    if (ov) {
      ...
      const triggerIdsStr = Array.isArray(ov.provenance?.triggerMemoryIds)
        ? ov.provenance.triggerMemoryIds.join(",")
        : "";
      overlayBlock = `\n    <interpretation-overlay shift-type="${safeShiftType}" confidence="0.60" weeks-ago="${weeksAgo}" trigger-memory-ids="${triggerIdsStr}">${safeShiftDesc}</interpretation-overlay>`;
    }
