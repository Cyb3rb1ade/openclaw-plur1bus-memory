...
function containsPhrase(text, phrases) {
  ...
  for (const phrase of phrases) {
    ...
    // Allow flexible spacing/punctuation between words, but only match whole
    // words: without the boundaries the German markers "nie" and "immer" fired
    // inside "Knie", "Melanie", "Ingenieur" or "Zimmer" and pushed those
    // memories to the 0.7 importance floor (found 18.09.2026). Lookarounds
    // instead of \b because \b does not treat umlauts as word characters.
    const body = pattern.replace(/\s+/g, "\\s*[^\\p{L}\\p{N}]*\\s*");
    const regex = new RegExp(`(?<![\\p{L}\\p{N}])${body}(?![\\p{L}\\p{N}])`, "iu");
...
/**
 * Wendet die Obergrenze fuer automatische Wichtigkeitswerte an.
 * Explizite Werte passieren ungekuerzt durch; automatische Werte werden auf
 * AUTOMATIC_IMPORTANCE_MAX begrenzt.
 *
 * @param {number} value — zu cappender Wert
 * @param {boolean} isExplicit — true, wenn vom Agenten explizit gesetzt
 * @returns {number} gepufferte oder durchgelassene Wichtigkeit
 */
export function capAutomaticImportance(value, isExplicit) {
  return isExplicit ? value : Math.min(value, AUTOMATIC_IMPORTANCE_MAX);
}
 ...
 *
 * Automatische Werte werden auf AUTOMATIC_IMPORTANCE_MAX (0.94) begrenzt;
 * der Bereich 0.95 bis 1.00 ist der Entscheidung des Agenten vorbehalten.
 *
 * @param {Object} params
...
export function computeMemoryImportance(params = {}) {
  ...
  const capped = capAutomaticImportance(normalized, isExplicit);

  // Append cap reason if it was actually applied
  let importanceReason = reasons.join("; ") || "default importance";
  if (!isExplicit && capped < normalized) {
    importanceReason = importanceReason + `; automatic cap ${AUTOMATIC_IMPORTANCE_MAX} applied (was ${normalized})`;
  }
