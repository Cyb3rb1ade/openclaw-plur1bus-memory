...
function extractParticipants(turns, { names = null, voiceSpeakers = [] } = {}) {
  ...
  for (const speaker of Array.isArray(voiceSpeakers) ? voiceSpeakers : []) push(speaker);
  return out.slice(0, 10);
}

// Woerter, die in Transkripten vor einem Doppelpunkt stehen, aber keine
// Sprecher sind (Ueberschriften, Metadaten des Bots, Cron-Prosa).
const VOICE_LABEL_STOP = new Set([
  "stimmung", "source", "hinweis", "achtung", "beispiel", "frage", "antwort", "thema",
  "status", "kommentar", "titel", "fazit", "notiz", "update", "ergebnis", "info", "betreff",
  "sprecher", "speaker", "transcript", "audio", "note", "warnung", "tipp", "kurz", "wichtig",
  "zusammenfassung", "link", "quelle", "datum", "uhrzeit", "ort", "name", "nutzer", "user",
]);
const VOICE_LABEL_RE = /(?:^|\n|"\s*|\|\s*|\.\s+|\?\s+|!\s+)([\p{Lu}][\p{Ll}]+(?:[ -][\p{Lu}][\p{Ll}]+)?):\s/gu;

/**
 * 7.12.43: Mitsprecher aus diarisierten Sprachnachrichten. Die STT-Bruecke
 * stellt erkannte Stimmen als `Name: …` vor den Text (`Eva: …`); wer per
 * Stimmprofil erkannt wurde, hat also gesprochen und gehoert zu den
 * Teilnehmern, nicht zu den Erwaehnten (Bernd 10.09.2026 23:3x). Unerkannte
 * Stimmen (`Sprecher 1`) bleiben aussen vor.
 */
export function extractVoiceSpeakers(turns, { names = null } = {}) {
  const own = new Set([names?.user, names?.assistant].filter(Boolean).map((x) => x.toLowerCase()));
  const out = [];
  for (const turn of Array.isArray(turns) ? turns : []) {
    if (turn?.role !== "user") continue;
    const text = String(turn.content || "");
    if (!/\[Audio transcript/i.test(text)) continue;
    for (const match of text.matchAll(VOICE_LABEL_RE)) {
      const name = match[1].trim();
      const lower = name.toLowerCase();
      if (VOICE_LABEL_STOP.has(lower) || VOICE_LABEL_STOP.has(lower.split(/[ -]/)[0])) continue;
...
export function createEpisode(turns, opts = {}) {
  ...
  const voiceSpeakers = extractVoiceSpeakers(turns, { names: opts.participantNames || null });
  const participants = extractParticipants(turns, { names: opts.participantNames || null, voiceSpeakers });
...
export async function enrichEpisodeNarratively(episode, turns, llmCfg, callLlm, opts = {}) {
  ...
  const voiceSpeakers = Array.isArray(episode.voiceSpeakers) ? episode.voiceSpeakers : [];
  const voiceLine = voiceSpeakers.length > 0
    ? `\nPer Stimme erkannte Mitsprecher (haben selbst gesprochen, zählen NICHT als „people"): ${voiceSpeakers.join(", ")}.`
    : "";
  const prompt = `Analysiere das folgende Gespräch zwischen ${names.user} (Mensch) und ${names.assistant} (Agent) und beschreibe es als Geschichte.${voiceLine}
...
Antworte NUR mit diesem JSON-Format (keine Code-Zäune, kein Text davor oder danach):
{
  ...
  "emotion": "Grundstimmung, wie sie im Gespräch spürbar ist — auch bei sachlichem Thema, wenn es jemanden berührt: joy|trust|anticipation|sadness|anger|fear|surprise|disgust; neutral nur, wenn keine Gefühlsregung erkennbar ist",
  "emotionIntensity": "niedrig = klingt an, mittel = prägt Teile des Gesprächs, hoch = bestimmt das Gespräch"
}
...
export function applyEnrichment(episode, parsed, { turns = [], names = null } = {}) {
  ...
  const voiceSpeakers = Array.isArray(episode.voiceSpeakers) ? episode.voiceSpeakers : [];
  const speakerNames = [...(names ? [names.user, names.assistant] : []), ...voiceSpeakers];
  ...
  const out = {
    ...
    participants: extractParticipants(turns, { names, voiceSpeakers }),
    mentioned,
  };
  if (llmEmotion) {
 ...
 * @returns {Promise<{episodes: Array, openEpisode: object|null, continuedId: string|null, skippedEpisodedSpans: number}>}
 */
export async function extractEpisodesWithState(turns, opts = {}) {
  ...
  if (fresh.length === 0) {
    return {
      episodes: [],
      openEpisode: opts.openEpisode || null,
      continuedId: null,
      skippedEpisodedSpans: 0,
    };
  }
  const maxGapMs = (opts.maxGapMinutes || DEFAULT_MAX_GAP_MINUTES) * 60 * 1000;
  ...
  // 7.12.55: Bereits episodierte Turns kennt der Aufrufer; ohne die Angabe
  // bleibt es beim bisherigen Verhalten.
  const episodedTurnIds = opts.episodedTurnIds instanceof Set
    ? opts.episodedTurnIds
    : new Set(Array.isArray(opts.episodedTurnIds) ? opts.episodedTurnIds : []);
  let skippedEpisodedSpans = 0;
  let continuedId = null;
  let lastGroupAlreadyEpisoded = false;
  for (let index = 0; index < groups.length; index++) {
    ...
    // 7.12.55: Spannen, deren Turns alle schon episodiert sind, verwirft der
    // Aufrufer ohnehin (filterAlreadyEpisoded). Bis hierher zahlte jede von
    // ihnen trotzdem einen Modellaufruf — am 12.09.2026 waren das nach einem
    // Turn 15 Anreicherungen für 16 verworfene Spannen. Der Abgleich gehoert
    // vor die Anreicherung.
    const alreadyEpisoded = episodedTurnIds.size > 0
      && group.length > 0
      && group.every((turn) => turn?.id && episodedTurnIds.has(turn.id));
    if (alreadyEpisoded) skippedEpisodedSpans += 1;
    lastGroupAlreadyEpisoded = index === groups.length - 1 && alreadyEpisoded;

    // LLM-basierte narrative Anreicherung
    if (!alreadyEpisoded && group.length >= MIN_TURNS_FOR_LLM_NARRATIVE && opts.llmCfg && opts.callLlm) {
  ...
  // A skipped final group is already persisted by the caller. Rebuilding an
  // open state from that group would invent a fresh episode id and make the
  // next continuation append under an episode that was never written. Keep
  // the caller's persisted state, including for mixed new/known groups.
  const openEpisode = lastGroupAlreadyEpisoded
    ...
    : (opts.openEpisode || null);
  return { episodes, openEpisode, continuedId, skippedEpisodedSpans };
}

/**
 * 7.12.43: Baut eine bestehende Episode aus ihren Turns neu (gleiche id,
 * gleiche Erstellzeit, Fassung +1) — fuer die Nachmigration aelterer Karten
 * auf das aktuelle Schema (`mentioned`, `voice_speakers`, Modell-Emotion).
 */
export async function rebuildEpisode(episode, turns, opts = {}) {
  const sorted = [...turns].sort((a, b) => new Date(a.createdAt || 0) - new Date(b.createdAt || 0));
  if (sorted.length === 0) return null;
  const participantNames = opts.participantNames
    || resolveParticipantNames(opts.workspaceDir, { agentId: opts.agentId || episode.agentId || "agent" });
  let rebuilt = createEpisode(sorted, {
    workspaceKey: episode.workspaceKey,
    ...
    lastReplayed: episode.lastReplayed,
    visibility: episode.visibility,
  });
  rebuilt = { ...rebuilt, id: episode.id, createdAt: episode.createdAt || rebuilt.createdAt, revision: (Number(episode.revision) || 0) + 1, rebuilt: true };
  if (sorted.length >= MIN_TURNS_FOR_LLM_NARRATIVE && opts.llmCfg && opts.callLlm) {
    rebuilt = await enrichEpisodeNarratively(rebuilt, sorted, opts.llmCfg, opts.callLlm, { ...opts, participantNames });
    throwIfAborted(opts.signal, "episode rebuild aborted");
  }
  rebuilt.vividness = calculateVividness(rebuilt);
  return rebuilt;
}

/**
 * Sucht die Kartendatei einer Episode im Vault (Monatsordner ihres
 * Starttags). Liefert den Pfad nur, wenn die Datei genau diese eine Episode
 * enthaelt — Sammeldateien aelterer Versionen werden nicht ersetzt.
 */
export function findEpisodeCardPath(episode, workspaceDir) {
  try {
    const date = new Date(episode.startTime).toISOString().slice(0, 10);
    const dir = join(workspaceDir, "memory", "episodes", date.slice(0, 4), date.slice(5, 7));
    if (!existsSync(dir)) return null;
    for (const file of readdirSync(dir)) {
      if (!file.endsWith(".md")) continue;
      const path = join(dir, file);
      const text = readFileSync(path, "utf8");
      const ids = text.match(/^episode_id: .+$/gm) || [];
      if (ids.length === 1 && ids[0] === `episode_id: ${episode.id}`) return path;
    }
    return null;
  } catch (_) {
    return null;
...
export function writeEpisodeToVault(episode, workspaceDir, { replacePath = null } = {}) {
  try {
    ...
    if (episode.voiceSpeakers?.length > 0) {
      lines.push(`voice_speakers: [${episode.voiceSpeakers.join(", ")}]`);
    }
