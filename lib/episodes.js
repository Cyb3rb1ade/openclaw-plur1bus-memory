 ...
 */
function extractParticipants(turns, { names = null, people = [] } = {}) {
  ...
  for (const person of Array.isArray(people) ? people : []) {
    if (out.length >= 2 + MAX_PEOPLE) break;
    push(person);
  }
...
Antworte NUR mit diesem JSON-Format (keine Code-Zäune, kein Text davor oder danach):
{
  ...
  "people": ["weitere Personen, die im Gespräch vorkommen — ohne ${names.user} und ${names.assistant}; sonst leer"],
 ...
 * dazu topics (bis 5), people (bis 5, ohne die beiden Sprecher) und die
 * Gespraechs-Emotion — letztere nur, wenn die EmotionEngine keinen
 * mindestens mittleren Ausschlag gemeldet hat (dann zaehlt die Engine).
 */
export function applyEnrichment(episode, parsed, { turns = [], names = null } = {}) {
  ...
  const people = cleanStringList(parsed.people, { max: MAX_PEOPLE, maxLen: 60, exclude: speakerNames });
  const llmEmotion = typeof parsed.emotion === "string" && LLM_EMOTIONS.has(parsed.emotion.trim().toLowerCase())
    ...
    : null;
  const engineStrong = episode.emotionalDominant && episode.emotionalDominant !== "neutral";
  const emotionalDominant = engineStrong ? episode.emotionalDominant : (llmEmotion || episode.emotionalDominant || "neutral");
  return {
    ...episode,
    ...
    participants: extractParticipants(turns, { names, people }),
    emotionalDominant,
  };
}
