...
export function createRecallPhaseTimer({
  ...
  return {
    ...
    /**
     * Record an already-measured phase duration directly, without a
     * start/end pair. Additive observability hook (Task 19 fix round): lets
     * a caller that ran its own nested timer (e.g. one child timer per
     * namespace in `runMergedNamespaceRecall`, kept separate from this timer
     * specifically to avoid interleaving corruption under concurrent
     * `Promise.allSettled` namespace reads) fold its finished phases back
     * into this timer's `completed` list once that nested work is done. Does
     * not touch `firstStart`/`activePhaseValue`/`elapsedMs()`.
     * @param {string} phase
     * @param {number} ms
     */
    record(phase, ms) {
      if (typeof phase !== "string" || phase.length === 0) return;
      pushCompleted({ phase, ms: Math.max(0, Number(ms) || 0) });
    },
