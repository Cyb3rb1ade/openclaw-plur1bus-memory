...
export const DEFAULT_SEMANTIC_LENS_CONFIG = Object.freeze({
  // Opt-out like every other PLUR1BUS feature.
  enabled: true,
  maxLensMemories: 3,
  maxBridgeMemories: 2,
  maxFadedMemories: 1,
...
// Tokens too generic to count as thematic overlap evidence.
...
function extractText(memory) {
  const entry = memory?.entry ?? memory;
  if (!entry || typeof entry !== "object") return "";
  return [entry.text, entry.summary, entry.display].filter(Boolean).join(" ");
}
...
function hasSignificantTokenOverlap(candidateText, baseTokenSet, minOverlap = MIN_SIGNIFICANT_OVERLAP) {
  const candidateTokens = significantTokenSet(candidateText);
  if (candidateTokens.size === 0 || baseTokenSet.size === 0) return false;
  let overlap = 0;
...
// Lifecycle statuses that must never be surfaced by the lens (mirrors
// INACTIVE_REACTIVATION_STATUSES in conversation-reactivation-recall.js).
const INACTIVE_LENS_STATUSES = new Set(["superseded", "archived", "deleted", "tombstoned"]);
...
async function selectLensMemories(baseMemories, options, config) {
  ...
  const baseText = (baseMemories || []).map(extractText).filter(Boolean).join(" ");
  const baseTokens = significantTokenSet(baseText);
  ...
  const tryPick = async (id, role, communityId) => {
    ...
    if (!entry) return;
    // Drop explicitly-inactive memories. The lens index is precomputed, so a
    // memory archived/superseded/deleted after index build is still listed in
    // the community id sets. Parity with the active-only filter in the main
    // recall pipeline and Conversation-Reactivation-Recall.
    const status = entry.status ?? entry.plur1bus_status;
    if (status && INACTIVE_LENS_STATUSES.has(String(status))) return;
    if (normalizeEpistemicStatus(entry.epistemicStatus) === "invalidated") return;
    const candidateText = extractText(entry);
    if (!hasSignificantTokenOverlap(candidateText, baseTokens)) return;
    pickedIds.add(id);
