/**
 * lib/encoding-llm.js — LLM-Einstiegspunkt fuer Emotion UND Bedeutung in einem Call.
 *
 * Tier 3 (lib/tier3-llm.js) fragt das Modell ohnehin nach der Emotion einer
 * Erinnerung. Da der Text dabei schon gelesen wird, kostet eine zusaetzliche
 ...
 * `inferEmotionalValenceAsync()` und `lib/tier3-llm.js` bleiben unveraendert,
 * weil andere Aufrufer sich auf deren Rueckgabeform verlassen.
 *
 * Der Wert, den `classifyEncoding` liefert, wird spaeter zum Ausgangspunkt
 * fuer die Halbwertszeit einer Erinnerung (Task 3) und, zusammen mit der
 * emotionalen Intensitaet, fuer die Entscheidung, ob ein Ereignis als
 ...
 * der Bereich darueber ist der expliziten Entscheidung des Agenten
...
/**
 * Ein LLM-Call fuer beide Urteile. Baut Messages im selben Format wie
 ...
 * gleichermassen ok:false — nie einen erratenen Wert; die Zeile bleibt
 * unresolved und wird spaeter erneut versucht.
 *
 * @param {string} text — der zu bewertende Erinnerungstext
