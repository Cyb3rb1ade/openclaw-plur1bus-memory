...
import { AGENT_BAND_MIN, resolveHalfLifeFromEncoding } from "./memory-dynamics.js";
...
export function buildRefinePatch(row = {}, encoding = {}, now = Date.now(), opts = {}) {
  ...
  if (!isAgentBand) {
    ...
    patch.halfLifeDays = resolveHalfLifeFromEncoding(encoding.importance);
  ...
  // Dieser Cron brennt NICHT ein (Phase 3, 21.09.2026). Er lief stündlich über
  // den gesamten Bestand und entschied bei jedem Lauf neu — mit der Wirkung,
  // dass eine vier Jahre alte Zeile rückwirkend eine Halbwertszeit von zehn
  // Jahren bekam, ohne dass sie je jemand als wichtig markiert hätte. Der
  // Pilotlauf hätte so 10,3 % des Bestands eingebrannt.
  //
  // Einbrennen ist eine Entscheidung beim Erleben, nicht beim Nachsortieren:
  // Sie gehört in den Capture-Pfad (applyDynamicsDefaults, einmal je neuer
  // Zeile). Dieser Cron bewertet weiter Emotion und Wichtigkeit und setzt die
  // Halbwertszeit aus dem Importance-Band — er ändert nur nichts mehr an
  // Stärke, Klasse und Kodierungszeitpunkt.
  return patch;
}
