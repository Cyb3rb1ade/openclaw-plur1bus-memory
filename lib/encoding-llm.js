...
export function buildEncodingPrompt(text) {
  return [
    ...
    `  emotions: Objekt mit einem Wert 0.0 bis 1.0 je Dimension (${EMOTION_DIMENSIONS.join(", ")}) — wie stark jede Dimension für sich mitschwingt, nicht nur die dominante.`,
...
export function buildRefinePatch(row = {}, encoding = {}, now = Date.now(), opts = {}) {
  ...
  if (!isAgentBand) {
    ...
    // Abschluss-Review, Important 5b: coreMemoryReason ist ein enger,
    // provenienzartiger Marker ("manual_importance_marker",
    // "deep_flashbulb_threshold" — siehe applyCoreMemoryEncoding), kein
    // Freitext-Eimer. Stand vorher hier und unbedingt geschrieben, überschrieb
    // er bei einer Agentenband-Core-Memory deren Provenienz-Marker mit einem
    // deutschen Satz. updateSource/updateEvidence existieren genau für
    // Freitext-Begründungen automatischer Änderungen (siehe
    // lib/safe-update.js) — und wie importance/halfLifeDays gehört auch diese
    // Begründung nur in den nicht-Agentenband-Zweig, nie in eine Zeile, die
    // der Agent bewusst gesetzt hat.
    patch.updateSource = "emotion-refine";
    patch.updateEvidence = String(encoding.reason || "").slice(0, 200);
  }
  if (flash) {
