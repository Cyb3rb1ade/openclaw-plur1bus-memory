...
/** Wie viel Text dem Modell zur Beurteilung vorgelegt wird. Die Erinnerung
 *  selbst wird davon nicht berührt — sie steht vollständig in der Zeile. */
export const PROMPT_TEXT_MAX = 2000;

/**
 * Kürzt auf höchstens `max` UTF-16-Code-Einheiten, aber niemals mitten durch
 * ein Surrogatpaar — und räumt zusätzlich eine halbe Einheit weg, die ein
 * Aufrufer schon abgetrennt hat (index.js schneidet vor `classifyEncoding`
 * selbst auf 2000).
 *
 * Warum das nötig ist: Zeichen jenseits von U+FFFF (Emoji) bestehen in
 * JavaScript aus ZWEI Code-Einheiten. `slice()` zählt Einheiten, nicht
 * Zeichen, und kann deshalb zwischen die beiden Hälften schneiden. Der Fehler
 * ist still — `JSON.stringify` schreibt bereitwillig `\ud83c`, und in
 * JavaScript übersteht das sogar `JSON.parse`. Erst beim Kodieren des
 * HTTP-Körpers als UTF-8 gibt es für ein einsames Surrogat keine Darstellung.
 *
 * Live am 19.09.2026 im Bestands-Backfill: eine Zeile von 22.000 trägt an
 * Position 1999 ein 🎙 (U+1F399); die Route antwortete mit HTTP 400
 * ("unexpected end of hex escape"), die Zeile blieb unbewertet. Gegenprobe
 * mit demselben Text auf Codepoint-Grenze: HTTP 200.
 */
export function truncateForPrompt(text, max = PROMPT_TEXT_MAX) {
  const value = String(text ?? "");
  let end = Math.min(value.length, Math.max(0, max));
  // Ein High-Surrogat an letzter Stelle hat seinen Partner verloren (oder
  // würde ihn beim Schneiden verlieren) — eine Einheit zurück.
  if (end > 0) {
...
export function buildRefinePatch(row = {}, encoding = {}, now = Date.now(), opts = {}) {
  ...
  // Hier faellt die Blitzlicht-Entscheidung, und hier faellt sie genau einmal
  // je Zeile: Der Cron liest ausschliesslich Zeilen mit
  // `emotionStatus='pending_t3' OR importanceStatus='pending'` (index.js) und
  // setzt beide Felder anschliessend auf `final` — die Zeile verlaesst die
  // Warteschlange dauerhaft. Live nachgemessen am 21.09.2026: von 24.509
  // aktiven Zeilen standen 15 darin, 0,06 %.
  //
  // Warum nicht im Capture-Pfad: Dort liegt nur die Tier-2-Heuristik vor, und
  // die erreicht auf echten Texten hoechstens 0,60 Intensitaet (Median 0,00,
  // 90. Perzentil 0,57). Fuer die Schwelle 0,80 braeuchte es bei maximaler
  // automatischer Wichtigkeit 0,66 — in einer Woche mit 903 neuen Zeilen kam
  // diese Kombination kein einziges Mal vor. Das Modellurteil aus Tier 3 ist
  // der erste Zeitpunkt, an dem ueber ein Einbrennen ueberhaupt belastbar
  ...
  // Der ausdrueckliche Agentenband-Schutz geht dem automatischen Blitzlicht vor.
  if (flash && !isAgentBand) {
    ...
    // nur ueber die zufaellige Signatur ihrer Halbwertszeit auffindbar.
