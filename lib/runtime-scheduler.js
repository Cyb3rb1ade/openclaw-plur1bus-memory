...
export function isBackgroundTurn(event = {}, ctx = {}) {
  ...
  const trigger = textOf(event.trigger || ctx.trigger).toLowerCase();
  if (origin === "cron" || origin === "internal") return true;
  if (kind === "cron" || kind === "heartbeat" || kind === "background") return true;
  if (["cron", "heartbeat", "background", "manual"].includes(trigger)) return true;
...
export function createBackgroundMemoryScheduler({ config = {}, logger = console, now = Date.now } = {}) {
    ...
    while (stats.recallActive < cfg.maxConcurrentRecall && recallQueue.length > 0) {
      ...
      Promise.resolve()
        ...
        .then((value) => {
          ...
          // A job whose signal fired (caller abort or timeout) already
          // resolved its caller; its late value must not become the cached
          // answer for the next turn.
          if (job.cacheKey && value !== undefined && !job.signal?.aborted) {
  ...
  function runRecall(meta, fn) {
    ...
    const phaseTimer = meta?.phaseTimer || createRecallPhaseTimer({
      // Das weiche Budget lag als feste Differenz von 10 s unter dem harten.
      // Das setzt ein grosses hartes Budget voraus: sobald es unter das
      // Hook-Fenster des Hosts gezogen wird (Default 15 s fuer
      // before_prompt_build), blieben davon nur noch Millisekunden uebrig.
      // Anteilig bleibt es in jeder Groessenordnung brauchbar und behaelt beim
      // bisherigen Wert von 20 s dieselben 10 s wie zuvor.
      softBudgetMs: Math.max(1_000, Math.min(cfg.recallTimeoutMs - 2_000, Math.round(cfg.recallTimeoutMs * 0.5))),
    ...
    const timeout = makeTimeout(timeoutMs, () => {
      ...
      // Ein Job, der bis zum Timeout nie einen Slot bekam, hat auf belegte
      // Slots gewartet. Er wird aus der Queue genommen, damit er nicht spaeter
      // noch sinnlos anlaeuft, und die Zeile nennt die Slot-Lage.
      const started = Boolean(job?.startedAt);
      if (!started && job) {
        const idx = recallQueue.queue.indexOf(job);
        if (idx >= 0) recallQueue.queue.splice(idx, 1);
      }
      const queueWaitMs = (job?.startedAt ?? Date.now()) - enqueuedAt;
      logger?.warn?.(
        ...
        `activeCount=${stats.recallActive} ` +
        `maxConcurrent=${cfg.maxConcurrentRecall} ` +
        `queueDepth=${recallQueue.length} ` +
  ...
  function drainCapture(key) {
    ...
    while (state.active < cfg.maxConcurrentCapturePerAgent && state.queue.length > 0) {
      ...
      stats.captureActive++;
      const controller = new AbortController();
      const jobSignal = job.callerSignal && typeof AbortSignal.any === "function"
        ? AbortSignal.any([controller.signal, job.callerSignal])
        : controller.signal;
      ...
      const fnPromise = Promise.resolve().then(() => job.fn(jobSignal));
  ...
  function enqueueCapture(agentId, meta, fn) {
    ...
    // A caller signal (Engine.capture's CaptureHandle) is linked exactly as
    // runRecall links it: already aborted means nothing is enqueued.
    const callerSignal = meta?.signal ?? null;
    if (callerSignal?.aborted) {
      return Promise.resolve({ ok: false, aborted: true, reason: "aborted", background });
    }
