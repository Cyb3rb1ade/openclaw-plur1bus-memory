...
export class OverlayGenerator {
  ...
  async generate({
    ...
    return this._resolveContradictions(result, memory);
  }
...
Current conversation context: ${safeContext.slice(0, 500)}`,
  ...
  async _resolveContradictions(result, memory) {
    ...
    const contradictions = await detector.findContradictionsForNewOverlay(result, meaningOverlays);
