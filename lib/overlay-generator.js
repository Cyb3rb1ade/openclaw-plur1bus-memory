...
export class OverlayGenerator {
  ...
  async generate({
    ...
    // Early dedupe with inferred shift type to avoid unnecessary LLM calls.
    let earlyDedupeKey = null;
    if (this.overlayStore) {
      earlyDedupeKey = this.overlayStore.computeDedupeKey(
        memory.id,
        this._inferShiftType(memory, conversationContext),
        conversationContext,
      );
    ...
    if (confidence === null || confidence < this.confidenceThreshold) return null;
    ...
    return {
      ...
      confidence,
      confidenceDelta: typeof parsed.confidenceDelta === "number" ? parsed.confidenceDelta : 0,
      triggerContext: conversationContext.slice(0, 500),
      dedupeKey,
      provenance: {
  ...
  _hasContextualSignal(memory, currentRegister, conversationContext) {
    ...
    if (!currentRegister || !memory.emotionalValence) return false;
...
Current conversation context: ${conversationContext.slice(0, 500)}`,
      },
    ];
  }
}
