...
const UNRESOLVED_HINT_RE = /\b(unresolved|offen|noch nicht|pending|follow.?up)\b/i;
...
export class OverlayGenerator {
  constructor({
    ...
    this.logger = logger;
    this.provisionalByDefault = provisionalByDefault;
    this.contradictionLlm = contradictionLlm;
    this.autoResolveContradictions = autoResolveContradictions;
   ...
   * @param {AbortSignal} [opts.signal] — scheduler cancellation signal
  ...
  async generate({
    ...
    throwIfAborted(signal, "overlay generation aborted");
    ...
    let completion;
    try {
      completion = await this.llm(prompt);
      throwIfAborted(signal, "overlay generation aborted");
    } catch (error) {
      throwIfAborted(signal, "overlay generation aborted");
      safeWarnLlmFailure(this.logger, "OverlayGenerator.llm", error, { memoryId: memory.id });
      return null;
    }

    const response = typeof completion === "string" ? completion : completion?.text;
    const resolvedModel = typeof completion?.model === "string" && completion.model.trim()
      ? completion.model.trim()
      : null;

    if (typeof response !== "string" || !response || response.trim().toLowerCase() === "no shift") return null;
    ...
    const confidenceDelta = typeof parsed.confidenceDelta === "number"
      ? Math.max(-1, Math.min(1, parsed.confidenceDelta))
      : 0;
    ...
    const result = {
      ...
      status: this.provisionalByDefault ? "provisional" : "active",
      ...
      provenance: {
        ...
        ...(resolvedModel ? { llmModel: resolvedModel } : {}),
      },
    };

    const resolved = await this._resolveContradictions(result, memory, signal);
    throwIfAborted(signal, "overlay generation aborted");
    return resolved;
  }
  ...
  _hasContextualSignal(memory, currentRegister, conversationContext) {
    ...
    if (UNRESOLVED_HINT_RE.test(context)) return true;

    // Explicit contextual shift signal.
    const contextualShiftPhrases = [
      "since then", "since we last", "since that", "no longer",
    ...
    if (/\bnow\b/i.test(context)) return true;
  ...
  _inferShiftType(memory, conversationContext) {
    if (UNRESOLVED_HINT_RE.test(conversationContext)) {
...
Confidence must be 0.0–1.0. "confidenceDelta" must be -1.0 to 1.0. If no shift is needed, reply exactly with: no shift`,
      },
      {
        role: "user",
...
Current conversation context: ${safeContext.slice(0, 500)}`,
  ...
  async _resolveContradictions(result, memory, signal = null) {
    throwIfAborted(signal, "overlay generation aborted");
    if (!this.autoResolveContradictions || !this.contradictionLlm) return result;
    if (!this.overlayStore || result.shiftType !== "meaning") return result;

    const { ContradictionDetector } = await import("./contradiction-detector.js");
    const detector = new ContradictionDetector({
      llm: this.contradictionLlm,
      workspaceDir: this.workspaceDir,
      logger: this.logger,
    });

    const existing = await this.overlayStore.loadAllOverlays([memory.id], {
      includeProvisional: false,
      includeSuperseded: false,
      includeDisabled: false,
      maxAgeDays: this.maxAgeDays,
    });
    const meaningOverlays = existing.filter((o) => o.shiftType === "meaning");
    const contradictions = await detector.findContradictionsForNewOverlay(result, meaningOverlays, { signal });
    throwIfAborted(signal, "overlay generation aborted");
    if (contradictions.length === 0) return result;

    // Supersede the most recent contradictory overlay.
    const byId = new Map(meaningOverlays.map((o) => [o.id, o]));
    const sorted = contradictions.sort((a, b) => {
      const aTime = new Date(byId.get(a.overlayB)?.createdAt || 0).getTime();
      const bTime = new Date(byId.get(b.overlayB)?.createdAt || 0).getTime();
      return bTime - aTime;
    });
    const target = sorted[0];

    result.supersedes = target.overlayB;
    result.status = "active";
    result.triggerContext += " [auto-resolved contradiction]";
    result.autoContradiction = {
      targetMemoryId: memory.id,
      overlayA: result.id,
      overlayB: target.overlayB,
      descriptionA: result.shiftDescription,
      descriptionB: byId.get(target.overlayB)?.shiftDescription,
    };
