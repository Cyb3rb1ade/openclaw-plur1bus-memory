...
export class OverlayGenerator {
  ...
  async generate({
    ...
    try {
      ...
      safeDebug(this.logger, "OverlayGenerator.parse", error, { memoryId: memory.id, response });
      return null;
    }
    ...
    const confidenceDelta = Number.isFinite(parsed.confidenceDelta) ? parsed.confidenceDelta : 0;
