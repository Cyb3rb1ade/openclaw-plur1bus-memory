...
import { getProcessSingleton, resetProcessSingleton } from "./process-singleton.js";
 ...
 */
export const SHARED_NEO_WORKER_KEY = "plur1bus.neoWorkerRuntime.v1";

/**
 * 7.12.36: Ein Worker-Thread je Prozess. Jede Plugin-Instanz startete sonst
 * ihren eigenen (5 WorkerThreads bei 4 Instanzen, 10.09.2026); eine
 * geschlossene Laufzeit wird ersetzt.
 */
export function getSharedNeoWorkerRuntime(options = {}) {
  const existing = getProcessSingleton(SHARED_NEO_WORKER_KEY, () => createNeoWorkerRuntime(options));
  if (typeof existing.isClosed === "function" && existing.isClosed()) {
    resetProcessSingleton(SHARED_NEO_WORKER_KEY);
    return getProcessSingleton(SHARED_NEO_WORKER_KEY, () => createNeoWorkerRuntime(options));
  }
  return existing;
}
...
export function createNeoWorkerRuntime(options = {}) {
  ...
  const ensureWorker = () => {
    ...
    lastSpawn = { startedAt: spawnStartedAt, onlineAt: null };
    currentWorker.once("online", () => {
      if (lastSpawn && lastSpawn.startedAt === spawnStartedAt) lastSpawn.onlineAt = Date.now();
    });
  ...
  const postActiveJob = (job) => {
    ...
    pending.set(job.id, {
      resolve: (value) => {
        // 7.12.24: Wartezeit in der Queue, Worker-Start und Umlauf sichtbar
        // machen — die Worker-Phasen allein erklaerten die 8–18 s nach
        // Gateway-Neustarts nicht.
        if (value && typeof value === "object") {
          const doneAt = Date.now();
          value.timings = {
            ...(value.timings || {}),
            queueWaitMs: Math.max(0, postedAt - job.enqueuedAt),
            spawnMs: hadWorker ? 0 : Math.max(0, (lastSpawn?.onlineAt || doneAt) - (lastSpawn?.startedAt || postedAt)),
            roundTripMs: Math.max(0, doneAt - postedAt),
          };
        }
        finishActiveJob(job, null, value);
      },
  ...
  return {
    ...
    /**
     * 7.12.24: Worker-Thread vorab starten (Gateway-Start), damit der erste
     * agent_end nach einem Neustart nicht auf Thread-Start und Modul-Laden
     * wartet. Fehler sind unkritisch, der naechste Job startet ihn erneut.
     * @returns {boolean} true, wenn ein Worker existiert
     */
    warmUp() {
      ...
      try {
        ...
        safeWarn(logger, `plur1bus-neo: worker warm-up failed: ${error?.message || String(error)}`);
        return false;
      }
    },
