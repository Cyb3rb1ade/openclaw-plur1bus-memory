...
/**
 * Bündelt kurze Stücke mit einem Nachbarn, statt ihre Aussage zu verwerfen.
 *
 * Ein kurzes Stück ist fast immer eine Antwort oder ein Nachsatz — „Nein.",
 * „Ja.", „Halt!" — und bezieht sich auf das, was DAVOR stand. Deshalb hängt
 * es an das vorige Stück; nur am Anfang, wo es kein voriges gibt, wandert es
 * an das nächste. So bleibt die Reihenfolge der Wörter erhalten und die
 * Antwort steht bei ihrer Frage, nicht vor der nächsten Aussage.
 */
function preserveShortParts(parts) {
  ...
  for (const part of parts) {
    ...
    if (text.length >= CHUNK_MIN_PART_CHARS) { out.push(text); pending = ""; }
