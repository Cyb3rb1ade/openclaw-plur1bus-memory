...
import { safeStatus } from "./sql-safety.js";
 ...
 */
function isProtectedFromGc(m) {
  // neverForget and core-class memories must never be archived by GC — same
  // contract memory-compaction enforces. neverForget may be stored as true or 1.
  return m && (m.memoryClass === "core" || m.neverForget === true || m.neverForget === 1);
}

export function selectCandidatesForGc(memories, policy = {}) {
  const list = Array.isArray(memories) ? memories : [];
  // Exclude protected memories up front so no policy branch can select them.
  const active = list.filter((m) => isActiveMemory(m) && !isProtectedFromGc(m));
...
export async function archiveMemories(db, memoryIds, archiveDir) {
  ...
  for (const id of memoryIds) {
    ...
    try {
      await db.update(id, { status: safeStatus("archived") });
      archived++;
    } catch {
      skipped++;
    }
