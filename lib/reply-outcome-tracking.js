...
import { readJsonl, readJsonlTail } from "./jsonl-utils.js";
import { recordFeedbackBatch } from "./feedback-log.js";
...
export const DEFAULT_MAX_REPLY_OUTCOME_LOG_ENTRIES = 5000;
...
function appendJsonl(path, entry, options = {}) {
  const existing = readJsonl(path);
  const idx = existing.findIndex((e) => e.id === entry.id);
  if (idx >= 0) existing.splice(idx, 1);
  existing.push(entry);
  const maxEntries = normalizeMaxEntries(options.maxEntries, DEFAULT_MAX_REPLY_OUTCOME_LOG_ENTRIES);
  const bounded = existing.length > maxEntries ? existing.slice(-maxEntries) : existing;
  const tmp = `${path}.tmp`;
  writeFileSync(tmp, bounded.map((e) => JSON.stringify(e)).join("\n") + "\n", "utf8");
  renameSync(tmp, path);
}
...
export async function completePendingReplyOutcomes(workspaceDir, params = {}) {
  ...
  for (const pending of matched) {
    ...
    appendJsonl(logPath(workspaceDir), entry, { maxEntries: params.maxOutcomeLogEntries });
    ...
    const dynamics = params.applyDynamics === true
      ? {
        ...
        ...(typeof params.dynamicsScheduler === "function" ? { dynamicsScheduler: params.dynamicsScheduler } : {}),
      }
      : {};
    const feedbackEntries = (pending.memoryIds || []).map((memoryId) => ({
      query: pending.userPrompt,
      memoryId,
    ...
    const maybePromise = recordFeedbackBatch(workspaceDir, feedbackEntries, {
      ...dynamics,
      maxEntries: params.maxFeedbackLogEntries,
    });
...
function normalizeReplyOutcomeReadArgs(limitOrOptions = 0, maybeOptions = {}) {
  if (limitOrOptions && typeof limitOrOptions === "object" && !Array.isArray(limitOrOptions)) {
    return {
      limit: normalizeMaxEntries(limitOrOptions.limit, 0),
      options: limitOrOptions,
    };
  }
  return {
    limit: normalizeMaxEntries(limitOrOptions, 0),
    options: maybeOptions && typeof maybeOptions === "object" ? maybeOptions : {},
  };
}

/**
 * Reads reply-outcome log entries newest-first with optional tail limits.
 * `maxBytes` is a bounded read, not a skip: oversized logs are read from the
 * end (last maxBytes), so the newest entries survive; `onSkip` fires as an
 * info hook when the read is truncated.
 *
 * @param {string} workspaceDir
 * @param {number|{limit?: number, maxBytes?: number, onSkip?: (msg: string) => void}} [limitOrOptions=0]
 * @param {{maxBytes?: number, onSkip?: (msg: string) => void}} [maybeOptions={}]
 * @returns {object[]}
 */
export function readReplyOutcomeLog(workspaceDir, limitOrOptions = 0, maybeOptions = {}) {
  const { limit, options } = normalizeReplyOutcomeReadArgs(limitOrOptions, maybeOptions);
  const entries = readJsonlTail(logPath(workspaceDir), options);
