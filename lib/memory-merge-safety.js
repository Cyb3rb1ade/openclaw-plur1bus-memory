...
export function hasMeaningfulDifference(a, b) {
  if (a === b) return false;
  if (normalizeMemoryText(a) === normalizeMemoryText(b)) return false;
  return extractStructuredDifferences(a, b).length > 0;
}

export function isSafeDuplicate(a, b) {
  if (a === b) return true;
  if (normalizeMemoryText(a) === normalizeMemoryText(b)) return true;
  return !hasMeaningfulDifference(a, b);
}
