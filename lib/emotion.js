...
function getEngine() {
  if (!_engine) {
    ...
    _engine = new EmotionEngine({
      ...
      tier3: {
        ...(typeof cfg.t3?.model === "string" && cfg.t3.model.trim()
          ? { model: cfg.t3.model.trim() }
          : {}),
        enabled: cfg.t3?.enabled !== false,
...
function emotionScoreToLegacy(score) {
  ...
  const legacy = {
    ...
    // 7.12.22: Konfidenz und Stufe wandern mit, damit der Capture-Pfad
    // entscheiden kann, ob der Cron `emotion-refine` mit Tier 3 nachbessert.
    confidence: clamp01(score.confidence ?? 0),
    tierUsed: score.tier_used ?? 1,
 ...
 * @param {{agentId?: string, runtimeLlm?: object, signal?: AbortSignal}} [context] — call-local provider context
