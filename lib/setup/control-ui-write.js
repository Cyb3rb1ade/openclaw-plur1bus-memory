...
import { isPartitionId } from "./control-ui-compaction.js";
import { validFeatureModelRequest } from "../featureModels.js";
import { validChatModelRequest } from "../chat-model.js";
import { dashboardSetting, parseSettingValue } from "../dashboard-settings.js";
export { createSettingMutator } from "../dashboard-settings.js";
export { createFeatureModelMutator } from "../featureModels.js";
export { createChatModelMutator } from "../chat-model.js";
...
// Die drei Speicherweisen beim Aufteilen. Die Prozentzahlen stammen aus dem
// Lauf vom 20.09.2026 ueber 100 gezielt ausgewaehlte schwere Faelle aus echten
// Daten (Abruf, Antwortmodell, Judge) und stehen wortgleich in der Schema-
// Beschreibung von captureChunkingMode.
export const CAPTURE_CHUNKING_CHOICES = Object.freeze([
  Object.freeze({
    id: "beides",
    label: "Both",
    detail: "Keeps the whole message and stores its parts alongside. Best recall in the measurement: 64 %.",
    enabled: true,
    mode: "beides",
  }),
  Object.freeze({
    id: "geteilt",
    label: "Parts only",
    detail: "Stores only the parts. Saves about one row per split message, but loses the context: 49 %.",
    enabled: true,
    mode: "geteilt",
  }),
  Object.freeze({
    id: "ganz",
    label: "Whole",
    detail: "No splitting at all, the message stays as one row: 36 %.",
    enabled: false,
    mode: null,
  }),
]);

const CAPTURE_CHUNKING_BY_ID = new Map(CAPTURE_CHUNKING_CHOICES.map((choice) => [choice.id, choice]));

/** Resolve one storage mode by its closed id. */
export function captureChunkingChoice(id) {
  return CAPTURE_CHUNKING_BY_ID.get(id) || null;
}

/**
 * Name the storage mode the running config expresses.
 * Beide Schluessel fehlen in einer frischen Konfiguration; die Vorgabe ist
 * dieselbe wie im Capture-Pfad: aufteilen und die Ursprungszeile behalten.
 * @param {object} config PLUR1BUS-Konfiguration.
 * @returns {string} "ganz", "geteilt" oder "beides".
 */
export function activeCaptureChunkingId(config = {}) {
  if (config?.captureChunking === false) return "ganz";
  return config?.captureChunkingMode === "geteilt" ? "geteilt" : "beides";
}
...
/**
 * Create an OpenClaw config mutator for the capture storage mode.
 * Schreibt bewusst beide Schluessel: captureChunking ist der Hauptschalter,
 * captureChunkingMode waehlt zwischen den beiden geteilten Weisen. Bei "ganz"
 * bleibt der zuletzt gewaehlte Modus stehen, damit das Zurueckschalten die
 * vorige Weise wiederherstellt.
 */
export function createCaptureChunkingMutator({ api, pluginId = "memory-lancedb-namespaced" } = {}) {
  const mutateConfigFile = api?.runtime?.config?.mutateConfigFile;
  if (typeof mutateConfigFile !== "function") {
    throw new Error("OpenClaw mutateConfigFile capability is required for the storage mode switch");
  }
  return async (choice) => mutateConfigFile({
    afterWrite: { mode: "auto" },
    mutate(draft) {
      ...
      entry.config = {
        ...config,
        captureChunking: choice.enabled,
        ...(choice.mode ? { captureChunkingMode: choice.mode } : {}),
      };
      return Object.freeze({ mode: choice.id });
    },
  });
}
...
const RESULTS = Object.freeze({
  setting_saved: "Saved to openclaw.json. OpenClaw now reloads the plugin, which takes about a minute; until then this page shows the running value and marks the saved one as pending.",
  capture_chunking_switched: "Storage mode saved. The plugin reload takes about a minute; it then applies to newly captured turns, and memories already stored stay as they are.",
  chat_model_saved: "Chat model saved: new sessions use it after the configuration reload (about a minute), and pinned active sessions of this agent were released so they follow it from their next turn. Heartbeat sessions keep their own model.",
  chat_model_reset: "Chat model choice removed. The agent keeps its current model until the next gateway start, when the model profile sets its default again.",
  feature_model_saved: "Model selection saved to openclaw.json. The plugin reload takes about a minute; until then this page shows the running choice and marks the saved one as pending. Running jobs finish with their previous settings.",
  ...
  compaction_started: "Compaction started. The partition row shows its progress; storage numbers refresh with the next health scan.",
  denied_busy: "A compaction is already running. Let it finish first.",
  denied_partition: "Unknown partition.",
  ...
  denied_value: "The value was refused. It is outside the allowed range, or it would archive memories that are still active.",
  denied_unknown_count: "Nothing was changed: the current memory counts are unavailable, so this cap cannot be checked against them. Wait for the next health scan and try again.",
  ...
  skill_approved: "Skill approved and applied through the OpenClaw Skill Workshop. New sessions of that agent load it.",
  skill_approved_partial: "Skill applied, but some evidence memories could not be marked yet. Approve once more to finish.",
  skill_rejected: "Skill declined. The Workshop draft is rejected and the name is blocked for future mining.",
  skill_retired: "Skill withdrawn: its Workshop directory was removed and the name is blocked for future mining.",
  denied_skill: "Unknown agent or proposal.",
  skill_not_found: "That proposal no longer exists in the ledger.",
  skill_not_pending: "That proposal is not pending any more. Reload the page.",
  skill_not_active: "That skill is not active, so there is nothing to withdraw.",
  skill_unbound: "That proposal has no Workshop binding (mined before the Workshop integration). It can only be declined.",
  skill_failed: "The Skill Workshop did not accept the change. The proposal is unchanged; see the gateway log.",
  failed: "The change did not go through. Nothing was altered.",
});

const SKILL_AGENT_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/i;
const SKILL_PROPOSAL_ID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

function skillResultCode(result, okCode) {
  if (result?.ok === true) {
    return result.partial === true || result.status === "activation_partial" ? "skill_approved_partial" : okCode;
  }
  const reason = typeof result?.reason === "string" ? result.reason : "";
  if (reason === "not_found") return "skill_not_found";
  if (reason === "not_pending" || reason === "workshop_not_pending") return "skill_not_pending";
  if (reason === "not_active") return "skill_not_active";
  if (reason === "workshop_binding_missing" || reason === "unbound") return "skill_unbound";
  return "skill_failed";
}
...
export async function applyControlUiWriteAction({ action, form, mode = "off", deps = {} } = {}) {
  ...
  try {
    if (action === "setting.set") {
      const setting = dashboardSetting(value("setting"));
      const parsed = parseSettingValue(setting, typeof form?.get?.("value") === "string" ? form.get("value") : "");
      if (!setting || !parsed.ok) return { ok: false, code: "denied_value" };
      if (typeof deps.setSetting !== "function") return { ok: false, code: "denied_action" };
      try {
        await deps.setSetting({ id: setting.id, value: form.get("value") });
      } catch (error) {
        if (error?.code === "denied_value" || error?.code === "denied_unknown_count") return { ok: false, code: error.code };
        throw error;
      }
      return { ok: true, code: "setting_saved" };
    }
    if (action === "capture.chunking") {
      const choice = captureChunkingChoice(value("choice"));
      if (!choice) return { ok: false, code: "denied_choice" };
      if (typeof deps.setCaptureChunking !== "function") return { ok: false, code: "denied_action" };
      await deps.setCaptureChunking(choice);
      return { ok: true, code: "capture_chunking_switched" };
    }
    if (action === "feature.model") {
      const request = { agentId: form?.get?.("agent"), feature: form?.get?.("feature"), model: form?.get?.("model") };
      if (!validFeatureModelRequest(request)) return { ok: false, code: "denied_choice" };
      if (typeof deps.setFeatureModel !== "function") return { ok: false, code: "denied_action" };
      await deps.setFeatureModel(request);
      return { ok: true, code: "feature_model_saved" };
    }
    if (action === "chat.model") {
      const request = { agentId: form?.get?.("agent"), model: form?.get?.("chat_model") };
      if (!validChatModelRequest(request)) return { ok: false, code: "denied_choice" };
      if (typeof deps.setChatModel !== "function") return { ok: false, code: "denied_action" };
      await deps.setChatModel(request);
      return { ok: true, code: request.model ? "chat_model_saved" : "chat_model_reset" };
    }
    ...
    // 7.12.48: geminte Skills freigeben, ablehnen oder zurueckziehen. Die
    // Dependencies kapseln Ledger-Suche und Workshop-Aufruf; hier werden nur
    // die Kennungen geprueft, bevor irgendetwas angefasst wird.
    if (action === "skill.approve" || action === "skill.reject" || action === "skill.retire") {
      const agentId = value("agent");
      const proposalId = value("proposal");
      if (!SKILL_AGENT_RE.test(agentId) || !SKILL_PROPOSAL_ID_RE.test(proposalId)) return { ok: false, code: "denied_skill" };
      const handler = action === "skill.approve"
        ? deps.approveSkill
        : action === "skill.reject"
          ? deps.rejectSkill
          : deps.retireSkill;
      if (typeof handler !== "function") return { ok: false, code: "denied_action" };
      const result = await handler({ agentId, proposalId });
      const code = skillResultCode(
        result,
        action === "skill.approve" ? "skill_approved" : action === "skill.reject" ? "skill_rejected" : "skill_retired",
      );
      if (result?.ok !== true) {
        deps.logger?.warn?.(`memory-lancedb-namespaced: dashboard ${action} for ${agentId}/${proposalId} declined: ${result?.reason || result?.text || "unknown"}`);
      }
      return { ok: result?.ok === true, code };
    }
    if (action === "compaction.start") {
      if (typeof deps.startCompaction !== "function") return { ok: false, code: "denied_action" };
      const partition = value("partition");
      if (!isPartitionId(partition)) return { ok: false, code: "denied_partition" };
      const started = await deps.startCompaction({ id: partition });
      return started?.ok === true
        ? { ok: true, code: "compaction_started" }
        : { ok: false, code: typeof started?.code === "string" ? started.code : "failed" };
    }
  } catch (error) {
