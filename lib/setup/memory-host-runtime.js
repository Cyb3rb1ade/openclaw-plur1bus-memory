 ...
 */
import { classifyWorkspaceMemoryPaths as classifyWorkspaceMemoryPathsOnDisk } from "./workspace-memory-provenance.js";
...
export function createMemoryHostRuntime({
  ...
  // Not frozen on purpose. The host wraps the manager in a Proxy whose `get`
  // returns a bound copy of every function; a frozen (non-configurable,
  // non-writable) property makes that violate the Proxy invariant and every
  // access throws "property 'status' is a read-only ... data property".
  const createManager = (agentId, chunks) => {
    ...
    return {
      async search(query, opts = {}) {
        ...
        // The host's signal cancels the recall pipeline's embedder and LanceDB waits (PR-05).
  ...
  // Same reason as the manager: leave the runtime configurable for the host.
  return {
    ...
    // Without this the host logs "excluding automatic memory context: selected
    // memory runtime does not support provenance classification" and leaves
    // MEMORY.md and USER.md out of every session. Same rules as memory-core's
    // own classifier for files without a recorded write ledger.
    async classifyWorkspaceMemoryPaths(params = {}) {
      const workspaceDir = typeof params.workspaceDir === "string" && params.workspaceDir
        ? params.workspaceDir
        : resolveAgentWorkspaceDir(hostConfig(), params.agentId);
      return classifyWorkspaceMemoryPathsOnDisk({ workspaceDir, relativePaths: params.relativePaths });
    },
