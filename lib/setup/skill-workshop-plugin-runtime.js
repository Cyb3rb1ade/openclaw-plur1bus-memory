...
const SKILL_WORKSHOP_DESCRIPTION_MAX_BYTES = 160;
const SKILL_WORKSHOP_DESCRIPTION_TRUNCATION_SUFFIX = "...";
...
function boundUtf8Description(value) {
  const description = requireString(value, "skill description");
  if (Buffer.byteLength(description, "utf8") <= SKILL_WORKSHOP_DESCRIPTION_MAX_BYTES) {
    return description;
  }
  const suffixBytes = Buffer.byteLength(SKILL_WORKSHOP_DESCRIPTION_TRUNCATION_SUFFIX, "utf8");
  let bounded = "";
  let boundedBytes = 0;
  for (const character of description) {
    const characterBytes = Buffer.byteLength(character, "utf8");
    if (boundedBytes + characterBytes + suffixBytes > SKILL_WORKSHOP_DESCRIPTION_MAX_BYTES) break;
    bounded += character;
    boundedBytes += characterBytes;
  }
  return `${bounded.trimEnd()}${SKILL_WORKSHOP_DESCRIPTION_TRUNCATION_SUFFIX}`;
}
    ...
    async createProposal({ agentId, proposal }) {
      ...
      const response = await call(
        "skills.proposals.create",
        {
          ...
          description: boundUtf8Description(proposal.description),
