...
function enforceRerankerInvariants(config) {
  if (getPath(config, ["reranker", "enabled"]) === true) {
    setPath(config, ["emotion", "t2", "enabled"], true, { overwrite: false });
    setPath(config, ["emotion", "t3", "enabled"], true, { overwrite: false });
    ...
    setPath(config, ["metaCognition", "enabled"], true, { overwrite: false });
