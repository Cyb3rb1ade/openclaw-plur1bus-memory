...
export function renderPlur1busStartStatus(config = {}, opts = {}) {
  ...
  const lines = [
    "+++ PLUR1BUS — Make your agent yours! +++",
    "",
    "PLUR1BUS Full Experience Status",
    `Active core features: ${status.active.length}`,
    `Disabled core features: ${status.disabled.length}`,
    `New/missing core features: ${status.missing.length}`,
    "",
    "Active:",
  ...
  if (status.missing.length > 0) {
    lines.push("", "New or missing:", ...status.missing.map((feature) => `- ${feature.label} (defaults on during update)`));
  }
  lines.push(
    "",
    "Safety Gates:",
    "- Merging auto-apply is low-risk only; high-risk merges stay proposals.",
    "- Semantic Graph writes derived metadata/edges only; memory text mutation remains off.",
    "- SoulPatch force is off; legacy migration requires notice and backup.",
    "- Destructive repair, hard delete, aggressive compaction, and .obsidian internals writes remain confirmation-gated.",
    "- Provider-backed LLM work is budgeted/fail-soft and does not run without configured provider credentials.",
    "",
    "Obsidian / Reviews / Dashboard:",
    `- vaultPath: ${vaultPath || bridge.vaultPath || "(auto-bootstrap)"}`,
    `- workspaceRoot: ${workspaceRoot || bridge.workspaceRoot || "(system-wide)"}`,
    `- reviewRoot: ${resolvedReviewRoot}`,
    `- morningReview: ${config.morningReview?.enabled === false ? "disabled" : "enabled"}`,
    `- eveningReview: ${config.eveningReview?.enabled === false ? "disabled" : "enabled"}`,
    `- dashboardLayer: ${bridge.dashboardLayer?.enabled === false ? "disabled" : "enabled"}`,
    "",
    "Change features with /plur1bus enable <feature> or /plur1bus disable <feature>.",
    "Installation status: complete once this command has been reviewed."
  );
  return lines.join("\n");
}
