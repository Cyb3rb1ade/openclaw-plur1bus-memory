 ...
 * @param {{vaultConfirmed?: boolean}} [options] — runtime confirmation facts.
 * @returns {Array<{feature, reason}>}
 */
/**
 * Report a feature that is switched on but cannot do its work yet.
 *
 * Since 7.5.0 every feature is opt-out, so "on" no longer means the operator
 * asked for it. One they explicitly enabled and left unusable deserves a
 * warning; one that is merely on by default does not, because that would warn
 * every user on every start about something they never requested.
 *
 * @param {object} logger Plugin logger.
 * @param {{explicit?: boolean, message: string}} report Explicitness and text.
 */
export function reportDormantFeature(logger, { explicit, message } = {}) {
  if (typeof message !== "string" || !message) return;
  if (explicit === true) logger?.warn?.(message);
  else logger?.info?.(message);
}

export function detectPendingFeatures(config, { vaultConfirmed = false } = {}) {
  const pending = [];
  ...
  for (const review of reviews) {
    if (review.config?.enabled !== false && review.config?.status === "pending_setup") {
      pending.push({
        feature: review.key,
        reason: "delivery_target_not_confirmed",
        explicit: review.config?.enabled === true,
      });
    }
  }
  // Opt-out: the bridge runs unless it is explicitly switched off, so the
  // pending check has to use the same rule the runtime uses. `explicit` marks
  // whether the operator asked for it — an unconfigured feature they never
  // requested is a notice, not a warning.
  if (bridge.enabled !== false && bridge.requireVaultPathConfirmation === true && vaultConfirmed !== true) {
    pending.push({ feature: "obsidianBridge", reason: "vault_path_not_confirmed", explicit: bridge.enabled === true });
  }
 ...
 */
export function isApplyBlocked(config, options = {}) {
  const pending = detectPendingFeatures(config, options);
  if (pending.length > 0) {
...
const WORKSPACE_DIR_RE = /^workspace(?:-[A-Za-z0-9_.-]{1,64})?$/;

/**
 * Names of the agent workspace directories under the OpenClaw home: the
 * default `workspace` plus every `workspace-<agent>`. Falls back to the
 * default suffixes when the home cannot be read, so detection never throws.
 *
 * @param {string} openclawHome
 * @returns {string[]} sorted directory names, default first
 */
export function listWorkspaceDirectories(openclawHome) {
  let entries;
  try {
    entries = readdirSync(openclawHome, { withFileTypes: true });
  } catch {
    return [...DEFAULT_WS_SUFFIXES];
  }
  const names = entries
    .filter((entry) => entry.isDirectory() && WORKSPACE_DIR_RE.test(entry.name))
    .map((entry) => entry.name)
    .sort((left, right) => (left === "workspace" ? -1 : right === "workspace" ? 1 : left.localeCompare(right)));
  return names.length > 0 ? names : [...DEFAULT_WS_SUFFIXES];
}
...
export function detectObsidianVaults(obsidianBridgeCfg = {}) {
  const home = homedir();
  const openclawHome = hostStateDir();
  ...
  if (rawPaths.length === 0) {
    // Without an explicit list, every agent workspace under the OpenClaw home
    // is a candidate: `workspace` and `workspace-<agent>`. Probing only the
    // default suffix hid the per-agent vaults of a multi-agent install.
    for (const suffix of listWorkspaceDirectories(openclawHome)) {
...
export function pendingNoticePath(openclawHome = hostStateDir()) {
