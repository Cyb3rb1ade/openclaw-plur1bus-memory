...
export const OBSIDIAN_VAULT_CLI_TIMEOUT_MS = 30_000;
    ...
    ({ program }) => {
      program
        ...
        .action(async (operation, options) => {
          ...
          // The host's CLI RPC helper is (method, opts, params, extra); the
          // params used to be handed over in the opts slot, so every method
          // saw an empty request and `use`/`confirm` answered "Invalid agent
          // ID" while `detect` printed nothing. The result is written as JSON
          // because the host does not print an action's return value.
          const call = async (method, params, scope) => {
            const result = await gatewayRuntime.callGatewayFromCli(
              method,
              { json: true, timeout: String(OBSIDIAN_VAULT_CLI_TIMEOUT_MS) },
              params,
              { progress: false, scopes: [scope] },
            );
            write(`${JSON.stringify(result ?? null, null, 2)}\n`);
            return result;
          };
          const session = options.session;
          if (operation === "detect") {
            return call(OBSIDIAN_VAULT_GATEWAY_METHODS.detect, { session }, "operator.read");
          }
