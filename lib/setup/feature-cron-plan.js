 ...
 *   with a hint on how to wire delivery — never silently deliver to
 *   nobody, never fail the install.
...
function buildAgentHint(perAgentName, agentId) {
  return (
    ...
    `openclaw cron edit --name "${perAgentName}" --announce --channel telegram --to <chatId>, ` +
 ...
 *   jobs (`plur1bus <feature> <agentId>`) for every agent, keeping
 *   delivery-needing jobs disabled until an operator enables them
 *   explicitly.
