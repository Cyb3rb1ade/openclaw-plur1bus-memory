...
/**
 * Fallback delivery derivation from the OpenClaw channel config (openclaw.json)
 * when no existing cron of the agent carries a usable target — e.g. on a fresh
 * install with zero delivery-capable crons. Chain: `bindings` maps the agent to
 * exactly one telegram account; that account's `allowFrom` allowlist must
 * contain exactly one chat id. Any ambiguity (multiple bound accounts, zero or
 * multiple allowFrom entries, disabled account) returns null — never guess a
 * recipient.
 *
 * @param {string} agentId
...
export function deriveDeliveryFromChannelConfig(agentId, config) {
  const normAgentId = normalize(agentId);
  if (!normAgentId || !config || typeof config !== "object") return null;
  const bindings = Array.isArray(config.bindings) ? config.bindings : [];
  const telegramBindings = bindings.filter(
    (b) => normalize(b?.agentId) === normAgentId && b?.match?.channel === "telegram" && typeof b?.match?.accountId === "string",
  );
  const accountIds = [...new Set(telegramBindings.map((b) => b.match.accountId))];
  ...
  const ids = [...new Set(allowFrom.map((v) => String(v)))].filter((v) => v.length > 0);
  if (ids.length !== 1) return null;
  return { channel: "telegram", to: ids[0], accountId };
}
 ...
 */
function planSpecForAgents(jobs, spec, agents, channelConfig = null) {
  const create = [];
  const skip = [];
...
export function planFeatureCrons(existingJobs, specs, opts = {}) {
  ...
  if (Array.isArray(agents) && agents.length > 0) {
    ...
    for (const spec of specs) {
      const planned = planSpecForAgents(jobs, spec, agents, channelConfig);
...
/**
 * Plant eine Delivery-Migration für einen existierenden Job, der ohne
 * Delivery-Bedarf (spec.needsDelivery === false) versehentlich mit dem
 * `cron add`-Default `announce -> channel "last"` (ohne konkretes `to`)
 * angelegt wurde. Isolierte Cron-Sessions haben kein "last active chat" —
 * diese Zustellung schlägt zur Laufzeit fail-closed fehl (beobachtet
 * 2026-07-18 auf den drei persona-evolve-Jobs der 7.0.0-Installation).
 * Delivery-bedürftige Jobs (afterthought) werden hier NIE stummgeschaltet.
 * Gibt null zurück, wenn nichts zu migrieren ist.
 */
export function planDeliveryMigration(job, spec) {
  if (!job?.id) return null;
  if (spec?.needsDelivery !== false) return null;
  const delivery = job.delivery;
  if (!delivery || delivery.mode === "none" || !delivery.mode) return null;
  if (delivery.to) return null; // konkretes Ziel — Operator-Entscheidung, nicht anfassen
...
function collectMessageMigrations(skip) {
  ...
  for (const entry of skip) {
    ...
    if (migration && !seenMessage.has(migration.id)) {
      seenMessage.add(migration.id);
      update.push(migration);
    }
    const deliveryMigration = planDeliveryMigration(entry.existingJob, entry.spec);
    if (deliveryMigration && !seenDelivery.has(deliveryMigration.id)) {
      seenDelivery.add(deliveryMigration.id);
      update.push(deliveryMigration);
    }
  }
  return update;
}
