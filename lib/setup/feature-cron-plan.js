...
function isValidCronExpression(value) {
  ...
  return fields.every((field, index) => {
    ...
    return field.split(",").every((part) => {
      ...
      const isRange = rangeParts.length === 2 && rangeParts.every(isAtom);
      const isDayOfMonthModifier = index === dayOfMonthIndex && /^(?:L|LW|\d+W)$/i.test(base);
      const isDayOfWeekModifier = index === dayOfWeekIndex && /^(?:\d+|[A-Z]+)#[1-5]$/i.test(base);
      if (!(base === "*" || base === "?" || isAtom(base) || isRange || isDayOfMonthModifier || isDayOfWeekModifier)) {
        return false;
      }
