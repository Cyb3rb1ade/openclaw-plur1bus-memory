...
function isValidCronExpression(value) {
  ...
  return fields.every((field, index) => {
    ...
    const allowedNames = index === monthIndex
      ...
      : index === dayOfWeekIndex
        ...
        : index === dayOfMonthIndex
          ? new Set(["L", "W", "LW"])
          : new Set();
    const alphaTokens = [...field.toUpperCase().matchAll(/[A-Z]+/g)].map(([token]) => token);
    ...
    return field.split(",").every((part) => {
      ...
      return [...stepParts[0].matchAll(/\d+/g)].every(([digits]) => {
...
export function selectEnabledFeatureCronSpecs(sourceConfig) {
  ...
  const explicitlyEnabled = (featureConfig) => featureConfig?.enabled === true;
  ...
  for (const spec of REQUIRED_FEATURE_CRONS) {
    ...
    if (spec.feature === "skill-miner") {
      const cron = cfg.skillMiner.cron === undefined ? spec.schedule.expr : cfg.skillMiner.cron;
      const timezone = cfg.skillMiner.timezone === undefined ? spec.timezone : cfg.skillMiner.timezone;
...
function normalizeDelivery(delivery) {
  if (!isPlainObject(delivery) || normalize(delivery.mode) !== "announce") return null;
...
function isLegacyExactMatch(job, spec, agentId) {
  return job?.agentId === agentId && job?.name === spec?.name;
}
...
export function deriveDeliveryFromChannelConfig(agentId, config) {
  ...
  const telegramBindings = bindings.filter(
    (binding) =>
      ...
      && normalize(binding?.match?.channel) === "telegram",
  );
  if (telegramBindings.length === 0) return null;
  ...
  const implicitDefaultAccount = explicitDefaultAccount
    || (Object.hasOwn(accounts, "default") ? "default" : configuredAccountIds.length === 1 ? configuredAccountIds[0] : "default");

  const candidates = [];
  for (const binding of telegramBindings) {
    const rawAccountId = binding.match?.accountId;
    const hasExplicitBindingAccount = rawAccountId !== undefined && rawAccountId !== "";
    ...
    if (account === undefined && (hasExplicitBindingAccount || accountId !== "default" || configuredAccountIds.length > 0)) {
      return null;
    }
...
function planSpecForAgents(jobs, spec, agents, channelConfig = null, accountConstraint = null) {
  ...
  agents.forEach((agentEntry, agentIndex) => {
    ...
    if (isDefault) {
      const legacyJob = jobs.find((job) => isLegacyExactMatch(job, spec, agentId));
      if (legacyJob) {
        skip.push({
    ...
    const existingJob = jobs.find((job) => matchesPerAgent(job, spec, agentId));
    if (existingJob) {
      skip.push({
        spec: { ...spec, name: perAgentName, agentId },
...
export function planFeatureCrons(existingJobs, specs, opts = {}) {
  ...
  for (const spec of specs) {
    const existingJob = jobs.find((job) => matchesLegacyOwned(job, spec, agent));
    if (existingJob) {
