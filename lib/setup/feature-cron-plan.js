...
export const REQUIRED_FEATURE_CRONS = [
  ...
  },
  {
    ...
    // Must remain byte-for-byte identical to command. The installed
    // plur1bus-cron-direct-dispatch host patch recognizes this exact payload,
    // returns before OpenClaw's model executor, and lets the plugin handler
    // provide either composed text or the NO_REPLY suppression token.
  ...
  },
  {
    ...
    // and the audited host patch finalizes it without a model-backed carrier.
...
export function planUnsafeDirectCronDisables(existingJobs) {
  const directSpecs = REQUIRED_FEATURE_CRONS.filter(
    (spec) => spec.feature === "afterthought" || spec.feature === "classify-recent",
  );
  const planned = [];
  ...
  for (const job of Array.isArray(existingJobs) ? existingJobs : []) {
    ...
    const spec = directSpecs.find((candidate) => job?.payload?.message === candidate.message);
    if (!spec) continue;
    const canonicalPerAgentName = typeof job?.agentId === "string" && job.agentId.length > 0
      ? `${spec.name} ${job.agentId}`
      : null;
    if (job?.name !== spec.name && job?.name !== canonicalPerAgentName) continue;
    if (seenIds.has(job.id)) continue;
    seenIds.add(job.id);
    planned.push({
      id: job.id,
      name: job.name,
      safetyName: `${job.name}${CRON_SAFETY_DISABLED_SUFFIX}`,
      disable: true,
    });
  }
  return planned;
 ...
 *
 * @param {Array<object>} existingJobs
 * @param {Array<object>} enabledSpecs
 * @returns {Array<{id: string, name: string, rename: string, enable: true}>}
 */
export function planSafetyDisabledCronRecoveries(existingJobs, enabledSpecs) {
  const specs = (Array.isArray(enabledSpecs) ? enabledSpecs : []).filter(
    (spec) => spec.feature === "afterthought" || spec.feature === "classify-recent",
  );
  const recoveries = [];
  for (const job of Array.isArray(existingJobs) ? existingJobs : []) {
    ...
    const spec = specs.find((candidate) => job?.payload?.message === candidate.message);
    if (!spec) continue;
    const canonicalPerAgentName = typeof job?.agentId === "string" && job.agentId.length > 0
      ? `${spec.name} ${job.agentId}`
      : null;
