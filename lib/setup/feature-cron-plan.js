...
export const REQUIRED_FEATURE_CRONS = [
  ...
  },
  {
    ...
    // Wie bei consolidate-daily beansprucht das Kommando den Job: die auf
    // dieser Installation gewachsenen `auto-accept-stale-criticals-<agent>`
    // werden darueber erkannt und auf die native Form umgezogen, statt dass
    // daneben ein kanonisch benannter Zwilling entsteht.
    ...
    // Bewusst nicht 04:45 wie gc-run: auf dieser Installation liefen
    // `auto-accept-stale-criticals-main` und `plur1bus gc-run` am 09.09.2026
    // beide um 04:45:00 an, und der auto-accept-Job lief in den Timeout.
    // Per-Agent wird ab dieser Basis gestaffelt — siehe
    // staggerAutoAcceptStaleSchedule.
  ...
  },
  {
    ...
    // Der Drain im agent_end-Hook bekommt seit 7.12.20 nur den Rest des
    // Capture-Budgets; was liegen bleibt, holt dieser Job nachts nach —
    // vor gc-run (04:45) und ausserhalb des 01:15–02:15-Wartungsfensters.
    // Per-Agent gestaffelt, siehe staggerEmbeddingDrainSchedule.
  ...
  },
  {
    ...
    // Seit 7.12.22 klassifiziert der Capture-Pfad neue Erinnerungen nur noch
    // lexikalisch (Tier 1/2); Zeilen unter der Eskalationsschwelle stehen auf
    // emotionStatus=pending_t3 und bekommen hier den LLM-Feinschliff. Kein
    // Stagger noetig: der Job ist kurz, laeuft "every" und hat keine Zustellung.
  ...
  },
  {
    ...
    // 7.12.50: taeglich 05:00 (je Agent +15 min), nach Konsolidierung
    // (04:00-04:30), Persona-Evolution (04:15-04:25), GC (04:45) und
    // auto-accept-stale (04:50-04:54). Der Miner liest genau die Erinnerungen,
    // die diese Jobs vorher bewertet haben. Woechentlich brauchte der Rueckstau
    // (je Lauf hoechstens maxPerRun Vorschlaege) Wochen; bekannte Cluster
    // kosten dank Fingerabdruck-Memo keinen Modellaufruf mehr.
  ...
  },
  {
    ...
    // erzeugt, und nach persona-evolve (täglich 04:15, seit 7.12.38).
...
export function selectEnabledFeatureCronSpecs(sourceConfig) {
  ...
  // Die Warteschlange existiert nur mit Neo; der Job ist model-frei.
  if (explicitlyEnabled(cfg.neo)) enabledFeatures.add("embedding-drain");
  // emotion-refine haengt an Tier 3 und am Capture-Modus: wer Tier 3 abschaltet
  // oder captureMode "inline" setzt, hat nichts zum Nachbessern.
  if (explicitlyEnabled(cfg.emotion?.t3) && cfg.emotion?.t3?.captureMode !== "inline") {
    enabledFeatures.add("emotion-refine");
  }
  if (explicitlyEnabled(cfg.criticalPush)) {
    enabledFeatures.add("classify-recent");
    // Derselbe Schalter: der Job raeumt liegengebliebene Critical-Karten auf,
    // die ohne Critical-Push gar nicht erst entstehen.
    enabledFeatures.add("auto-accept-stale");
  }
  ...
  for (const spec of REQUIRED_FEATURE_CRONS) {
    ...
    if (spec.feature === "persona-evolve") {
      // 7.12.38: same override pattern as skill-miner — cadence is operator
      // configurable, invalid values drop the job instead of guessing.
      // The shipped spec carries no timezone (local time); only an explicit
      // override is validated, so the spec shape stays as before.
      const personaCfg = cfg.personaVoice ?? {};
      const cron = personaCfg.cron === undefined ? spec.schedule.expr : personaCfg.cron;
      const timezone = personaCfg.timezone === undefined ? spec.timezone : personaCfg.timezone;
      if (!isValidCronExpression(cron) || (timezone !== undefined && !isValidTimeZone(timezone))) continue;
      selected.push({ ...spec, schedule: { kind: "cron", expr: cron.trim() }, ...(timezone === undefined ? {} : { timezone }) });
      continue;
    }
 ...
 * case-sensitive. Consolidation jobs are claimed by their exact command alone:
 * an operator-named job such as `daily-memory-consolidation-main` that runs
 * `/plur1bus internal consolidate-daily` is the consolidation for that agent,
 * and creating a canonical sibling next to it would consolidate twice on the
 * same minute (found live on 04.09.2026). A canonical name with a foreign
 * payload still never claims (impostor guard); schedule migration is limited
 * to canonical names separately (see planConsolidationScheduleMigration).
 * Other jobs retain compatibility with the legacy name-or-command identity.
 */
...
/**
 * Singleton specs (gc-run) are owned installation-wide: one job on ANY agent
 * satisfies the spec. Matching per agent created a second collector on the
 * first selected agent whenever the existing job belonged to another one.
 */
function matchesSingleton(job, spec, agents) {
  const firstMessageLine = firstCommandLine(job);
  if (Boolean(firstMessageLine) && firstMessageLine === spec.command) return true;
  if (job?.name === spec?.name) return true;
  return agents.some((agent) => job?.name === `${spec.name} ${agent.id}`);
}
...
/**
 * Among owned-but-disabled jobs, pick the one to re-enable: the canonical
 * per-agent name first, then the bare legacy name, then the first owned job.
 */
function pickEnableCandidate(ownedJobs, spec, agentId) {
  const perAgentName = `${spec.name} ${agentId}`;
  return ownedJobs.find((job) => job?.name === perAgentName)
    ?? ownedJobs.find((job) => job?.name === spec.name)
    ?? ownedJobs[0];
}
...
/**
 * Staggers the stale-critical auto-accept by 2 minutes per selected agent.
 *
 * Der Job ist kurz; zwei Minuten reichen, damit nicht drei Agenten auf
 * dieselbe Minute fallen und den 60s-Watchdog ausloesen.
 *
 * @param {object} baseSchedule
 * @param {number} agentIndex
 * @returns {object}
 */
/** Three minutes per agent: a full drain embeds hundreds of records. */
export function staggerEmbeddingDrainSchedule(baseSchedule, agentIndex) {
  return staggerCronSchedule(baseSchedule, agentIndex, 3);
}

export function staggerAutoAcceptStaleSchedule(baseSchedule, agentIndex) {
  return staggerCronSchedule(baseSchedule, agentIndex, 2);
}
...
const STAGGER_BY_FEATURE = {
  ...
  "auto-accept-stale": staggerAutoAcceptStaleSchedule,
  "embedding-drain": staggerEmbeddingDrainSchedule,
 ...
 *
 * Ausnahme: ist überhaupt kein Agent gebunden (CLI-/API-only-Installation),
 * fällt die Auswahl auf den Hauptagenten zurück — sonst bekäme eine solche
 * Installation stillschweigend gar keine Feature-Crons.
 *
 ...
 */
export function selectAgentsForCronSetup(agents, options = {}) {
  const all = (Array.isArray(agents) ? agents : []).filter((a) => a?.id);
  let bound = all.filter((a) => Number(a?.bindings) > 0);

  // Eine Installation ganz ohne Kanalbindungen — reiner CLI- oder API-Betrieb —
  // hat keinen einzigen gebundenen Agenten. Ohne Rückfall entstünden dort
  // stillschweigend null Feature-Crons: PLUR1BUS läuft, lernt aber nie nach.
  // Beobachtet 12.09.26 auf einer fremden Installation.
  //
  // Der Rückfall greift NUR, wenn niemand gebunden ist, und NUR auf einen
  // Agenten, der sich zweifelsfrei als Hauptagent ausweist: als Standard
  // markiert, vom Aufrufer benannt, oder OpenClaws konventionelles `main`.
  // Ein beliebiger Agent käme nicht in Frage — sonst bekäme in einer
  // Installation mit Subagenten ausgerechnet einer von ihnen die Crons, und
  // genau das soll der bindings-Filter verhindern.
  if (bound.length === 0 && all.length > 0) {
    const fallback = all.find((a) => a.isDefault)
      ?? (typeof options?.defaultAgentId === "string" && options.defaultAgentId
        ? all.find((a) => a.id === options.defaultAgentId)
        : undefined)
      ?? all.find((a) => a.id === "main");
    if (fallback) bound = [fallback];
  }
  ...
  const selected = [...byWorkspace.values()].map((a) => ({ id: a.id, isDefault: Boolean(a.isDefault) }));

  // `openclaw agents list --json` may flag no agent as default at all (seen
  // live on 2026.9.1 with directory-defined agents). Without a default the
  // stagger order turned alphabetical, legacy single-agent jobs stopped
  // counting for anybody, and the singleton collector moved to whichever
  // agent sorted first. Fall back to the operator-supplied id, then to
  // OpenClaw's conventional `main`.
  if (!selected.some((a) => a.isDefault)) {
    const preferred = [options?.defaultAgentId, "main"].filter((id) => typeof id === "string" && id.length > 0);
    const fallback = preferred.map((id) => selected.find((a) => a.id === id)).find(Boolean);
...
function planSpecForAgents(jobs, spec, agents, channelConfig = null, accountConstraint = null) {
  ...
  // Minute. Der erste (voreingestellte) Agent trägt den Job; ein bestehender
  // Job auf irgendeinem Agenten erfüllt die Spec (matchesSingleton).
  ...
  zielAgenten.forEach((agentEntry, agentIndex) => {
    ...
    const ownedJobs = spec.singleton
      ? jobs.filter((job) => matchesSingleton(job, spec, agents))
      : jobs.filter(
    ...
    if (ownedJobs.length > 0) {
      for (const existingJob of ownedJobs) {
        // A singleton owned by another agent keeps that agent's identity, so
        // payload migrations never rewrite its `--agent` argument.
        const ownerAgentId = spec.singleton && typeof existingJob?.agentId === "string" && existingJob.agentId
          ? existingJob.agentId
          : agentId;
        skip.push({
          spec: { ...spec, name: `${spec.name} ${ownerAgentId}`, agentId: ownerAgentId, agentIndex },
          reason: isLegacyExactMatch(existingJob, spec, ownerAgentId) ? "legacy" : "already-exists",
          existingJob,
        });
      }
      // Owned but entirely disabled: the feature silently never runs and the
      // planner would otherwise report "already present" forever (live: the
      // main REM dream since 27.07.2026). Non-delivery features are switched
      // back on; delivery features stay with the safety-recovery contract.
      if (spec.needsDelivery !== true && !ownedJobs.some(isJobEnabled)) {
        const candidate = pickEnableCandidate(ownedJobs, spec, agentId);
        if (typeof candidate?.id === "string" && candidate.id.length > 0) {
          enable.push({ id: candidate.id, name: candidate.name ?? "", enable: true, reason: "disabled-owned" });
        }
      }
      return;
    }
  ...
  return { create, skip, enable };
}

/**
 * Merge disabled-owned enables into the migration updates by job id. An
 * update that disables the job wins; everything else simply gains `enable`.
 */
function mergeEnablesIntoUpdates(updates, enables) {
  const merged = updates.map((update) => ({ ...update }));
  for (const entry of enables) {
    const existing = merged.find((update) => update.id === entry.id);
    if (!existing) {
      merged.push({ ...entry });
      continue;
    }
...
export function planFeatureCrons(existingJobs, specs, opts = {}) {
  ...
  if (Array.isArray(agents) && agents.length > 0) {
    ...
    for (const spec of specs) {
      ...
      enable.push(...planned.enable);
    }
    return { create, skip, update: mergeEnablesIntoUpdates(collectMessageMigrations(skip), enable) };
  }
...
export function planConsolidationScheduleMigration(job, spec) {
  ...
  // Only PLUR1BUS-named jobs are eligible: ownership now also comes from the
  // exact command, but an operator-named consolidation keeps its schedule.
  const canonicalNames = new Set([
    spec?.name,
    ...(typeof spec?.agentId === "string" && spec.agentId ? [`${spec.name} ${spec.agentId}`] : []),
    ...(typeof job?.agentId === "string" && job.agentId ? [`${spec?.name} ${job.agentId}`] : []),
  ]);
  if (!canonicalNames.has(job?.name)) return null;
...
// 7.12.38: die bis 7.12.37 ausgelieferte Wochen-Kadenz (Sonntag 04:15,
// je Agent um fünf Minuten versetzt) — nur genau diese Ausdrücke werden auf
// die tägliche Kadenz migriert; ein vom Operator gesetzter Ausdruck bleibt.
const SHIPPED_WEEKLY_PERSONA_EVOLVE_BASE = { kind: "cron", expr: "15 4 * * 0" };
const SHIPPED_WEEKLY_PERSONA_EVOLVE_SCHEDULES = new Set(
  Array.from({ length: 24 }, (_, i) => staggerPersonaEvolveSchedule(SHIPPED_WEEKLY_PERSONA_EVOLVE_BASE, i).expr),
);

/**
 * Migrates only the exact PLUR1BUS-shipped weekly persona-evolve cadence to
 * the daily schedule for the job's selected-agent index. Operator-defined
 * expressions (including a personaVoice.cron override that still says
 ...
 * @returns {{id: string, name: string, schedule: object, timezone: string|null} | null}
 */
export function planPersonaEvolveScheduleMigration(job, spec) {
  if (!job?.id || spec?.feature !== "persona-evolve") return null;
  const canonicalNames = new Set([
    spec?.name,
    ...(typeof spec?.agentId === "string" && spec.agentId ? [`${spec.name} ${spec.agentId}`] : []),
    ...(typeof job?.agentId === "string" && job.agentId ? [`${spec?.name} ${job.agentId}`] : []),
  ...
  if (!canonicalNames.has(job?.name)) return null;
  if (job?.schedule?.kind !== "cron" || spec?.schedule?.kind !== "cron") return null;
  if (!SHIPPED_WEEKLY_PERSONA_EVOLVE_SCHEDULES.has(job.schedule.expr)) return null;
  const target = staggerPersonaEvolveSchedule(spec.schedule, spec.agentIndex ?? 0);
  if (job.schedule.expr === target.expr) return null;
  const currentTimezone = job.schedule.tz ?? job.timezone ?? null;
  const targetTimezone = spec.timezone ?? currentTimezone ?? null;
  return {
    id: job.id,
    name: job.name ?? "",
    schedule: target,
...
function collectMessageMigrations(skip) {
  ...
  for (const entry of skip) {
    ...
    mergeUpdate(planPersonaEvolveScheduleMigration(entry.existingJob, entry.spec));
  }
  return [...byId.values()];
}

/**
 * Zustellziel eines bestimmten Feature-Crons dieses Agenten, gelesen aus dem
 * eigenen Cron-Job (dem Ziel, an das seine Textausgabe heute geht).
 *
 * @param {Array<object>} jobs - Cron-Jobs des Hosts
 * @param {string} agentId
 * @param {string} feature - z. B. "classify-recent"
 * @returns {{channel: string, to: string, accountId?: string} | null}
 */
export function findFeatureCronDelivery(jobs, agentId, feature) {
  if (!Array.isArray(jobs) || typeof agentId !== "string" || !agentId || typeof feature !== "string") return null;
  const command = `/plur1bus internal ${feature}`;
  const matches = jobs.filter((job) => job?.agentId === agentId
    && job?.enabled !== false
    && (runsFeatureCron(job?.payload, agentId, feature) || firstCommandLine(job) === command));
  const deliveries = matches.map((job) => normalizeDelivery(job?.delivery)).filter(Boolean);
  if (deliveries.length === 0) return null;
  const key = (delivery) => JSON.stringify([delivery.channel, delivery.to, delivery.accountId ?? null]);
  return deliveries.every((delivery) => key(delivery) === key(deliveries[0])) ? { ...deliveries[0] } : null;
}

/**
 * Das Telegram-Konto (der Bot), an das ein Agent gebunden ist. Nur eine
 * ausdrückliche, eindeutige Bindung zählt; Platzhalter wie `*` nicht.
 *
 * @param {string} agentId
 * @param {{bindings?: Array<object>} | null} config
 * @returns {string | null}
 */
export function boundTelegramAccountId(agentId, config) {
  if (typeof agentId !== "string" || !agentId || !isPlainObject(config)) return null;
  const bindings = Array.isArray(config.bindings) ? config.bindings : [];
  const accounts = new Set();
  for (const binding of bindings) {
    if (binding?.type === "acp" || binding?.agentId !== agentId) continue;
    if (normalizeChannel(binding?.match?.channel) !== "telegram") continue;
    const accountId = normalizeAccountId(binding?.match?.accountId);
    if (accountId && accountId !== "*") accounts.add(accountId);
  }
  return accounts.size === 1 ? [...accounts][0] : null;
}

/**
 * Erkennt einen nativen Feature-Cron an Runner-Dateiname, `--agent` und
 * `--feature`, unabhängig vom Installationspfad und vom Node-Binary. Für die
 * Zielsuche genügt das; die Migration nutzt weiter den byte-genauen Vergleich.
 */
function runsFeatureCron(payload, agentId, feature) {
  if (payload?.kind !== "command" || !Array.isArray(payload.argv)) return false;
  const argv = payload.argv.map(String);
  if (!argv.some((part) => /(^|\/)run-feature-cron\.mjs$/.test(part))) return false;
  const valueOf = (flag) => {
    const index = argv.indexOf(flag);
    return index >= 0 ? argv[index + 1] : undefined;
  };
  return valueOf("--agent") === agentId && valueOf("--feature") === feature;
}
