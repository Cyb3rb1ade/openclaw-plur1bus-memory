...
function setHeaders(response, contentType = "text/html; charset=utf-8", { allowForms = false } = {}) {
  response.setHeader("cache-control", "no-store, max-age=0");
  response.setHeader("content-type", contentType);
  // form-action stays 'none' while the tab is read-only, so a page that cannot
  // change anything also cannot be made to post anywhere.
  response.setHeader(
    "content-security-policy",
    `default-src 'none'; style-src 'unsafe-inline'; frame-ancestors 'self'; base-uri 'none'; form-action ${allowForms ? "'self'" : "'none'"}`,
  );
...
function resultFromUrl(url) {
  ...
  if (!query) return "";
  const value = new URLSearchParams(query).get("result");
...
export function createControlUiHttpHandler({ getProjection, now = Date.now, write = null } = {}) {
  ...
  const renderPage = async (request, response, result) => {
    ...
    setHeaders(response, "text/html; charset=utf-8", { allowForms: writable });
  ...
  return async (request, response) => {
    ...
    if (method === "POST" && writable) {
      try {
        ...
        // The tab cookie is SameSite=None, so a valid single-use token from a
        // page this operator loaded is what separates a click from a
        // cross-site post.
