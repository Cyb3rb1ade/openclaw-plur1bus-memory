...
function renderRerankerCard(providers, features) {
  const reranker = asObject(providers).reranker;
  const state = asObject(asObject(features).reranker);
  if (!reranker) {
    return `<article class="card"><h3>Reranking</h3><p>No reranker is active. Recall returns candidates in raw similarity order.</p>
<p>Switch one on in OpenClaw config under <code>reranker.provider</code>: <code>cohere</code> or <code>jina</code> for a hosted API (key in <code>reranker.apiKey</code> or <code>reranker.apiKeyEnv</code>), <code>local</code> for the bundled BGE model, which needs no key.</p></article>`;
  }
  ...
  const note = blocked
    ...
    : "";
  return `<article class="card"><h3>Reranking</h3><dl><dt>Provider</dt><dd>${safeText(provider.provider)}</dd><dt>Model</dt><dd>${safeText(provider.model)}</dd>${
    provider.revision ? `<dt>Revision</dt><dd><code>${safeText(provider.revision)}</code></dd>` : ""
  }</dl>${note}
<p>To change it, set <code>reranker.provider</code> in OpenClaw config to <code>cohere</code>, <code>jina</code> or <code>local</code>. The two hosted providers read their key from <code>reranker.apiKey</code> or <code>reranker.apiKeyEnv</code>; <code>local</code> runs the bundled BGE model without a key. A chained value such as <code>chained:cohere-&gt;local</code> tries them in order.</p></article>`;
}

function renderMemoryHealth(value, providers, features, nowMs = Date.now()) {
...
${renderRerankerCard(providers, features)}
</div>
...
const CREDENTIAL_STATUS_HELP = Object.freeze({
  ...
  missing: "Nothing is configured at this path and there is no host route to fall back on. The feature that needs it stays off.",
...
function renderObsidianVault(value) {
  ...
  const state = vault.confirmed === true ? "ready" : vault.configured === true ? "degraded" : "unavailable";
  const summary = vault.confirmed === true
    ? "A target is configured and confirmed. Mirroring is active."
    : vault.configured === true
      ? "A target is configured but not yet confirmed. Run the confirm step below."
...
function renderCredentials(value) {
  ...
  const rows = CREDENTIAL_UI_DEFINITIONS.map(([key, label, defaultPath, purpose]) => {
    ...
    const source = rawSource === "host_route" ? "OpenClaw default route" : rawSource;
    ...
    const path = typeof credential.path === "string" && credential.path ? credential.path : defaultPath;
...
function renderProjection(projection, nowMs = Date.now()) {
...
${renderMemoryHealth(status.memoryHealth, status.providers, status.features, nowMs)}
...
export function createControlUiHttpHandler({ getProjection, now = Date.now } = {}) {
  ...
  return async (request, response) => {
    ...
    try {
      ...
      const body = renderProjection(projection, now());
