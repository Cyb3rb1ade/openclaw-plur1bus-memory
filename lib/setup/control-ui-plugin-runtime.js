...
function renderFeatureControls(values, modelControls, write = writeContext(null)) {
  ...
  const rendered = cards.map((entry) => {
    ...
    return `<li class="card feature-card"><div class="card-heading"><h3>${safeText(entry.label)}</h3>${renderBadge(state)}</div>${details}${renderFeatureModelControls(entry.id, modelControls, write)}</li>`;
  }).join("");
...
${renderFeatureControls(status.featureCards, status.featureModels, write)}
