...
function renderFeatureControls(values) {
  const cards = asArray(values)
    ...
    .map((entry) => {
      ...
      const purpose = typeof entry.purpose === "string" && entry.purpose ? entry.purpose : "No description available.";
      return `<article class="card feature-card"><div class="card-heading"><h3 title="${escapeHtml(purpose)}">${safeText(entry.label)}</h3>${renderBadge(state)}</div><dl><dt title="Whether a switch is set or inherited from the default. Says nothing about whether the feature can actually run.">Configured</dt><dd>${entry.configured === true ? "yes" : "no"}</dd><dt>Dependencies</dt><dd>${dependencies}</dd><dt title="Why an enabled feature is not effective \u2014 usually a missing dependency or a disabled workspace.">Reason</dt><dd><code>${safeText(entry.reason, "none")}</code></dd><dt>Audit</dt><dd><code>${safeText(entry.audit, "openclaw_config_audit")}</code></dd></dl><p><a href="/config">Configure</a> · <a href="/secrets">Credentials</a></p></article>`;
    })
    .join("");
  return `<section class="panel" aria-labelledby="feature-controls-title">
<div class="section-heading"><div><h2 id="feature-controls-title">Feature Controls</h2><p>All PLUR1BUS features are opt-out: an absent switch means on, only an explicit <code>enabled: false</code> turns one off. Hover a title for what it does. <strong>enabled</strong> means running, <strong>degraded</strong> means switched on but blocked by a dependency, <strong>disabled</strong> means switched off.</p></div></div>
...
function renderCredentials(value) {
  ...
  const rows = CREDENTIAL_UI_DEFINITIONS.map(([key, label, path, purpose]) => {
    ...
    return `<tr>`
      + `<td><span title="${escapeHtml(purpose)}">${escapeHtml(label)}</span><br><code class="hint" title="Set this key at this path in openclaw.json, or point it at a SecretRef.">${escapeHtml(path)}</code></td>`
      + `<td title="${credentialHelp(CREDENTIAL_STATUS_HELP, status, "Unknown readiness state.")}">${renderBadge(credential.status)}</td>`
      + `<td title="${source ? credentialHelp(CREDENTIAL_SOURCE_HELP, source, "Unrecognised source type.") : "Nothing configured at this path yet."}">${safeText(source, "not configured")}</td>`
      + `</tr>`;
  }).join("");
  return `<section class="panel" aria-labelledby="credentials-title"><div class="section-heading"><div><h2 id="credentials-title">Credential Readiness</h2><p>Readiness only &mdash; values and references are never shown, not even redacted. Hover any cell for what it means. To change a key, edit the listed path in your OpenClaw config or manage it under OpenClaw Secrets; this tab is read-only by design.</p></div><a class="button-link" href="/secrets" title="Opens OpenClaw's own secret management. PLUR1BUS deliberately does not store or edit secrets itself.">OpenClaw Secrets</a></div><div class="table-wrap"><table><thead><tr><th scope="col" title="What the credential is used for.">Capability and config path</th><th scope="col" title="Whether something is configured — not whether it still works.">Status</th><th scope="col" title="How the value is resolved at runtime.">Source type</th></tr></thead><tbody>${rows}</tbody></table></div></section>`;
}
