...
function renderLlmTaskMatrix(modelControls, write) {
...
function renderFeatureControls(values, captureChunking, write = writeContext(null)) {
  ...
  const rendered = cards.map((entry) => {
    ...
    return `<li class="card feature-card"><div class="card-heading"><h3>${safeText(entry.label)}</h3>${renderBadge(state)}</div>${details}${renderCaptureChunkingSwitch(entry.id, captureChunking, write)}</li>`;
  }).join("");
...
<p>All PLUR1BUS features are opt-out: an absent switch means on, only an explicit <code>enabled: false</code> turns one off. <strong>Running</strong> is active, <strong>blocked</strong> is switched on but held back by a dependency, <strong>off</strong> is switched off. Feature switches are managed in <a href="/config">OpenClaw config</a>.</p>
...
${renderFeatureControls(status.featureCards, status.captureChunking, write)}
${renderLlmTaskMatrix(status.featureModels, write)}
