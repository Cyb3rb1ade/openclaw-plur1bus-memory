...
function formatBytes(value) {
  ...
  if (bytes < 1000) return `${bytes.toLocaleString("en-US")} B`;
  // Operators read "9.3 GB", not "9,300,000,000 B"; the exact count stays in the tooltip.
  const units = ["kB", "MB", "GB", "TB"];
  let scaled = bytes; let unit = -1;
  while (scaled >= 1000 && unit < units.length - 1) { scaled /= 1000; unit += 1; }
  return `<span title="${bytes.toLocaleString("en-US")} B">${scaled.toLocaleString("en-US", { maximumFractionDigits: scaled >= 100 ? 0 : 1 })} ${units[unit]}</span>`;
}
...
const EDIT_CELL_RE = /^[A-Za-z0-9_-]{1,64}\.[A-Za-z0-9*-]{1,64}$/;
const VIEW_AGENT_RE = /^[A-Za-z0-9_-]{1,64}$/;
// Above this many agents the matrix gives each column ~100px and every select
// shrinks to its arrow. One agent per view then, chosen by ?agent=.
const MATRIX_MAX_COLUMNS = 4;
// A reload that has not landed after this long is not slow, it failed.
...
function writeContext(value) {
  ...
  return {
    ...
    reranker: mode !== "off",
    embedding: mode === "all",
    // Skill approval changes what an agent does; full write surface only.
    skills: mode === "all",
    ...
    // "<agent>.<task>" from ?edit=: the one collapsed matrix cell to render as a select.
    edit: mode === "all" && typeof context.edit === "string" && EDIT_CELL_RE.test(context.edit) ? context.edit : "",
...
// Saved but not yet running: the file is ahead of this plugin instance.
function pendingContext(status, nowMs) {
  const pending = asObject(status.pending);
  const ids = new Set(asArray(pending.ids).filter((id) => typeof id === "string"));
  const savedAt = safeInteger(pending.savedAt);
  const stale = savedAt !== null && nowMs - savedAt > PENDING_STALE_MS;
  return { status: typeof pending.status === "string" ? pending.status : "unknown", ids, count: ids.size, savedAt, stale };
}

function pendingMark(pending, id) {
  return pending.ids.has(id) ? '<span class="pending-mark" title="Saved, not yet running">pending</span>' : "";
}

function renderPendingNotice(pending, nowMs) {
  if (pending.status === "unreadable") {
    return `<div class="notice notice-pending">${renderBadge("attention")} The config file could not be read, so this page cannot tell whether a saved change is already running.</div>`;
  }
  if (!pending.count) return "";
  const age = pending.savedAt ? describeObservedAge(pending.savedAt, nowMs) : "just now";
  const what = `${pending.count} saved change${pending.count === 1 ? "" : "s"}`;
  return `<div class="notice notice-pending">${renderBadge(pending.stale ? "attention" : "current")} <strong>${escapeHtml(what)} not yet running.</strong> `
    + (pending.stale
      ? `Saved ${escapeHtml(age)}; a plugin reload normally lands within a minute, so this one probably failed — restart the gateway to apply it. `
      : `Saved ${escapeHtml(age)}; the plugin reload takes about a minute. `)
    + `Until then every value below is the one actually running, and the changed ones are marked <span class="pending-mark">pending</span>.</div>`;
}
...
function renderCountList(values, emptyMessage, { zeroHint = "", allowUnknown = false } = {}) {
  const valid = asArray(values).filter((entry) => asObject(entry) && (
    safeInteger(entry?.cards) !== null || (allowUnknown && entry?.cards === null)
  ));
  const entries = valid.map((entry) => `<li><code>${safeText(entry.id)}</code><strong>${entry.cards === null ? "Unavailable" : safeInteger(entry.cards)}</strong></li>`);
...
// The scan records the first failure as {component, code}; the code alone is
// an identifier, not an explanation, and it sits in a card far from the badge.
const HEALTH_REASONS = Object.freeze({
  "lancedb:partition_count_failed": "A partition could not be counted — usually a cold or busy LanceDB right after a plugin reload. The card counts below may be incomplete.",
  "lancedb:partition_list_failed": "A partition directory could not be listed. Stores that were not read are missing from the counts.",
  "storage:storage_measure_failed": "The size measurement did not finish, so Storage is a lower bound.",
  "health:partition_id_unsupported": "A partition name does not match the supported id format and was skipped.",
  "health:partition_limit_reached": "The scan stopped at its partition limit; agents beyond it are not counted.",
  "health:health_scan_failed": "The health scan itself failed. The numbers below are from the last successful run, or missing.",
  "health:health_unavailable": "No health snapshot has been taken yet — right after a gateway start the first scan is still running.",
});

function healthExplanation(health, nowMs) {
  const lastError = asObject(health.lastError);
  const parts = [];
  if (lastError.component && lastError.code) {
    const key = `${lastError.component}:${lastError.code}`;
    parts.push(HEALTH_REASONS[key] || `The scan reported ${escapeHtml(key)}.`);
    const observedAt = safeInteger(health.observedAt);
    // A failed snapshot is retried sooner than a good one, so say when.
    if (observedAt) parts.push(`Observed ${escapeHtml(describeObservedAge(observedAt, nowMs))}; the next scan runs within the minute.`);
  }
  // Only worth saying when a number is actually shown; without one the
  // missing snapshot above already explains everything.
  if (health.storage?.complete === false && safeInteger(health.storage?.bytes) !== null) {
    parts.push("The size scan was incomplete, so <strong>Storage</strong> is a lower bound, not the full figure.");
  }
  return parts.length ? `<p class="health-reason">${parts.join(" ")}</p>` : "";
}
...
<div class="section-heading"><div><h2 id="memory-health-title">Memory Health</h2><p>Aggregate-only LanceDB status; no memory card content is shown. Snapshot observed ${observed}; it is refreshed in the background, so this page never waits for a scan.</p>${healthExplanation(health, nowMs)}</div>${renderBadge(health.status)}</div>
...
<div class="grid grid-2"><article class="card"><h3>Cards by workspace</h3>${renderCountList(cards.byWorkspace, "No shared workspace cards observed.", { zeroHint: "These shared workspace partitions exist but hold no cards yet. A card gets there through /share <id>; recall still runs on each agent's private partition." })}</article><article class="card"><h3>Cards by primary agent</h3>${renderCountList(cards.byPrimaryAgent, "No channel-bound agent found.", { allowUnknown: true })}</article></div>
...
// Capacity & Runtime: read-only, the questions an operator asks when recall
// slows down or rows disappear — how full, when did gc last run, is the
// process under pressure, which limits apply.
function renderOperations(value, nowMs = Date.now()) {
  const ops = asObject(value);
  const gc = asObject(ops.gc);
  const cap = safeInteger(gc.maxMemoryCount);
  const last = asObject(gc.lastRun);
  const lastAt = safeInteger(last.at);
  const agentRows = asArray(gc.agents).filter((row) => asObject(row) && typeof row.id === "string").map((row) => {
    const pct = safeInteger(row.usedPct);
    const tone = pct === null ? "" : pct >= 90 ? " is-critical" : pct >= 75 ? " is-warning" : "";
    return `<tr><th scope="row"><code>${safeText(row.id)}</code></th><td>${safeInteger(row.cards, 0).toLocaleString("en-US")}</td>`
      + `<td>${cap ? `${cap.toLocaleString("en-US")}` : "—"}</td>`
      + `<td>${pct === null ? "—" : `<span class="headroom${tone}"><progress value="${Math.min(pct, 100)}" max="100" aria-label="${escapeHtml(`${row.id}: ${pct} % of the cap`)}"></progress> ${pct} %</span>`}</td>`
      + `<td>${safeInteger(row.activeAtGc) === null ? "—" : safeInteger(row.activeAtGc).toLocaleString("en-US")}</td>`
      + `<td>${safeInteger(row.archivedLastRun) === null ? "—" : safeInteger(row.archivedLastRun).toLocaleString("en-US")}</td></tr>`;
  }).join("");
  // Already escaped piece by piece: the <time> element must survive.
  const gcSummary = lastAt
    ? `Last run <time datetime="${escapeHtml(new Date(lastAt).toISOString())}">${escapeHtml(describeObservedAge(lastAt, nowMs))}</time>: ${safeInteger(last.totalArchived, 0).toLocaleString("en-US")} archived, ${safeInteger(last.totalSkipped, 0).toLocaleString("en-US")} skipped across ${safeInteger(last.agentsChecked, 0)} agents (${safeInteger(last.runs, 0)} runs on record).`
    : "No gc report found yet; the job writes one after its first run.";
  const pressure = asObject(ops.pressure);
  const level = typeof pressure.level === "string" ? pressure.level : "unknown";
  const pressureCard = ops.pressure
    ? `<article class="card"><div class="card-heading"><h3>Process pressure</h3>${renderBadge(level === "ok" ? "ready" : level === "warning" ? "degraded" : level === "critical" ? "failed" : "unavailable")}</div>
<dl><dt>RSS</dt><dd>${formatBytes(pressure.rssBytes)}</dd><dt>Heap used</dt><dd>${formatBytes(pressure.heapUsedBytes)}</dd><dt>Warning at</dt><dd>${formatBytes(pressure.warningBytes)}</dd><dt>Critical at</dt><dd>${formatBytes(pressure.criticalBytes)}</dd><dt>Gate</dt><dd>${pressure.gateEnabled === false ? "off — background work is never shed" : "on — low-priority work is shed above warning"}</dd></dl></article>`
    : `<article class="card"><h3>Process pressure</h3><p class="empty">No pressure reading available.</p></article>`;
  const runtimeRows = asArray(ops.runtime).filter((row) => asObject(row) && typeof row.key === "string").map((row) => {
    const value = row.value === null || row.value === undefined ? "—"
      : typeof row.value === "boolean" ? (row.value ? "on" : "off")
        : typeof row.value === "number" ? `${row.value.toLocaleString("en-US")}${row.unit ? ` ${escapeHtml(row.unit)}` : ""}` : safeText(row.value);
    return `<li><code>${safeText(row.key)}</code><span class="runtime-label">${safeText(row.label)}</span><strong>${value}</strong></li>`;
  }).join("");
  return `<section class="panel" aria-labelledby="operations-title">
<div class="section-heading"><div><h2 id="operations-title">Capacity &amp; Runtime</h2>
<p>How full each store is against <code>gc.maxMemoryCount</code>, what the last garbage-collection run did, whether the gateway process is under memory pressure, and the runtime limits in force. Read-only; the cap is changed on the Garbage Collection card, the limits in <a href="/config">OpenClaw config</a>.</p></div>${renderBadge(gc.enabled === false ? "disabled" : "enabled")}</div>
<div class="grid grid-2">
<article class="card card-wide"><h3>Store size against the cap</h3><p class="hint">${gcSummary}</p>
<div class="table-wrap"><table><thead><tr><th scope="col">Agent</th><th scope="col">Cards now</th><th scope="col">Cap</th><th scope="col">Used</th><th scope="col">Active at last gc</th><th scope="col">Archived last run</th></tr></thead><tbody>${agentRows || "<tr><td colspan=\"6\">No agent stores observed.</td></tr>"}</tbody></table></div>
${safeInteger(gc.moreAgents, 0) > 0 ? `<p class="hint">${safeInteger(gc.moreAgents, 0)} smaller stores not listed.</p>` : ""}
${cap ? "" : "<p class=\"hint\">No cap is set, so nothing is ever archived by count; the Used column stays empty.</p>"}</article>
${pressureCard}
<article class="card"><h3>Runtime limits</h3><ul class="count-list runtime-list">${runtimeRows || "<li>No runtime values available.</li>"}</ul></article>
</div>
</section>`;
}
...
// Die Speicherweise gilt fuer das ganze Plugin, nicht je Agent — deshalb ein
// einzelner Schalter auf der Capture-Karte statt einer Zeile je Agent.
function renderCaptureChunkingSwitch(card, captureChunking, write, pending = null) {
  if (card !== "capture") return "";
  const mark = pending ? pendingMark(pending, "capture.chunking") : "";
  const active = asObject(captureChunking).mode;
  const options = CAPTURE_CHUNKING_CHOICES.map((choice) => {
    const label = `<span class="switch-label">${escapeHtml(choice.label)}</span><span class="switch-detail">${escapeHtml(choice.detail)}</span>`;
    if (choice.id === active) return `<li class="switch-option is-current">${label}${renderBadge("current")}</li>`;
    if (write.mode !== "all") return `<li class="switch-option">${label}</li>`;
    return `<li class="switch-option">${label}`
      + `${formStart(write, "capture.chunking")}`
      + `<input type="hidden" name="choice" value="${escapeHtml(choice.id)}">`
      + `<button type="submit">Switch</button></form></li>`;
  }).join("");
  return `<fieldset class="feature-models"><legend>Storage mode${mark}</legend>`
    + `<p class="feature-note">How a captured turn is stored once it is long enough to split. Applies to new turns only.</p>`
    + `<ul class="switch-list switch-stack">${options}</ul>`
    + (write.mode === "all" ? "" : '<p class="hint">Set <code>controlUi.writeActions</code> to <code>all</code> in OpenClaw config to switch here.</p>')
    + `</fieldset>`;
}

// One matrix instead of a fieldset per card: `llmRouter.agentModels` is
// agent x task, and a table shows that shape. The default row is what most
// operators want; the per-task rows sit behind a disclosure so 21 x N selects
// do not dominate the page.
function renderModelCell(agent, feature, current, inherited, write, models, expanded = true, pending = null, viewAgent = "") {
  const id = `model-${agent.id}-${feature}`;
  const mark = pending ? pendingMark(pending, `model:${agent.id}.${feature}`) : "";
  // The inherited model belongs under the field, not inside the option text:
  // in a narrow cell "Inherit — anthropic/claude-haiku-" is cut off mid-name.
  const below = inherited ? `<span class="inherit">Inherits ${escapeHtml(inherited)}</span>` : "";
  if (!expanded) {
    const text = `<span class="inherit">${escapeHtml(inherited ? `Inherits ${inherited}` : "Inherits the OpenClaw route")}</span>`;
    if (write.mode !== "all") return `<td>${text}${mark}</td>`;
    // Progressive disclosure without script: the link re-renders the page with this one cell open.
    const params = `edit=${encodeURIComponent(`${agent.id}.${feature}`)}${viewAgent ? `&agent=${encodeURIComponent(viewAgent)}` : ""}`;
    return `<td>${text}${mark} <a class="cell-edit" href="?${escapeHtml(params)}#llm-tasks-title">Change</a></td>`;
  }
  // The default row has nothing to inherit from: without it, each task keeps
  // its own route, which the task rows spell out one by one.
  const emptyLabel = feature === "*" ? "No agent default — tasks keep their routes" : "Inherit";
  const options = `<option value=""${current ? "" : " selected"}>${escapeHtml(emptyLabel)}</option>`
    + (current && !models.some((model) => model.id === current)
      ? `<option value="${escapeHtml(current)}" selected disabled>${escapeHtml(current)} (no longer configured)</option>` : "")
    + models.map((model) => `<option value="${escapeHtml(model.id)}"${model.id === current ? " selected" : ""}>${safeText(model.label)}${model.requiresPermission ? " †" : ""}</option>`).join("");
  const select = `<select id="${escapeHtml(id)}" name="model" aria-label="${escapeHtml(`${feature === "*" ? "Default model" : feature} for ${agent.id}`)}"${write.mode === "all" ? "" : " disabled"}>${options}</select>`;
  if (write.mode !== "all") return `<td>${select}${below}${mark}</td>`;
  return `<td>${formStart(write, "feature.model")}<input type="hidden" name="agent" value="${escapeHtml(agent.id)}">`
    + `<input type="hidden" name="feature" value="${escapeHtml(feature)}">${select}<button type="submit">Save</button></form>${below}${mark}</td>`;
}

// Chat model per workspace: one row per chat agent (those with a heartbeat).
// Unlike the task table below, this is the model the agent answers with.
// One table row: the model select of one agent, shared by the chat card and
// the per-workspace subagent blocks.
function renderChatModelRow(agent, write) {
  const chosen = typeof agent.chosen === "string" ? agent.chosen : "";
  const models = asArray(agent.models);
  const options = `<option value=""${chosen ? "" : " selected"}>Profile default</option>`
    + (chosen && !models.some((model) => model.id === chosen)
      ? `<option value="${escapeHtml(chosen)}" selected disabled>${escapeHtml(chosen)} (no longer allowed)</option>` : "")
    + models.map((model) => `<option value="${escapeHtml(model.id)}"${model.id === chosen ? " selected" : ""}>${safeText(model.label)}</option>`).join("");
  // Its own field name: the task table's selects are all called "model".
  const select = `<select name="chat_model" aria-label="${escapeHtml(`Chat model for ${agent.id}`)}"${write.mode === "all" ? "" : " disabled"}>${options}</select>`;
  const running = `<span class="hint">Running now: ${safeText(agent.running || "none")}</span>`;
  const control = write.mode === "all"
    ? `${formStart(write, "chat.model")}<input type="hidden" name="agent" value="${escapeHtml(agent.id)}">${select}<button type="submit">Save</button></form>`
    : select;
  return `<tr><th scope="row">${safeText(agent.label)}${agent.label !== agent.id ? ` <code>${safeText(agent.id)}</code>` : ""}${running}</th><td>${control}</td></tr>`;
}

function renderChatModels(chatControls, write) {
  const agents = asArray(asObject(chatControls).agents).filter((agent) => asObject(agent) && typeof agent.id === "string");
  if (!agents.length) return "";
  const rows = agents.map((agent) => renderChatModelRow(agent, write)).join("");
  return `<section class="panel" aria-labelledby="chat-models-title">
<div class="section-heading"><div><h2 id="chat-models-title">Chat model</h2>
<p>The model each workspace's chat agent answers with. Saving applies to new sessions after the configuration reload and releases pinned active sessions of that agent, so they follow from their next turn; heartbeat sessions keep their own model. The list holds the models the agent knows and <code>agents.defaults.modelPolicy.allow</code> admits. <strong>Profile default</strong> removes the choice; the agent keeps its current model until the next gateway start. ${write.mode === "all" ? "" : 'To change models here, set <code>controlUi.writeActions</code> to <code>all</code> in OpenClaw config.'}</p></div></div>
<div class="table-wrap"><table><tbody>${rows}</tbody></table></div>
</section>`;
}

// Subagent models: one collapsible block per chat workspace, listing the
// subagents that serve it. Same row, same write action as the chat card.
function renderSubagentModels(subagentControls, write) {
  const workspaces = asArray(asObject(subagentControls).workspaces).filter((ws) => asObject(ws) && typeof ws.id === "string");
  const blocks = workspaces.map((ws) => {
    const agents = asArray(ws.agents).filter((agent) => asObject(agent) && typeof agent.id === "string");
    if (!agents.length) return "";
    const rows = agents.map((agent) => renderChatModelRow(agent, write)).join("");
    return `<details class="matrix-tasks"><summary>Subagents of ${safeText(ws.label)} (${agents.length})</summary>`
      + `<div class="table-wrap"><table><tbody>${rows}</tbody></table></div></details>`;
  }).filter(Boolean).join("");
  if (!blocks) return "";
  return `<section class="panel" aria-labelledby="subagent-models-title">
<div class="section-heading"><div><h2 id="subagent-models-title">Subagent models</h2>
<p>The model each subagent of a workspace answers with. Saving works like the chat model above: new sessions after the configuration reload, pinned active sessions of that subagent are released.</p></div></div>
${blocks}
</section>`;
}

function renderLlmTaskMatrix(modelControls, settings, write, pending = null, view = {}) {
  const controls = asObject(modelControls);
  const all = asArray(controls.agents).filter((agent) => asObject(agent) && typeof agent.id === "string");
  if (!all.length) return "";
  // Beyond four columns the table starves every select; show one agent then,
  // picked by ?agent= — a view parameter, so it works read-only too.
  const tabbed = all.length > MATRIX_MAX_COLUMNS;
  const chosen = tabbed ? (all.find((agent) => agent.id === view.agent) || all[0]) : null;
  const agents = tabbed ? [chosen] : all;
  const viewAgent = tabbed ? chosen.id : "";
  const catalogue = [...new Map(all.flatMap((agent) => asArray(agent.models)).map((model) => [model.id, model])).values()];
  const hostDefault = asArray(asObject(asObject(settings).byCard)["llm-tasks"]).find((setting) => setting?.id === "llmRouter.defaultModel");
  // The background default is the reference for every task on this page; the
  // agent's chat model only applies when nothing else is set. Its own box, so
  // the table below cannot be mistaken for the place where it lives.
  const hostDefaultRow = hostDefault
    ? `<div class="host-default"><h3>Background default for every agent</h3><div class="settings settings-wide">${renderSettingRow(hostDefault, write, catalogue, pending)}</div></div>`
    : "";
  const tabs = tabbed
    ? `<nav class="agent-tabs" aria-label="Agent"><span class="hint">Agent:</span>${all.map((agent) => (agent.id === chosen.id
        ? `<span class="agent-tab is-current" aria-current="true">${safeText(agent.label)}</span>`
        : `<a class="agent-tab" href="?agent=${encodeURIComponent(agent.id)}#llm-tasks-title">${safeText(agent.label)}</a>`)).join("")}</nav>`
    : "";
  const head = `<tr><th scope="col">Task</th>${agents.map((agent) => `<th scope="col">${safeText(agent.label)}${agent.label !== agent.id ? ` <code>${safeText(agent.id)}</code>` : ""}`
    + `<span class="hint">Background default: ${safeText(agent.agentDefault || asObject(hostDefault).value || "none")}</span>`
    + `<span class="hint">Chat model (last resort): ${safeText(agent.defaultModel || "none")}</span></th>`).join("")}</tr>`;
  const defaultRow = `<tr><th scope="row">Default for all tasks<span class="hint">Applies to every task of this agent without its own choice.</span></th>${agents.map((agent) =>
    renderModelCell(agent, "*", typeof agent.agentDefault === "string" ? agent.agentDefault : "", "", write, asArray(agent.models), true, pending, viewAgent)).join("")}</tr>`;
  const tasks = asArray(all[0].features).filter((feature) => asObject(feature) && typeof feature.id === "string");
  const taskRows = tasks.map((task) => `<tr><th scope="row">${safeText(task.label)}<span class="hint">${safeText(task.card)}</span></th>${agents.map((agent) => {
    const feature = asArray(agent.features).find((entry) => entry.id === task.id) || {};
    const inherited = feature.inheritedModel ? `${feature.inheritedModel}${feature.direct ? (agent.agentDefault ? " (replaces direct route)" : " (direct route)") : ""}` : "";
    const current = typeof feature.model === "string" ? feature.model : "";
    // 21 tasks x N agents x the whole catalogue would be several hundred KB of
    // <option>s; a cell only carries the select while it holds a choice or was opened.
    return renderModelCell(agent, task.id, current, inherited, write, asArray(agent.models), Boolean(current) || write.edit === `${agent.id}.${task.id}`, pending, viewAgent);
  }).join("")}</tr>`).join("");
  const overridden = all.reduce((n, agent) => n + asArray(agent.features).filter((f) => f.model).length, 0);
  const selection = controls.selection === "configured"
    ? "Columns follow <code>llmRouter.dashboardAgents</code>."
    : "Agents are the standing ones (those with a heartbeat) plus any agent with a saved choice; set <code>llmRouter.dashboardAgents</code> to pick others.";
  const taskTable = `<div class="table-wrap matrix"><table><thead>${head}</thead><tbody>${taskRows}</tbody></table></div>`;
  return `<section class="panel" aria-labelledby="llm-tasks-title">
<div class="section-heading"><div><h2 id="llm-tasks-title">LLM Tasks</h2>
<p>Which model each agent uses for PLUR1BUS's background tasks — not for chatting. A task choice beats the agent default, the agent default beats the background default above, and that beats every route from the config file. <strong>Inherit</strong> removes the choice; the line under each field says what runs instead. Models marked † are not yet in the plugin's model permissions; saving one grants exactly that model. ${selection} ${write.mode === "all" ? "Changes go through OpenClaw's normal configuration reload, which takes about a minute." : 'To change models here, set <code>controlUi.writeActions</code> to <code>all</code> in OpenClaw config.'}</p>
<p class="summary" role="status">${escapeHtml(`${all.length} agent${all.length === 1 ? "" : "s"}, ${all.filter((a) => a.agentDefault).length} with a default, ${overridden} task override${overridden === 1 ? "" : "s"}`)}</p></div></div>
${hostDefaultRow}${tabs}
<div class="table-wrap matrix"><table><thead>${head}</thead><tbody>${defaultRow}</tbody></table></div>
${tabbed ? taskTable : `<details class="matrix-tasks"${overridden || write.edit ? " open" : ""}><summary>Per-task overrides (${tasks.length} tasks)</summary>${taskTable}</details>`}
</section>`;
}

// One compact row per setting: label, current value as a select or number
// field, Save. Toggles and operator decisions share this — the table in
// lib/dashboard-settings.js decides what is offered, the page only renders it.
function settingLabel(setting, value) {
  if (setting.type === "boolean") return value ? "On" : "Off";
  if (setting.type === "enum") return value === true ? "On" : value === false ? "Off" : String(value);
  return value === null || value === undefined || value === "" ? "not set" : String(value);
}

function renderSettingRow(setting, write, models = [], pending = null) {
  const id = `setting-${setting.id.replaceAll(/[^A-Za-z0-9_-]/g, "-")}`;
  const value = setting.value;
  let field;
  if (setting.type === "number") {
    field = `<input type="number" id="${escapeHtml(id)}" name="value" value="${escapeHtml(value ?? "")}"${Number.isFinite(setting.min) ? ` min="${setting.min}"` : ""}${Number.isFinite(setting.max) ? ` max="${setting.max}"` : ""} step="1"${write.mode === "all" ? "" : " disabled"}>`;
  } else if (setting.type === "model") {
    const current = typeof value === "string" ? value : "";
    field = `<select id="${escapeHtml(id)}" name="value"${write.mode === "all" ? "" : " disabled"}><option value=""${current ? "" : " selected"}>No host-wide default</option>`
      + (current && !models.some((model) => model.id === current) ? `<option value="${escapeHtml(current)}" selected disabled>${escapeHtml(current)} (no longer configured)</option>` : "")
      + models.map((model) => `<option value="${escapeHtml(model.id)}"${model.id === current ? " selected" : ""}>${safeText(model.label)}${model.requiresPermission ? " †" : ""}</option>`).join("") + "</select>";
  } else {
    field = `<select id="${escapeHtml(id)}" name="value"${write.mode === "all" ? "" : " disabled"}>`
      + asArray(setting.values).map((candidate) => `<option value="${escapeHtml(String(candidate))}"${candidate === value ? " selected" : ""}>${escapeHtml(settingLabel(setting, candidate))}</option>`).join("") + "</select>";
  }
  const mark = pending ? pendingMark(pending, setting.id) : "";
  const label = `<label for="${escapeHtml(id)}">${safeText(setting.label)}${mark}${setting.hint ? `<span class="hint">${safeText(setting.hint)}</span>` : ""}</label>`;
  if (write.mode !== "all") return `<div class="setting-row">${label}<div class="setting-field">${field}</div></div>`;
  return `<div class="setting-row">${label}${formStart(write, "setting.set")}<input type="hidden" name="setting" value="${escapeHtml(setting.id)}"><div class="setting-field">${field}<button type="submit">Save</button></div></form></div>`;
}

function renderCardSettings(card, settings, write, pending) {
  const rows = asArray(asObject(asObject(settings).byCard)[card]).filter((setting) => asObject(setting) && typeof setting.id === "string");
  if (!rows.length) return "";
  return `<div class="settings">${rows.map((setting) => renderSettingRow(setting, write, [], pending)).join("")}</div>`;
}

function renderFeatureControls(values, captureChunking, settings, write = writeContext(null), pending = null) {
  ...
  const rendered = cards.map((entry) => {
    ...
    return `<li class="card feature-card"><div class="card-heading"><h3>${safeText(entry.label)}</h3>${renderBadge(state)}</div>${details}${renderCaptureChunkingSwitch(entry.id, captureChunking, write, pending)}${renderCardSettings(entry.id, settings, write, pending)}</li>`;
  }).join("");
...
<p>Most PLUR1BUS features are opt-out: an absent switch means on, only an explicit <code>enabled: false</code> turns one off; a card marked "off by default" is the exception. <strong>Running</strong> is active, <strong>blocked</strong> is switched on but held back by a dependency, <strong>off</strong> is switched off. ${write.mode === "all" ? "Each card carries its switch and its operating decisions; the thresholds stay in <a href=\"/config\">OpenClaw config</a>." : 'Switches and decisions are shown here; to change them, set <code>controlUi.writeActions</code> to <code>all</code> in OpenClaw config.'}</p>
<p>Which model a task uses is chosen per agent under <a href="#llm-tasks-title">LLM Tasks</a>.</p>
...
// 7.12.48: geminte Skills je Workspace, mit Freigabe, Ablehnung und Rueckzug.
// Jede Angabe auf einer Karte ist Modellausgabe aus Gedaechtnis-Auszuegen und
// geht ausschliesslich durch escapeHtml; die Projektion hat die Laengen
// bereits gedeckelt.
const SKILL_STATUS_LABEL = Object.freeze({
  pending_review: "pending",
  activation_partial: "partially applied",
  active: "active",
});

function renderSkillActionButton(write, action, agentId, proposalId, label, secondary = false) {
  return formStart(write, action)
    + `<input type="hidden" name="agent" value="${escapeHtml(agentId)}">`
    + `<input type="hidden" name="proposal" value="${escapeHtml(proposalId)}">`
    + `<button type="submit"${secondary ? " class=\"button-secondary\"" : ""}>${escapeHtml(label)}</button></form>`;
}

function renderSkillCard(proposal, agentId, write, available) {
  const status = SKILL_STATUS_LABEL[proposal.status] ? proposal.status : "pending_review";
  const confidence = Number.isFinite(proposal.confidence) ? `${Math.round(proposal.confidence * 100)} %` : "unknown";
  const evidence = [
    `${Number.isFinite(proposal.memories) ? proposal.memories : 0} memories`,
    Number.isFinite(proposal.score) ? `score ${proposal.score}` : "",
    proposal.grade ? `grade ${proposal.grade}` : "",
  ].filter(Boolean).join(" · ");
  const discovered = proposal.discoveredAt ? proposal.discoveredAt.slice(0, 10) : "unknown";
  const benefit = proposal.benefit
    || (proposal.description ? "Saves the agent from rediscovering this procedure; the evidence shows it recurred." : "");
  const examples = asArray(proposal.examples).map((example) => `<li>${escapeHtml(example)}</li>`).join("");
  const actions = [];
  if (write.skills && available) {
    if (status === "pending_review" && proposal.bound) {
      actions.push(renderSkillActionButton(write, "skill.approve", agentId, proposal.id, "Approve"));
      actions.push(renderSkillActionButton(write, "skill.reject", agentId, proposal.id, "Decline", true));
    } else if (status === "pending_review") {
      actions.push(renderSkillActionButton(write, "skill.reject", agentId, proposal.id, "Decline", true));
    } else if (status === "activation_partial") {
      actions.push(renderSkillActionButton(write, "skill.approve", agentId, proposal.id, "Finish approval"));
      actions.push(renderSkillActionButton(write, "skill.retire", agentId, proposal.id, "Withdraw", true));
    } else {
      actions.push(renderSkillActionButton(write, "skill.retire", agentId, proposal.id, "Withdraw", true));
    }
  }
  const notes = [];
  if (status === "pending_review" && !proposal.bound) {
    notes.push("No Workshop binding: mined before the Workshop integration, so it can only be declined.");
  }
  if (status === "active" && proposal.location === "workshop") {
    notes.push("Applied through the OpenClaw Skill Workshop. Withdraw removes its Workshop directory and blocks the name for future mining.");
  }
  return `<li class="card skill-card">
<div class="card-heading"><h3>${escapeHtml(proposal.title)}</h3>${renderBadge(status, SKILL_STATUS_LABEL[status])}</div>
<dl><dt>Skill</dt><dd><code>${escapeHtml(proposal.skillName)}</code></dd>
<dt>Category</dt><dd>${escapeHtml(proposal.category || "workflow")}</dd>
<dt>Evidence</dt><dd>${escapeHtml(evidence)} · confidence ${escapeHtml(confidence)}</dd>
<dt>Discovered</dt><dd>${escapeHtml(discovered)}</dd></dl>
<p class="feature-purpose">${escapeHtml(proposal.description || "No description was extracted.")}</p>
${benefit ? `<p class="skill-benefit"><span class="label">Benefit</span> ${escapeHtml(benefit)}</p>` : ""}
<details><summary>Instructions the agent would follow</summary><pre>${escapeHtml(proposal.instructions || "No instructions were extracted.")}</pre>${examples ? `<ul>${examples}</ul>` : ""}</details>
${notes.map((note) => `<p class="hint">${escapeHtml(note)}</p>`).join("")}
${actions.length ? `<div class="skill-actions">${actions.join("")}</div>` : ""}
</li>`;
}

function renderSkillWorkshop(value, write = writeContext(null)) {
  const status = asObject(value);
  const counts = asObject(status.counts);
  const available = status.available === true;
  const modeText = status.mode === "on"
    ? "always on (skillMiner.autoApply = on)"
    : status.mode === "off"
      ? "off (skillMiner.autoApply = off): every mined skill waits here for approval"
      : `follows the host (skillMiner.autoApply = host, skills.workshop.autonomous.mode = ${escapeHtml(status.hostMode || "auto")})`;
  const summary = `${Number(counts.pending) || 0} pending, ${Number(counts.active) || 0} active, ${Number(counts.rejected) || 0} declined`;
  const groups = asArray(status.workspaces).map((workspace) => {
    const agents = asArray(workspace.agents).map((agent) => {
      const cards = asArray(agent.proposals).map((proposal) => renderSkillCard(proposal, agent.agentId, write, available)).join("");
      return `<h4>Agent <code>${escapeHtml(agent.agentId)}</code></h4><ul class="grid grid-2 card-list">${cards || "<li class=\"empty\">No open or active mined skills.</li>"}</ul>`;
    }).join("");
    return `<div class="skill-group"><h3>Workspace <code>${escapeHtml(workspace.id)}</code></h3>${agents}</div>`;
  }).join("");
  const readOnlyNote = !write.skills
    ? `<p class="hint">Approve, decline and withdraw need <code>controlUi.writeActions: "all"</code>; this page is ${write.mode === "off" ? "read-only" : "limited to reranker switching"}.</p>`
    : !available
      ? "<p class=\"hint\">The OpenClaw Skill Workshop capability is unavailable, so proposals cannot be applied or rejected from here.</p>"
      : "";
  return `<section class="panel" aria-labelledby="skill-workshop-title">
<div class="section-heading"><div><h2 id="skill-workshop-title">Mined Skills</h2>
<p>The Skill Miner runs weekly per agent, clusters corroborated memories of the last 30 days, and lets the model extract procedures that recurred. Each result is filed as a draft in the OpenClaw Skill Workshop. <strong>Approve</strong> applies the draft to the agent's Workshop directory, so future sessions can follow it, and marks its evidence as corroborated. <strong>Decline</strong> rejects the draft and blocks the name for future mining. <strong>Withdraw</strong> removes an already applied skill. Auto-apply: ${modeText}.</p>
<p class="summary" role="status">${escapeHtml(summary)}</p>${readOnlyNote}</div></div>
${groups || "<p class=\"empty\">No mined skills yet. The next weekly run files its findings here.</p>"}
</section>`;
}
...
function renderEmbeddingDimensionPlanner(values) {
  ...
  const cards = profiles.map((profile) => {
    ...
    const recommended = profile.model === RECOMMENDED_EMBEDDING_MODEL && profile.current !== true
      ? renderBadge("recommended")
      : "";
    return `<article class="card"><div class="card-heading"><h3>${safeText(profile.model)}</h3>${renderBadge(profile.current === true ? "current" : profile.mode)}${recommended}</div><p><code>${safeText(profile.provider)}</code></p>${control}<p>Allowed range: ${profile.minDimensions === null ? "probe result" : `${safeText(profile.minDimensions)}–${safeText(profile.maxDimensions)}`}.${selectionRule} Every migration still validates a real returned vector.</p>${license}</article>`;
  }).join("");
  const current = profiles.find((profile) => profile.current === true);
  const migrationAdvice = current && current.model !== RECOMMENDED_EMBEDDING_MODEL
    ? `<div class="notice"><strong>Recommended for existing installs: migrate to JinaAI v5 Text Nano.</strong> In the PLUR1BUS lab test on real memory cards it ranked level with Jina v3, separated hits from noise more clearly, embedded cards three times faster and needed half the memory. Prepare the <code>jina-v5-nano-768</code> target under Model Preparation, then run the re-embedding migration below: dry run, copy, and a separate switch. Nothing changes until you switch, and the previous generation stays for rollback.</div>`
    : "";
  return `<section class="panel" aria-labelledby="embedding-dimension-planner-title"><div class="section-heading"><div><h2 id="embedding-dimension-planner-title">Embedding Dimension Planner</h2><p>Model-dependent choices for re-embedding. JinaAI v3 and JinaAI v5 Text Nano are embedding models; the separate JinaAI and BGE rerankers do not define memory-vector dimensions.</p></div></div>${migrationAdvice}<div class="grid grid-3">${cards || "<p class=\"empty\">No verified dimension profiles are available.</p>"}</div><div class="notice"><strong>Read-only planning controls.</strong> Apply a target only through the confirmed operator-admin re-embedding action. A reload restores the persisted active or migration state.</div></section>`;
}

/** The embedding model the installer proposes since 7.12.0, after the lab test. */
const RECOMMENDED_EMBEDDING_MODEL = "jinaai/jina-embeddings-v5-text-nano-retrieval";

const EMBEDDING_PROFILE_CHOICES = Object.freeze([
  Object.freeze({ id: "jina-v5-nano-768", label: "JinaAI v5 Text Nano, 768 dimensions", detail: "Recommended. Level with v3 on ranking in the lab test, clearer separation from noise, three times the migration speed, half the memory. Non-commercial license, must be acknowledged." }),
  Object.freeze({ id: "jina-v5-nano-512", label: "JinaAI v5 Text Nano, 512 dimensions", detail: "The economical width of the recommended model. Non-commercial license." }),
  Object.freeze({ id: "jina-v3-multilingual-1024", label: "JinaAI v3 multilingual, 1024 dimensions", detail: "Kept for existing installs; 89 languages, largest vectors. Non-commercial license." }),
  Object.freeze({ id: "jina-v3-multilingual-512", label: "JinaAI v3 multilingual, 512 dimensions", detail: "Kept for existing installs. Non-commercial license, must be acknowledged." }),
  Object.freeze({ id: "e5-multilingual-384", label: "E5 multilingual, 384 dimensions", detail: "Small, keyless, commercially usable; weakest separation of hits from noise." }),
]);
...
// Nine panels and several screens of scrolling: a sticky row of anchors is
// the cheapest way to make any of them one click away. No script involved.
const JUMP_TARGETS = Object.freeze([
  ["memory-health-title", "Health"],
  ["operations-title", "Capacity"],
  ["workspace-matrix-title", "Workspaces"],
  ["feature-controls-title", "Features"],
  ["llm-tasks-title", "LLM Tasks"],
  ["skill-workshop-title", "Skills"],
  ["obsidian-title", "Obsidian"],
  ["credentials-title", "Credentials"],
  ["embedding-dimension-planner-title", "Dimensions"],
  ["model-preparation-title", "Preparation"],
  ["reembedding-workflow-title", "Re-Embedding"],
]);

function renderJumpBar(status) {
  const hasMatrix = asArray(asObject(status.featureModels).agents).length > 0;
  const links = JUMP_TARGETS.filter(([id]) => hasMatrix || id !== "llm-tasks-title")
    .map(([id, label]) => `<a href="#${id}">${escapeHtml(label)}</a>`).join("");
  return `<nav class="jump" aria-label="Sections">${links}</nav>`;
}

function renderProjection(projection, nowMs = Date.now(), rawWrite = null, view = {}) {
  const status = asObject(projection);
  const write = writeContext(rawWrite);
  const pending = pendingContext(status, nowMs);
...
.feature-models { min-width: 0; margin: 10px 0 0; padding: 8px; border: 1px solid var(--border); border-radius: var(--radius-sm); }
.feature-models legend { padding: 0 4px; color: var(--muted); font-size: 12px; }
/* In a 14rem card the reranker-style row has no room: stack label, text and button. */
.switch-stack .switch-option { flex-direction: column; align-items: stretch; gap: 4px; }
.switch-stack .switch-option .badge, .switch-stack .switch-form { align-self: flex-end; }
.jump { position: sticky; top: 0; z-index: 2; display: flex; flex-wrap: wrap; gap: 4px 6px; margin: 0 0 12px; padding: 8px 0; background: var(--bg); border-bottom: 1px solid var(--border); }
.jump a { padding: 3px 9px; border: 1px solid var(--border); border-radius: var(--radius-full); background: var(--panel-strong); color: var(--text); font-size: 12px; font-weight: 500; white-space: nowrap; }
.jump a:hover { color: var(--text-strong); border-color: var(--border-strong); text-decoration: none; }
/* Fixed layout: the agent columns share the width evenly instead of growing
   with the longest option text, so three agents fit without a scrollbar. */
.matrix table { margin-top: 0; table-layout: fixed; min-width: 38rem; } .matrix th[scope="col"] { text-transform: none; letter-spacing: 0; font-size: 13px; color: var(--text-strong); font-weight: 600; overflow-wrap: anywhere; }
.matrix th .hint { display: block; font-weight: 400; text-transform: none; letter-spacing: 0; }
.matrix th[scope="row"] { width: 11rem; }
.matrix td { min-width: 0; }
.matrix td form { display: flex; gap: 6px; align-items: center; margin: 0; }
.matrix select { flex: 1 1 auto; width: 100%; min-width: 0; max-width: 100%; min-height: 32px; padding: 4px 6px; font: inherit; font-size: 13px; color: var(--text-strong); background: var(--panel-strong); border: 1px solid var(--border-strong); border-radius: var(--radius-sm); }
.matrix td button { padding: 4px 10px; font-size: 12px; }
.notice-pending { border-color: color-mix(in srgb, var(--warn) 45%, transparent); }
.pending-mark { display: inline-block; margin-left: 6px; padding: 1px 7px; border: 1px solid color-mix(in srgb, var(--warn) 45%, transparent); border-radius: var(--radius-full); background: color-mix(in srgb, var(--warn) 12%, transparent); color: var(--warn); font-size: 11px; font-weight: 600; vertical-align: middle; }
.health-reason { margin-top: 6px; color: var(--text); }
.host-default { margin-top: 12px; padding: 10px 12px; border: 1px solid var(--border-strong); border-radius: var(--radius); background: var(--panel-strong); }
.host-default h3 { margin-bottom: 2px; }
.agent-tabs { display: flex; flex-wrap: wrap; gap: 6px; align-items: baseline; margin-top: 12px; }
.agent-tab { padding: 3px 10px; border: 1px solid var(--border); border-radius: var(--radius-full); background: var(--panel-strong); color: var(--text); font-size: 12.5px; font-weight: 500; }
.agent-tab:hover { color: var(--text-strong); border-color: var(--border-strong); text-decoration: none; }
.agent-tab.is-current { color: var(--text-strong); border-color: color-mix(in srgb, var(--ok) 45%, transparent); background: color-mix(in srgb, var(--ok) 12%, transparent); }
.matrix .inherit { display: block; margin-top: 4px; font-size: 12px; }
.headroom { display: inline-flex; align-items: center; gap: 8px; white-space: nowrap; } .headroom progress { width: 7rem; margin: 0; }
.headroom.is-warning { color: var(--warn); } .headroom.is-critical { color: var(--danger); }
.headroom.is-warning progress { accent-color: var(--warn); } .headroom.is-critical progress { accent-color: var(--danger); }
.runtime-list li { display: grid; grid-template-columns: minmax(9rem, auto) 1fr auto; gap: 8px; align-items: baseline; }
.runtime-label { color: var(--muted); font-size: 12.5px; }
.settings { display: grid; gap: 6px; margin-top: 10px; padding-top: 8px; border-top: 1px solid var(--border); }
.settings-wide { margin: 0 0 12px; padding: 0; border: 0; }
.setting-row { display: grid; gap: 4px; } .setting-row label { font-size: 12.5px; color: var(--text-strong); font-weight: 600; }
.setting-row label .hint { display: block; font-weight: 400; }
.setting-row form { margin: 0; }
.setting-field { display: flex; gap: 6px; align-items: center; }
.setting-field select, .setting-field input { flex: 1 1 auto; width: 100%; min-width: 0; min-height: 32px; padding: 4px 6px; font: inherit; font-size: 13px; color: var(--text-strong); background: var(--panel-strong); border: 1px solid var(--border-strong); border-radius: var(--radius-sm); }
.setting-field button { padding: 4px 10px; font-size: 12px; }
.matrix .cell-edit { margin-left: 6px; font-size: 12px; white-space: nowrap; }
.matrix-tasks { margin-top: 12px; } .matrix-tasks summary { cursor: pointer; font-weight: 600; color: var(--text-strong); padding: 4px 0; }
.matrix-tasks[open] summary { margin-bottom: 8px; }
...
.skill-group { margin-top: 12px; } .skill-group h3 { margin-bottom: 4px; } .skill-group h4 { margin: 10px 0 6px; color: var(--muted); font-size: 12px; font-weight: 600; letter-spacing: .04em; text-transform: uppercase; }
.skill-card { display: flex; flex-direction: column; gap: 6px; }
.skill-card dl { grid-template-columns: minmax(6rem, auto) 1fr; font-size: 12.5px; }
.skill-card details { margin-top: 4px; font-size: 13px; } .skill-card summary { cursor: pointer; color: var(--text-strong); font-weight: 600; }
.skill-card pre { margin: 6px 0 0; padding: 8px 10px; white-space: pre-wrap; overflow-wrap: anywhere; font: 12px/1.5 var(--font-mono); background: var(--panel-strong); border: 1px solid var(--border); border-radius: var(--radius-sm); }
.skill-card ul { margin: 6px 0 0 18px; padding: 0; }
.skill-benefit { margin: 0; color: var(--text); } .skill-benefit .label { color: var(--muted); }
.skill-actions { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px; } .skill-actions form { margin: 0; }
.button-secondary { background: var(--panel-strong); color: var(--text-strong); } .button-secondary:hover { background: var(--border-strong); }
.badge-pending_review, .badge-activation_partial { color: var(--warn); border-color: color-mix(in srgb, var(--warn) 45%, transparent); background: color-mix(in srgb, var(--warn) 12%, transparent); }
.badge-active { color: var(--ok); border-color: color-mix(in srgb, var(--ok) 45%, transparent); background: color-mix(in srgb, var(--ok) 12%, transparent); }
...
${renderJumpBar(status)}
${renderResultBanner(write)}
${renderPendingNotice(pending, nowMs)}
...
${renderOperations(status.operations, nowMs)}
${renderWorkspaceMatrix(status.workspaceMatrix)}
${renderFeatureControls(status.featureCards, status.captureChunking, status.settings, write, pending)}
${renderChatModels(status.chatModels, write)}
${renderLlmTaskMatrix(status.featureModels, status.settings, write, pending, view)}
${renderSubagentModels(status.subagentModels, write)}
${renderSkillWorkshop(status.skillWorkshop, write)}
...
// ?agent=<id> only chooses which agent the LLM Tasks panel shows. Like ?edit=
// it writes nothing — but unlike ?edit= it also applies to a read-only page.
function viewFromUrl(url) {
  const value = queryOf(url).get("agent");
  return { agent: typeof value === "string" && VIEW_AGENT_RE.test(value) ? value : "" };
}

// ?edit=<agent>.<task> only changes which matrix cell shows a select; it never
// writes anything and is ignored on read-only pages.
function editFromUrl(url) {
  const value = queryOf(url).get("edit");
  return typeof value === "string" && EDIT_CELL_RE.test(value) ? value : "";
}
