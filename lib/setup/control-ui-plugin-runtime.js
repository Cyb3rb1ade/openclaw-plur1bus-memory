...
function renderMemoryHealth(value, providers, features) {
...
const CREDENTIAL_STATUS_HELP = Object.freeze({
  ...
  missing: "No key of its own is configured. Whether that stops the feature depends on the source column: without a host route it stays off.",
...
const CREDENTIAL_SOURCE_HELP = Object.freeze({
  host_route: "No key of its own; this capability uses OpenClaw's configured model route. It runs, and a key here would only override the host default.",
...
${renderMemoryHealth(status.memoryHealth, status.providers, status.features)}
