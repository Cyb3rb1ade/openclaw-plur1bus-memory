...
function renderCaptureChunkingSwitch(card, captureChunking, write) {
  ...
  return `<fieldset class="feature-models"><legend>Storage mode</legend>`
...
function renderModelCell(agent, feature, current, inherited, write, models, expanded = true) {
  const id = `model-${agent.id}-${feature}`;
  if (!expanded) {
    const text = `<span class="inherit">${escapeHtml(inherited ? `Inherit — ${inherited}` : "Inherit OpenClaw route")}</span>`;
    if (write.mode !== "all") return `<td>${text}</td>`;
    // Progressive disclosure without script: the link re-renders the page with this one cell open.
    return `<td>${text} <a class="cell-edit" href="?edit=${escapeHtml(`${agent.id}.${feature}`)}#llm-tasks-title">Change</a></td>`;
  }
  ...
  const emptyLabel = feature === "*" ? "No agent default — tasks keep their routes" : inherited ? `Inherit — ${inherited}` : "Inherit OpenClaw route";
  ...
  if (write.mode !== "all") return `<td>${select}</td>`;
  return `<td>${formStart(write, "feature.model")}<input type="hidden" name="agent" value="${escapeHtml(agent.id)}">`
    + `<input type="hidden" name="feature" value="${escapeHtml(feature)}">${select}<button type="submit">Save</button></form></td>`;
}

function renderLlmTaskMatrix(modelControls, settings, write) {
  const controls = asObject(modelControls);
  const agents = asArray(controls.agents).filter((agent) => asObject(agent) && typeof agent.id === "string");
  if (!agents.length) return "";
  const catalogue = [...new Map(agents.flatMap((agent) => asArray(agent.models)).map((model) => [model.id, model])).values()];
  const hostDefault = asArray(asObject(asObject(settings).byCard)["llm-tasks"]).find((setting) => setting?.id === "llmRouter.defaultModel");
  const hostDefaultRow = hostDefault ? `<div class="settings settings-wide">${renderSettingRow(hostDefault, write, catalogue)}</div>` : "";
  const head = `<tr><th scope="col">Task</th>${agents.map((agent) => `<th scope="col">${safeText(agent.label)}${agent.label !== agent.id ? ` <code>${safeText(agent.id)}</code>` : ""}<span class="hint">OpenClaw default: ${safeText(agent.defaultModel || "none")}</span></th>`).join("")}</tr>`;
  const defaultRow = `<tr><th scope="row">Default for all tasks<span class="hint">Applies to every task without its own choice.</span></th>${agents.map((agent) =>
    renderModelCell(agent, "*", typeof agent.agentDefault === "string" ? agent.agentDefault : "", "", write, asArray(agent.models))).join("")}</tr>`;
  const tasks = asArray(agents[0].features).filter((feature) => asObject(feature) && typeof feature.id === "string");
  const taskRows = tasks.map((task) => `<tr><th scope="row">${safeText(task.label)}<span class="hint">${safeText(task.card)}</span></th>${agents.map((agent) => {
    ...
    return renderModelCell(agent, task.id, current, inherited, write, asArray(agent.models), Boolean(current) || write.edit === `${agent.id}.${task.id}`);
  }).join("")}</tr>`).join("");
  const overridden = agents.reduce((n, agent) => n + asArray(agent.features).filter((f) => f.model).length, 0);
  const selection = controls.selection === "configured"
    ? "Columns follow <code>llmRouter.dashboardAgents</code>."
    : "Columns are the standing agents (those with a heartbeat) plus any agent with a saved choice; set <code>llmRouter.dashboardAgents</code> to pick others.";
...
<p>Which model each agent uses for PLUR1BUS's background tasks. A task choice beats the agent default; the agent default beats every route from the config file. <strong>Inherit</strong> removes the choice and shows what runs instead. Models marked † are not yet in the plugin's model permissions; saving one grants exactly that model. ${selection} ${write.mode === "all" ? "Changes go through OpenClaw's normal configuration reload; running jobs finish with their previous settings." : 'To change models here, set <code>controlUi.writeActions</code> to <code>all</code> in OpenClaw config.'}</p>
<p class="summary" role="status">${escapeHtml(`${agents.length} agent${agents.length === 1 ? "" : "s"}, ${agents.filter((a) => a.agentDefault).length} with a default, ${overridden} task override${overridden === 1 ? "" : "s"}`)}</p></div></div>
${hostDefaultRow}
<div class="table-wrap matrix"><table><thead>${head}</thead><tbody>${defaultRow}</tbody></table></div>
<details class="matrix-tasks"${overridden || write.edit ? " open" : ""}><summary>Per-task overrides (${tasks.length} tasks)</summary>
<div class="table-wrap matrix"><table><thead>${head}</thead><tbody>${taskRows}</tbody></table></div></details>
</section>`;
}
...
function renderSettingRow(setting, write, models = []) {
  ...
  const label = `<label for="${escapeHtml(id)}">${safeText(setting.label)}${setting.hint ? `<span class="hint">${safeText(setting.hint)}</span>` : ""}</label>`;
...
function renderCardSettings(card, settings, write) {
  const rows = asArray(asObject(asObject(settings).byCard)[card]).filter((setting) => asObject(setting) && typeof setting.id === "string");
  if (!rows.length) return "";
  return `<div class="settings">${rows.map((setting) => renderSettingRow(setting, write)).join("")}</div>`;
}

function renderFeatureControls(values, captureChunking, settings, write = writeContext(null)) {
  ...
  const rendered = cards.map((entry) => {
    ...
    return `<li class="card feature-card"><div class="card-heading"><h3>${safeText(entry.label)}</h3>${renderBadge(state)}</div>${details}${renderCaptureChunkingSwitch(entry.id, captureChunking, write)}${renderCardSettings(entry.id, settings, write)}</li>`;
  }).join("");
...
.matrix .inherit { color: var(--muted); } .matrix .cell-edit { margin-left: 6px; font-size: 12px; white-space: nowrap; }
...
${renderFeatureControls(status.featureCards, status.captureChunking, status.settings, write)}
${renderLlmTaskMatrix(status.featureModels, status.settings, write)}
