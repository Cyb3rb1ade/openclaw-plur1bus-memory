...
function renderFeatureModelControls(card, modelControls, write) {
  return asArray(asObject(modelControls).agents).map((agent) => {
    const features = asArray(agent.features).filter((feature) => feature.card === card);
    if (!features.length) return "";
    const rows = features.map((feature) => {
      const id = `model-${agent.id}-${feature.id}`;
      const models = asArray(agent.models);
      const current = typeof feature.model === "string" ? feature.model : "";
      const inherited = feature.inheritedModel || "OpenClaw agent default";
      const defaultLabel = `Use default — ${inherited}${feature.direct ? " (existing direct route)" : ""}`;
      const options = `<option value=""${current ? "" : " selected"}>${escapeHtml(defaultLabel)}</option>`
        ...
        + models.map((model) => `<option value="${escapeHtml(model.id)}"${model.id === current ? " selected" : ""}>${safeText(model.label)}${model.label !== model.id ? ` — ${safeText(model.id)}` : ""}${model.requiresPermission ? " (grants PLUR1BUS access)" : ""}</option>`).join("");
      const fields = `<label for="${escapeHtml(id)}">${safeText(feature.label)}</label>`
        + `<select id="${escapeHtml(id)}" name="model"${write.mode === "all" ? "" : " disabled"}>${options}</select>`;
      if (write.mode !== "all") return `<div class="feature-model-row">${fields}</div>`;
      return `${formStart(write, "feature.model")}<div class="feature-model-row">`
        + `<input type="hidden" name="agent" value="${escapeHtml(agent.id)}">`
        + `<input type="hidden" name="feature" value="${escapeHtml(feature.id)}">`
        + `${fields}<button type="submit">Save model</button></div></form>`;
    }).join("");
    return `<fieldset class="feature-models"><legend>${safeText(agent.label)}${agent.label !== agent.id ? ` (${safeText(agent.id)})` : ""}</legend>${rows}</fieldset>`;
  }).join("");
}

function renderFeatureControls(values, modelControls, captureChunking, write = writeContext(null)) {
  ...
  const rendered = cards.map((entry) => {
    ...
    return `<li class="card feature-card"><div class="card-heading"><h3>${safeText(entry.label)}</h3>${renderBadge(state)}</div>${details}${renderCaptureChunkingSwitch(entry.id, captureChunking, write)}${renderFeatureModelControls(entry.id, modelControls, write)}</li>`;
  }).join("");
...
<p>LLM tasks offer the models configured for each OpenClaw agent. Saving a model uses OpenClaw's model route and, when indicated, grants PLUR1BUS access to that specific model. <strong>Use default</strong> restores the existing feature settings; model permissions remain available for other features. Existing settings remain unchanged until you save a choice. ${write.mode === "all" ? "Model changes use OpenClaw's normal configuration reload." : 'To save models here, set <code>controlUi.writeActions</code> to <code>all</code> in OpenClaw config.'}</p>
...
.feature-model-row { display: grid; gap: 6px; margin: 6px 0 10px; }
.feature-model-row label { font-size: 12px; }
.feature-model-row select { width: 100%; min-width: 0; max-width: 100%; min-height: 36px; padding: 6px; font: inherit; color: var(--text-strong); background: var(--panel-strong); border: 1px solid var(--border-strong); border-radius: var(--radius-sm); }
.feature-model-row button { justify-self: end; }
...
${renderFeatureControls(status.featureCards, status.featureModels, status.captureChunking, write)}
