...
import { findFeatureCronDelivery } from "./feature-cron-plan.js";
  ...
  return async ({ params, respond, context }) => {
    try {
      ...
      const reply = validateFeatureCronReplyPayload(await runFeatureCommand({
        ...
        // 7.16.10: Das Zustellziel dieses Crons, gelesen aus dem eigenen Job
        // über den Cron-Dienst des Gateways; der Critical Push sendet damit
        // seine Knopf-Nachrichten an denselben Chat wie bisher den Text.
        resolveCronDelivery: async () => {
          if (typeof context?.cron?.list !== "function") return null;
          try {
            return findFeatureCronDelivery(await context.cron.list({ includeDisabled: false }), agentId, feature);
          } catch (error) {
            logger?.warn?.(`memory-lancedb-namespaced: feature cron delivery lookup failed for agent=${agentId}: ${errorMessage(error)}`);
            return null;
          }
        },
      }));
...
export async function loadOpenClawPluginSdkRuntime(subpath, options = {}) {
  ...
  // memory-host-core: the facade that enumerates public memory artifacts, so a
  // companion plugin can read MEMORY.md, the daily notes and the event log
  // without reaching into our private layout.
  // model-session-runtime: the host's own session model-override helper, so the
  // chat-model switch releases session pins exactly as the host would.
  if (!new Set(["gateway-runtime", "secret-input-runtime", "memory-host-events", "memory-host-core", "model-session-runtime"]).has(subpath)) {
...
/**
 * Lists the public memory artifacts of every configured workspace.
 *
 * The host facade owns the definition of "public" — MEMORY.md, the markdown
 * notes under memory/, dream reports and the materialized event log. Filtering
 * here would make our answer disagree with the host's own view, so this only
 * carries the call across. `importHostCore` exists for tests; leaving it
 * undefined takes the ordinary SDK path.
 */
export async function listPluginPublicArtifacts(params, { importHostCore } = {}) {
  const host = await (importHostCore ?? (() => loadOpenClawPluginSdkRuntime("memory-host-core")))();
  if (typeof host?.listMemoryHostPublicArtifacts !== "function") {
    throw new Error("memory-host-core facade does not provide listMemoryHostPublicArtifacts");
  }
  return await host.listMemoryHostPublicArtifacts(params);
}
...
const PLUGIN_COMMAND_LOCALE_RE = /^[a-z]{2}(?:-[A-Za-z]{2})?$/;
...
export function validatePluginCommandRequest(params) {
  ...
  const keys = Object.keys(params).sort();
  // 7.12.24: `locale` ist optional (Operator-Pfad ohne Chatverlauf); sonst
  // bleibt der Feldsatz absichtlich streng.
  const expected = keys.includes("locale")
    ? ["agentId", "command", "locale", "sessionKey"]
    : ["agentId", "command", "sessionKey"];
  if (keys.length !== expected.length || keys.some((key, index) => key !== expected[index])) {
    throw new Error("invalid PLUR1BUS command request fields");
  }
  let locale;
  if (keys.includes("locale")) {
    locale = typeof params.locale === "string" ? params.locale.trim() : "";
    if (!PLUGIN_COMMAND_LOCALE_RE.test(locale)) throw new Error("invalid PLUR1BUS command locale");
  }
  const agentId = safeAgentId(params.agentId);
  ...
  return locale ? { agentId, sessionKey, command, locale } : { agentId, sessionKey, command };
}
...
export async function executePluginCommandCli({
  ...
  const request = validatePluginCommandRequest({ agentId, sessionKey, command, ...(locale ? { locale } : {}) });
  if (typeof callGateway !== "function") throw new Error("Gateway RPC capability unavailable");
  const response = await callGateway(
    ...
    api.registerCli(
      ({ program }) => {
        program
          ...
          .option("--locale <code>", "reply language, e.g. de or en (default: plugin config `language`)")
