...
import { resolveOpenClawDistDir } from "../../patches/apply-cron-plugin-direct-dispatch.mjs";
...
function findOpenClawPackageManifest(entryPath = process.argv[1]) {
  ...
  const distDir = resolveOpenClawDistDir({ entryPath });
  const manifestPath = join(dirname(realpathSync(distDir)), "package.json");
  const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
  if (manifest?.name !== "openclaw") throw new Error("could not resolve the active OpenClaw package");
  return manifestPath;
}
...
export async function loadOpenClawGatewayRuntime(options = {}) {
  const manifestPath = findOpenClawPackageManifest(options.entryPath);
