...
const ENV_OR_STORE_SECRET_ID_RE = /^[A-Z][A-Z0-9_]{0,127}$/;
const EXEC_SECRET_ID_RE = /^[A-Za-z0-9][A-Za-z0-9._:/#-]{0,255}$/;
...
function resolveSchema(schema) {
  ...
  if (schema.$ref !== "#/$defs/secretInput") throw new Error(`unsupported plugin config schema reference: ${schema.$ref}`);
  const target = configSchema?.$defs?.secretInput;
  if (!target) throw new Error("plugin config SecretInput schema definition is missing");
...
function isValidFileSecretId(value) {
  if (value === "value") return true;
  if (!value.startsWith("/")) return false;
  return value.slice(1).split("/").every((segment) => /^(?:[^~]|~0|~1)*$/.test(segment));
}

function validateSecretInputs(config, path) {
  for (const parts of SECRET_INPUT_PATHS) {
    const value = getPath(config, parts);
    if (value === MISSING || typeof value === "string") continue;
    const configPath = `${path}.${parts.join(".")}`;
    if (!isPlainObject(value)) errorAt(configPath, "expected a string or SecretRef");
    if (!SECRET_PROVIDER_RE.test(value.provider)) errorAt(configPath, "invalid SecretRef provider alias");
    if ((value.source === "env" || value.source === "store") && !ENV_OR_STORE_SECRET_ID_RE.test(value.id)) {
      errorAt(configPath, `invalid ${value.source} SecretRef id`);
    }
    if (value.source === "file" && !isValidFileSecretId(value.id)) {
      errorAt(configPath, "invalid file SecretRef id");
    }
    if (value.source === "exec") {
      if (!EXEC_SECRET_ID_RE.test(value.id) || value.id.split("/").some((segment) => segment === "." || segment === "..")) {
        errorAt(configPath, "invalid exec SecretRef id");
      }
    }
  }
}
...
export function validatePluginConfig(rawConfig, { path = PLUGIN_CONFIG_PATH } = {}) {
  ...
  validateSecretInputs(normalized, path);
