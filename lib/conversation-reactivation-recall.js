...
export async function selectReactivationMemories({
  ...
  for (const community of lens.communities) {
    ...
    for (const id of candidateIds) {
      ...
      const memory = await resolveMemoryById(id, lens, getMemoryById);
      if (!memory) continue;
      if (!hasDisplayText(memory)) continue;
      const text = memory.display || memory.text || memory.summary || "";
...
export async function runConversationReactivationRecall({
  ...
  try {
    ...
    const safeGraphEdges = Array.isArray(graphEdges)
      ? graphEdges
      : (neoStore && typeof neoStore.readGraphEdges === "function"
          ? neoStore.readGraphEdges(1000)
          : []);
    const patterns = neoStore && typeof neoStore.readPatterns === "function"
      ? neoStore.readPatterns(200)
      : [];
    const episodes = neoStore && typeof neoStore.readEpisodes === "function"
      ? neoStore.readEpisodes(200)
      : [];
