...
/** Returns whether a projected memory is active and has a strictly live expiry. */
export function isRecallEntryLive(entry, now = Date.now()) {
...
function filterRecallCandidatesByLifecycle(candidates, now, trace) {
  return candidates.filter((candidate) => {
    const live = isRecallEntryLive(candidate.entry, now);
    if (!live && trace) {
...
export function mergeNamespaceRecallResults(namespaceResults, options = {}) {
  ...
  for (const candidate of memoryCandidates) {
    ...
    const existing = originWinners.get(key);
    if (!existing) {
    ...
    const replace = candidatePriority < existingPriority
      || (candidatePriority === existingPriority && compareRecallCandidate(candidate, existing) < 0);
    const rejected = replace ? existing : candidate;
    if (replace) originWinners.set(key, candidate);
  ...
  originUnique.push(...originWinners.values());
 ...
 * @param {{queryVector?: Array, embeddings?: Object, graphConfig?: Object, decisionTrace?: Object, strictReadErrors?: boolean, aclCtx?: Object|null, embeddingContext?: Object}} [opts]
...
export async function hydrateGraphResults(dbTable, results, logger, opts = {}) {
  ...
  if (graphOnly.length === 0) {
    const active = filterRecallCandidatesByLifecycle(results, now, decisionTrace);
  ...
  for (const r of results) {
    ...
    if (isGraphOnly) {
      ...
      if (!isRecallEntryLive(entry, now)) {
        if (decisionTrace) {
          addTraceDecision(decisionTrace, {
            memoryId: row.id,
            action: "rejection",
      ...
      if (isRecallEntryLive(r.entry, now)) {
...
export async function runRecallPipeline({
  ...
  const fetchLimit = deferFinalCap
    ? hardCandidateLimit
    : Math.min(hardCandidateLimit, reranker ? Math.max(rerankCandidates, topN * 3) : topN);
  let rows = [];
  try {
  ...
  results = filterRecallCandidatesByLifecycle(results, now, trace);
  ...
  if (queryRefinerEnabled && shouldRefineQuery(results, recallMinScore)) {
    ...
    try {
      ...
      const liveRefinedResults = filterRecallCandidatesByLifecycle(refinedResults, now, trace);
  ...
  boosted = filterRecallCandidatesByLifecycle(boosted, now, trace);
  ...
  if (associativeEnabled && graphEdges.length > 0 && boosted.length > 0) {
    ...
    if (authorizedGraphEdges) {
      try {
        const { adjacency } = readGraph(authorizedGraphEdges);
