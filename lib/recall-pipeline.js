...
function filterRecallCandidatesByAcl(candidates, aclCtx, trace, logger, stage) {
  const requestAclCtx = aclCtx ?? DENY_ALL_RECALL_ACL_CONTEXT;
  const filtered = candidates.filter((r) => {
    const acl = checkAccess(requestAclCtx, r.entry);
    if (!acl.allowed) {
...
export async function hydrateGraphResults(dbTable, results, logger, opts = {}) {
  ...
  if (graphOnly.length === 0) {
    const active = results.filter(r => !r.entry?.status || r.entry.status === "active");
    ...
    if (entry.status !== "active" && entry.status !== "") {
      if (decisionTrace) {
        addTraceDecision(decisionTrace, {
          ...
          action: "rejection",
          stage: "graph-endpoint-acl",
          reason: `inactive status: ${entry.status}`,
        });
      }
      continue;
    }
