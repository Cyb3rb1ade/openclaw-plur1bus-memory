...
import { applyTemporalFilterSafe, temporalRangeFromAnchor } from "./temporal-filter.js";
import { checkAccess } from "./acl-middleware.js";
import { isAbortError, raceAbort, throwIfAborted } from "./abort.js";
import { rerankSignal } from "./providers/rerank-signal.js";
 ...
 */
/**
 * LanceDB liefert Int64-Spalten (expiresAt, validFrom, validUntil, updatedAt,
 * remindAt, …) als BigInt. Die Lebenszeit- und Zeitfenster-Pruefungen
 * vergleichen mit JavaScript-Zahlen; ein BigInt `0n` ist weder `0` noch vom
 * Typ "number" und galt deshalb als abgelaufen — jede Erinnerung fiel im
 * Lifecycle-Filter durch. Hier wird einmal zentral auf eine sichere Zahl
 * gebracht; nicht darstellbare oder unbrauchbare Werte werden zum Fallback.
 * @param {unknown} value
 * @param {number|null|undefined} fallback
 * @returns {number|null|undefined}
 */
export function toEpochNumber(value, fallback = 0) {
  if (typeof value === "bigint") {
    if (value < BigInt(Number.MIN_SAFE_INTEGER) || value > BigInt(Number.MAX_SAFE_INTEGER)) return fallback;
    return Number(value);
  }
  if (typeof value === "number") return Number.isFinite(value) ? value : fallback;
  if (value == null) return fallback;
  // Fremde Typen (z. B. ein String "20000") bleiben unveraendert, damit die
  // nachgelagerten Pruefungen sie weiterhin als fehlerhaft verwerfen.
  return value;
}
...
export function projectRecallEntry(row) {
  return {
    ...
    importance: toEpochNumber(row.importance, 0.5),
    createdAt: toEpochNumber(row.createdAt, row.createdAt),
    ...
    retrievalCount: toEpochNumber(row.retrievalCount, 0),
    lastRetrievedAt: toEpochNumber(row.lastRetrievedAt, 0),
    memoryStrength: row.memoryStrength ?? 1,
    halfLifeDays: row.halfLifeDays ?? 30,
    lastStrengthenedAt: toEpochNumber(row.lastStrengthenedAt, 0),
    lastDynamicsAt: toEpochNumber(row.lastDynamicsAt, 0),
    memoryClass: row.memoryClass || "standard",
    neverForget: toEpochNumber(row.neverForget, 0),
    coreMemoryScore: row.coreMemoryScore ?? 0,
    coreMemoryReason: row.coreMemoryReason || "",
    versionNumber: toEpochNumber(row.versionNumber, 1),
    ...
    versionCreatedAt: toEpochNumber(row.versionCreatedAt, 0),
    updatedAt: toEpochNumber(row.updatedAt, 0),
    expiresAt: toEpochNumber(row.expiresAt, row.expiresAt),
    memoryKind: row.memoryKind ?? "memory",
    type: row.type || "",
    confirmed: toEpochNumber(row.confirmed, 0),
    confirmationStatus: row.confirmationStatus || "",
    reminderStatus: row.reminderStatus || "",
    reminderKey: row.reminderKey || "",
    remindAt: toEpochNumber(row.remindAt, 0),
    reminderTimezone: row.reminderTimezone || "",
    reminderRecurrence: row.reminderRecurrence || "",
    epistemicStatus: row.epistemicStatus || "",
    validFrom: toEpochNumber(row.validFrom, 0),
    validUntil: toEpochNumber(row.validUntil, 0),
  };
}
...
export function isRecallEntryLive(entry, now = Date.now(), validAt = null) {
  ...
  const expiry = toEpochNumber(entry?.expiresAt, entry?.expiresAt);
...
async function runVectorSearchWithValidTimeFallback({
  ...
  try {
    ...
    return await withTimeout(
      raceAbort(query.limit(fetchLimit).toArray(), signal, "recall aborted"),
    ...
    return withTimeout(
      raceAbort(dbTable.vectorSearch(vector).limit(fetchLimit).toArray(), signal, "recall aborted"),
      LANCEDB_READ_TIMEOUT_MS,
 ...
 */
/**
 * Wie viele Teilstuecke derselben Nachricht hoechstens in eine Trefferliste
 * duerfen.
 *
 * Sobald eine Nachricht in mehrere Vektoren aufgeteilt gespeichert wird
 * (lib/memory-chunking.js), passen auf eine Frage oft gleich mehrere ihrer
 * Teile. Ohne Deckel belegen sie drei der zwoelf Plaetze mit
 * zusammengehoerigem Inhalt und verdraengen genau die Vielfalt, die die
 * Aufteilung gewinnen soll. Zwei Plaetze lassen einer wirklich mehrteiligen
 * Antwort Raum, ohne die Liste zu fluten.
 */
export const DEFAULT_MAX_PER_GROUP = 2;

/**
 * Woran haengt eine Zeile, wenn sie Teil einer groesseren Nachricht ist?
 * `chunkGroupId` setzt die Aufteilung; `sourceTurnId` ist der bereits
 * vorhandene Herkunftsschluessel (bei 59 % der Produktionszeilen gefuellt und
 * bislang nirgends gelesen). Ohne beides gibt es keine Gruppe.
 */
export function chunkGroupKey(entry) {
  const key = entry?.chunkGroupId || entry?.sourceTurnId || "";
  return String(key).trim() || null;
}

export function dedupResults(results, maxOut, jaccardThreshold = 0.78, { maxPerGroup = DEFAULT_MAX_PER_GROUP } = {}) {
  if (maxOut <= 0) return [];
  const out = [];
  const jeGruppe = new Map();
  for (const r of results) {
    // Der Deckel steht VOR der Textpruefung: eine dritte Passage derselben
    // Nachricht faellt raus, auch wenn sie inhaltlich etwas anderes sagt.
    const gruppe = chunkGroupKey(r.entry);
    if (gruppe && (jeGruppe.get(gruppe) || 0) >= maxPerGroup) continue;
    ...
    if (!isDup) {
      out.push(r);
      if (gruppe) jeGruppe.set(gruppe, (jeGruppe.get(gruppe) || 0) + 1);
    }
 ...
 * @param {number} opts.importanceBoost — historisch; seit 19.09.2026 ohne Wirkung im Ranking (Default 0). Bedeutung wirkt stattdessen über die Kodierungs-Halbwertszeit auf `memoryStrength`.
...
function combineStrictReadFailures(failures) {
  // Fix round 2: a raceAbort rejection (its own fresh error per call — see
  // lib/abort.js) carries `.settlement` exactly like an ETIMEOUT error and
  // must combine the same way when several concurrent strict reads share one
  // caller signal.
  const timeoutFailures = failures.filter((error) => (
    (error?.code === "ETIMEOUT" || isAbortError(error))
    && error.settlement
 ...
 * @param {AbortSignal|null} [options.signal=null] Caller cancellation (PR-05).
...
export async function runRecallPipeline({
  ...
  budget = topN,
  recallMinScore = 0.15,
  // Seit 19.09.2026 ohne Wirkung: Bedeutung wirkt über die Stärke, die sie
  ...
  const embeddingContext = Object.freeze(signal ? { agentId: requestAgentId, signal } : { agentId: requestAgentId });
  ...
  let canonicalHits = [];
  // Ob der Scoring-Block unten schon gelaufen ist. Der Notausgang greift an
  // neun Stellen, vier davon VOR dem Scoring — dort muss er die Staerke
  // selbst verrechnen, danach waere es eine Doppelanwendung.
  ...
  function softBudgetPartial(currentList) {
    ...
    // Zerfall und Gebrauch gelten auch unter Zeitdruck. Ohne das rangierte
    // der Notausgang allein nach Aehnlichkeit, und eine seit Monaten
    // unbenutzte Zeile stand gleichauf mit einer taeglich gebrauchten —
    // zwei Wege mit zwei Regeln. Vor dem Zuschneiden, damit die Auswahl
    // selbst schon die richtige ist und nicht nur ihre Reihenfolge.
    if (!scoringApplied && partial.length > 0) {
      partial = partial
        .map((r) => ({ ...r, score: r.score + ((r.entry?.memoryStrength ?? 1.0) - 1.0) }))
    ...
    if (temporal.type === "anchor") {
      try {
        ...
        if (resolved) {
          ...
          const safe = applyTemporalFilterSafe(results, resolved);
          results = safe.results;
          if (safe.dropped) {
            logger?.info?.(`recall-pipeline: temporal anchor "${temporal.referenceQuery}" would remove all ${before} results — kept unfiltered`);
          } else {
      ...
      const beforeIds = new Set(results.map(r => r.entry.id));
      const before = results.length;
      const safe = applyTemporalFilterSafe(results, temporal);
      results = safe.results;
      if (safe.dropped) {
        // 7.12.41: Bernd 10.09.2026 21:42 — „bis heute" → Fenster „heute" → 39 → 0.
        logger?.info?.(`recall-pipeline: temporal filter would remove all ${before} results — kept unfiltered`);
      } else if (before !== results.length) {
  ...
  // 4. Single-Pass-Scoring: Emotional-, Strength- und Epistemic-Boost werden
  // in EINEM map + EINEM sort kombiniert statt in separaten map+sort-
  // Durchläufen (das Endergebnis ist identisch, da nur der finale Sort die
  // Reihenfolge bestimmt).
  phaseTimer?.start("scoring");
  let boosted = results;
  if (results.length > 0) {
    // 4a. Importance-Boost entfernt (19.09.2026): Bedeutung fließt seither
    // über die Kodierungs-Halbwertszeit in memoryStrength ein statt als
    // zweiter additiver Term hier. `applyImportance` bleibt fest auf false,
    // damit die Trace-Beschriftung unten ("boostParts") ohne weitere
    // Änderung korrekt bleibt.
  ...
  if (reranker && boosted.length > 1) {
    ...
    try {
      ...
      let reranked;
      try {
        // "One timeout owner" means one timer, not no bound: a provider that
        // ignores its own signal is still cut off here, on the SAME
        // `rerankAbort` signal the provider was given — no second timer.
        reranked = await raceAbort(
          reranker.rerank(query, docs, rerankLimit, rerankAbort ? { signal: rerankAbort } : undefined),
          rerankAbort,
          "reranker timeout",
        );
      } catch (rerankError) {
  ...
  // 5b. Budget-Kappung — NACH dem Reranking.
  //
  // Bis zum 20.09.2026 stand dieser Block VOR dem Rerank (Zeile 2012). Der
  // Reranker durfte damit nur umsortieren, was die Kappung uebrig liess, und
  // konnte eine Zeile von Platz 16 nicht mehr hereinholen — obwohl genau das
  // seine Aufgabe ist. Gemessen an 80 Benchmark-Fragen mit verfehltem Beleg:
  // Kappung nach dem Rerank findet 65 davon (81 %), Kappung davor 11 (14 %).
  //
  // Produktiv lief bereits die bessere Variante, weil index.js:882
  // `deferFinalCap: true` setzt und selbst kappt; jeder Direktaufruf bekam
  // stillschweigend die schwaechere. Die Kappung zu verschieben statt nur die
  // Vorgabe umzudrehen haelt den Vertrag der Funktion intakt: sie liefert
  // weiterhin hoechstens topN, und kein Aufrufer muss etwas aendern.
  phaseTimer?.start("budget");
  if (!deferFinalCap && budget > 0 && ordered.length > 0) {
    const beforeBudget = ordered.map(r => r.entry.id);
    ...
    const budgetResult = applyRecallBudget(ordered, { canonical: [], budget });
    ordered = budgetResult.selected;
    if (trace) {
      const selectedIds = new Set(ordered.map(r => r.entry.id));
