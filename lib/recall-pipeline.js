...
import { safeWarn, trySafeWarn } from "./safe-logging.js";
 ...
 * @returns {{ok: true, emitted: boolean}|{ok: false, error: unknown, loggingError: unknown|null}} Emission outcome.
 */
export function emitRetrievalLedger({ retrievalLogger = null, logger = null, entry } = {}) {
  ...
  try {
    retrievalLogger(entry);
    return { ok: true, emitted: true };
  } catch (error) {
    const warning = trySafeWarn(logger, "recall-pipeline.retrievalLogger", error);
    return {
      ok: false,
      error,
 ...
 * @param {{queryVector?: Array, embeddings?: Object, graphConfig?: Object, decisionTrace?: Object, strictReadErrors?: boolean}} [opts]
...
export async function hydrateGraphResults(dbTable, results, logger, opts = {}) {
  const { queryVector, embeddings, graphConfig = {}, decisionTrace, strictReadErrors = false } = opts;
  ...
  const hydratedMap = await getByIds(dbTable, ids, { strictReadErrors });
 ...
 */
async function getByIds(dbTable, ids, { strictReadErrors = false } = {}) {
  ...
  for (let i = 0; i < cleanIds.length; i += BATCH) {
    ...
    const lookups = batch.map(async (id) => {
      try {
        const rows = await withTimeout(
          dbTable.query()
    ...
    if (strictReadErrors) {
      ...
      if (failures.length > 0) throw combineStrictReadFailures(failures);
    } else {
