 ...
 */
export function applyFlashbulbEncoding(row = {}, now = Date.now(), threshold = 0.70, baseHalfLifeDays = 0, floorDays = FLASHBULB_HALF_LIFE_DAYS) {
...
export function applyDynamicsDefaults(entry = {}, now = Date.now(), halfLifeOverrides = {}, opts = {}) {
  ...
  if (isNew) {
    ...
    const flashbulb = core ? null : applyFlashbulbEncoding(out, now, 0.70, baseHalfLifeDays, flashbulbFloorDays);
    if (core) {
