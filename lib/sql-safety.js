...
const DEFAULT_PATH_OPERATIONS = Object.freeze({ relative, isAbsolute, sep });

/**
 * Return whether an absolute target is contained by an absolute base using the supplied platform path semantics.
 * @param {string} baseDir Absolute containing directory.
 * @param {string} targetPath Absolute candidate path.
 * @param {{relative: Function, isAbsolute: Function, sep: string}} [pathOperations]
 * @returns {boolean}
 */
export function isPathInside(baseDir, targetPath, pathOperations = DEFAULT_PATH_OPERATIONS) {
  const relativePath = pathOperations.relative(baseDir, targetPath);
  return relativePath === ""
    || (relativePath !== ".."
...
/**
 * Genau die IDs, die `safeUuidList()` in seinen IN-Clause aufnimmt — valide
 * UUIDs, auf `maxItems` gekappt, in Eingabereihenfolge. Wer die Antwortzeilen
 * einer solchen Abfrage gegen die Anfrage abgleicht, braucht diese Liste:
 * IDs jenseits der Kappungsgrenze wurden nie abgefragt und dürfen nicht als
 * „nicht gefunden" gelten.
 * @returns {string[]} die tatsächlich abgefragten IDs.
 */
export function selectSafeUuids(ids, maxItems = 100) {
  if (!Array.isArray(ids)) throw new Error("selectSafeUuids: not an array");
  return ids
    .filter(id => typeof id === "string" && id.length === 36 && UUID_RE.test(id))
    .slice(0, maxItems);
}
...
export function safeUuidList(ids, maxItems = 100) {
  ...
  const valid = selectSafeUuids(ids, maxItems);
  if (valid.length === 0) return null;
  return valid.map(id => `'${id}'`).join(",");
}

/**
 * Prüft, ob ein Name als Agent-Kennung durchgeht — ohne zu werfen.
 * Wer wissen will, welche Verzeichnisse als Agent-Speicher gelten (der
 * GC-Job tut das), fragt hier, statt safeAgentId in try/catch zu wickeln.
 * @param {unknown} id
 * @returns {boolean}
 */
export function isSafeAgentId(id) {
  return typeof id === "string" && SAFE_AGENT_RE.test(id);
}
 ...
 *
 * `fakt` ist der vom Klassifizierer-Prompt vorgeschriebene Default
 * (lib/critical-push-classifier.js: „Wenn nichts passt: 'fakt'") und wird in
 * lib/jobs/critical-classifier.js ausdrücklich vor der Deklassierung zu `note`
 * geschützt. Er fehlte hier, wodurch updateCardType jede nicht-kritische
 * Klassifikation abwies.
 */
const ALLOWED_TYPES = new Set([
  "", "person", "beziehung", "geburtstag", "geld_konto", "gesundheit",
  "zugang_passwort", "critical", "insight", "task", "project", "note",
  "event", "preference", "goal", "habit", "learning", "decision", "fakt",
]);
...
/**
 * Audit-Log für destruktive DB-Operationen. Non-blocking in dem Sinne, dass es
 * selbst nie wirft — signalisiert aber den Erfolg/Misserfolg über den Rückgabewert,
 * damit sicherheitsrelevante Pfade einen Audit-Schreibfehler nicht still als
 * Erfolg behandeln.
 *
 * @returns {boolean} true bei erfolgreichem Schreiben, false bei Fehler.
 */
export function appendDestructiveOpLog(workspaceDir, entry) {
  ...
  try {
    ...
    // Sichtbar, aber kein silent success — der Rückgabewert meldet den Fehlschlag.
