...
import { sanitizeMemoryContextAttribute, sanitizeMemoryTextForPrompt } from "./memory-context-sanitize.js";
...
function weeksSince(dateValue) {
  ...
  return Math.max(0, (Date.now() - createdTime) / MS_PER_WEEK);
}
...
export function computePatternScore(pattern, candidateIds, weeksSince) {
  ...
  const boundedWeeksSince = Math.max(0, Number.isFinite(Number(weeksSince)) ? Number(weeksSince) : 0);
  const recencyFactor = Math.exp(-boundedWeeksSince / 12);
...
export function formatPatternBlock(pattern, triggerIds, score) {
  ...
  // Attributes
  const triggerIdsStr = Array.isArray(triggerIds)
    ? triggerIds.map((tid) => sanitizeMemoryContextAttribute(String(tid), "id")).join(",")
    : "";
