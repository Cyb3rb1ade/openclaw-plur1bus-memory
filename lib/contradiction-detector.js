...
export class ContradictionDetector {
   ...
   * @returns {Promise<Array<object>>} Detected contradiction records.
   */
  async findContradictionsForNewOverlay(newOverlay, existingOverlays) {
  ...
  async loadAll() {
    ...
    for (const line of content.split("\n").filter(Boolean)) {
      try {
        const rec = JSON.parse(line);
        if (rec.recordType === "contradiction") {
          results.push(rec);
        }
      } catch (err) {
        safeWarn(this.logger, "ContradictionDetector:loadAll", err, { lineIndex: results.length });
      }
    }
    return results;
  }
