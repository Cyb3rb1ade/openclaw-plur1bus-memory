...
export class ContradictionDetector {
   ...
   * @param {{maxPairs?: number, signal?: AbortSignal}} [opts]
   * @returns {Promise<Array<object>>} Detected contradiction records.
   */
  async findContradictions(overlays, opts = {}) {
    throwIfAborted(opts.signal, "contradiction detection aborted");
    ...
    for (let i = 0; i < meaningOverlays.length; i++) {
      for (let j = i + 1; j < meaningOverlays.length; j++) {
        ...
        pairsChecked++;
        const conflict = await this._askLLM(a, b, opts.signal);
        throwIfAborted(opts.signal, "contradiction detection aborted");
        if (conflict) {
          contradictions.push({
            id: randomUUID(),
   ...
   */
  async detectContradiction(a, b, opts = {}) {
    throwIfAborted(opts.signal, "contradiction detection aborted");
    if (!a || !b) return false;
    ...
    return this._askLLM(a, b, opts.signal);
  }
  ...
  async findContradictionsForNewOverlay(newOverlay, existingOverlays, opts = {}) {
    ...
    for (const other of existingOverlays) {
      ...
      pairsChecked++;
      const conflict = await this.detectContradiction(newOverlay, other, opts);
      throwIfAborted(opts.signal, "contradiction detection aborted");
      if (conflict) {
        contradictions.push({
          id: randomUUID(),
   ...
   */
  async persistContradiction(record, opts = {}) {
    throwIfAborted(opts.signal, "contradiction persistence aborted");
   ...
   */
  async detectMemoryTextContradiction(a, b, opts = {}) {
    throwIfAborted(opts.signal, "memory contradiction detection aborted");
    if (!a || !b) return false;
  ...
  async findMemoryTextContradictions(memories, opts = {}) {
    ...
    for (let i = 0; i < memories.length; i++) {
      for (let j = i + 1; j < memories.length; j++) {
        ...
        pairsChecked++;
        const conflict = await this.detectMemoryTextContradiction(a, b, opts || {});
        throwIfAborted(opts?.signal, "memory contradiction detection aborted");
        if (conflict) {
          contradictions.push({
            id: randomUUID(),
   ...
   * @private
   */
  async _askLLM(a, b, signal = null) {
    throwIfAborted(signal, "contradiction detection aborted");
    const prompt = [
      {
        role: "system",
...
Do these contradict each other?`,
    ...
    try {
      ...
      throwIfAborted(signal, "contradiction detection aborted");
      safeWarnLlmFailure(this.logger, "ContradictionDetector:_askLLM", err, {
        overlayA: a.id,
        overlayB: b.id,
        targetMemoryId: a.targetMemoryId,
      });
   ...
   * not interleave lines. Non-abort errors are logged and swallowed; aborts
   * are returned to the caller while the internal queue tail remains usable.
   *
  ...
  _append(record, opts = {}) {
    ...
    const writeAttempt = this._writeQueue
      .then(() => {
        throwIfAborted(opts?.signal, "contradiction persistence aborted");
        return appendFile(this.filePath, line);
      })
      .catch((err) => {
        if (opts?.signal?.aborted) throw err;
        safeWarn(this.logger, "ContradictionDetector:_append", err, { recordId: record.id });
      });
    this._writeQueue = writeAttempt.catch((err) => {
      safeDebug(this.logger, "ContradictionDetector:_appendQueueTail", err, { recordId: record.id });
    });
    return writeAttempt;
  }
}
