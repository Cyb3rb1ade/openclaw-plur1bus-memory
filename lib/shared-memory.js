...
export async function ensureSharedMemoryColumns(db) {
  ...
  const task = (async () => {
    await db.init();
    let schema = await db.table.schema();
    ...
    if (missing.length) {
      try { await db.table.addColumns(missing.map((name) => ({ name, type: textField.type, valueSql: defaults[name] }))); }
      catch (error) {
        schema = await db.table.schema(); fields = fieldsOf(schema);
 ...
 */
export async function storeSharedMemory(targetDb, source, ctx, { targetScope, vector, action = "explicit_share", allowSensitiveShare = false, logger = null } = {}) {
  ...
  const sourceAgentId = String(source.agentId || source.storedBy || "");
  if (!sourceAgentId || !source.id) throw new Error("shared source provenance is incomplete");
  const idempotencyKey = hash([action, targetScope, principal, sourceAgentId, source.id, hash(source.text || source.summary || "")].join("\0"));
  return runKeyed(idempotencyKey, async () => {
    const rows = await targetDb.table.query().where(`shareIdempotencyKey = ${sqlString(idempotencyKey)} AND status = 'active'`).limit(2).toArray();
    if (rows.length > 1) throw new Error("shared idempotency conflict");
    if (rows.length === 1) return { ok: true, id: rows[0].id };
    const now = Date.now();
    const copy = {
      ...source, id: randomUUID(), vector: [...vector], scope: targetScope, status: "active", storedBy: sourceAgentId, agentId: sourceAgentId,
      ...
      sourceMemoryId: source.id, sourceAgentId, shareIdempotencyKey: idempotencyKey,
      shareProvenance: JSON.stringify({ schemaVersion: 1, action, targetScope, targetPrincipalHash: hash(principal), sourceMemoryId: source.id, sourceAgentId, sharedAt: now }),
      supersededBy: "", previousVersion: "",
    };
    await targetDb.store(copy);
    const readbackRows = await targetDb.table.query().where(`id = ${sqlString(copy.id)}`).limit(2).toArray();
