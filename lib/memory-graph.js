...
/**
 * 7.12.24: Entfernt Kanten, deren Endpunkte es nicht mehr gibt, plus die
 * schwachen Altkanten aus shouldPrune. Datensaetze werden woertlich behalten
 * (Ownership-Stempel bleiben). Endpunkte werden nur beurteilt, wenn sie zur
 * Agententabelle gehoeren koennen: ohne Ownership (Altbestand) oder mit Scope
 * agent-private; Kanten in andere Scopes bleiben unangetastet. Episode-Anker
 * (`episode-<id>`) werden gegen episodeIds aufgeloest, wenn uebergeben.
 *
 * @param {Array<object>} edgeRecords
 * @param {{liveMemoryIds: Set<string>, episodeIds?: Set<string>|null, now?: number}} options
 * @returns {{kept: Array<object>, before: number, after: number, removedMissing: number, removedWeak: number, skippedForeign: number}}
 */
export function pruneGraphEdges(edgeRecords, { liveMemoryIds, episodeIds = null, now = Date.now() } = {}) {
  const live = liveMemoryIds instanceof Set ? liveMemoryIds : new Set(liveMemoryIds || []);
  const episodes = episodeIds instanceof Set ? episodeIds : episodeIds ? new Set(episodeIds) : null;
  const judgeable = (ownership) => !ownership || !ownership.scope || ownership.scope === "agent-private";
  const resolvable = (id) => {
    const key = String(id || "");
    if (!key) return false;
    if (key.startsWith("episode-")) return episodes ? episodes.has(key.slice("episode-".length)) : true;
    return live.has(key);
  };
  const kept = [];
  ...
  const records = Array.isArray(edgeRecords) ? edgeRecords : [];
  for (const edge of records) {
    if (!edge || !edge.source || !edge.target) { removedMissing++; continue; }
    if (!judgeable(edge.sourceOwnership) || !judgeable(edge.targetOwnership)) {
      skippedForeign++;
      kept.push(edge);
      continue;
    }
    if (!resolvable(edge.source) || !resolvable(edge.target)) { removedMissing++; continue; }
    if (shouldPruneAt(edge, now)) { removedWeak++; continue; }
    kept.push(edge);
  }
  return { kept, before: records.length, after: kept.length, removedMissing, removedWeak, skippedForeign };
}

function shouldPruneAt(edge, now) {
  const ageDays = (now - new Date(edge.createdAt).getTime()) / (1000 * 60 * 60 * 24);
  const lastReinforcedDays = edge.lastReinforcedAt
    ? (now - new Date(edge.lastReinforcedAt).getTime()) / (1000 * 60 * 60 * 24)
    : ageDays;
  return (
    edge.strength < 0.2 &&
    (edge.observations || 1) <= 1 &&
