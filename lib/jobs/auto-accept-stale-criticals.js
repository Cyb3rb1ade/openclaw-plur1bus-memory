...
/**
 * @deprecated Seit 7.16.10 nicht mehr verdrahtet: unbestätigte Criticals
 * verfallen per {@link expireStaleCriticals} zur normalen Notiz, statt
 * automatisch akzeptiert zu werden. Nicht wieder anschließen.
 */
...
export async function expireStaleCriticals(db, agent, opts = {}) {
  ...
  for (const card of pending) {
    try {
      const result = await db.markCriticalRejected(agent, card.id);
      if (result && result.ok === false) throw new Error(result.error || "markCriticalRejected returned ok=false");
      expired += 1;
    } catch (err) {
