...
import { readEpistemicCutoff } from "../epistemic-cutoff.js";
...
function filterSkillRows(rows, lookbackCutoff, loadScope, admission = {}) {
  return rows
    ...
    .filter(r => (r.createdAt || 0) >= lookbackCutoff)
    .filter((row) => isAdmissibleSkillEvidence(row, admission))
    .filter((row) => {
...
export async function runSkillMiner(db, agent, opts = {}) {
  ...
  try {
    ...
    const cutoffState = opts.cutoffState
      || (opts.baseDbPath ? readEpistemicCutoff(opts.baseDbPath) : { ok: false, since: 0, legacyOpen: false, reason: "cutoff_absent" });
