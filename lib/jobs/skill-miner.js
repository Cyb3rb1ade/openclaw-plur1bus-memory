...
async function loadMemories(db, lookbackDays) {
  ...
  return rows
    ...
    // A disputed or invalidated memory is not reliable evidence for a
    // mined skill pattern — exclude both, not just invalidated, since
    // skill-mining is a soft-scoring/pattern-extraction use, not a hard
    // recall boundary (see plan §6d row 6).
    .filter(r => !["disputed", "invalidated"].includes(normalizeEpistemicStatus(r.epistemicStatus)))
