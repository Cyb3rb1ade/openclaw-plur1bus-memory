...
// 7.12.50: Der Miner laeuft naechtlich. Die Sperre faengt nur doppelte
// Ausloeser ab (Nachholstart nach Neustart, Handlauf); 20 Stunden lassen den
// naechsten Nachtlauf durch, auch wenn der Zeitstempel erst am Laufende
// entsteht.
export const SKILL_MINER_RATE_LIMIT_MS = 20 * 3600000;
const SKILL_MINER_CLUSTER_MEMO_KEY = "skillMinerClusterMemo";
const SKILL_MINER_CLUSTER_MEMO_MAX = 300;
...
/**
 * Fingerabdruck eines Evidenz-Clusters: die exakte Menge seiner Erinnerungs-IDs.
 * Kommt eine Erinnerung hinzu, aendert sich der Fingerabdruck — ein Cluster,
 * der gewachsen ist, wird also erneut geprueft.
 * @param {object} group
 * @returns {string}
 */
export function clusterFingerprint(group) {
  const ids = (Array.isArray(group?.memories) ? group.memories : [])
    .map((memory) => String(memory?.id || ""))
    .filter(Boolean)
    .sort();
  return createHash("sha256").update(JSON.stringify(ids)).digest("hex").slice(0, 24);
}

function readClusterMemo(statePath, stateId, logger) {
  if (!statePath || !existsSync(statePath)) return {};
  try {
    const state = JSON.parse(readFileSync(statePath, "utf8"));
    const memo = state?.[SKILL_MINER_CLUSTER_MEMO_KEY]?.[stateId];
    return memo && typeof memo === "object" ? memo : {};
  } catch (error) {
    safeDebug(logger, "skill-miner.cluster-memo-read", error);
    return {};
  }
}

/**
 * Merkt sich Cluster, die nichts ergeben haben (zu unsicher, Name gesperrt).
 * Ohne das zahlt jeder Nachtlauf denselben Modellaufruf fuer dieselbe Evidenz.
 */
async function writeClusterMemo(statePath, stateId, entries) {
  if (!statePath || entries.size === 0) return;
  await atomicJsonUpdate(statePath, (data) => {
    const state = data || {};
    state[SKILL_MINER_CLUSTER_MEMO_KEY] = state[SKILL_MINER_CLUSTER_MEMO_KEY] || {};
    const memo = { ...(state[SKILL_MINER_CLUSTER_MEMO_KEY][stateId] || {}) };
    for (const [fingerprint, reason] of entries) memo[fingerprint] = { reason, at: Date.now() };
    const pruned = Object.entries(memo)
      .sort(([, left], [, right]) => (right?.at || 0) - (left?.at || 0))
      .slice(0, SKILL_MINER_CLUSTER_MEMO_MAX);
    state[SKILL_MINER_CLUSTER_MEMO_KEY][stateId] = Object.fromEntries(pruned);
    return state;
  });
}
...
export async function runSkillMiner(db, agent, opts = {}) {
  ...
  // Rate limit gegen doppelte Ausloeser, nicht gegen die naechste Nacht.
  const statePath = join(workspaceDir, "run-state.json");
  const rateLimit = checkJobRateLimit("skill-miner", agent, rateLimitWorkspaceKey, SKILL_MINER_RATE_LIMIT_MS, statePath);
  ...
  try {
    ...
    const clusterMemo = readClusterMemo(statePath, scanStateId, logger);
    const newClusterMemo = new Map();
    ...
    for (const group of groups) {
      ...
      // 7.12.50: Dieselbe Evidenzmenge hat schon einmal nichts ergeben — ohne
      // neue Erinnerung im Cluster waere auch dieser Aufruf umsonst.
      const fingerprint = clusterFingerprint(group);
      if (clusterMemo[fingerprint]) {
        skippedKnownCluster++;
        continue;
      }
      ...
      if (candidate.skip) {
        ...
        newClusterMemo.set(fingerprint, "low_confidence");
        continue;
      }
      ...
      if (isSkillNameBlocked(workspaceDir, proposal.skillName)) {
        skippedDuplicate++;
        newClusterMemo.set(fingerprint, "blocked_name");
        continue;
      }
      ...
      if (writeResult.written) {
        ...
        // 7.12.48: Auto-Apply. Der Aufrufer reicht die Aktivierung durch
        // (inspect → hash-gebundenes apply → Evidenz-Promotion, derselbe Pfad
        // wie eine manuelle Freigabe). Schlaegt sie fehl, bleibt der Vorschlag
        // pending_review und wartet im Dashboard; verloren geht nichts.
        if (!opts.dryRun && opts.autoApply === true && typeof opts.activateProposal === "function") {
          try {
            const activation = await opts.activateProposal(proposal);
            if (activation?.ok === true) {
              autoApplied++;
              pushMessage.autoApplied = true;
              pushMessage.activationStatus = activation.status || "active";
            } else {
              autoApplyFailed++;
              logger.warn?.(`skill-miner[${agent}]: auto-apply of ${proposal.skillName} declined: ${activation?.reason || activation?.text || "unknown"}`);
            }
          } catch (error) {
            autoApplyFailed++;
            safeWarn(logger, "skill-miner.auto-apply", error, { agent, skillName: proposal.skillName });
          }
        }
        pushMessages.push(pushMessage);
      } else {
        if (writeResult.reason !== "persist_failed") {
          skippedDuplicate++;
          newClusterMemo.set(fingerprint, "blocked_name");
        }
    ...
    logger.info?.(`skill-miner[${agent}]: ${proposalsCreated} proposals created, ${skippedLowEvidence} low-evidence skipped, ${skippedLowConfidence} low-confidence skipped, ${skippedDuplicate} duplicate skipped, ${autoApplied} auto-applied, ${autoApplyFailed} auto-apply failed, ${skippedKnownCluster} known clusters skipped`);

    if (!opts.dryRun) {
      try {
        await writeClusterMemo(statePath, scanStateId, newClusterMemo);
      } catch (error) {
        safeWarn(logger, "skill-miner.cluster-memo-write", error, { agent });
      }
    }
