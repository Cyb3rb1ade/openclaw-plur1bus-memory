...
export const SKILL_MINER_RATE_LIMIT_MS = 7 * 86400000 - 6 * 3600000; // Woche minus Laufzeit-Drift
...
export async function runSkillMiner(db, agent, opts = {}) {
  ...
  // Rate limit: 1× pro Woche pro Agent. 7.12.48: sechs Stunden Toleranz.
  // Der Zeitstempel wird erst am Ende eines Laufs geschrieben (06.09. 06:03
  // fuer den 06:00-Cron); exakt sieben Tage sperrten den naechsten Sonntag um
  // wenige Minuten, der Miner lief faktisch nur alle zwei Wochen (30.08. fehlt).
  ...
  try {
    ...
    logger.info?.(`skill-miner[${agent}]: ${proposalsCreated} proposals created, ${skippedLowEvidence} low-evidence skipped, ${skippedLowConfidence} low-confidence skipped, ${skippedDuplicate} duplicate skipped, ${autoApplied} auto-applied, ${autoApplyFailed} auto-apply failed`);
