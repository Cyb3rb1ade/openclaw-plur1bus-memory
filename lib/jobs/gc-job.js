 ...
 * @param {object} opts.dbPool — AgentDbPool mit getDb(agentId) → MemoryDB
...
export async function runGcJob(opts = {}) {
  ...
  for (const agentId of agentIds) {
    ...
    let memories = [];
    try {
      memories = await db.scanActive();
    } catch (e) {
      logger.warn?.(`gc-job: scanActive failed for agent ${agentId}: ${e.message}`);
      results.push({ agentId, ok: false, error: e.message, archived: 0, skipped: 0 });
      continue;
    }
    ...
    if (candidates.length === 0) {
      results.push({ agentId, ok: true, archived: 0, skipped: 0, dbSizeMb: mergedPolicy.dbSizeMb, memoryCount: memories.length });
      continue;
    }
    ...
    totalSkipped += archiveResult.skipped;
