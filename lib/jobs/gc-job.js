...
function listAgentIds(baseDbPath, logger) {
  if (!baseDbPath || !existsSync(baseDbPath)) return [];
  const ids = [];
  try {
    for (const entry of readdirSync(baseDbPath, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.isSymbolicLink()) continue;
      if (!existsSync(join(baseDbPath, entry.name, "memories.lance"))) continue;
      try {
        ids.push(safeAgentId(entry.name));
      } catch (error) {
        logger?.warn?.(`gc-job: skipped invalid agent directory: ${String(error)}`);
      }
    }
  } catch (error) {
    logger?.warn?.(`gc-job: agent discovery failed: ${String(error)}`);
    return [];
  }
  return ids;
}
...
async function runGcJobBody({ baseDbPath, dbPool, policy, workspaceDir, archiveDir, logger, now }) {
  const agentIds = listAgentIds(baseDbPath, logger);
