...
function readCompactionScanCursor(db, neoStore, stateId, persistent, logger) {
  if (persistent && typeof neoStore?.readRunState === "function") {
    try {
      const state = neoStore.readRunState() || {};
      return normalizeCompactionScanCursor(state?.[COMPACTION_SCAN_STATE_KEY]?.[stateId]?.cursor);
    } catch (error) {
      safeDebug(logger, "memory-compaction.scan-state-read", error);
    }
  }
  return normalizeCompactionScanCursor(getInMemoryCompactionState(db)?.get(stateId)?.cursor);
}

async function writeCompactionScanCursor(db, neoStore, stateId, securityScope, cursor, persistent, logger) {
...
async function executeActions(table, actions, candidates, dryRun, autoApply, logger, outputDir, embeddings, securityScope) {
  if (dryRun) {
  ...
  if (!autoApply) {
    persistProposals(outputDir, actions, logger);
  ...
  if (proposalActions.length > 0) {
    persistProposals(outputDir, proposalActions, logger);
  }
...
async function runMemoryCompactionBody(db, mergedOpts) {
  ...
  const scanCursor = readCompactionScanCursor(db, neoStore, scanStateId, persistScanState, logger);
  ...
  const scanMeta = candidates.scanMeta || {};
  const nextScanCursor = scanMeta.exhausted
    ? null
    : (scanMeta.lastScannedCursor || normalizeCompactionScanCursor(candidates[candidates.length - 1]));
  const advanceScan = async (successful = true) => {
    if (!dryRun && autoApply && successful) {
      await writeCompactionScanCursor(
        db,
        neoStore,
        scanStateId,
        securityScope,
  ...
  const outputDir = securityScope.partition && securityScope.partition.scope !== "workspace"
    ? null
    : workspaceDir;
