 ...
 */
async function callMergeCheck(existingText, newText, llmCfg, callLlm, timeoutMs, agentId) {
  ...
  try {
    ...
    console.warn(`[memory-compaction] Merge check failed: ${err.message}`);
    return null;
  }
}
...
async function generateCompactionActions(cluster, opts) {
  ...
  for (const mem of remaining) {
    if (isCompatibleText(mem, keep)) {
      ...
      if (llmCfg && callLlm) {
        const mergeResult = await callMergeCheck(keep.text, mem.text, llmCfg, callLlm, llmMergeTimeoutMs, agentId);
