...
export async function loadCompactionCandidates(table, lookbackDays, opts = {}) {
  ...
  const scanLimit = Number(opts.scanLimit) > 0 ? Number(opts.scanLimit) : DEFAULT_COMPACTION_SCAN_LIMIT;

  // Pushdown statt Nachfiltern: vorher entschied der Deckel, WELCHE Zeilen
  // überhaupt betrachtet wurden. LanceDB liefert in Einfügereihenfolge, der
  // Präfix waren also die ältesten Zeilen — bei Tabellen über dem Deckel lagen
  // sie sämtlich außerhalb des Rückschaufensters und der Job fand nie einen
  // Kandidaten (live: bernhardine 0 statt 1.965, main 0 statt 1.071).
  //
  // `epistemicStatus` bleibt bewusst im JS-Filter: `!= 'invalidated'` ist in SQL
  // NULL-asymmetrisch und würde Zeilen ohne gesetzten Status still verwerfen.
  const whereClause = `createdAt >= ${cutoffMs} AND (status = 'active' OR status IS NULL OR status = '')`;
  let rows = null;
  try {
    const builder = table.query();
    if (typeof builder.where === "function") {
      rows = await builder.where(whereClause).limit(scanLimit).toArray();
    }
  } catch {
    rows = null;
  }
  if (rows === null) rows = await table.query().limit(scanLimit).toArray();
...
async function generateCompactionActions(cluster, opts) {
  ...
  for (const mem of remaining) {
    ...
    if (isCompatibleText(mem, keep) && !hasDisjointValidityWindows(mem, keep)) {
      ...
      actions.push({ type: "mark_redundant", id: mem.id, targetId: keep.id, reason: "compatible_text_disjoint_validity_window" });
      continue;
    } else {
