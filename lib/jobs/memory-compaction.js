...
async function readPagedRows(table, whereClause, { pageSize, maxRows, logger, rowFilter = null }) {
  ...
  while (rows.length < maxRows) {
    ...
    try {
      ...
      // 7.12.29: Eine ACL-Ablehnung ist kein Query-Fehler. Ohne Where liefert
      // die Wiederholung nur noch mehr Fremd-Scope-Zeilen und scheitert erneut
      // (consolidate-daily[bernhardine] 10.09.2026: "ACL denied for query").
      if (error?.code === "PLUR1BUS_ACL_DENIED") {
        safeDebug(logger, "memory-compaction.acl", error, { offset, rowId: error.rowId || "", reason: error.aclReason || "" });
        throw error;
      }
      if (useWhere) {
...
/**
 * Apply one compaction merge, or refuse if the merged text is tombstoned.
 * Sources stay active when the write is blocked.
 * @param {object} table
 * @param {object} action
 * @param {Map<string, object>} memoryMap
 * @param {object} [logger]
 * @param {string|null} outputDir
 * @param {object} [embeddings]
 * @param {object} [tombstoneCtx]
 * @returns {Promise<{ok: boolean, added: boolean, archived: string[], error?: string}>}
 */
export async function executeCompactionMergeAction(table, action, memoryMap, logger, outputDir, embeddings, tombstoneCtx = {}) {
  const target = memoryMap.get(action.id);
  if (!target) {
    return { ok: false, added: false, archived: [], error: "target not found in candidates" };
  }
  ...
  if (!mergeGuard.allowed) {
    ...
    return { ok: false, added: false, archived: [], error: "tombstone_blocked" };
  }
  ...
  if (!archivedTarget || !archivedOther) {
    return { ok: false, added: false, archived: archivedTarget ? [action.id] : [], error: "merge_archive_failed" };
  }
  try {
    ...
    return { ok: false, added: false, archived: [action.id, action.targetId], error: "merge_add_failed" };
  }
  logger?.info?.(`memory-compaction: aliased merge ${action.id} + ${action.targetId} → ${mergedId}`);
  return { ok: true, added: true, archived: [action.id, action.targetId] };
}
  ...
  for (const action of lowRiskActions) {
    try {
      switch (action.type) {
        ...
        case "merge": {
          const mergeResult = await executeCompactionMergeAction(
            table, action, memoryMap, logger, outputDir, embeddings, tombstoneCtx,
          );
          if (mergeResult.error) errors.push({ action, error: mergeResult.error });
          if (mergeResult.ok) {
            executed++;
            merged++;
          }
