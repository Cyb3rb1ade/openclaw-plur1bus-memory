...
export async function applyConflictViaSafeUpdate(db, conflict, opts = {}) {
  ...
  const id = conflict?.id || conflict?.original?.id || conflict?.memoryId;
  const text = conflict?.mergedText || conflict?.text || conflict?.resolutionText;
  const vector = conflict?.vector;
