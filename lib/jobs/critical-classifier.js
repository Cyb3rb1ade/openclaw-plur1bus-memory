...
export async function runClassifier(db, agent, opts = {}) {
  ...
  // im Ergebnis zurückgegeben, damit der aufrufende Cron-Carrier sie zustellen
  // kann.
  const pushMessages = [];
  ...
  for (const card of recent) {
    ...
    const message = buildPushMessage({ ...card, type });
    ...
    pushMessages.push({ id: card.id, type, text: message.text, inline_keyboard: message.inline_keyboard });
    pushed += 1;
  }
