...
function buildPrompt(group) {
  ...
  return `You are a skill designer. Given the following memories from an agent's experience, extract a repeatable skill, workflow, or domain rule.
...
export async function extractSkillFromEvidence(group, opts = {}) {
  const { callLlm, llmCfg, timeoutMs = 30000, logger } = opts;
  ...
  try {
    const result = await Promise.race([
      callLlm([{ role: "user", content: prompt }], { ...llmCfg, jsonMode: true, maxTokens: 800 }),
    ...
    if (parsed.skip === true || (typeof parsed.confidence === "number" && parsed.confidence < 0.6)) {
      return { skip: true, confidence: parsed.confidence || 0, reason: "low_confidence" };
    }
