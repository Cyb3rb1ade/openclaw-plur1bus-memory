...
import { applyConflictViaSafeUpdate } from "./apply-conflict-resolution.js";
  ...
  for (const group of groups) {
    for (const conflict of group) {
      ...
      if (!dryRun) {
        ...
        if (opts.confirm === true && resolvedEntry.recommendation === "apply_via_safe_reconsolidation" && opts.db) {
          const applied = await applyConflictViaSafeUpdate(opts.db, { ...conflict, ...decision }, { confirm: true, neoStore: opts.neoStore, logger, agentId });
          resolvedEntry.apply = applied;
        }
      }
    }
  }
