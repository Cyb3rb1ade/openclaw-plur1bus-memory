...
export async function runConsolidation(db, agent, opts = {}) {
  ...
  let ownershipDiscovery = { readable: true, explicit: false, invalid: false, partitions: [] };
  if (db?.table) {
    ownershipDiscovery = await enumerateCompactionPartitions(db.table, agent, workspaceAliases, logger);
  }
  const hasProtectedOwnership = ownershipDiscovery.explicit && (
  ...
  try {
    return await withTimeout(
      runConsolidationBody(
        ...
        timestamp,
        logger,
        workspaceOutputAllowed ? neoStore : null,
        workspaceDir,
        workspaceKey,
        statePath,
        ownershipDiscovery,
