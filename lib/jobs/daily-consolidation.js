...
export async function runConsolidation(db, agent, opts = {}) {
  ...
  try {
    return await withTimeout(runConsolidationBody(db, agent, opts, timestamp, logger, neoStore, workspaceDir, workspaceKey), timeoutMs, `daily-consolidation:${agent}`);
  } catch (err) {
...
async function runConsolidationBody(db, agent, opts, timestamp, logger, neoStore, workspaceDir, workspaceKey) {
  try {
    ...
    if (db && neoStore) {
      try {
        ...
        logger.info?.(`daily-consolidation[${agent}]: dynamics ledger processed=${dynamicsLedger.processed} failed=${dynamicsLedger.failed} watermark=${dynamicsLedger.watermark}`);
      } catch (err) {
        logger.warn?.(`daily-consolidation[${agent}]: dynamics ledger threw: ${err.message}`);
      }
      try {
        dynamicsDecay = await applyDailyDecayToAll(db, { batchSize: 500, logger, dryRun: opts.dryRun === true });
        logger.info?.(`daily-consolidation[${agent}]: dynamics decay decayed=${dynamicsDecay.decayed} errors=${dynamicsDecay.errors}`);
      } catch (err) {
