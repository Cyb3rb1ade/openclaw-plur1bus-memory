...
import { processRetrievalLedger, applyDailyDecayToAll, applyDailyDecayBatch } from "./memory-dynamics-maintenance.js";
...
export const DAILY_CONSOLIDATION_RATE_LIMIT_MS = 12 * 60 * 60 * 1000;
...
// 7.12.46: 50 je Nacht hiess bei 9385 Zeilen (main) 188 Naechte fuer einen
// Durchgang — und der Cursor wanderte gar nicht (s. readDynamicsDecayCursor).
// Jetzt 300 Zeilen, gedeckelt durch ein Zeitbudget von zwei Minuten.
const DEFAULT_DYNAMICS_DECAY_MAX_ROWS = 300;
const DEFAULT_DYNAMICS_DECAY_DEADLINE_MS = 120_000;
...
export async function runConsolidation(db, agent, opts = {}) {
  ...
  // Rate Limit: faengt doppelte Ausloeser ab (Nachholstart nach Neustart,
  // Handlauf), nicht den naechsten Tag. 7.12.48: 12 statt 24 Stunden. Der
  // Zeitstempel entsteht am Ende des Laufs (21-37 s nach dem Slot); exakt 24 h
  // sperrten den naechsten Nachtlauf um eine Minute, jede zweite Nacht fiel
  // aus (bernhardine 10.09. 04:15, main 11.09. 04:00, "1min remaining").
  const statePath = workspaceOutputAllowed ? join(workspaceDir, "run-state.json") : null;
  const rateLimit = workspaceOutputAllowed
    ? checkJobRateLimit("daily-consolidation", agent, workspaceKey, DAILY_CONSOLIDATION_RATE_LIMIT_MS, statePath)
    : { allowed: true };
...
/**
 * 7.12.46: Der Cursor lag nur in `<workspace>/run-state.json`, und die gibt es
 * fuer partitionsgebundene Laeufe (agent-private, seit der Ownership-
 * Erkennung) nicht: `statePath` ist dort null, weil `workspaceOutputAllowed`
 * bei geschuetzter Partition false ist. Folge seit 14.08.2026: jede Nacht
 * dieselben ersten 50 Zeilen (heisenberg: `nextCursorId` 154407e2… am 10.09.
 * UND 11.09.). Der Cursor wohnt jetzt im Hook-Record des partitionseigenen
 * Neo-Stores (`daily-consolidation.dynamicsDecay[<agent>:<workspace>]`), die
 * Datei bleibt als Rueckfall fuer Laeufe ohne Store.
 */
export function readDynamicsDecayCursor(statePath, agent, workspaceKey, logger, neoStore = null) {
  const stateKey = dynamicsDecayStateKey(agent, workspaceKey);
  if (neoStore && typeof neoStore.readHooks === "function") {
    try {
      const cursor = neoStore.readHooks()?.["daily-consolidation"]?.dynamicsDecay?.[stateKey]?.cursorId;
      if (typeof cursor === "string" && cursor) return cursor;
    } catch (err) {
      logger?.warn?.(`daily-consolidation[${agent}]: failed to read dynamics decay cursor from neo store: ${err.message}`);
    }
  }
  if (!statePath || !existsSync(statePath)) return null;
  try {
    const parsed = JSON.parse(readFileSync(statePath, "utf8"));
    const cursor = parsed?.[DYNAMICS_DECAY_STATE_KEY]?.[stateKey]?.cursorId;
...
export async function recordDynamicsDecayCursor(statePath, agent, workspaceKey, cursorId, neoStore = null) {
  if (!cursorId) return false;
  const stateKey = dynamicsDecayStateKey(agent, workspaceKey);
  let stored = false;
  if (neoStore && typeof neoStore.recordHook === "function") {
    try {
      const previous = (typeof neoStore.readHooks === "function" ? neoStore.readHooks()?.["daily-consolidation"]?.dynamicsDecay : null) || {};
      neoStore.recordHook("daily-consolidation", {
        dynamicsDecay: { ...previous, [stateKey]: { cursorId, updatedAt: Date.now() } },
      });
      stored = true;
    } catch (_) {
      stored = false;
...
async function runConsolidationBody(
  ...
  try {
    ...
    if (db && neoStore) {
      ...
      try {
        const decayPartition = (ownershipDiscovery?.explicit && ownershipDiscovery.partitions?.length === 1 ? ownershipDiscovery.partitions[0].aclPartition : null)
          || opts.aclPartition || opts.ownershipPartition || opts.partition
          || { scope: "agent-private", agentId: agent, workspaceIdentity: "", ownerUserId: "" };
        // 7.12.47: erst der Batch (ein Statement), der Zeilenpfad bleibt Rueckfall.
        let batchDecay = null;
        if (opts.dynamicsDecayMode !== "rows") {
          try {
            batchDecay = await applyDailyDecayBatch(db, { partition: decayPartition, logger, dryRun: opts.dryRun === true });
          } catch (batchErr) {
            logger.warn?.(`daily-consolidation[${agent}]: batch decay unavailable, falling back to rows: ${batchErr.message}`);
            batchDecay = null;
          }
        }
        const decayCursorId = opts.dynamicsDecayCursorId ?? readDynamicsDecayCursor(statePath, agent, workspaceKey, logger, neoStore);
        dynamicsDecay = batchDecay || await applyDailyDecayToAll(db, {
          batchSize: 500,
          maxRows: opts.dynamicsDecayMaxRows ?? DEFAULT_DYNAMICS_DECAY_MAX_ROWS,
          deadlineMs: opts.dynamicsDecayDeadlineMs ?? DEFAULT_DYNAMICS_DECAY_DEADLINE_MS,
          cursorId: decayCursorId,
          logger,
          dryRun: opts.dryRun === true,
          // 7.12.29: Decay nur innerhalb der Partition, die auch der Guard
          // freigibt (sonst ACL-Abbruch an der ersten Fremd-Scope-Zeile).
          partition: decayPartition,
        });
        if (!opts.dryRun && dynamicsDecay?.nextCursorId) {
          await recordDynamicsDecayCursor(statePath, agent, workspaceKey, dynamicsDecay.nextCursorId, neoStore);
        }
