...
export async function applyDailyDecayToAll(db, opts = {}) {
  ...
  try {
    ...
    const startIndex = cursorId
      ? Math.max(0, eligibleRows.findIndex((row) => String(row.id) > cursorId))
      : 0;
    const normalizedStartIndex = startIndex === -1 ? 0 : startIndex;
    const maxAttempts = Math.min(maxValidUpdates, eligibleRows.length);
    truncated = maxAttempts < eligibleRows.length;

    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      const row = eligibleRows[(normalizedStartIndex + attempt) % eligibleRows.length];
