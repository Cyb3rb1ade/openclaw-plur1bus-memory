import { isSafeAgentId } from "./sql-safety.js";
...
const nonNegativeSafeIntegerOrNull = (value) => (
...
function projectCountGroup(value, name, { allowUnknown = false } = {}) {
  ...
  const projected = value.map((entry) => {
    ...
    seen.add(id);
    const cards = allowUnknown && entry.cards === null
      ? null : nonNegativeSafeInteger(entry.cards, `${name} cards`);
    return Object.freeze({ id, cards });
  });
...
function projectSnapshot(value, observedAt) {
  ...
  return Object.freeze({
    ...
    cards: Object.freeze({
      // Fail closed: anything but a usable count means unknown. A layer that
      // forgets to carry the number must refuse, not permit.
      largestAgentCards: nonNegativeSafeIntegerOrNull(value.cards.largestAgentCards),
      ...
      byPrimaryAgent: projectCountGroup(value.cards.byPrimaryAgent ?? [], "primary agent cards", { allowUnknown: true }),
    }),
...
/**
 * Create an aggregate-only, read-only scan; unknown primary-agent counts are null.
...
export function createControlPlaneHealthScan({
  ...
  userIdentityForKey = () => null,
  primaryAgentIds = () => [],
  maxPartitions = 128,
} = {}) {
  if (typeof primaryAgentIds !== "function") {
    throw new Error("PLUR1BUS health primary agent resolver is required");
  }
  if (typeof userIdentityForKey !== "function") {
    throw new Error("PLUR1BUS health user identity resolver is required");
  }
  ...
  return async () => {
    ...
    const unknownAgentCounts = new Set();
    let agentListingsComplete = roots.length > 0;
    // The gc cap guard asks one question: how big is the largest store the gc
    // job would prune? `byAgent` cannot answer it. The two id contracts differ
    // in both directions — `safeAgentId` admits `_neo` and `_customer`, which
    // the public contract strips as reserved, and it rejects `agent.v2`, which
    // the public contract publishes. Counting the published list therefore both
    // misses stores gc prunes and would refuse over stores it never touches. So
    // the basis follows gc's own contract, counted whether or not the name may
    // be shown, and the vouch is withdrawn as soon as one of those counts is
    // missing. Absence is invisible downstream; it has to be said here.
    let gcVouch = roots.length > 0;
    let largestGcCards = 0;
    ...
    const scanRoot = async (root, kind, counts) => {
      let rows = 0;
      let partitionIds = [];
      // Agent-root names the gc job would prune, by its contract, not ours.
      const gcAgentIds = new Set();
      try {
        ...
        if (Array.isArray(listed)) {
          if (kind === "agent") for (const entry of listed) if (isSafeAgentId(entry)) gcAgentIds.add(entry);
          // Names with a leading underscore or dot are PLUR1BUS's own reserved
          // directories inside the store (`_neo` for the Neo workspace,
          // `.plur1bus-authority` and `.plur1bus-shared` for the shared pools),
          // not partitions. They are skipped silently; flagging them pinned
          // "degraded" on every dashboard that had a Neo workspace, and the two
          // dot directories kept doing exactly that after the `_neo` fix.
          const candidates = listed.filter((entry) => !(
            typeof entry === "string" && (entry.startsWith("_") || entry.startsWith("."))
          ));
          usable = candidates.filter((entry) => typeof entry === "string" && PUBLIC_ID_RE.test(entry));
          // A name dropped here is either one gc prunes — then it is already in
          // gcAgentIds and gets counted below, just never shown — or one gc
          // ignores, and then it is none of the guard's business. Neither case
          // touches the vouch.
        ...
        if (kind === "agent") {
          agentListingsComplete = false;
          gcVouch = false;
        }
        recordFailure("lancedb", "partition_list_failed");
      }
      // Published first, then the stores gc prunes that may not be shown.
      const publishable = new Set(partitionIds);
      const inspectIds = kind === "agent"
        ? [...partitionIds, ...[...gcAgentIds].filter((id) => !publishable.has(id))]
        : partitionIds;
      for (const partitionId of inspectIds) {
        const publish = publishable.has(partitionId);
        const prunedByGc = gcAgentIds.has(partitionId);
        if (inspectedPartitions >= maxPartitions) {
          if (publish && kind === "agent") unknownAgentCounts.add(partitionId);
          if (prunedByGc) gcVouch = false;
        ...
        try {
          ...
          if (prunedByGc && cards > largestGcCards) largestGcCards = cards;
          if (!publish) continue; // counted for the guard, not part of the view
          rows += cards;
          let publicPartitionId = partitionId;
          if (kind === "workspace" || kind === "user") {
            const resolver = kind === "workspace" ? workspaceIdentityForKey : userIdentityForKey;
            try {
              const resolved = resolver(partitionId);
              if (typeof resolved === "string" && PUBLIC_ID_RE.test(resolved)) publicPartitionId = resolved;
              else if (resolved !== null && resolved !== undefined) recordFailure("health", `${kind}_identity_invalid`);
            } catch {
              recordFailure("health", `${kind}_identity_unavailable`);
            }
          }
    ...
    // The primary agents' private counts, read from the agent scan above. An
    // agent absent from every successfully enumerated namespace shows 0.
    // Failed/capped counts or any failed private listing make the aggregate
    // unknown, even if another namespace contributed a successful subtotal.
    // Shared-pool failures cannot invalidate a private count. The roster comes from the host
    // config; when it is unavailable the list stays empty, which is no
    // storage problem and does not degrade the snapshot.
    const primaryAgentCards = new Map();
    try {
      const ids = primaryAgentIds();
      for (const id of Array.isArray(ids) ? ids : []) {
        if (typeof id === "string" && PUBLIC_ID_RE.test(id)) {
          const known = agentListingsComplete && !unknownAgentCounts.has(id);
          primaryAgentCards.set(id, known ? (agentCards.get(id) ?? 0) : null);
        }
      }
    } catch {
      primaryAgentCards.clear();
    }

    let storage;
    try {
      storage = normalizeStorage(await measureStorage());
      // Eine abgeschnittene Groessenmessung ist kein Gesundheitsproblem, sondern
      // eine Messgrenze: `measureControlHealthStorage` steigt nach 10 000
      // Verzeichniseintraegen aus, damit der Scan beschraenkt bleibt. Am
      // 09.09.2026 hatte der Store 12 646 Eintraege — die Karte stand deshalb in
      // jedem Durchlauf auf "degraded", obwohl jede Partition sauber war. Der
      // Snapshot traegt `storage.complete: false` bereits selbst bis in die
      // Oberflaeche; daraus zusaetzlich den Gesamtzustand herabzustufen
      // ueberzeichnet. Ein echter Fehler beim Messen bleibt degradierend.
    } catch {
