...
function projectSnapshot(value, observedAt) {
  ...
  return Object.freeze({
    ...
    cards: Object.freeze({
      ...
      byPrimaryAgent: projectCountGroup(value.cards.byPrimaryAgent ?? [], "primary agent cards"),
    }),
...
export function createControlPlaneHealthScan({
  ...
  return async () => {
    ...
    // agent without a partition yet shows 0. The roster comes from the host
    ...
    try {
      ...
      for (const id of Array.isArray(ids) ? ids : []) {
        if (typeof id === "string" && PUBLIC_ID_RE.test(id)) primaryAgentCards.set(id, agentCards.get(id) ?? 0);
      }
    } catch {
