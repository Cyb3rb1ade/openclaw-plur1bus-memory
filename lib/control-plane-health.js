...
function projectSnapshot(value, observedAt) {
  ...
  return Object.freeze({
    ...
    cards: Object.freeze({
      // Fail closed: anything but an explicit true means the inventory is not
      // vouched for. A layer that forgets to carry the flag must refuse, not
      // permit.
...
export function createControlPlaneHealthScan({
  ...
  return async () => {
    ...
    // Separate from `agentListingsComplete`, which answers "do we know the agent
    // ids?" for byPrimaryAgent. This one answers "is byAgent the whole
    // inventory?" — the gc guard needs that, because a partition whose count
    // failed is *dropped* from the map rather than nulled, and an id the public
    // contract rejects never enters it at all. Absence is invisible downstream,
    // so it has to be said here.
    let agentCountsComplete = roots.length > 0;
    ...
    const scanRoot = async (root, kind, counts) => {
      ...
      try {
        ...
        if (Array.isArray(listed)) {
          ...
          if (usable.length !== candidates.length) {
            recordFailure("health", "partition_id_unsupported");
            // A dropped name cannot match a validated primary id, so
            // byPrimaryAgent stays exact; byAgent, however, is now short one
            // partition whose size nobody measured.
            if (kind === "agent") agentCountsComplete = false;
          }
        }
      ...
      for (const partitionId of partitionIds) {
        if (inspectedPartitions >= maxPartitions) {
          if (kind === "agent") unknownAgentCounts.add(partitionId);
    ...
    return {
      ...
      namespaces,
      cards: {
        agentCountsComplete: agentCountsComplete && unknownAgentCounts.size === 0,
