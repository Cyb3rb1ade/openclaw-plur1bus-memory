...
export class MemoryDoctor {
  ...
  async summarize() {
    ...
    return {
      type: "summary",
      totalOverlays:
        classified.active.length +
        classified.provisional.length +
        classified.superseded.length +
        classified.disabled.length,
        ...
        if (loser.id === current.id) {
          if (current.status === "provisional") {
            suggestions.push({
              ...
              reason: `This provisional overlay contradicts ${partnerId} and is the weaker interpretation`,
              command: `/plur1bus memory disable-overlay ${current.id}`,
            });
          } else {
            suggestions.push({
              action: "supersede",
