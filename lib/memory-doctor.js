...
export class MemoryDoctor {
  ...
  async summarize() {
    ...
    const contradictions = await this.detector.loadAll();
  ...
  async diagnoseMemory(memoryId) {
    ...
    const activeIds = new Set(classified.active.map((o) => o.id));
    const activeContradictions = contradictions.filter(
      (c) => activeIds.has(c.overlayA) && activeIds.has(c.overlayB),
    );
    const suggestions = this._suggestForMemory(classified, activeContradictions);
  ...
  async diagnoseOverlay(overlayId) {
    ...
    const suggestions = this._suggestForOverlay(lineage, involving);
  ...
  _classify(overlays) {
    ...
    for (const rec of overlays) {
      ...
      } else if (rec.supersededBy || supersededIds.has(rec.id)) {
        superseded.push(rec);
      } else {
        active.push(rec);
      }
  ...
  _suggestForMemory(classified, activeContradictions) {
    const suggestions = [];
    const byId = new Map([
    ...
    for (const c of activeContradictions) {
      ...
      if (loser.status === "provisional") {
        suggestions.push({
          ...
          reason: `Provisional overlay contradicts active overlay ${winner.id}`,
        ...
        suggestions.push({
          ...
          reason: `Contradicts active overlay ${winner.id} (${winner.confidence ?? "no"} confidence)`,
    ...
    for (const prov of classified.provisional) {
      if (!activeContradictions.some((c) => c.overlayA === prov.id || c.overlayB === prov.id)) {
        suggestions.push({
          action: "review",
  ...
  _suggestForOverlay(lineage, involving) {
    const suggestions = [];
    ...
    if (involving.length > 0) {
      const byId = new Map();
      for (const o of [...lineage.predecessors, current, ...lineage.successors]) {
        byId.set(o.id, o);
      }
