...
export function createEmbeddingCache({
  ...
  function _dbSize(targetDbPath) {
    if (!targetDbPath || !existsSync(targetDbPath)) return 0;
    try {
      return statSync(targetDbPath).size;
    } catch {
      return 0;
    }
  }
  ...
  async function _ensureDb(options = {}) {
    ...
    if (!targetDbPath || failedDbPaths.has(targetDbPath)) return null;
    ...
    try {
      ...
      const database = new sqliteModule.DatabaseSync(targetDbPath);
      ...
      failedDbPaths.add(targetDbPath);
  ...
  function _cleanupDb(handle, targetBytes = 0) {
    if (!handle?.database) return;
    const { database, dbPath } = handle;
    const now = Date.now();
    try {
      database.prepare("DELETE FROM embeddings WHERE expires_at <= ?").run(now);
      if (targetBytes > 0) {
        while (_dbSize(dbPath) > targetBytes) {
          const oldest = database
            .prepare("SELECT key_hash FROM embeddings ORDER BY accessed_at ASC, created_at ASC LIMIT 1")
            .get();
          if (!oldest) break;
          database.prepare("DELETE FROM embeddings WHERE key_hash = ?").run(oldest.key_hash);
        }
      }
    } catch (err) {
      safeWarn(logger, "embedding-cache", err, { scope: "cleanupDb", dbPath });
    }
  }
  ...
  async function _persistSet(rows, options = {}) {
    ...
    _cleanupDb(handle);
    const currentDbSize = _dbSize(dbPath);
    ...
    try {
      ...
      if (_dbSize(dbPath) > softTarget) {
        _cleanupDb(handle, softTarget);
      }
    } catch (err) {
