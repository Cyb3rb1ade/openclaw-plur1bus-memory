...
const SQLITE_CHECKPOINT_BUSY_TIMEOUT_MS = 50;
const SQLITE_CHECKPOINT_MAX_ATTEMPTS = 64;
 ...
 */
function buildKey({ provider, model, dimensions, scopeId, cacheVersion, textHash, purpose = "passage" }) {
  // `purpose` belongs in the key: asymmetric models embed the same string
  // differently as a question than as a stored passage, so without it a
  // question would be served the passage vector that happens to share its text.
  const key = `${provider}\x00${model}\x00${dimensions}\x00${scopeId}\x00${cacheVersion}\x00${purpose}\x00${textHash}`;
...
export function createEmbeddingCache({
  ...
  function _hasCheckpointBudget(handle, checkpointBudget, requiredAttempts) {
    const { dbPath } = handle;
    if (checkpointBudget.attempts + requiredAttempts > SQLITE_CHECKPOINT_MAX_ATTEMPTS) {
      safeWarn(logger, "embedding-cache", "checkpoint_budget_exhausted", {
        dbPath,
        checkpointAttempts: checkpointBudget.attempts,
      });
      return false;
    }
    return true;
  ...
  function _checkpoint(handle, checkpointBudget) {
    const { database, dbPath } = handle;
    if (!_hasCheckpointBudget(handle, checkpointBudget, 1)) return false;
    checkpointBudget.attempts += 1;
    const result = database.prepare("PRAGMA wal_checkpoint(TRUNCATE)").get();
    if (Number(result?.busy ?? 0) === 0) return true;
    safeWarn(logger, "embedding-cache", "checkpoint_busy", {
      dbPath,
      checkpointAttempts: checkpointBudget.attempts,
  ...
  function _compactAndMeasure(handle, checkpointBudget) {
    const { database, dbPath } = handle;
    if (!_hasCheckpointBudget(handle, checkpointBudget, 2)) {
      return { currentBytes: _dbSize(dbPath), checkpointBlocked: true };
    }
    const previousBusyTimeout = Number(
      database.prepare("PRAGMA busy_timeout").get()?.timeout ?? 5_000,
    );
    database.exec(`PRAGMA busy_timeout=${SQLITE_CHECKPOINT_BUSY_TIMEOUT_MS};`);
    try {
      if (!_checkpoint(handle, checkpointBudget)) {
        return { currentBytes: _dbSize(dbPath), checkpointBlocked: true };
      }
      database.exec("PRAGMA incremental_vacuum;");
      const checkpointBlocked = !_checkpoint(handle, checkpointBudget);
      return { currentBytes: _dbSize(dbPath), checkpointBlocked };
    } finally {
      try {
        database.exec(`PRAGMA busy_timeout=${previousBusyTimeout};`);
      } catch (err) {
        safeDebug(logger, "embedding-cache", err, { scope: "restoreBusyTimeout", dbPath });
      }
    }
  }
  ...
  function _trimToSize(handle, targetBytes, checkpointBudget, removedRows = 0, protectedAt) {
    const { database } = handle;
    let compacted = _compactAndMeasure(handle, checkpointBudget);
    let { currentBytes } = compacted;
    if (compacted.checkpointBlocked) {
      return { currentBytes, removedRows, checkpointBlocked: true };
    }
    while (currentBytes > targetBytes && removedRows < SQLITE_CLEANUP_MAX_ROWS) {
      ...
      compacted = _compactAndMeasure(handle, checkpointBudget);
      currentBytes = compacted.currentBytes;
      if (compacted.checkpointBlocked) {
        return { currentBytes, removedRows, checkpointBlocked: true };
      }
    }
    return { currentBytes, removedRows, checkpointBlocked: false };
  }
  ...
  async function _persistSet(rows, options = {}) {
    ...
    try {
      ...
      const checkpointBudget = { attempts: 0 };
      ...
      if (removedRows > 0 || currentDbSize + incomingBytes > limit || currentDbSize > softTarget) {
        ...
        const trimmed = _trimToSize(handle, capacityTarget, checkpointBudget, removedRows);
      ...
      if (currentDbSize <= limit && currentDbSize > softTarget) {
        const trimmed = _trimToSize(handle, softTarget, checkpointBudget, removedRows, now);
        currentDbSize = trimmed.currentBytes;
        removedRows = trimmed.removedRows;
      }
      if (currentDbSize > limit) {
        _restorePreviousRows(database, persistedRows, previousRows);
        const rolledBack = _compactAndMeasure(handle, checkpointBudget);
        _skipPersistRows(persistedRows, "persist_write_rolled_back_size_limit", {
          dbSize: rolledBack.currentBytes,
  ...
  function _keyPartsFor(options = {}) {
    return {
      ...
      purpose: options.purpose || "passage",
    };
  }
