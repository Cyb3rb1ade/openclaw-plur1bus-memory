...
function sha256Hex(value) {
  return cryptoHash("sha256", value, "hex");
}
...
export function createEmbeddingCache({
  ...
  function _resolveDbPath(options = {}) {
    ...
    try {
      // resolveInside() realpath()s the base directory, so a configured but
      // not-yet-created cacheBasePath threw ENOENT here and silently turned
      // embeddingCachePersist into a no-op. The caller asked for a persistent
      // cache, so the directory is ours to create.
      mkdirSync(cacheBasePath, { recursive: true, mode: 0o700 });
