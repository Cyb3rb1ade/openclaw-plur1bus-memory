...
function buildKey(agentId, normalizedQuery, modelVersion) {
  const MAX_KEY_QUERY_CHARS = 256;
  const cappedQuery =
    normalizedQuery.length > MAX_KEY_QUERY_CHARS
      ? normalizedQuery.slice(0, MAX_KEY_QUERY_CHARS)
      : normalizedQuery;
  return `${agentId}\x00${cappedQuery}\x00${modelVersion}`;
}
 ...
 */
export function createEmbeddingCache({ maxEntries = 128, ttlMs = 300000 } = {}) {
