...
import { appendDestructiveOpLog, safeUuid } from "./sql-safety.js";
 ...
 */
async function searchByKind(table, vector, kind, limit, minScore, requestNow, logger) {
  const kindFilter = kind === "memory"
    ? "memoryKind = 'memory' OR memoryKind IS NULL OR memoryKind = ''"
    : `memoryKind = '${kind}'`;
  const whereClause = `(${kindFilter}) AND (status = 'active' OR status IS NULL OR status = '') AND (expiresAt IS NULL OR expiresAt = 0 OR expiresAt > ${requestNow})`;
...
async function wikiSearch(
  ...
  const wikiResults = visibleResults(
    ctx,
    await searchByKind(db.table, vector, "wiki", 8, 0.2, requestNow, logger),
    "wiki",
    requestNow,
  );
  ...
  const memoryResults = visibleResults(
    ctx,
    await searchByKind(db.table, vector, "memory", 8, 0.2, requestNow, logger),
    "memory",
    requestNow,
  );
...
async function wikiDelete(
  ...
  const results = visibleResults(
    ctx,
    await searchByKind(db.table, vector, "wiki", 5, 0.25, requestNow, logger),
    "wiki",
    requestNow,
  );
...
export async function runWikiCommand(
  ...
  const validation = validateCommandArgs(commandCtx?.args);
  ...
  if (!args) return { text: t("wiki.usage", { lang, tone }) };
  ...
  const requestNow = typeof suppliedNow === "number" && Number.isFinite(suppliedNow)
    ? suppliedNow
    : Date.now();
  const workspaceDir = suppliedWorkspaceDir ?? memoryCtx.workspaceDir;
