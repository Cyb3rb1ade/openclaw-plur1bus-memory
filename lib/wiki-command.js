...
async function wikiAdd(rawArgs, { db, embeddings, agentId, workspaceKey, lang, tone }) {
 ...
 */
export async function runWikiCommand(commandCtx, { pool, embeddings, reranker, callLlm, cfg, api, llmCfg, archiveDir }) {
  const { lang, tone } = resolveLocaleFromCtx(commandCtx);
  ...
  const workspaceKey = commandCtx.workspaceKey || commandCtx.workspaceId || (commandCtx.workspaceDir ? basename(commandCtx.workspaceDir) : "default");
  ...
  if (subCmd === "add") {
    ...
    try {
      return await withDbLease(pool, agentId, async (db) => {
        await db.init();
        return wikiAdd(rest, { db, embeddings, agentId, workspaceKey, lang, tone });
      });
    } catch (err) {
      return { text: t("wiki.search_error", { lang, tone, vars: { error: err.message } }) };
    }
  ...
  try {
    return await withDbLease(pool, agentId, async (db) => {
      await db.init();
      return wikiSearch(args, { db, embeddings, reranker, callLlm, llmCfg, lang, tone, logger: api.logger, ctx: aclCtx });
    });
  } catch (err) {
    return { text: t("wiki.search_error", { lang, tone, vars: { error: err.message } }) };
  }
