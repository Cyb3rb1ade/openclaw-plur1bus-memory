...
function checkWikiAuth(memoryCtx, commandCtx, cfg, { destructive = false } = {}) {
  const auth = isAuthorized(memoryCtx, cfg, { destructive, chatKind: memoryCtx.chatKind });
  if (!auth.authorized) {
...
function filterWikiResultsByAcl(ctx, results) {
  if (!ctx || !Array.isArray(results)) return results || [];
  return results.filter((result) => checkAccess(ctx, result?.entry || result).allowed);
}

async function wikiSearch(query, { db, embeddings, callLlm, llmCfg, lang, tone, ctx }) {
  let vector;
  try {
  ...
  const wikiResults = filterWikiResultsByAcl(ctx, await searchByKind(db.table, vector, "wiki", 8, 0.2));
  ...
  const memoryResults = filterWikiResultsByAcl(ctx, await searchByKind(db.table, vector, "memory", 8, 0.2));
...
async function wikiAdd(rawArgs, { db, embeddings, memoryCtx, lang, tone }) {
...
async function wikiDelete(rawArgs, { db, embeddings, agentId, lang, tone, archiveDir }) {
  ...
  if (rawArgs.startsWith("id:")) {
    ...
    let archivePath;
    try {
      archivePath = archiveCard(card, agentId || "default", archiveDir);
    } catch (err) {
      return { text: t("wiki.archive_failed", { lang, tone, vars: { error: err.message } }) };
    }
    try {
      await db.table.delete(`id = "${safeId}"`);
    } catch (err) {
      return { text: t("wiki.archive_failed", { lang, tone, vars: { error: err.message } }), archivePath };
    }
 ...
 */
export async function runWikiCommand(commandCtx, { pool, embeddings, reranker, callLlm, cfg, api, llmCfg, archiveDir, memoryCtx: suppliedMemoryCtx, workspaceAliases }) {
  const { lang, tone } = resolveLocaleFromCtx(commandCtx);
  const memoryCtx = suppliedMemoryCtx || resolveMemoryRequestContext(commandCtx, { workspaceAliases });
  ...
  if (subCmd === "add") {
    const denied = checkWikiAuth(memoryCtx, commandCtx, cfg, { destructive: true });
    if (denied) return denied;
    const rest = args.slice(tokens[0].length).trim();
    if (!rest) return { text: t("wiki.add_usage", { lang, tone }) };
    if (!memoryCtx.workspaceIdentity) return { text: t("wiki.search_error", { lang, tone, vars: { error: "memory context requires a bound workspace" } }) };
    try {
      return await withDbLease(pool, agentId, async (db) => {
        await db.init();
        return wikiAdd(rest, { db, embeddings, memoryCtx, lang, tone });
      });
    } catch (err) {
      return { text: t("wiki.search_error", { lang, tone, vars: { error: err.message } }) };
    }
  ...
  if (subCmd === "delete" || subCmd === "löschen" || subCmd === "loeschen") {
    ...
    try {
      return await withDbLease(pool, agentId, async (db) => {
        await db.init();
        return wikiDelete(rest, { db, embeddings, agentId, lang, tone, archiveDir });
      });
    } catch (err) {
      return { text: t("wiki.search_error", { lang, tone, vars: { error: err.message } }) };
    }
  ...
  try {
    return await withDbLease(pool, agentId, async (db) => {
      await db.init();
      return wikiSearch(args, { db, embeddings, reranker, callLlm, llmCfg, lang, tone, logger: api.logger, ctx: memoryCtx });
    });
  } catch (err) {
    return { text: t("wiki.search_error", { lang, tone, vars: { error: err.message } }) };
  }
