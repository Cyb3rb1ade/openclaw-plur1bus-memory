...
export async function runWikiCommand(commandCtx, { pool, embeddings, reranker, callLlm, cfg, api, llmCfg, archiveDir }) {
  ...
  if (subCmd === "add") {
    ...
    await db.init();
    try {
      return await wikiAdd(rest, { db, embeddings, agentId, workspaceKey, lang, tone });
    } catch (err) {
      return { text: t("wiki.search_error", { lang, tone, vars: { error: err.message } }) };
    }
  }
  ...
  if (subCmd === "delete" || subCmd === "löschen" || subCmd === "loeschen") {
    ...
    await db.init();
    try {
      return await wikiDelete(rest, { db, embeddings, agentId, lang, tone, archiveDir });
    } catch (err) {
      return { text: t("wiki.search_error", { lang, tone, vars: { error: err.message } }) };
    }
  }
  ...
  await db.init();
  try {
    return await wikiSearch(args, { db, embeddings, reranker, callLlm, llmCfg, lang, tone, logger: api.logger, ctx: aclCtx });
  } catch (err) {
    return { text: t("wiki.search_error", { lang, tone, vars: { error: err.message } }) };
  }
}
