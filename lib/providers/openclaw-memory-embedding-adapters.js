...
import { ReloadSafeIpcScopedEmbeddingProvider } from "./scoped-embedding-ipc.js";
...
function resolveDimension(options, model, fallbackDim) {
  const requested = Number(options.dimensions || options.outputDimensionality || 0);
...
function extractTextEmbeddingInputs(inputs) {
  return inputs.map((input, index) => {
    ...
    if (!input || typeof input !== "object") {
      throw new Error(`EmbeddingInput at index ${index} is neither text nor a structured input`);
    }
...
function createRemoteProvider({ id, model, dim, baseUrl, apiKey, headers }) {
  ...
  return {
    ...
    dimensions: dim,
    maxInputTokens: 8192,
    embed: async (input) => (await embedBatch(extractTextEmbeddingInputs([input])))[0],
    embedBatch: async (inputs) => embedBatch(extractTextEmbeddingInputs(inputs)),
  };
}

function createLocalE5Provider({
  model,
  dim,
  cacheDir,
  ...
  const local = scopedEmbeddingIpc?.stateRoot
    ? new ReloadSafeIpcScopedEmbeddingProvider({
        stateRoot: scopedEmbeddingIpc.stateRoot,
        model,
        dimensions: dim,
        fingerprintId: scopedEmbeddingIpc.fingerprintId,
      })
  ...
  return {
    ...
    maxInputTokens: 512,
    embed: (input, options = {}) => {
      const text = extractTextEmbeddingInputs([input])[0];
      return options.inputType === "query" ? local.embedQuery(text) : local.embedPassage(text);
    },
    embedBatch: (inputs, options = {}) => {
      const texts = extractTextEmbeddingInputs(inputs);
      return options.inputType === "query"
        ? Promise.all(texts.map((text) => local.embedQuery(text)))
        : local.embedBatch(texts);
    },
    close: () => local.shutdown(),
  };
}
...
/**
 * Build embedding adapters whose provider instances are owned by OpenClaw.
 * @param {object} [pluginConfig] PLUR1BUS plugin configuration.
 * @param {{scopedEmbeddingIpc?: {stateRoot: string, fingerprintId: string}}} [runtime] Optional request-scoped IPC bridge.
 * @returns {object[]} OpenClaw embedding-provider adapters.
 */
export function createOpenClawMemoryEmbeddingProviderAdapters(pluginConfig = {}, runtime = {}) {
      ...
      create: async (options) => {
        ...
        return {
          provider: createLocalE5Provider({
            ...
            scopedEmbeddingIpc: runtime.scopedEmbeddingIpc,
          }),
          runtime: {
            id: "plur1bus-e5-small",
...
/**
 * Register PLUR1BUS embedding providers with OpenClaw when the capability exists.
 * @param {object} api OpenClaw plugin API.
 * @param {object} [pluginConfig] PLUR1BUS plugin configuration.
 * @param {{scopedEmbeddingIpc?: {stateRoot: string, fingerprintId: string}}} [runtime] Optional request-scoped IPC bridge.
 * @returns {object[]} Registered adapters.
 */
export function registerOpenClawMemoryEmbeddingProviders(api, pluginConfig = {}, runtime = {}) {
  if (typeof api.registerEmbeddingProvider !== "function") {
    api.logger?.info?.(
      "memory-lancedb-namespaced: optional OpenClaw registerEmbeddingProvider API unavailable; PLUR1BUS continues with its configured embedding provider."
    );
    return [];
  }
  const adapters = createOpenClawMemoryEmbeddingProviderAdapters(pluginConfig, runtime);
  for (const adapter of adapters) api.registerEmbeddingProvider(adapter);
  return adapters;
}
