...
function normalizeIpcAddress(value) {
  ...
  if (
    ...
    throw new Error("scoped embedding IPC address is invalid");
  }
 ...
 * so no claim listener (a loopback TCP port off Linux) is opened. The token
 * always lives at `resolveScopedEmbeddingIpcPaths(stateRoot).tokenPath`.
...
export function createScopedEmbeddingIpcServer({
  ...
  if (typeof claim !== "boolean") throw new Error("scoped embedding IPC claim must be a boolean");
