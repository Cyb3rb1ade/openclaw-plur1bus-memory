 ...
 * @param {{stateRoot: string, embeddings: object, logger?: object}} options Owner inputs.
 * @returns {{start: () => Promise<void>, shutdown: () => Promise<void>}} Idempotent service resource.
 */
export function createScopedEmbeddingIpcServer({ stateRoot, embeddings, logger = null } = {}) {
  ...
  const processSocket = async (socket) => {
    ...
    const line = await new Promise((resolve, reject) => {
      ...
      const onData = (chunk) => {
        ...
        if (trailing) {
          reject(new Error("scoped embedding connection accepts exactly one request"));
          return;
        }
      ...
      socket.once("end", () => fail(new Error("scoped embedding request ended before a complete frame")));
    });
    ...
    if (
      envelopeKeys.length !== 4
      || !["dimensions", "model", "request", "token"].every((key, index) => envelopeKeys[index] === key)
    ) {
    ...
    startPromise = (async () => {
      if (existsSync(paths.socketPath)) {
        ...
        if (await existingSocketAcceptsConnections(paths.socketPath)) {
          const error = new Error("scoped embedding IPC owner is already active");
 ...
 * @param {{stateRoot: string, model: string, dimensions: number}} cfg Scoped provider configuration.
 */
export class IpcScopedEmbeddingProvider {
  ...
  async _request(operation, texts) {
    ...
    if (this._ownerToken && !tokensEqual(this._ownerToken, token)) throw ownerChangedError();
    this._ownerToken ||= token;
