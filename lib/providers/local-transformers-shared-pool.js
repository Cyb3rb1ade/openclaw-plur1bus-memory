...
function modelIdentityDigest(key) {
  return createHash("sha256").update(key).digest("hex").slice(0, 16);
}

function ownerUnavailableError(key) {
  const requestedIdentityDigest = modelIdentityDigest(key);
  const activeOwnerIdentityDigests = [...poolState().entries.values()]
    .filter((entry) => !entry.closing && entry.owners.size > 0)
    .map((entry) => modelIdentityDigest(entry.key))
    .sort();
  const error = new Error(
    "shared local model runtime has no activated full-runtime owner "
    + `(requested=${requestedIdentityDigest}; activeOwners=${activeOwnerIdentityDigests.join(",") || "none"})`,
  );
  error.code = "shared_local_model_owner_unavailable";
  error.requestedIdentityDigest = requestedIdentityDigest;
  error.activeOwnerIdentityDigests = activeOwnerIdentityDigests;
  return error;
}
...
export function createSharedLocalModelLease({
  ...
  if (!owner && requireOwner) {
    ...
    if (candidate && !candidate.closing && candidate.owners.size > 0) {
      ...
      invalidated = true;
      admissionError = ownerUnavailableError(key);
    }
  }
  ...
  const attach = async () => {
    ...
    while (true) {
      ...
      if (!candidate) {
        if (!owner && requireOwner) {
          invalidated = true;
          throw ownerUnavailableError(key);
        }
