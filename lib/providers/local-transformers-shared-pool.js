 ...
 * @param {{key: string, owner?: boolean, requireOwner?: boolean}} options Immutable model identity and lifecycle role.
 ...
 */
export function createSharedLocalModelLease({ key, owner = false, requireOwner = false } = {}) {
  ...
  const attach = async () => {
    ...
    while (true) {
      ...
      if (!candidate) {
        if (!owner && requireOwner) throw ownerUnavailableError();
