...
// Sent with queries only, never with stored passages. Wording follows the
// Qwen3-Embedding convention: state the task, not the question.
const DEFAULT_QUERY_INSTRUCTION =
  "Given a question about stored personal memories, retrieve the memory that answers it";
...
export class OpenAIEmbeddingProvider {
  constructor(cfg = {}) {
    ...
    // Qwen3-Embedding is trained asymmetrically: questions carry an
    // instruction, stored passages do not. Sending queries bare makes the model
    // embed them as if they were more conversation, so a rare decisive term
    // ("Sklerose", 7 of 13104 rows) is averaged away by whatever name dominates
    // the sentence. Measured on real memories, this one field moved the
    // answering document from rank 347 to rank 1.
    this.queryInstruction = cfg.queryInstruction === undefined
      ? DEFAULT_QUERY_INSTRUCTION
      : cfg.queryInstruction;
  ...
  _resolveApiKey() {
    if (this.apiKeyEnv) return process.env[this.apiKeyEnv] || undefined;
  ...
  _buildRequest(model, textOrTexts, purpose = "passage") {
    const req = { model, input: textOrTexts, encoding_format: "float" };
    if (isOpenAiEmbeddingModel(model) && this.dim) req.dimensions = this.dim;
    // Servers that do not know the field ignore it; OpenAI itself is excluded
    // because its models are symmetric and would only see an unknown argument.
    if (purpose === "query" && this.queryInstruction && !isOpenAiEmbeddingModel(model)) {
      req.instruction = this.queryInstruction;
    }
    return req;
  }
  ...
  async _computeBatch(texts, retries = 3, purpose = "passage") {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        const response = await client.embeddings.create(this._buildRequest(this.model, input, purpose));
    ...
    try {
      ...
      for (let i = 0; i < input.length; i++) {
        const singleResponse = await client.embeddings.create(this._buildRequest(this.model, input[i], purpose));
  ...
  async embedBatch(texts, retries = 3, options = {}) {
    ...
    if (!this._cache) {
      return this._computeBatch(input, retries, purpose);
    }
    ...
    const cacheOptions = {
      ...
      // Without this a question and a stored passage with the same text share
      // a cache entry, and whichever ran first wins.
      purpose,
    };

    return this._cache.getMany(input, cacheOptions, (missing) => this._computeBatch(missing, retries, purpose));
  }

  async embedRaw(text, purpose = "passage", retries = 3, options = {}) {
    const [vector] = await this.embedBatch([text], retries, { ...options, purpose });
    return vector;
  }
