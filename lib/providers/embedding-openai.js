...
export class OpenAIEmbeddingProvider {
  constructor(cfg = {}) {
    ...
    this.requestTimeoutMs = Number.isFinite(Number(cfg.requestTimeoutMs)) && Number(cfg.requestTimeoutMs) >= 1000
      ? Math.floor(Number(cfg.requestTimeoutMs))
      : 15_000;
  ...
  async _clientForPrimary() {
    ...
    if (!this._client) {
      ...
      this._client = new OpenAI(this._clientOptions(apiKey));
    }
    return this._client;
  }

  /**
   * Client options for the OpenAI SDK. The SDK's own default is a ten-minute
   * timeout with two retries; the provider retries batches itself, so the SDK
   * gets a bounded per-request timeout and no retries of its own.
   */
  ...
  async _computeBatch(texts, retries = 3, signal = null) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        const response = await client.embeddings.create(this._buildRequest(this.model, input), requestOptions);
        ...
        await raceAbort(new Promise((r) => setTimeout(r, delay)), signal, "embedding aborted");
      }
    }
    ...
    try {
      ...
      for (let i = 0; i < input.length; i++) {
        const singleResponse = await client.embeddings.create(this._buildRequest(this.model, input[i]), requestOptions);
  ...
  async embedBatch(texts, retries = 3, options = {}) {
    throwIfAborted(options.signal, "embedding aborted");
    const input = Array.isArray(texts) ? texts : [texts];
    if (input.length === 0) return [];

    if (!this._cache) {
      // No cache to share: this call is the only consumer of the compute, so
      // the SDK-level request itself can honour the caller's signal directly.
      return this._computeBatch(input, retries, options.signal);
    }
    ...
    // `getMany` coalesces identical in-flight cache keys into one shared
    // compute promise. Passing this caller's signal into `_computeBatch`
    // would let one caller's abort cancel the OpenAI request for every other
    // concurrent caller waiting on the same key (PR-05 fix round 1) — so the
    // signal only bounds how long *this* call waits for the shared result.
    return raceAbort(
      this._cache.getMany(input, cacheOptions, (missing) => this._computeBatch(missing, retries)),
      options.signal,
      "embedding aborted",
    );
  }
