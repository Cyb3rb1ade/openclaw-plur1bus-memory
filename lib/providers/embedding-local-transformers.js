...
export class LocalTransformersEmbeddingProvider {
  ...
  async _embedBatchForPurpose(texts, purpose = "passage", options = {}) {
    ...
    // `finish()` must not fire while `work` (the cache fetch or the raw
    // compute) is still running: raceAbort settling first on an abort would
    // otherwise let `_activeOperations` hit 0 and a concurrent `shutdown()`
    // dispose the pipeline out from under a live inference call (PR-05 fix
    // round 1). Once `work` exists, its own `.finally(finish)` owns the call;
    // the outer `finally` below only covers the early-throw paths above it.
    ...
    try {
      ...
      const work = this._cache
        ? this._cache.getMany(input, {
            provider: this.id,
            model: `${this.model}:${purpose}`,
            dimensions: this.dim,
            agentId: options.agentId,
          }, (missing) => this._computeBatch(missing, purpose))
        : this._computeBatch(input, purpose);
      workStarted = true;
      ...
      return await raceAbort(work, options.signal, "embedding aborted");
    } finally {
