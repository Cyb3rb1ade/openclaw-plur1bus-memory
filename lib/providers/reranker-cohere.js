...
import { rerankSignal } from "./rerank-signal.js";
...
export class CohereRerankerProvider {
   ...
   * @param {number} topN
   * @param {{ signal?: AbortSignal }} [options] When `signal` is given (the
   *   normal case: the recall pipeline always supplies one via
   *   `rerankSignal`), this provider drives its HTTP request on that signal
   *   alone — `this.timeoutMs` is never consulted, even if it is a positive
   *   number. This is accepted: the caller (the pipeline, or the outer
   *   scheduler budget when the pipeline itself has `rerankerTimeoutMs: 0`)
   *   already owns exactly one timer for this rerank; a second one here
   *   would reintroduce the two-timers-race this class exists to avoid.
   *   `this.timeoutMs` only builds a timer of its own when called directly
   *   with no `signal` at all (e.g. this provider's own unit tests).
