...
// No tensor disposal here, deliberately. `modelInputs` and `outputs` look like
// candidates for a try/finally that calls dispose(), but under onnxruntime-node
// these are CPU tensors, and a CPU tensor carries no disposer: the runtime sets
// one only for GPU textures, GPU buffers and ML tensors. dispose() would just
// null a JavaScript typed array that goes out of scope one line later anyway.
// The library's own TextClassificationPipeline._call does exactly the same and
// disposes nothing. What does hold native memory is the inference session, and
// that is released through the pipeline's dispose() when the reranker shuts
// down. If this ever runs on a GPU execution provider, the picture changes:
// then the outputs do carry a disposer and this function needs a try/finally
// that disposes both modelInputs and outputs.
...
export class LocalTransformersRerankerProvider {
  ...
  async rerank(query, documents, topN, { signal } = {}) {
    ...
    // An already-aborted call never starts scoring at all — zero calls to
    // `_beginOperation()`/the classifier, not just a fast fallback.
    throwIfAborted(signal, "rerank aborted");
    const finish = this._beginOperation();
    // `finish()` must not fire while `work` (the actual model scoring) is
    // still running: raceAbort settling first on an abort would otherwise let
    // `_activeOperations` hit 0 and a concurrent `shutdown()` dispose the
    // pipeline out from under a live scoring call (mirrors PR-05 fix round 1
    // in embedding-local-transformers.js). Once `work` exists, its own
    // `.finally(finish)` owns the call; the outer `finally` below only covers
    // the early-throw path above it (`_getPipeline()` failing or being
    // aborted while the shared pipeline promise itself keeps loading in the
    // background for the next caller).
    let workStarted = false;
    try {
      const classifier = await raceAbort(this._getPipeline(), signal, "rerank aborted");
      const work = (async () => {
        if (typeof classifier?.tokenizer === "function" && typeof classifier?.model === "function") {
          return await scoresFromSequenceClassifier(classifier, query, documents);
        }
        const pairs = documents.map((document) => ({ text: query, text_pair: document }));
        try {
          return scoresFromBatchOutput(await classifier(pairs), documents.length);
        } catch (err) {
          safeDebug(this.logger, "local-transformers-reranker.batch-fallback", err, { count: documents.length });
          const fallbackScores = [];
          for (const document of documents) {
            fallbackScores.push(await this._scorePair(classifier, query, document));
          }
          return fallbackScores;
        }
      })();
      workStarted = true;
      // This chain exists only to run `finish()` once `work` settles; its own
      // rejection is not silently dropped — it is the exact same rejection
      // `raceAbort(work, ...)` below already awaits and surfaces to the
      // caller. The debug log here only records that this second promise
      // chain (not `work` itself) also observed the settlement, per AGENTS.md's
      // no-silent-catch rule.
      work.finally(finish).catch((error) => {
        safeDebug(this.logger, "local-transformers-reranker.work-settle-after-abort", error);
      });
      const scores = await raceAbort(work, signal, "rerank aborted");
