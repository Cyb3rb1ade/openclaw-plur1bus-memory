...
export class LocalTransformersRerankerProvider {
  ...
  async rerank(query, documents, topN, { signal } = {}) {
    ...
    // the early-throw path above it (`_getPipeline()` failing).
    ...
    try {
      ...
      // caller. The `.catch(() => {})` here only stops this second promise
      // chain (not `work` itself) from also reporting an unhandled rejection.
      work.finally(finish).catch(() => {});
