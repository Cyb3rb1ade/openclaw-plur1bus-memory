 ...
 * @param {{runtimeLlm?: object, signal?: AbortSignal}} [options]
 * @returns {Object}
 */
...
/**
 * Add an explicit cache scope plus matching agent and purpose call context.
 * @param {Object} llmCfg
...
export function withLlmResultCacheContext(llmCfg, scopeId, purpose) {
  const existingCallContext = llmCfg?.callContext || {};
  const runtimeLlm = typeof existingCallContext.runtimeLlm?.complete === "function"
    ? existingCallContext.runtimeLlm
    : undefined;
  const hasExplicitAgent = Boolean(existingCallContext.agentId);
  return withLlmCallContext(
    { ...llmCfg, resultCacheContext: { scopeId, purpose } },
    runtimeLlm && !hasExplicitAgent ? undefined : scopeId,
    purpose,
    {
      runtimeLlm,
      signal: existingCallContext.signal,
    },
  );
}
