...
export function createLlmResultCache({
  ...
  function recordHitUsage(scopeId, value) {
    ...
    increment(scopeId, "avoidedInputTokens", inputTokens);
    increment(scopeId, "avoidedOutputTokens", outputTokens);
  }
  ...
  async function openDb(scopeId) {
    ...
    try {
      ...
      dbPath = resolveInside(baseDbPath, CACHE_VERSION, `${agentId}.db`);
  ...
  function deleteExpired(database, timestamp) {
    database.prepare("DELETE FROM llm_results WHERE expires_at <= ?").run(timestamp);
  }

  function trimToSize(handle, targetBytes, timestamp) {
    const { database, dbPath } = handle;
    deleteExpired(database, timestamp);
    while (databaseSize(dbPath) > targetBytes) {
      ...
      `).get();
      if (!oldest) break;
      database.prepare("DELETE FROM llm_results WHERE key_hash = ?").run(oldest.key_hash);
  ...
  async function setPersistent(keyHash, request, value, createdAt, expiresAt) {
    ...
    const { database, dbPath } = handle;
    try {
      deleteExpired(database, createdAt);
      if (databaseSize(dbPath) >= normalizedMaxBytes) {
      ...
      if (databaseSize(dbPath) > softTarget) {
        trimToSize(handle, softTarget, createdAt);
      }
    } catch (error) {
