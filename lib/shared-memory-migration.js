...
class MigrationOperationStop extends Error {
  constructor(label, reason, { commitUncertain = false, commitCompleted = false } = {}) {
    super(`migration ${reason} during ${label}`);
    this.name = "MigrationOperationStop";
    this.label = label;
    this.reason = reason;
    this.commitUncertain = commitUncertain;
    this.commitCompleted = commitCompleted;
  }
}
 ...
 */
export async function ensureLegacySourceMarkerColumn(sourceDb, { runDbOperation = null } = {}) {
  ...
  let fields = fieldMap(await run("source-marker-schema-read", () => sourceDb.table.schema()));
  ...
  if (existing) {
    ...
    await run("source-marker-schema-refresh", () => sourceDb.refreshSchemaFields());
    return;
  }
  try {
    await run("source-marker-schema-add", () =>
      sourceDb.table.addColumns([{
        name: "legacyShareMigrationMarker",
        type: textField.type,
        valueSql: "'{}'",
      }]));
  } catch (error) {
    fields = fieldMap(await run("source-marker-schema-race-read", () => sourceDb.table.schema()));
    const raced = fields.get("legacyShareMigrationMarker");
    if (!raced || !sameDataType(raced.type, textField.type)) throw error;
  }
  fields = fieldMap(await run("source-marker-schema-verify", () => sourceDb.table.schema()));
...
async function verifySharedMarker(sharedPool, ctx, marker, runDbOperation) {
  return sharedPool.withWorkspaceDb(ctx, async (targetDb) => {
    await runDbOperation("target-marker-init", () => targetDb.init());
    const rows = await runDbOperation("target-marker-readback", () =>
...
async function runBoundedDbOperation({
  label,
  operation,
  deadline,
  now,
  signal,
  logger,
  commitMayComplete = false,
}) {
  if (signal?.aborted) throw new MigrationOperationStop(label, "aborted");
  const timeoutMs = remaining(deadline, now);
  if (timeoutMs <= 0) throw new MigrationOperationStop(label, "deadline");
  let started = false;
  let detached = false;
  const raw = Promise.resolve().then(() => {
    if (signal?.aborted) throw new MigrationOperationStop(label, "aborted");
    if (remaining(deadline, now) <= 0) throw new MigrationOperationStop(label, "deadline");
    started = true;
    return operation();
  });
  raw.then(
    ...
    (error) => {
      if (detached) {
        trySafeWarn(logger, "legacy-shared-migration.late-db", error, { operation: label });
      }
    },
  );
  let timer = null;
  ...
  const barrier = new Promise((_, reject) => {
    timer = setTimeout(() => {
      barrierError = new MigrationOperationStop(label, "deadline", {
        commitUncertain: commitMayComplete && started,
      });
      reject(barrierError);
    }, Math.max(1, timeoutMs));
    if (signal) {
      abortHandler = () => {
        barrierError = new MigrationOperationStop(label, "aborted", {
          commitUncertain: commitMayComplete && started,
        });
        reject(barrierError);
      };
  ...
  let result;
  try {
    result = await Promise.race([raw, barrier]);
  } catch (error) {
    if (error === barrierError) detached = true;
    throw error;
  } finally {
  ...
  if (signal?.aborted) {
    throw new MigrationOperationStop(label, "aborted", {
      commitCompleted: commitMayComplete,
    });
  }
  if (remaining(deadline, now) <= 0) {
    throw new MigrationOperationStop(label, "deadline", {
      commitCompleted: commitMayComplete,
    });
  }
  return result;
...
export async function migrateLegacySharedRows({
  ...
  now = Date.now,
  logger = null,
  writeReport = writeLegacyRepairReport,
}) {
  ...
  if (typeof writeReport !== "function") throw new Error("migration report writer is required");
  ...
  const runDbOperation = (label, operation, options = {}) => runBoundedDbOperation({
    label,
    operation,
    deadline,
    now,
  ...
  const scan = async (sourceDb, writerDb = null) => {
    ...
    const initialized = await runDbOperation("source-read-init", () => sourceDb.init());
    if (initialized === false || !sourceDb.table) {
      sourceAvailable = false;
      return;
    }
    if (sourceVersion === null) {
      sourceVersion = await runDbOperation("source-version-read", () => sourceDb.table.version());
      if (!Number.isSafeInteger(sourceVersion) || sourceVersion < 0) {
        throw new Error("authoritative source version is invalid");
      }
    }
    try {
      await runDbOperation("source-version-checkout", () => sourceDb.table.checkout(sourceVersion));
    } catch (error) {
      if (error instanceof MigrationOperationStop) throw error;
      throw new Error("pinned source version unavailable; restart without a cursor", { cause: error });
    }
    const schemaFields = fieldMap(await runDbOperation(
      "source-pinned-schema-read",
    ...
    let page;
    try {
      page = await runDbOperation("source-pinned-query", () =>
        executePinnedPage(
          sourceDb.table,
          selectedColumns,
          startOffset,
      ...
      stoppedOperation = error instanceof MigrationOperationStop ? error.label : null;
      incomplete = true;
      return;
    }
    ...
    for (const snapshot of candidates) {
      ...
      let current;
      try {
        current = await runDbOperation(
          "source-current-read",
          () => getCurrentSourceRow(writerDb, sourceId),
        );
      } catch (error) {
        operationalError = error;
        stoppedOperation = error instanceof MigrationOperationStop ? error.label : null;
      ...
      if (existingMarker) {
        ...
        let verified;
        try {
          verified = await verifySharedMarker(
            sharedPool,
            targetCtx,
            existingMarker,
            runDbOperation,
      ...
      let beforeCopy;
      try {
        beforeCopy = await runDbOperation(
          "source-current-reread",
          () => getCurrentSourceRow(writerDb, sourceId),
        );
      } catch (error) {
        operationalError = error;
      ...
      try {
        const sharedResult = await sharedPool.withWorkspaceDb(targetCtx, async (targetDb) => {
          await runDbOperation("target-init", () => targetDb.init());
        ...
        await runDbOperation(
          "source-marker-update",
          () => writerDb.update(sourceId, { legacyShareMigrationMarker: marker }),
          { commitMayComplete: true },
        );
        counts.migrated += 1;
        counts.terminallyConsumedRows += 1;
      } catch (error) {
        operationalError = error;
        stoppedOperation = error instanceof MigrationOperationStop ? error.label : null;
        if (error instanceof MigrationOperationStop
          && (error.commitUncertain || error.commitCompleted)
          && error.label === "source-marker-update") {
          counts.uncertainSourceMarkerWrites += 1;
        }
        incomplete = true;
        break;
      }
  ...
  if (!incomplete) {
    try {
      if (apply) {
        ...
        const initialized = await runDbOperation("source-preflight-init", () => probeDb.init());
        ...
        if (sourceVersion === null) {
          sourceVersion = await runDbOperation("source-preflight-version", () => table.version());
          if (!Number.isSafeInteger(sourceVersion) || sourceVersion < 0) {
            throw new Error("authoritative source version is invalid");
          }
        }
        try {
          await runDbOperation("source-preflight-checkout", () => table.checkout(sourceVersion));
        } catch (error) {
          if (error instanceof MigrationOperationStop) throw error;
          throw new Error("pinned source version unavailable; restart without a cursor", { cause: error });
        }
        ...
        if (!exists) sourceAvailable = false;
          ...
          await runDbOperation("source-writer-init", () => writerDb.init());
          if (!writerDb.table) {
            sourceAvailable = false;
            return;
          }
          ...
          await ensureLegacySourceMarkerColumn(writerDb, { runDbOperation });
          if (stopped(signal, deadline, now)) {
            incomplete = true;
            return;
          }
        ...
        await privatePool.withAuthoritativeReadDb(
          sourceAgentId,
          (sourceDb) => scan(sourceDb),
        );
      }
    } catch (error) {
      if (!(error instanceof MigrationOperationStop)) throw error;
      operationalError = error;
      stoppedOperation = error.label;
      if ((error.commitUncertain || error.commitCompleted)
        && error.label === "source-marker-update") {
        counts.uncertainSourceMarkerWrites += 1;
      }
      incomplete = true;
  ...
  return Object.freeze({
    ...
    nextOffset,
    stoppedOperation,
    uncertainSourceMarkerWrites: counts.uncertainSourceMarkerWrites,
    reportPath,
  });
}
