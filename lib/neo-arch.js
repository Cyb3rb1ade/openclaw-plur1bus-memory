...
function drainEmbeddingQueueFile(paths, options = {}) {
  ...
  const rewritten = queue.map((item) => {
    if (item?.__parseError) {
      parseErrors++;
      return item.__raw ? { status: "invalid", raw: item.__raw, error: item.__parseError, processedAt: nowIsoStr } : item;
    }
    if (processed >= maxItems || !embeddingQueueItemMatches(item, options)) return item;
  ...
  if (processed > 0 || missingTargets > 0 || parseErrors > 0) {
    ...
    writeJsonlAtomic(paths.embeddings, rewritten);
  }

  const pending = rewritten.filter(item => item && !item.__parseError && item.status === "pending").length;
  return {
    processed,
    pending,
