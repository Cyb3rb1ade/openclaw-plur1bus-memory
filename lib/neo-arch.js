...
export function formatNeoRecallContext(lanes, opts = {}) {
  ...
  for (const [lane, rows] of Object.entries(lanes || {})) {
    for (const row of rows || []) {
      ...
      const line = `  <memory-record lane="${safeLane}" category="${safeCategory}" trust="${safeTrust}" status="${safeStatus}" id="${safeId}" score="${row.score.toFixed(2)}"><quoted-evidence>${text}</quoted-evidence></memory-record>`;
