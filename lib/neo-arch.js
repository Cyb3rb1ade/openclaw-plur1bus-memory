...
const NEO_EMBEDDING_BUDGET_STOP = "NEO_EMBEDDING_BUDGET_STOP";

function embeddingBudgetStopError() {
  const error = new Error("Neo embedding drain budget exhausted");
  error.name = "AbortError";
  error.code = NEO_EMBEDDING_BUDGET_STOP;
  return error;
}

async function awaitEmbeddingWithinBudget(embedder, text, target, options = {}) {
  const deadlineAt = Number(options.deadlineAt);
  const hasDeadline = Number.isFinite(deadlineAt);
  if (options.signal?.aborted || (hasDeadline && Date.now() >= deadlineAt)) {
    throw embeddingBudgetStopError();
  }

  // Promise.race observes both late resolution and rejection. This lets the
  // maintenance drain release the capture slot at its budget boundary without
  // producing an unhandled rejection when an abort-ignoring provider settles.
  const rawEmbedding = Promise.resolve().then(() => embedder(text, target));
  let timer = null;
  let abortListener = null;
  const stop = new Promise((_, reject) => {
    if (hasDeadline) {
      timer = setTimeout(
        () => reject(embeddingBudgetStopError()),
        Math.max(0, deadlineAt - Date.now()),
      );
    }
    if (typeof options.signal?.addEventListener === "function") {
      abortListener = () => reject(embeddingBudgetStopError());
      options.signal.addEventListener("abort", abortListener, { once: true });
    }
  });

  try {
    const embedding = await Promise.race([rawEmbedding, stop]);
    if (options.signal?.aborted || (hasDeadline && Date.now() >= deadlineAt)) {
      throw embeddingBudgetStopError();
    }
    return embedding;
...
async function markEmbeddingTargetFresh(paths, queueItem, nowIsoStr, targetState = null, options = {}) {
  ...
  let embedding;
  try {
    embedding = await awaitEmbeddingWithinBudget(
      options.embedder,
      target.statement || target.content || "",
      target,
      options,
    );
  } catch (error) {
    if (error?.code === NEO_EMBEDDING_BUDGET_STOP) {
      return { ok: false, reason: "budget_exhausted", stoppedEarly: true };
    }
...
async function drainEmbeddingQueueFile(paths, options = {}) {
  ...
  const drainOptions = deadlineAt === null ? options : { ...options, deadlineAt };
  ...
  for (const { key, item } of latestEntries) {
    ...
    const target = await markEmbeddingTargetFresh(paths, item, nowIsoStr, targetState, drainOptions);
    if (!target.ok) {
      if (target.stoppedEarly) {
        stoppedEarly = true;
        break;
      }
