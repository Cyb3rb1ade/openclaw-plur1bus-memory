...
export function searchNeoCandidatesGlobal(store, options = {}) {
  ...
  const result = { hits: [], scanned: 0, unique: 0, eligible: 0, withVector: 0, ms: 0 };
  ...
  for (const [id, record] of latestById) {
    ...
    if (record[NEO_LAZY_VECTOR]) lazyIds.push(id);
  }
  ...
  for (const record of eligible) {
    const id = String(record.id);
    const typedVector = record[NEO_LAZY_VECTOR] ? batch.get(id) : null;
    const inlineVector = typedVector ? null : record.embedding;
    let similarity;
    if (typedVector) {
  ...
  result.hits = scored.slice(0, topK).map((hit) => {
    // Nur Treffer bekommen den Vektor als plain Array auf den Datensatz, damit
    // das Lane-Scoring ihn wiederverwendet.
    if (hit.typedVector) hit.item.embedding = Array.from(hit.typedVector);
    return { item: hit.item, similarity: hit.similarity, score: hit.score, ageDays: hit.ageDays };
  });
