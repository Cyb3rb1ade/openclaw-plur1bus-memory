...
export function turnEventsFromMessages(messages = [], params = {}) {
  ...
  for (const msg of messages) {
    ...
    const sourceToolCallIds = msg.tool_call_id ? [msg.tool_call_id] : [];
...
export function routeNeoRecall(items = [], query, opts = {}) {
  ...
  uniqueItems.forEach((item, itemIndex) => {
    lanes.forEach((lane, laneIndex) => {
      const score = scoreNeoRecallItem(item, query, lane);
...
export function createNeoStore(rootDir, workspaceKey = "default") {
  ...
  return {
    paths,
    appendTurns: (items) => appendJsonlDedupe(paths.turns, items, { indexPath: paths.index, bucket: "turns" }),
    appendCandidates: (items) => appendJsonlDedupe(paths.candidates, items, { indexPath: paths.index, bucket: "candidates" }),
    appendReactions: (items) => appendJsonlDedupe(paths.reactions, items, { indexPath: paths.index, bucket: "reactions" }),
    appendBehaviorCards: (items) => appendJsonlDedupe(paths.behavior, items, { indexPath: paths.index, bucket: "behavior" }),
    appendEmbeddingQueue: (items) => appendEmbeddingQueueDedupe(paths.embeddings, items.map(item => {
    ...
    drainEmbeddingQueue: (opts = {}) => drainEmbeddingQueueFile(paths, opts),
    appendDreams: (items) => appendJsonl(paths.dreams, items),
    appendEpisodes: (items) => appendJsonl(paths.episodes, items),
    appendGraphEdges: (items) => appendJsonl(paths.graph, items),
    appendPatterns: (items) => appendJsonl(paths.patterns, items),
    appendRetrievalLedger: (items) => appendJsonl(paths.retrievalLedger, items),
    appendReconsolidationEvents: (items) => appendJsonl(paths.reconsolidationEvents, items),
    ...
    readDreams: (limit = 100) => readJsonlTail(paths.dreams, limit),
    readEpisodes: (limit = 100) => readJsonlTail(paths.episodes, limit),
    readGraphEdges: (limit = 10_000) => readJsonlTail(paths.graph, limit),
    readPatterns: (limit = 500) => readJsonlTail(paths.patterns, limit),
    ...
    pruneAll: (opts = {}) => {
      ...
      for (const [name, p] of Object.entries(targets)) {
        ...
        report[name] = pruneNeoJsonlFile(p, { ...opts, dedup: opts.dedup !== false && name !== "embeddings" && name !== "retrievalLedger" && name !== "reconsolidationEvents" });
      }
      return report;
    },
...
function normalizeRecordIndex(raw = {}) {
  ...
  const normalizedIds = {};
  for (const bucket of ["turns", "candidates", "reactions", "behavior"]) {
...
function markEmbeddingTargetFresh(paths, queueItem, nowIsoStr, targetState = null) {
...
function drainEmbeddingQueueFile(paths, options = {}) {
  ...
  for (const { key, item } of latestEntries) {
    ...
    const target = markEmbeddingTargetFresh(paths, item, nowIsoStr, targetState);
    if (!target.ok) {
      missingTargets++;
      const skipped = {
        ...item,
  ...
  if (processed > 0 || missingTargets > 0 || parseErrors > 0) {
