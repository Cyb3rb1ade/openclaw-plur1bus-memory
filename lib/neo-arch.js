...
import { appendFileSync, closeSync, existsSync, fstatSync, mkdirSync, openSync, readFileSync, readSync, readdirSync, realpathSync, renameSync, rmdirSync, statSync, unlinkSync, writeFileSync, writeSync } from "node:fs";
...
const INJECTED_QUICK_MARKERS = [
  ...
  "</knowledge-update-reminder",
  // 7.12.24: Die Reminder-Nudge zitiert den Reminder-Text samt Zeitfloskel;
  // landet sie im Capture, parst der Reminder-Pfad sie erneut (Schleife
  // "in 1 minute" alle paar Minuten, bernhardine 09.09.2026).
  "<reminder-nudge",
  "</reminder-nudge",
  "<adaptive-learning",
  "</adaptive-learning",
 ...
 */
const INJECTED_HEADER_RE = /^(?:<<<)?begin_openclaw_internal_context\b|^\[subagent context\]|^\[inter-session message\]/i;

function lineHasInjectedHeader(line) {
  const trimmed = String(line || "").trim();
  if (!trimmed) return false;
  if (INJECTED_HEADER_RE.test(trimmed)) return true;
  const withoutTimestamp = trimmed.replace(/^\[[^\]]{0,80}\]\s+/, "");
  return withoutTimestamp !== trimmed && INJECTED_HEADER_RE.test(withoutTimestamp);
}

export function isInjectedContextText(text) {
  const s = String(text || "");
  if (s.length === 0) return false;
  if (s.split("\n").some(lineHasInjectedHeader)) return true;
...
export function turnEventsFromMessages(messages = [], params = {}) {
  ...
  // Turn-Identitaet muss ueber Laeufe hinweg stabil sein: der Host reicht bei
  // jedem agent_end den ganzen Verlauf herein, und der Hook journaliert ihn
  // komplett neu, wenn sich auch nur ein Bestandteil der ID aendert. Bis
  // 7.12.19 standen `sessionId` (wechselt beim Host pro Lauf/Kompaktierung)
  // und `turnIndex` (verschiebt sich bei jeder Kompaktierung) in der ID —
  // Bernhardines Journal hatte 5273 Eintraege mit 258 verschiedenen Inhalten,
  // und jeder Turn stellte ~240 Embeddings neu in die Warteschlange. Jetzt:
  // stabiler Sitzungsschluessel, Rolle, normierter Inhalt, Tool-Call-IDs und
  // die Ordnungszahl gleicher Nachrichten im Verlauf ("ok" bleibt zweimal).
  const identityKey = String(params.sessionKey || params.sessionId || "");
  ...
  for (const msg of messages) {
    ...
    const statement = normalizeStatement(content);
    const ordinalKey = `${role}\n${statement}\n${sourceToolCallIds.join(",")}`;
    const ordinal = identityOrdinals.get(ordinalKey) || 0;
    identityOrdinals.set(ordinalKey, ordinal + 1);
...
/**
 * Stamp visibility on a new derived record. Existing stamps win. No invent
 * when ownership cannot be inferred.
 * @param {object} record
 * @param {object} [binding]
 * @returns {object}
 */
export function stampDerivedVisibility(record, binding = {}) {
  if (!record || typeof record !== "object") return record;
  if (record.visibility?.scope) return record;
  const agentId = binding.agentId || record.agentId || record.aclBindings?.agentId || "";
  const workspaceIdentity = binding.workspaceIdentity
    || record.workspaceKey
    || record.aclBindings?.workspaceIdentity
    || "";
  const ownerUserId = binding.ownerUserId || record.ownerUserId || record.aclBindings?.ownerUserId || "";
  if (!agentId && !workspaceIdentity && !ownerUserId) return record;
  return {
    ...record,
    visibility: {
      scope: normalizeNeoScope(binding.scope || record.aclBindings?.scope, workspaceIdentity ? "workspace_shared" : "agent_private"),
      agentId,
      workspaceIdentity,
      ownerUserId,
    },
...
function stampDerivedList(items, binding) {
  return (Array.isArray(items) ? items : []).map((item) => stampDerivedVisibility(item, binding));
}

/**
 * ACL for dreams/episodes/edges/patterns. Legacy rows without visibility
 * are visible only to the owning agent.
 * @param {object} record
 * @param {object} requester
 * @returns {boolean}
 */
export function isDerivedRecordAccessible(record, requester = {}) {
  const requesterAgent = requester.requesterAgentId || requester.agentId;
  ...
  if (record?.visibility?.scope) {
    return isNeoRecordAccessible({
      ...record,
      visibility: record.visibility,
      origin: { ...(record.origin || {}), scope: record.visibility.scope },
      agentId: record.visibility.agentId || record.agentId,
      workspaceKey: record.visibility.workspaceIdentity || record.workspaceKey,
    }, {
      requesterAgentId: requesterAgent,
      requesterWorkspaceKey: requester.workspaceIdentity || requester.requesterWorkspaceKey,
      requesterOwnerId: requester.ownerUserId || requester.requesterOwnerId,
    });
  }
  const owner = record?.agentId || record?.visibility?.agentId;
  return Boolean(requesterAgent && owner && requesterAgent === owner);
}
...
export function scoreNeoRecallItem(item, query, lane = "workspace_facts", opts = {}) {
  ...
  // Hard exclusion — an invalidated NEO record must never surface via recall,
  // same rule as the LanceDB layer (see plan §6a/§6b). epistemicStatus is a
  // top-level field on NEO records, distinct from origin.trustLevel below.
  if (normalizeEpistemicStatus(item.epistemicStatus) === "invalidated") return -Infinity;
  // conflict is not a hard filter: the detector is an unvalidated LLM, 4017
  // newest-revision records on the 2026-08-16 live probe carried conflict
  // (2505 on one agent) with no resolve path that clears the status, and a
  // 20-row sample was not pairwise contradiction. Atlas gap left open on purpose.
  ...
  // Soft epistemic boost — uses epistemicScoreBoost() (absent/unrecognized ->
  // neutral 0), not normalizeEpistemicStatus(), so a legacy NEO record's
  // score is unaffected (see plan §4e, same split as the LanceDB layer).
  const epistemicBoost = epistemicScoreBoost(item.epistemicStatus);
  const penalties =
  ...
  return semantic + (Number.isFinite(vectorScore) ? vectorScore * 0.75 : 0) + categoryBoost + trustBoost + curationBoost + epistemicBoost + salience + recency - penalties;
}

/**
 * Schreibzeitpunkt einer Neo-Record-Revision in Epoch-ms.
 *
 * Bewusst `updatedAt` zuerst — das setzt `transitionRecordStatus` bei jedem
 * Statuswechsel, während `createdAt` über alle Revisionen gleich bleibt. Der
 * bestehende `recordTimeMs` taugt hier nicht: er liest nur `startTime`/
 * `createdAt` und könnte zwei Revisionen desselben Records nicht unterscheiden.
 *
 * Nicht datierbare Records liefern `-Infinity`, damit jede datierte Revision
 * sie schlägt und der Vergleich nie an NaN scheitert.
 *
 * @param {Object} record
 * @returns {number}
 */
function neoRevisionTimeMs(record) {
  const raw = record?.updatedAt ?? record?.startTime ?? record?.createdAt;
...
export function routeNeoRecall(items = [], query, opts = {}) {
  ...
  // Dedupe by id keeping the NEWEST revision, not the first one seen.
  //
  // Die JSONL-Stores sind append-only Event-Logs: `transitionRecordStatus`
  // hängt eine neue Zeile unter derselben id an, statt die alte zu ersetzen.
  // Wer hier die erste Fundstelle behält, bewertet die Kopie von VOR der
  // Transition — ein auf `demoted`/`conflict` gesetzter Record würde also mit
  // seinem alten `active`-Status bewertet und die Statusstrafe liefe komplett
  // ins Leere. Genau das war der Fall (siehe
  // tests/neo-status-transition-dedupe.test.js).
  //
  // Die Reihenfolge des ersten Auftretens bleibt erhalten, damit der
  // itemIndex-Tiebreak unten stabil bleibt; nur der Inhalt wird auf die
  // jüngste Revision gehoben.
  const uniqueItems = [];
  ...
  for (const item of items) {
    ...
    if (slot === undefined) {
      slotById.set(id, uniqueItems.length);
      uniqueItems.push(item);
      continue;
    }
    if (neoRevisionTimeMs(item) >= neoRevisionTimeMs(uniqueItems[slot])) {
...
export function formatNeoRecallContext(lanes, opts = {}) {
  ...
  for (const [lane, rows] of Object.entries(lanes || {})) {
    for (const row of rows || []) {
      ...
      // Ohne `status` kann das Modell `active`/`promoted` nicht von `demoted`/
      // `conflict` unterscheiden — obwohl das Memory-Prompt-Supplement genau
      // das verlangt ("use active/promoted BehaviorCards ... only when they do
      // not conflict"). Der Score allein trägt die Information nicht: er ist
      // eine undurchsichtige Summe, aus der sich "war umstritten" nicht ablesen
      // lässt.
      const promptAttrs = renderPromptMemoryAttrs(item);
      const safeStatus = sanitizePromptIdentifier(promptAttrs.status, "active");
      const safeEpistemic = sanitizePromptIdentifier(promptAttrs.epistemic, "untrusted");
      const line = `  <memory-record lane="${safeLane}" category="${safeCategory}" trust="${safeTrust}" status="${safeStatus}" epistemic="${safeEpistemic}" id="${safeId}" score="${row.score.toFixed(2)}"><quoted-evidence>${text}</quoted-evidence></memory-record>`;
 ...
 */
// 7.12.24: capJsonl/pruneAll schreiben `<datei>.<pid>.<ts>.tmp` und benennen
// dann um. Wird der Worker mitten im Rewrite beendet (Deadline, Neustart),
// bleibt die Leiche liegen — bernhardine hatte am 09.09.2026 zwei davon mit
// zusammen 388 MB. Beim naechsten Oeffnen des Stores weg damit, sobald sie
// sicher nicht mehr in Arbeit sind.
const NEO_STALE_TMP_MS = 15 * 60 * 1000;
const NEO_STALE_TMP_RE = /\.\d+\.\d+\.tmp$/;
function cleanupStaleNeoTempFiles(workspaceDir, now = Date.now()) {
  let removed = 0;
  let entries = [];
  try { entries = readdirSync(workspaceDir); } catch (_) { return 0; }
  for (const name of entries) {
    if (!NEO_STALE_TMP_RE.test(name)) continue;
    const full = join(workspaceDir, name);
    try {
      if (now - statSync(full).mtimeMs < NEO_STALE_TMP_MS) continue;
      unlinkSync(full);
      removed++;
    } catch (_) { /* Konkurrent war schneller oder Datei ist weg */ }
  }
  return removed;
}
...
export function createNeoStore(rootDir, workspaceKey = "default") {
  ...
  cleanupStaleNeoTempFiles(workspaceDir);
  ...
  const paths = {
    ...
    // 7.12.26: Vektoren liegen nicht mehr in den JSONL-Zeilen, sondern als
    // rohes Float32 in vectors.<gen>.f32 mit Byte-Offsets in vector-index.json.
    vectorIndex: join(workspaceDir, "vector-index.json"),
    // 7.12.28: Metadatenindex der Kandidaten (juengste Revision je ID, ohne
    // Vektor) fuer die globale Suche; unabhaengig vom 5000-Zeilen-Cap.
    candidateIndex: join(workspaceDir, CANDIDATE_INDEX_FILE),
  };
  ...
  const vectorBearingPaths = new Set([paths.candidates, paths.behavior, paths.turns]);
  const readMerged = (path, legacyPath, limit, requester) => {
    ...
    const records = mergeNeoRecordsById(canonical, legacy)
      .filter(record => !requester || isNeoRecordAccessible(record, requester))
      .slice(-limit);
    if (vectorBearingPaths.has(path)) attachSidecarVectors(paths, records);
    return records;
  };
  ...
  return {
    ...
    appendCandidates: (items) => write(() => {
      // 7.12.28: Der Metadatenindex wird im selben Lock gepflegt. Fehlt er
      // (erster Lauf nach dem Update), entsteht er hier aus dem Journal.
      const legacyCandidatesPath = legacyReadIsSafe ? legacyPaths.candidates : "";
      ensureCandidateIndex(paths, { legacyCandidatesPath });
      const appended = appendJsonlDedupe(paths.candidates, items, { indexPath: paths.index, bucket: "candidateContent", dedupeKey: appendCandidateContentDedupeKey });
      maintainCandidateIndexAfterAppend(paths, appended);
      return appended;
    }),
    ...
    appendDreams: (items, binding) => write(() => appendJsonl(paths.dreams, stampDerivedList(items, binding))),
    appendEpisodes: (items, binding) => write(() => appendJsonl(paths.episodes, stampDerivedList(items, binding))),
    // 7.12.40: Der Episoden-Commit laeuft in der Nachverarbeitung nach dem
    // Turn und konkurriert dort mit dem Embedding-Drain des Workers, der das
    // Lock sekundenlang haelt. Mit dem 5-s-Sync-Lock lief er zweimal am
    // 10.09.2026 in NEO_WRITE_BACKPRESSURE (19:32, 21:07) — die Karte kam
    // erst beim naechsten Turn (Watermark-Retry). Async und mit langer Frist,
    // damit der Event-Loop frei bleibt und die Karte im selben Lauf landet.
    appendEpisodesAsync: (items, binding, { timeoutMs = NEO_LOCK_LONG_ACQUIRE_MS } = {}) =>
      workspaceWriteLockAsync(paths, () => appendJsonl(paths.episodes, stampDerivedList(items, binding)), { timeoutMs }),
    appendGraphEdges: (items, binding) => write(() => appendJsonl(paths.graph, stampDerivedList(items, binding))),
    // 7.12.24: Vollersatz fuer den Graph-Prune (consolidate-daily); Datensaetze
    // kommen woertlich aus readGraphEdges zurueck, ohne neuen Stempel.
    rewriteGraphEdges: (items) => write(() => writeJsonlAtomic(paths.graph, Array.isArray(items) ? items : [])),
    // 7.12.26: Vektor-Sidecar. readVector liefert ein plain Array (float32-
    // exakt) oder undefined; compactVectors schreibt eine neue Generation nur
    // mit Vektoren, deren Datensatz noch in Journal, Kandidaten oder Karten liegt.
    readVector: (id) => readVectorFromSidecar(paths, readVectorIndex(paths), String(id || "")),
    readVectors: (ids) => readVectorsBatch(paths, readVectorIndex(paths), ids),
    vectorStats: () => vectorSidecarStats(paths),
    compactVectors: () => write(() => compactVectorSidecar(paths, liveVectorIds(paths))),
    // 7.12.28: Kandidaten-Metadatenindex. readCandidateIndex ist lockfrei
    // (Prozess-Cache, inkrementeller Tail-Read); Aufbau und Kompaktierung
    // laufen unter dem Write-Lock.
    readCandidateIndex: () => readCandidateIndex(paths),
    ensureCandidateIndex: (opts = {}) => write(() => ensureCandidateIndex(paths, { ...opts, legacyCandidatesPath: legacyReadIsSafe ? legacyPaths.candidates : "" })),
    rebuildCandidateIndex: (opts = {}) => write(() => buildCandidateIndex(paths, { ...opts, legacyCandidatesPath: legacyReadIsSafe ? legacyPaths.candidates : "" })),
    compactCandidateIndex: (opts = {}) => write(() => compactCandidateIndex(paths, opts)),
    candidateIndexStats: () => candidateIndexStats(paths),
    appendPatterns: (items, binding) => write(() => appendJsonl(paths.patterns, stampDerivedList(items, binding))),
    ...
    readDreams: (limit = 100, requester) => readMerged(paths.dreams, legacyPaths.dreams, limit)
      .filter((record) => !requester || isDerivedRecordAccessible(record, requester)),
    // 7.12.40: fortgeschriebene Episoden werden mit derselben id erneut
    // angehaengt — die juengste Fassung gewinnt.
    readEpisodes: (limit = 100, requester) => latestById(readMerged(paths.episodes, legacyPaths.episodes, limit))
      .filter((record) => !requester || isDerivedRecordAccessible(record, requester)),
    readGraphEdges: (limit = 10_000, requester) => readMerged(paths.graph, legacyPaths.graph, limit)
      .filter((record) => !requester || isDerivedRecordAccessible(record, requester)),
    readPatterns: (limit = 500, requester) => readMerged(paths.patterns, legacyPaths.patterns, limit)
      .filter((record) => !requester || isDerivedRecordAccessible(record, requester)),
    ...
    pruneAll: (opts = {}) => {
      ...
      // 7.12.28: Erst den Metadatenindex auf die juengste Revision je ID
      // ziehen, dann den Sidecar — liveVectorIds liest den Index mit.
      report.candidateIndex = workspaceWriteLock(paths, () => compactCandidateIndex(paths, { dryRun: opts.dryRun === true, maxEntries: opts.candidateIndexMaxEntries }), { timeoutMs: NEO_LOCK_LONG_ACQUIRE_MS });
      if (!opts.dryRun) {
        report.vectors = workspaceWriteLock(paths, () => compactVectorSidecar(paths, liveVectorIds(paths)), { timeoutMs: NEO_LOCK_LONG_ACQUIRE_MS });
      }
      return report;
    },
    recordHook: (hookName, meta = {}) => write(() => applyHookRecord(paths, hookName, meta)),
    // 7.12.30: Variante fuer den Prompt-Recall im Main-Thread. Der synchrone
    // Lock wartet mit Atomics.wait bis zu NEO_LOCK_ACQUIRE_MS und blockiert
    // dabei den Event-Loop; hier wird asynchron gewartet.
    recordHookAsync: (hookName, meta = {}) => workspaceWriteLockAsync(paths, () => applyHookRecord(paths, hookName, meta), { timeoutMs: NEO_LOCK_ACQUIRE_MS }),
  };
}

// Merge statt Replace: Teil-Updates (z.B. processedDreams, processedEpisodes,
// lastProcessedMessageCount aus den fire-and-forget Callbacks im agent_end)
// duerfen die Keys der jeweils anderen nicht ueberschreiben. Sonst geht die
// High-Watermark / Idempotenz verloren.
function applyHookRecord(paths, hookName, meta = {}) {
...
/**
 * 7.12.44: EINE Quelle fuer die Turn-Identitaet. Bis dahin baute index.js
 * (Episoden, Traeume, Watermark) die Turn-Events mit rohem ctx.workspaceKey
 * und der Host-sessionId, der Worker (Journal) mit normiertem Workspace-
 * Schluessel und stabilem Sitzungsschluessel — dieselbe Nachricht bekam zwei
 * IDs, und keine Episode fand ihre Turns im Journal (10.09.2026: 0 von 112).
 */
export function turnIdentityParams(event = {}, ctx = {}, workspaceKey) {
  const agentId = ctx?.agentId || "default";
  const sessionId = event?.sessionId || event?.sessionKey || event?.runId ||
    ctx?.sessionId || ctx?.sessionKey || ctx?.runId || "";
  // Der Sitzungsschluessel (agent:<id>:<kanal>:...) bleibt ueber Laeufe und
  // Kompaktierungen gleich; nur er darf in die Turn-Identitaet.
  const sessionKey = event?.sessionKey || event?.agentSessionKey ||
    ctx?.sessionKey || ctx?.agentSessionKey || sessionId;
  return { workspaceKey, agentId, sessionId, sessionKey };
}
...
export function captureNeoFromAgentEnd(event, ctx, store, options = {}) {
  ...
  const { agentId, sessionId, sessionKey } = turnIdentityParams(event, ctx, workspaceKey);
  ...
  const appendedReactions = store.appendReactions(reactions);
  ...
  // `turns` usw. sind der ganze hereingereichte Verlauf; `appended` ist, was
  // dieser Lauf wirklich neu geschrieben hat — nur das taugt als Kennzahl.
  return {
    turns,
    candidates,
    reactions,
    ...
    appended: {
      ...
      reactions: Array.isArray(appendedReactions) ? appendedReactions : reactions,
      behaviorCards: queueBehaviorCards,
    },
  };
}
...
// Hysterese für den Cap. Ohne sie steht eine Datei nach dem Cap auf exakt
// NEO_MAX_RECORDS und dauerhaft über NEO_CAP_CHECK_BYTES — jeder weitere Append
// erfüllt dann BEIDE Bedingungen und löst Zeilenzählung + Vollrewrite aus.
// Produktiv (bernhardine, 2026-08-26): 5000 Records / 161 MB, also ~161 MB
// synchron pro Append. Deshalb nach einem Cap erst wieder prüfen, wenn die
// Datei spürbar gewachsen ist; dazwischen kostet der Pfad nur ein statSync.
// Der Abstand wird in RECORDS gedacht, nicht in Bytes: nach einem Cap ist die
// durchschnittliche Recordgröße bekannt (size / NEO_MAX_RECORDS), daraus ergibt
// sich die Byte-Schwelle für den nächsten Check. Ein reiner Byte-Mindestabstand
// wäre bei kleinen Records ein Freibrief für unbegrenztes Wachstum.
const NEO_CAP_SLACK_RECORDS = Math.max(25, Math.ceil(NEO_MAX_RECORDS * 0.1));
// path -> Dateigröße, ab der die nächste (teure) Zeilenprüfung erlaubt ist.
const neoCapRecheckAt = new Map();
// 7.12.25: Die Map lebt nur im Prozess. Nach jedem Gateway-Neustart lief der
// erste Append deshalb wieder in die volle Pruefung (Tail-Read ueber 154 MB,
// dann Vollrewrite auf 5000 Zeilen) — 10–17 s im ersten agent_end nach jedem
// Neustart (bernhardine, 09.09.2026, offline reproduziert). Der Stand wandert
// deshalb in ein Sidecar `<datei>.cap.json`; schrumpft die Datei unter die
// dort notierte Groesse (Prune, Merge), gilt der Stand als ungueltig.
function neoCapStatePath(path) {
  return `${path}.cap.json`;
}
function readPersistedCapRecheckAt(path, currentSize) {
  try {
    const parsed = JSON.parse(readFileSync(neoCapStatePath(path), "utf8"));
    const recheckAt = Number(parsed?.recheckAt);
    const sizeAtCheck = Number(parsed?.size);
    if (!Number.isFinite(recheckAt) || recheckAt <= 0) return undefined;
    if (Number.isFinite(sizeAtCheck) && currentSize < sizeAtCheck) return undefined;
    return recheckAt;
  } catch (_) {
    return undefined;
  }
}
function writePersistedCapRecheckAt(path, recheckAt, size) {
  try {
    writeFileSync(neoCapStatePath(path), JSON.stringify({ recheckAt, size, updatedAt: Date.now() }), "utf8");
  } catch (_) { /* best-effort; ohne Sidecar prueft der naechste Prozess erneut */ }
}

/**
 * Read the last nonempty LF-delimited lines backwards in 64 KiB chunks.
 * Empty runs do not consume the line budget; detected short reads fail closed.
 * @param {string} path Journal path.
 * @param {number} limit Maximum nonempty lines to return, in original order.
 * @returns {string[]} The complete suffix, without decoding across UTF-8 boundaries.
 */
export function readJsonlTailLines(path, limit) {
  ...
  try {
    ...
    while (pos > 0 && completeLines < limit) {
      const readLen = Math.min(chunkSize, pos);
      pos -= readLen;
      const bytes = readSync(fd, buf, 0, readLen, pos);
      if (bytes !== readLen) throw new Error("journal changed during tail read");
      ...
      // counting complete nonempty segments backwards is safe without decoding.
      // Keep the segment state across chunks, including very long records.
      for (let i = bytes - 1; i >= 0; i--) {
        if (buf[i] === 0x0a) {
          if (lineHasBytes) completeLines++;
          lineHasBytes = false;
        } else {
          lineHasBytes = true;
        }
    ...
    let tail = Buffer.concat(chunks.reverse()).toString("utf8");
    // Unless we reached BOF, the first segment may start in the middle of a
    // record (or UTF-8 codepoint). Only segments after its LF are complete.
    if (pos > 0) tail = tail.slice(tail.indexOf("\n") + 1);
...
function appendJsonl(path, items) {
  ...
  try {
    ...
    if (size > NEO_CAP_CHECK_BYTES) {
      // Hysterese: nach einem Cap steht die Datei auf exakt NEO_MAX_RECORDS und
      // bleibt über der Byte-Schwelle. Ohne diese Sperre liest und schreibt
      // JEDER Folge-Append die ganze Datei neu.
      let recheckAt = neoCapRecheckAt.get(path);
      if (recheckAt === undefined) {
        recheckAt = readPersistedCapRecheckAt(path, size);
        if (recheckAt !== undefined) neoCapRecheckAt.set(path, recheckAt);
      }
      if (recheckAt === undefined || size >= recheckAt) {
        const recent = readJsonlTailLines(path, NEO_MAX_RECORDS + 1);
        if (recent.length > NEO_MAX_RECORDS) {
          capJsonl(path, NEO_MAX_RECORDS);
          size = statSync(path).size;
        }
        // Nächster Check, wenn ~NEO_CAP_SLACK_RECORDS weitere Records dazugekommen
        // sind — umgerechnet über die gerade gemessene Durchschnittsgröße.
        const bytesPerRecord = Math.max(1, Math.floor(size / NEO_MAX_RECORDS));
        const nextRecheckAt = size + bytesPerRecord * NEO_CAP_SLACK_RECORDS;
        neoCapRecheckAt.set(path, nextRecheckAt);
        writePersistedCapRecheckAt(path, nextRecheckAt, size);
      }
    }
  } catch (error) {
    // Der Append ist bereits dauerhaft geschrieben; ein fehlgeschlagener Cap
    // bleibt deshalb best-effort, darf aber als möglicher Growth-/I/O-Fehler
    // nicht unsichtbar werden. Nur der Dateiname wird als Kontext ausgegeben.
    safeWarn(console, "neo-arch.cap", error, { file: basename(path) });
  }
  return list;
}
...
function latestById(records) {
  ...
  for (const record of Array.isArray(records) ? records : []) {
    ...
    if (!id) { unidentified.push(record); continue; }
    byId.delete(id);
    byId.set(id, record);
  }
...
// ---------------------------------------------------------------------------
// Vektor-Sidecar (7.12.26)
//
// Bis 7.12.25 haengte der Drain jeden Datensatz samt Vektor als neue
// JSONL-Zeile an: 3072 Dezimalzahlen zu je bis zu 20 Zeichen, rund 64 kB je
// Vektor. Bei bernhardine waren das 151 von 154 MB der Kandidatendatei; jede
// Cap-Pruefung las Sekunden, jeder Recall parste 32 MB Ziffern fuer die
// letzten 500 Zeilen. Die Werte sind bitgenau float32 (geprueft am Bestand),
// also verlustfrei als rohes Float32 speicherbar: 12 kB je Vektor.
//
// Layout je Workspace:
//   vectors.<gen>.f32   rohe Float32, Little-Endian, Vektoren hintereinander
//   vector-index.json   { version, endian, file, entries: { id: { offset, length, dims } } }
// Schreiben haengt immer an (kein In-Place), erst Datei, dann Index atomar;
// ein Leser sieht damit nie einen halb geschriebenen Vektor ueber den Index.
// Re-Embeddings lassen den alten Slot verwaisen; compactVectorSidecar schreibt
// eine neue Generation nur mit lebenden IDs und stellt den Index um. Die
// Vorgaenger-Generation bleibt bis zur naechsten Kompaktierung liegen, damit
// ein Leser mit altem Index seine Offsets noch findet.
// ---------------------------------------------------------------------------
const VECTOR_INDEX_VERSION = 1;
const vectorIndexCache = new Map(); // indexPath -> { mtimeMs, size, index }

function emptyVectorIndex() {
  return { version: VECTOR_INDEX_VERSION, endian: "LE", file: "vectors.0.f32", entries: {} };
}

function normalizeVectorIndex(raw) {
  const index = emptyVectorIndex();
  if (!raw || typeof raw !== "object") return index;
  if (typeof raw.file === "string" && /^vectors\.\d+\.f32$/.test(raw.file)) index.file = raw.file;
  const entries = raw.entries && typeof raw.entries === "object" ? raw.entries : {};
  for (const [id, entry] of Object.entries(entries)) {
    const offset = Number(entry?.offset);
    const length = Number(entry?.length);
    const dims = Number(entry?.dims);
    if (!Number.isInteger(offset) || offset < 0 || !Number.isInteger(length) || length <= 0) continue;
    if (!Number.isInteger(dims) || dims <= 0 || dims * 4 !== length) continue;
    index.entries[id] = { offset, length, dims };
  }
  return index;
}

function readVectorIndex(paths) {
  const indexPath = paths.vectorIndex;
  let stat = null;
  try { stat = statSync(indexPath); } catch (_) { return emptyVectorIndex(); }
  const cached = vectorIndexCache.get(indexPath);
  if (cached && cached.mtimeMs === stat.mtimeMs && cached.size === stat.size) return cached.index;
  const index = normalizeVectorIndex(readJson(indexPath, null));
  vectorIndexCache.set(indexPath, { mtimeMs: stat.mtimeMs, size: stat.size, index });
  return index;
}

function writeVectorIndex(paths, index) {
  writeJsonAtomic(paths.vectorIndex, index);
  vectorIndexCache.delete(paths.vectorIndex);
}

function vectorFilePath(paths, index) {
  return join(paths.workspaceDir, index.file);
}

function vectorGeneration(file) {
  const match = /^vectors\.(\d+)\.f32$/.exec(String(file || ""));
  return match ? Number(match[1]) : 0;
}

function toFloat32Buffer(vector) {
  const f32 = Float32Array.from(vector, (value) => Number(value));
  return Buffer.from(f32.buffer, f32.byteOffset, f32.byteLength);
}

function float32BufferToArray(buffer) {
...
/**
 * Haengt Vektoren an die aktuelle Generation an und schreibt den Index neu.
 * Muss unter dem Workspace-Write-Lock laufen (Drain, Migration, pruneAll).
 * @param {object} paths
 * @param {Array<{id: string, vector: ArrayLike<number>}>} entries
 * @returns {number} geschriebene Vektoren
 */
function appendVectorsToSidecar(paths, entries) {
  const list = (Array.isArray(entries) ? entries : []).filter((entry) => entry && entry.id && entry.vector && entry.vector.length > 0);
  if (list.length === 0) return 0;
  const current = readVectorIndex(paths);
  const index = { ...current, entries: { ...current.entries } };
  const filePath = vectorFilePath(paths, index);
  mkdirSync(paths.workspaceDir, { recursive: true });
  let offset = 0;
  try { offset = statSync(filePath).size; } catch (_) { offset = 0; }
  const fd = openSync(filePath, "a");
  let written = 0;
  try {
    for (const entry of list) {
      const buffer = toFloat32Buffer(entry.vector);
      let done = 0;
      while (done < buffer.length) done += writeSync(fd, buffer, done, buffer.length - done);
      index.entries[String(entry.id)] = { offset, length: buffer.length, dims: entry.vector.length };
  ...
  writeVectorIndex(paths, index);
  return written;
}

function readVectorFromSidecar(paths, index, id) {
  const entry = index?.entries?.[id];
  if (!entry) return undefined;
  const filePath = vectorFilePath(paths, index);
  let fd;
  try { fd = openSync(filePath, "r"); } catch (_) { return undefined; }
  try {
    const buffer = Buffer.alloc(entry.length);
    const bytes = readSync(fd, buffer, 0, entry.length, entry.offset);
    if (bytes !== entry.length) return undefined;
    return float32BufferToArray(buffer);
  } catch (_) {
    return undefined;
  } finally {
    closeSync(fd);
...
/**
 * Haengt Datensaetzen ohne Inline-Vektor einen memoisierten, nicht
 * enumerierbaren Getter `embedding` an. JSON.stringify und Spreads sehen ihn
 * nicht, das Scoring (item.embedding) schon; ein Setter erlaubt Zuweisungen.
 */
function attachSidecarVectors(paths, records) {
  if (!Array.isArray(records) || records.length === 0) return records;
  const index = readVectorIndex(paths);
  const entries = index.entries;
  if (!entries || Object.keys(entries).length === 0) return records;
  for (const record of records) {
    if (!record || typeof record !== "object" || Object.hasOwn(record, "embedding")) continue;
    const id = record.id ? String(record.id) : "";
    if (!id || !entries[id]) continue;
    let loaded = false;
    let value;
    Object.defineProperty(record, NEO_LAZY_VECTOR, { value: true, enumerable: false, configurable: true, writable: true });
    Object.defineProperty(record, "embedding", {
      configurable: true,
      enumerable: false,
      get() {
        if (!loaded) {
          value = readVectorFromSidecar(paths, index, id);
          loaded = true;
        }
        return value;
      },
      set(next) {
        record[NEO_LAZY_VECTOR] = false;
        Object.defineProperty(record, "embedding", { value: next, writable: true, configurable: true, enumerable: true });
      },
    });
  }
  return records;
...
const NEO_LAZY_VECTOR = Symbol("plur1bus.neo.lazyVector");

/**
 * Liest viele Vektoren mit einem Deskriptor, nach Offset sortiert.
 * @returns {Map<string, Array<number>>}
 */
function readVectorsBatch(paths, index, ids, options = {}) {
  ...
  for (const id of ids || []) {
    ...
    if (entry && !out.has(key)) entries.push([key, entry]);
  }
  if (entries.length === 0) return out;
  entries.sort((a, b) => a[1].offset - b[1].offset);
  let fd;
  try { fd = openSync(vectorFilePath(paths, index), "r"); } catch (_) { return out; }
  try {
    for (const [key, entry] of entries) {
      const buffer = Buffer.alloc(entry.length);
      const bytes = readSync(fd, buffer, 0, entry.length, entry.offset);
      if (bytes !== entry.length) continue;
      if (typed) {
        const copy = new Uint8Array(buffer.byteLength);
        copy.set(buffer);
        out.set(key, new Float32Array(copy.buffer, 0, copy.byteLength / 4));
      } else {
        out.set(key, float32BufferToArray(buffer));
      }
    }
  } finally {
    closeSync(fd);
...
// Cosinus ohne Array-Konvertierung: der Kandidatenvektor bleibt Float32Array.
function cosineSimilarityTyped(query, queryNorm, vector) {
  if (!vector || vector.length !== query.length || queryNorm === 0) return NaN;
  let dot = 0;
  let norm = 0;
...
// 7.12.27: Suche ueber alle Kandidaten statt nur ueber das 500er-Fenster.
// Brute-Force-Cosinus ueber das Sidecar (bernhardine: 2400 x 3072 float32,
// rund 40 ms), kein Index noetig. Vorfilter auf lebende Status und ACL,
// Mindest-Cosinus, Rezenz-Abschlag, damit Altes nicht dominiert.
const NEO_GLOBAL_SEARCH_STATUSES = new Set(["candidate", "active", "promoted"]);

/**
 * @param {object} store  Neo-Store (readCandidates, readVectors)
 * @param {{queryVector: Array<number>, requester?: object, topK?: number, minSimilarity?: number, halfLifeDays?: number, maxCandidates?: number, excludeIds?: Set<string>, now?: number}} options
 * @returns {{hits: Array<{item: object, similarity: number, score: number, ageDays: number}>, scanned: number, eligible: number, withVector: number, ms: number}}
 */
export function searchNeoCandidatesGlobal(store, options = {}) {
  const started = Date.now();
  const result = { hits: [], scanned: 0, unique: 0, eligible: 0, withVector: 0, ms: 0, index: "missing", indexLines: 0 };
  const queryVector = options.queryVector;
  if (!Array.isArray(queryVector) || queryVector.length === 0) return result;
  const topK = Math.max(1, Math.min(200, Number(options.topK) || 30));
  const minSimilarity = Number.isFinite(Number(options.minSimilarity)) ? Math.max(0, Math.min(1, Number(options.minSimilarity))) : 0.35;
  const halfLifeDays = Number(options.halfLifeDays) > 0 ? Number(options.halfLifeDays) : 30;
  const maxCandidates = Math.max(100, Number(options.maxCandidates) || 20000);
  const excludeIds = options.excludeIds instanceof Set ? options.excludeIds : new Set(options.excludeIds || []);
  const now = Number.isFinite(options.now) ? options.now : Date.now();

  // 7.12.28: Primaer ueber den Metadatenindex (juengste Revision je ID, ohne
  // Vollparse des Journals, unabhaengig vom Zeilen-Cap). Ohne Index — etwa
  // vor dem ersten Kandidaten-Append nach dem Update — bleibt der Fensterpfad.
  const index = typeof store.readCandidateIndex === "function" ? store.readCandidateIndex() : null;
  const latestById = new Map();
  let fromIndex = false;
  if (index && index.exists) {
    fromIndex = true;
    result.index = index.mode === "tail" ? `tail:${index.parsedLines}` : index.mode;
    result.indexLines = index.lines;
    result.scanned = index.entries.size;
    let entries = index.entries;
    if (entries.size > maxCandidates) {
      const newest = [...entries.values()].sort((a, b) => neoRevisionTimeMs(b) - neoRevisionTimeMs(a)).slice(0, maxCandidates);
      entries = new Map(newest.map((entry) => [String(entry.id), entry]));
    }
    const requester = options.requester;
    for (const [id, entry] of entries) {
      if (requester && !isNeoRecordAccessible(entry, requester)) continue;
      latestById.set(id, entry);
    }
  } else {
    result.index = "missing";
    const records = store.readCandidates(maxCandidates, options.requester);
    result.scanned = records.length;
    // Die JSONL traegt je ID mehrere Revisionen (Statuszeilen); nur die juengste zaehlt.
    for (const record of records) {
      if (!record || !record.id) continue;
      const id = String(record.id);
      const prev = latestById.get(id);
      if (!prev || neoRevisionTimeMs(record) >= neoRevisionTimeMs(prev)) latestById.set(id, record);
    }
  }
  result.unique = latestById.size;
  const eligible = [];
  const lazyIds = [];
  for (const [id, record] of latestById) {
    if (excludeIds.has(id)) continue;
    if (!NEO_GLOBAL_SEARCH_STATUSES.has(record.status || "candidate")) continue;
    if (record.stale === true || record.hardDeleted === true) continue;
    eligible.push(record);
    if (fromIndex || record[NEO_LAZY_VECTOR]) lazyIds.push(id);
  }
  result.eligible = eligible.length;
  const batch = lazyIds.length > 0 ? readVectorsBatch(store.paths, readVectorIndex(store.paths), lazyIds, { typed: true }) : new Map();
  const query = Float64Array.from(queryVector, (value) => Number(value));
  let queryNorm = 0;
  ...
  for (const record of eligible) {
    const id = String(record.id);
    const typedVector = fromIndex || record[NEO_LAZY_VECTOR] ? batch.get(id) : null;
    const inlineVector = typedVector || fromIndex ? null : record.embedding;
    let similarity;
    if (typedVector) {
      similarity = cosineSimilarityTyped(query, queryNorm, typedVector);
    } else if (Array.isArray(inlineVector) && inlineVector.length === query.length) {
      similarity = cosineSimilarity(queryVector, inlineVector);
    } else {
      continue;
    }
    result.withVector++;
    ...
    const ageDays = Number.isFinite(timeMs) ? Math.max(0, (now - timeMs) / 86_400_000) : halfLifeDays * 4;
    const recency = Math.pow(0.5, ageDays / halfLifeDays);
    scored.push({ item: record, similarity, score: similarity * (0.85 + 0.15 * recency), ageDays, typedVector });
  }
  scored.sort((a, b) => b.score - a.score || b.similarity - a.similarity);
  result.hits = scored.slice(0, topK).map((hit) => {
    // Treffer sind Kopien: Index-Eintraege leben im Prozess-Cache und duerfen
    // weder den Vektor (24 KB je ID) noch Lane-Mutationen behalten. Nur
    // Treffer bekommen den Vektor als plain Array, damit das Lane-Scoring ihn
    // wiederverwendet.
    const item = hit.typedVector ? { ...hit.item, embedding: Array.from(hit.typedVector) } : { ...hit.item, embedding: hit.item.embedding };
    return { item, similarity: hit.similarity, score: hit.score, ageDays: hit.ageDays };
  });
...
/**
 * Entfernt aus den Recall-Lanes die Treffer (optional nur bestimmte IDs), deren
 * Text einem der uebergebenen Texte zu aehnlich ist — z. B. LanceDB-
 * Erinnerungen, die im selben Prompt schon stehen.
 * @returns {{lanes: object, dropped: number}}
 */
export function dedupeNeoLanesAgainstTexts(lanes, texts, options = {}) {
  const threshold = Number.isFinite(Number(options.threshold)) ? Number(options.threshold) : 0.8;
  const onlyIds = options.onlyIds instanceof Set ? options.onlyIds : options.onlyIds ? new Set(options.onlyIds) : null;
  const tokenSets = (Array.isArray(texts) ? texts : []).map((text) => tokenizeForScore(String(text || ""))).filter((set) => set.size > 0);
  const out = {};
  let dropped = 0;
  for (const [lane, rows] of Object.entries(lanes || {})) {
    out[lane] = [];
    for (const row of rows || []) {
      const id = row?.item?.id ? String(row.item.id) : "";
      if (tokenSets.length === 0 || (onlyIds && !onlyIds.has(id))) { out[lane].push(row); continue; }
      const own = tokenizeForScore(String(row.item?.statement || row.item?.content || ""));
      const duplicate = own.size > 0 && tokenSets.some((set) => jaccard(own, set) >= threshold);
      if (duplicate) dropped++; else out[lane].push(row);
    }
  }
...
//
// Die globale Suche (7.12.27) parste je Recall die komplette Kandidaten-JSONL
// (bernhardine: 5000 Zeilen, 10 MB, ~0,3 s auf dem Main-Thread) und sah nur,
// was der 5000-Zeilen-Cap noch enthielt — ein rollierendes Fenster von rund
// 2500 eindeutigen Kandidaten. Der Index haelt je Kandidat eine schlanke
// Projektion (Statement, Status, Sichtbarkeit, Herkunft, Zeiten; kein
// normalizedStatement, keine Turn-Listen, kein Vektor), wird bei jedem
// Kandidaten-Append mitgeschrieben (append-only, kein Zeilen-Cap) und beim
// Lesen inkrementell nachgeladen: Prozess-Cache je Datei mit (ino, size,
// mtime); waechst die Datei unter gleicher Inode, wird nur der neue Tail
// geparst. Ein Rewrite (Kompaktierung, neue Inode) oder eine geschrumpfte
// Datei loest einen Vollparse aus. Die Kompaktierung (consolidate-daily,
// pruneAll, oder wenn die Datei doppelt so viele Zeilen wie IDs hat) behaelt
// die juengste Revision je ID, nur suchbare Status, kein injizierter Kontext,
// hoechstens NEO_CANDIDATE_INDEX_MAX_ENTRIES nach Revisionszeit.
// Die Vektoren liegen weiter im Sidecar; liveVectorIds nimmt die Index-IDs
// mit, damit die Kompaktierung des Sidecars den Horizont nicht kuerzt.
// ---------------------------------------------------------------------------
const CANDIDATE_INDEX_FILE = "candidate-index.jsonl";
const NEO_CANDIDATE_INDEX_MAX_ENTRIES = Math.max(1000, Number(process.env.PLUR1BUS_NEO_CANDIDATE_INDEX_MAX) || 20000);
// Kompaktieren, sobald die Datei so viele ueberholte Zeilen traegt.
const NEO_CANDIDATE_INDEX_COMPACT_SLACK = 1000;
// Self-Heal je Append: so viele juengste Journalzeilen gegen den Index pruefen
// (faengt Zeilen, die ein aelterer Plugin-Stand ohne Index geschrieben hat).
const NEO_CANDIDATE_INDEX_HEAL_TAIL = 50;
const CANDIDATE_INDEX_FIELDS = Object.freeze([
  "id", "workspaceKey", "agentId", "statement", "category", "status", "stale", "hardDeleted",
  "confidence", "salience", "recency", "createdAt", "updatedAt", "startTime", "epistemicStatus",
  "visibility", "ownerId", "ownerUserId", "sourceTurnIds", "embeddingDims", "emotionalDominant",
]);
const CANDIDATE_INDEX_ORIGIN_FIELDS = Object.freeze(["kind", "role", "trustLevel", "ownerId", "scope"]);
// path -> { ino, size (konsumierte Bytes), mtimeMs, entries: Map<id, entry>, lines }
const candidateIndexCache = new Map();

/**
 * Schlanke Projektion eines Kandidaten fuer den Index. Enthaelt alles, was
 * ACL (isNeoRecordAccessible), Lane-Scoring (scoreNeoRecallItem), Rendering
 * (formatNeoRecallContext/renderPromptMemoryAttrs) und Dedupe lesen.
 * @param {object} record
 * @returns {object|null}
 */
export function candidateIndexEntry(record) {
  if (!record || typeof record !== "object" || record.__parseError || !record.id) return null;
  const entry = {};
  for (const key of CANDIDATE_INDEX_FIELDS) if (record[key] !== undefined) entry[key] = record[key];
  entry.id = String(record.id);
  if (typeof entry.statement !== "string") entry.statement = String(record.statement ?? record.content ?? record.text ?? "");
  if (record.origin && typeof record.origin === "object") {
    const origin = {};
    for (const key of CANDIDATE_INDEX_ORIGIN_FIELDS) if (record.origin[key] !== undefined) origin[key] = record.origin[key];
    entry.origin = origin;
  }
  return entry;
}

function upsertCandidateIndexEntries(entries, records) {
  for (const record of records) {
    if (!record || typeof record !== "object" || record.__parseError || !record.id) continue;
    const id = String(record.id);
    const prev = entries.get(id);
    if (!prev || neoRevisionTimeMs(record) >= neoRevisionTimeMs(prev)) entries.set(id, record);
  }
}

function parseCandidateIndexText(text, entries) {
  let lines = 0;
  for (const line of text.split("\n")) {
    ...
    upsertCandidateIndexEntries(entries, [record]);
  }
  return lines;
}

/**
 * Liest den Index lockfrei mit Prozess-Cache und inkrementellem Tail-Read.
 * @param {object} paths
 * @returns {{entries: Map<string, object>, lines: number, mode: "missing"|"cached"|"tail"|"full", parsedLines: number, exists: boolean}}
 */
export function readCandidateIndex(paths) {
  const path = paths.candidateIndex;
  let stat;
  try { stat = statSync(path); } catch (_) {
    candidateIndexCache.delete(path);
    return { entries: new Map(), lines: 0, mode: "missing", parsedLines: 0, exists: false };
  }
  const cached = candidateIndexCache.get(path);
  if (cached && cached.ino === stat.ino && cached.size === stat.size && cached.mtimeMs === stat.mtimeMs) {
    return { entries: cached.entries, lines: cached.lines, mode: "cached", parsedLines: 0, exists: true };
  }
  const incremental = Boolean(cached && cached.ino === stat.ino && stat.size > cached.size);
  const entries = incremental ? cached.entries : new Map();
  let lines = incremental ? cached.lines : 0;
  let consumed = incremental ? cached.size : 0;
  let parsedLines = 0;
  const fd = openSync(path, "r");
  try {
    const length = stat.size - consumed;
    if (length > 0) {
      const buf = Buffer.alloc(length);
      const bytes = readSync(fd, buf, 0, length, consumed);
      // appendFileSync ist gegenueber einem Leser im anderen Thread nicht
      // atomar: nur bis zum letzten vollstaendigen Zeilenende konsumieren.
      const end = bytes > 0 ? buf.lastIndexOf(0x0a, bytes - 1) : -1;
      if (end >= 0) {
        parsedLines = parseCandidateIndexText(buf.toString("utf8", 0, end + 1), entries);
        lines += parsedLines;
        consumed += end + 1;
      }
    }
  ...
  candidateIndexCache.set(path, { ino: stat.ino, size: consumed, mtimeMs: consumed === stat.size ? stat.mtimeMs : -1, entries, lines });
  return { entries, lines, mode: incremental ? "tail" : "full", parsedLines, exists: true };
}

function appendCandidateIndex(paths, records) {
  const entries = (Array.isArray(records) ? records : [records]).map(candidateIndexEntry).filter(Boolean);
  if (entries.length === 0) return 0;
  mkdirSync(dirname(paths.candidateIndex), { recursive: true });
  appendFileSync(paths.candidateIndex, entries.map((entry) => JSON.stringify(entry)).join("\n") + "\n", "utf8");
  return entries.length;
}

function candidateIndexKeeps(entry) {
  if (!entry || !entry.id) return false;
  if (!NEO_GLOBAL_SEARCH_STATUSES.has(entry.status || "candidate")) return false;
  if (entry.hardDeleted === true) return false;
  if (isInjectedContextText(entry.statement || "")) return false;
  return true;
}

function finalizeCandidateIndexEntries(latest, maxEntries) {
  const stats = { unique: latest.size, removedStatus: 0, removedInjected: 0, removedCap: 0 };
  const kept = [];
  for (const record of latest.values()) {
    const entry = candidateIndexEntry(record);
    if (!entry) continue;
    if (!NEO_GLOBAL_SEARCH_STATUSES.has(entry.status || "candidate") || entry.hardDeleted === true) { stats.removedStatus++; continue; }
    if (isInjectedContextText(entry.statement || "")) { stats.removedInjected++; continue; }
    kept.push(entry);
  }
  kept.sort((a, b) => neoRevisionTimeMs(a) - neoRevisionTimeMs(b));
  const limit = Math.max(1, Number(maxEntries) || NEO_CANDIDATE_INDEX_MAX_ENTRIES);
  const capped = kept.length > limit ? kept.slice(-limit) : kept;
  stats.removedCap = kept.length - capped.length;
  return { entries: capped, stats };
}

/**
 * Baut den Index aus dem Kandidaten-Journal (und einem sicheren Legacy-
 * Journal) neu: juengste Revision je ID. Muss unter dem Write-Lock laufen.
 * @param {object} paths
 * @param {{legacyCandidatesPath?: string, dryRun?: boolean, maxEntries?: number}} [options]
 */
export function buildCandidateIndex(paths, options = {}) {
  const latest = new Map();
  let sourceRecords = 0;
  const sources = [];
  if (options.legacyCandidatesPath && options.legacyCandidatesPath !== paths.candidates) sources.push(options.legacyCandidatesPath);
  sources.push(paths.candidates);
  for (const source of sources) {
    if (!existsSync(source)) continue;
    const records = readJsonlAll(source);
    sourceRecords += records.length;
    upsertCandidateIndexEntries(latest, records);
  }
  const { entries, stats } = finalizeCandidateIndexEntries(latest, options.maxEntries);
  if (options.dryRun !== true) {
    writeJsonlAtomic(paths.candidateIndex, entries);
    candidateIndexCache.delete(paths.candidateIndex);
  }
  return { built: true, dryRun: options.dryRun === true, sourceRecords, entries: entries.length, ...stats };
}

/**
 * Legt den Index an, falls er fehlt und ein Journal existiert.
 * Muss unter dem Write-Lock laufen.
 */
export function ensureCandidateIndex(paths, options = {}) {
  if (existsSync(paths.candidateIndex)) return { built: false, reason: "exists" };
  const hasJournal = existsSync(paths.candidates) || (options.legacyCandidatesPath && existsSync(options.legacyCandidatesPath));
  if (!hasJournal) return { built: false, reason: "no_journal" };
  return buildCandidateIndex(paths, options);
}

/**
 * Kompaktiert den Index auf die juengste Revision je ID (suchbare Status,
 * kein injizierter Kontext, Cap nach Revisionszeit). Muss unter dem
 * Write-Lock laufen.
 */
export function compactCandidateIndex(paths, options = {}) {
  const dryRun = options.dryRun === true;
  if (!existsSync(paths.candidateIndex)) return { exists: false, before: 0, after: 0, rewritten: false };
  const records = readJsonlAll(paths.candidateIndex);
  const latest = new Map();
  upsertCandidateIndexEntries(latest, records);
  const { entries, stats } = finalizeCandidateIndexEntries(latest, options.maxEntries);
  const report = { exists: true, dryRun, before: records.length, after: entries.length, ...stats, rewritten: false };
  const removed = stats.removedStatus + stats.removedInjected + stats.removedCap;
  if (dryRun || (records.length === entries.length && removed === 0)) return report;
  writeJsonlAtomic(paths.candidateIndex, entries);
  candidateIndexCache.delete(paths.candidateIndex);
...
function candidateIndexStats(paths) {
  const index = readCandidateIndex(paths);
  let bytes = 0;
  try { bytes = statSync(paths.candidateIndex).size; } catch (_) { bytes = 0; }
  return { exists: index.exists, lines: index.lines, entries: index.entries.size, bytes };
}

// Nach einem Kandidaten-Append (unter dem Write-Lock): neue Zeilen in den
// Index, Journal-Tail gegen den Index pruefen, bei Bedarf kompaktieren.
function maintainCandidateIndexAfterAppend(paths, appended) {
  try {
    appendCandidateIndex(paths, appended);
    const index = readCandidateIndex(paths);
    if (!index.exists) return;
    const missing = [];
    for (const record of readJsonlTail(paths.candidates, NEO_CANDIDATE_INDEX_HEAL_TAIL)) {
      if (!record?.id) continue;
      const prev = index.entries.get(String(record.id));
      if (!prev || neoRevisionTimeMs(record) > neoRevisionTimeMs(prev)) missing.push(record);
    }
    if (missing.length > 0) appendCandidateIndex(paths, missing);
    if (index.lines > 2 * index.entries.size + NEO_CANDIDATE_INDEX_COMPACT_SLACK) compactCandidateIndex(paths);
  } catch (error) {
    safeWarn(console, "neo-arch.candidateIndex", error, { file: CANDIDATE_INDEX_FILE });
  }
}

/**
 * Index eines Workspace-Verzeichnisses anlegen/neu bauen (Deploy-Skript).
 * @param {string} workspaceDir
 * @param {{dryRun?: boolean, rebuild?: boolean, maxEntries?: number}} [options]
 */
export function buildNeoCandidateIndexForDir(workspaceDir, options = {}) {
  const paths = neoStorePathsForDir(workspaceDir);
  const dryRun = options.dryRun === true;
  const run = () => {
    if (!existsSync(paths.candidates)) return { workspaceDir, built: false, reason: "no_journal", dryRun };
    if (existsSync(paths.candidateIndex) && options.rebuild !== true) {
      return { workspaceDir, built: false, reason: "exists", dryRun, ...candidateIndexStats(paths) };
    }
    return { workspaceDir, ...buildCandidateIndex(paths, { dryRun, maxEntries: options.maxEntries }) };
  };
  return dryRun ? run() : workspaceWriteLock(paths, run, { timeoutMs: NEO_LOCK_LONG_ACQUIRE_MS });
}

function liveVectorIds(paths) {
  const ids = new Set();
  for (const path of [paths.candidates, paths.behavior, paths.turns]) {
    for (const record of readJsonlAll(path)) {
      if (record && !record.__parseError && record.id) ids.add(String(record.id));
    }
  }
  // 7.12.28: Kandidaten jenseits des Journal-Fensters leben im Metadatenindex
  // weiter; ihre Vektoren muessen die Kompaktierung ueberstehen.
  for (const id of readCandidateIndex(paths).entries.keys()) ids.add(id);
  return ids;
}

/**
 * Neue Generation nur mit lebenden IDs; alte Generationen ausser dem direkten
 * Vorgaenger werden geloescht.
 * @param {object} paths
 * @param {Set<string>} liveIds
 */
function compactVectorSidecar(paths, liveIds) {
  const index = readVectorIndex(paths);
  const ids = Object.keys(index.entries);
  const stats = { before: ids.length, after: 0, orphans: 0, bytesBefore: 0, bytesAfter: 0, generation: vectorGeneration(index.file), rewritten: false };
  if (ids.length === 0) return stats;
  const sourcePath = vectorFilePath(paths, index);
  try { stats.bytesBefore = statSync(sourcePath).size; } catch (_) { stats.bytesBefore = 0; }
  const keep = ids.filter((id) => liveIds.has(id));
  stats.orphans = ids.length - keep.length;
  const liveBytes = keep.reduce((sum, id) => sum + index.entries[id].length, 0);
  stats.after = keep.length;
  stats.bytesAfter = liveBytes;
  // Lohnt sich nur, wenn wirklich etwas verwaist ist (inkl. ueberschriebener
  // Slots, die im Index nicht mehr referenziert sind).
  if (stats.orphans === 0 && liveBytes === stats.bytesBefore) return stats;
  const nextGeneration = stats.generation + 1;
  const nextFile = `vectors.${nextGeneration}.f32`;
  const nextPath = join(paths.workspaceDir, nextFile);
  const nextIndex = { version: VECTOR_INDEX_VERSION, endian: "LE", file: nextFile, entries: {} };
  let sourceFd;
  try { sourceFd = openSync(sourcePath, "r"); } catch (_) { return stats; }
  const targetFd = openSync(nextPath, "w");
  let offset = 0;
  try {
    for (const id of keep) {
      const entry = index.entries[id];
      const buffer = Buffer.alloc(entry.length);
      const bytes = readSync(sourceFd, buffer, 0, entry.length, entry.offset);
      if (bytes !== entry.length) continue;
      let done = 0;
      while (done < buffer.length) done += writeSync(targetFd, buffer, done, buffer.length - done);
      nextIndex.entries[id] = { offset, length: entry.length, dims: entry.dims };
      offset += entry.length;
    }
  } finally {
    closeSync(sourceFd);
  ...
  stats.after = Object.keys(nextIndex.entries).length;
  stats.bytesAfter = offset;
  writeVectorIndex(paths, nextIndex);
  stats.rewritten = true;
  stats.generation = nextGeneration;
  // Alles ausser aktueller und Vorgaenger-Generation aufraeumen.
  for (const name of readdirSync(paths.workspaceDir)) {
    const generation = /^vectors\.(\d+)\.f32$/.exec(name);
    if (!generation) continue;
    const value = Number(generation[1]);
    if (value >= nextGeneration - 1) continue;
    try { unlinkSync(join(paths.workspaceDir, name)); } catch (_) { /* best-effort */ }
  }
  return stats;
}

function vectorSidecarStats(paths) {
  const index = readVectorIndex(paths);
  let bytes = 0;
  try { bytes = statSync(vectorFilePath(paths, index)).size; } catch (_) { bytes = 0; }
  return { file: index.file, vectors: Object.keys(index.entries).length, bytes, generation: vectorGeneration(index.file) };
}

function neoStorePathsForDir(workspaceDir) {
  return {
    workspaceDir,
    turns: join(workspaceDir, "turn-journal.jsonl"),
    candidates: join(workspaceDir, "memory-candidates.jsonl"),
...
/**
 * Einmalige Migration: Inline-Vektoren aus Journal, Kandidaten und Karten ins
 * Sidecar (letzte Zeile je ID gewinnt), bitgenau zurueckgelesen, danach die
 * JSONL ohne `embedding` atomar neu geschrieben. Laeuft unter dem Write-Lock.
 * @param {string} workspaceDir  Workspace-Verzeichnis (…/_neo/workspaces/<name>)
 * @param {{dryRun?: boolean}} [options]
 */
export function migrateInlineVectorsToSidecar(workspaceDir, options = {}) {
  const paths = neoStorePathsForDir(workspaceDir);
  const dryRun = options.dryRun === true;
  const report = { workspaceDir, dryRun, files: {}, vectorsWritten: 0, verified: 0, mismatches: 0 };
  const run = () => {
    const latest = new Map();
    const perFile = [];
    for (const [name, path] of [["turns", paths.turns], ["candidates", paths.candidates], ["behavior", paths.behavior]]) {
      const lines = existsSync(path) ? readFileSync(path, "utf8").split("\n").filter(Boolean) : [];
      let inline = 0;
      const stripped = [];
      for (const line of lines) {
        let record;
        try { record = JSON.parse(line); } catch (_) { stripped.push(line); continue; }
        if (record && Array.isArray(record.embedding) && record.embedding.length > 0 && record.id) {
          if (validEmbedding(record.embedding)) latest.set(String(record.id), record.embedding);
          inline++;
          const { embedding: _vector, ...rest } = record;
          rest.embeddingDims = record.embedding.length;
          rest.embeddingStore = "sidecar";
          stripped.push(JSON.stringify(rest));
        } else {
          stripped.push(line);
        }
      }
      perFile.push({ path, inline, stripped });
      report.files[name] = {
        lines: lines.length,
        inlineVectors: inline,
        bytesBefore: lines.reduce((sum, line) => sum + line.length + 1, 0),
        bytesAfter: stripped.reduce((sum, line) => sum + line.length + 1, 0),
      };
    }
    if (dryRun) { report.vectorsWritten = latest.size; return; }
    const entries = [...latest.entries()].map(([id, vector]) => ({ id, vector }));
    report.vectorsWritten = appendVectorsToSidecar(paths, entries);
    const index = readVectorIndex(paths);
    for (const { id, vector } of entries) {
      const back = readVectorFromSidecar(paths, index, id);
      const same = Array.isArray(back) && back.length === vector.length && back.every((value, i) => value === Number(vector[i]));
      if (same) report.verified++; else report.mismatches++;
    }
    if (report.mismatches > 0) throw new Error(`vector sidecar verification failed: ${report.mismatches} mismatches in ${workspaceDir}`);
    for (const { path, stripped, inline } of perFile) {
      if (inline === 0) continue;
      const tmp = `${path}.${process.pid}.${Date.now()}.tmp`;
      writeFileSync(tmp, stripped.length ? stripped.join("\n") + "\n" : "", "utf8");
      renameSync(tmp, path);
    }
  };
  workspaceWriteLock(paths, run, { timeoutMs: NEO_LOCK_LONG_ACQUIRE_MS });
  return report;
}
...
function createEmbeddingTargetState(paths) {
  return {
    memory: { path: paths.candidates, byId: latestRecordById(paths.candidates), updates: [], vectors: [] },
    behavior: { path: paths.behavior, byId: latestRecordById(paths.behavior), updates: [], vectors: [] },
    turn: { path: paths.turns, byId: latestRecordById(paths.turns), updates: [], vectors: [] },
  };
}
...
async function markEmbeddingTargetFresh(paths, queueItem, nowIsoStr, targetState = null, options = {}) {
  ...
  // 7.12.26: Der Vektor geht ins Sidecar, die JSONL-Zeile traegt nur noch
  // Status und Dimension. Ein Inline-Vektor aus alten Zeilen wird dabei
  // abgestreift, damit er nicht bei jeder Statusaenderung mitkopiert wird.
  const { embedding: _inlineVector, ...targetWithoutVector } = target;
  const updated = {
    ...targetWithoutVector,
    embeddingStatus: "fresh",
    embeddingDims: embedding.length,
    embeddingStore: "sidecar",
    embeddingImpact: inferEmbeddingImpact(queueItem),
    embeddingUpdatedAt: nowIsoStr,
  };
  if (state) {
    state.vectors.push({ id: updated.id, vector: embedding });
    state.updates.push(updated);
    state.byId.set(updated.id, updated);
  } else {
    appendVectorsToSidecar(paths, [{ id: updated.id, vector: embedding }]);
    appendJsonl(targetPath, [updated]);
  }
  return { ok: true };
}

function flushEmbeddingTargetUpdates(targetState, paths = null) {
  // Erst die Vektoren (Datei, dann Index), dann die Zeilen: ein Leser, der die
  // neue Zeile sieht, findet den Vektor bereits im Index.
  const vectors = Object.values(targetState || {}).flatMap((state) => state.vectors || []);
  if (vectors.length > 0 && paths) appendVectorsToSidecar(paths, vectors);
...
async function drainEmbeddingQueueFile(paths, options = {}) {
  ...
  // Der Drain war bisher ausschliesslich mengenbegrenzt. Laeuft er im
  // Zeitbudget eines Aufrufers (auto-capture teilt sich 60s mit ihm), verbraucht
  // er es bei vollem maxItems komplett und haengert die eigentliche Arbeit aus.
  // deadlineMs/signal geben dem Aufrufer die Kontrolle zurueck; der Flush unten
  // schreibt den bis dahin erreichten Fortschritt regulaer weg.
  ...
  if (processed > 0 || missingTargets > 0 || deferred > 0 || parseErrors > 0) {
    flushEmbeddingTargetUpdates(targetState, paths);
