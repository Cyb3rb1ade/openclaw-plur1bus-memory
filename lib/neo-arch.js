...
import { appendFileSync, closeSync, existsSync, fstatSync, mkdirSync, openSync, readFileSync, readSync, readdirSync, realpathSync, renameSync, rmdirSync, statSync, writeFileSync } from "node:fs";
...
function normalizeWorkspaceKey(value) {
  return String(value || "default").trim().slice(0, 240) || "default";
}
...
function workspaceWriteLock(paths, action) {
  mkdirSync(paths.workspaceDir, { recursive: true });
  const lockPath = join(paths.workspaceDir, ".neo-write.lock");
...
async function workspaceWriteLockAsync(paths, action) {
  mkdirSync(paths.workspaceDir, { recursive: true });
  const lockPath = join(paths.workspaceDir, ".neo-write.lock");
  const deadline = Date.now() + 5_000;
  while (true) {
    try { mkdirSync(lockPath, { recursive: false }); break; }
    catch (error) {
      if (error?.code !== "EEXIST") throw error;
      if (Date.now() >= deadline) throw Object.assign(new Error("Neo workspace writer backpressure: lock deadline exceeded"), { code: "NEO_WRITE_BACKPRESSURE" });
      await new Promise(resolve => setTimeout(resolve, 10));
    }
  }
  try { return await action(); }
  finally { try { rmdirSync(lockPath); } catch (error) { if (error?.code !== "ENOENT") throw error; } }
}
...
export function migrateNeoWorkspaces(rootDir, options = {}) {
  ...
  for (const mapping of mappings) {
    ...
    const migrateMapping = () => {
      ...
      const manifest = { version: 1, workspaceKey: mapping.workspaceKey, legacyKey: mapping.legacyKey, migratedAt: new Date().toISOString() };
...
export function createNeoStore(rootDir, workspaceKey = "default") {
  ...
  const readMerged = (path, legacyPath, limit, requester) => {
    return readJsonlTail(path, limit)
  ...
  return {
    ...
    drainEmbeddingQueue: async (opts = {}) => workspaceWriteLockAsync(paths, () => drainEmbeddingQueueFile(paths, opts)),
