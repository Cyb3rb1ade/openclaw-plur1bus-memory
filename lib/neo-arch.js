...
export function formatNeoRecallContext(lanes, opts = {}) {
  ...
  for (const [lane, rows] of Object.entries(lanes || {})) {
    for (const row of rows || []) {
      ...
      // lässt.
      const safeStatus = sanitizePromptIdentifier(item.status, "active");
      const safeEpistemic = sanitizePromptIdentifier(normalizeEpistemicStatus(item.epistemicStatus), "untrusted");
