...
export function createNeoStore(rootDir, workspaceKey = "default") {
  ...
  const readMerged = (path, legacyPath, limit, requester) => {
    const legacyRecords = readJsonlTail(legacyPath, limit)
      .filter(record => record?.workspaceKey === normalizeWorkspaceKey(workspaceKey));
    return [...legacyRecords, ...readJsonlTail(path, limit)]
  ...
  return {
    ...
    drainEmbeddingQueue: (opts = {}) => write(() => drainEmbeddingQueueFile(paths, opts)),
...
function markEmbeddingTargetFresh(paths, queueItem, nowIsoStr, targetState = null, options = {}) {
  ...
  let embedding;
  try {
    embedding = options.embedder(target.statement || target.content || "", target);
  } catch (error) {
...
function drainEmbeddingQueueFile(paths, options = {}) {
  ...
  for (const { key, item } of latestEntries) {
    ...
    const target = markEmbeddingTargetFresh(paths, item, nowIsoStr, targetState, options);
    if (!target.ok) {
