...
function safeErrorMessage(err) {
  ...
  try {
    ...
    // A DOMException keeps `message` on its prototype, so the own-property
    // check above misses it and every aborted operation was reported as
    // "non-standard error". Reading it is safe: the platform defines the
    // getter, so no user code runs. Everything else keeps the strict
    // own-property rule, which exists to avoid invoking foreign getters.
...
function observeNativePromise(value, fulfill, reject) {
  ...
  const constructorObservation = observeNativePromiseWithOverride(
    value,
    fulfill,
    reject,
    value,
  ...
  if (constructorObservation.available) return constructorObservation.observed;

  const constructorLookup = findDataPropertyValue(value, "constructor");
  if (!constructorLookup.found
    || typeof constructorLookup.value !== "function"
    || nodeTypes.isProxy(constructorLookup.value)) {
    return false;
  }
  const speciesObservation = observeNativePromiseWithOverride(
    value,
    fulfill,
    reject,
    constructorLookup.value,
    Symbol.species,
  );
  return speciesObservation.available && speciesObservation.observed;
}

function observeNativePromiseWithOverride(value, fulfill, reject, target, key) {
  const override = installTemporaryOwnValue(target, key, INTRINSIC_PROMISE);
  if (!override) return { available: false, observed: false };
  try {
    try {
      const child = Reflect.apply(INTRINSIC_PROMISE_THEN, value, [fulfill, reject]);
      Reflect.apply(INTRINSIC_PROMISE_THEN, child, [() => {}, reject]);
      return { available: true, observed: true };
    } catch {
      return { available: true, observed: false };
    }
  } finally {
    override.restore();
  }
...
function installTemporaryOwnValue(target, key, value) {
  ...
  } else if ("value" in original && original.writable) {
    temporaryDescriptor = { ...original, value };
  } else {
    return null;
  }
  if (!Reflect.defineProperty(target, key, temporaryDescriptor)) return null;
  return {
    restore() {
      const restored = original
        ? Reflect.defineProperty(target, key, original)
        : Reflect.deleteProperty(target, key);
      if (!restored) throw new TypeError("native promise observation override restoration failed");
    },
  };
}

function findDataPropertyValue(value, key) {
  let current = value;
  while (current !== null) {
    if (nodeTypes.isProxy(current)) return { found: false };
