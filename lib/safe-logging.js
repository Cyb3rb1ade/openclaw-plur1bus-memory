...
const TOKEN_PATTERNS = [
  /\bBearer\s+(?:"[^"\r\n]*"|'[^'\r\n]*'|[^\s,;]+)/gi,
  /\b(?:api[_-]?key|access[_-]?token|refresh[_-]?token|bot[_-]?token|token|password|client[_-]?secret|secret|authorization)["']?\s*[:=]\s*(?:"[^"\r\n]*"|'[^'\r\n]*'|[^\s,;]+)/gi,
  /\bsk-[A-Za-z0-9_+=./-]{8,}/gi,
...
function observeThenableValue(value, then, seen, depth, finish) {
  if (depth >= MAX_THENABLE_CHAIN_DEPTH || seen.has(value)) {
