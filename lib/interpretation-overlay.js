 ...
 *       patternId: "pat-789",
 *       llmModel: "<resolved-model>" // optional
 *     }
 *   }
 *
...
export class InterpretationOverlayStore {
   ...
   */
  append(overlay, cooldownDays = 7, opts = {}) {
    throwIfAborted(opts?.signal, "overlay persistence aborted");
    ...
    this._writeQueue = this._writeQueue
      .catch((error) => {
        safeWarn(console, "InterpretationOverlayStore.append:previousWrite", error, {
          targetMemoryId: overlay.targetMemoryId,
        });
      })
      .then(() => this._doAppend(overlay, cooldownDays, opts?.signal));
   ...
   * @returns {Promise<boolean>}
   */
  async _doAppend(overlay, cooldownDays, signal = null) {
    throwIfAborted(signal, "overlay persistence aborted");
