...
export class InterpretationOverlayStore {
   ...
   *
   * Returns only the latest, non-superseded overlay per target memory so that
   * stale interpretations cannot be presented as live evidence. This preserves
   * the core architectural rule: factual records are append-only, but meaning
   * revisions are versioned and reversible.
   *
   * @param {string[]} memoryIds
   * @param {number} [maxAgeDays=30]
   * @returns {Promise<Array<object>>}
   */
  async loadForTargets(memoryIds, maxAgeDays = 30) {
