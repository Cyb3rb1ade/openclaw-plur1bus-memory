...
async function applyFeedbackDynamics(entry, options) {
  try {
    await withDbLease(options.dbPool, options.agentId, async (db) => {
...
export function recordFeedbackBatch(workspaceDir, feedbackEntries, options = {}) {
  ...
  if (dynamicEntries.length > 0) {
    ...
    // 7.12.30: Ein Aufrufer im Prompt-Hook uebergibt die DB-Arbeit einer
    // Warteschlange statt darauf zu warten (bis zu zwoelf LanceDB-Updates je
    // Turn, ~1,2 s je Update auf fragmentierten Tabellen).
    if (typeof options.dynamicsScheduler === "function") {
      options.dynamicsScheduler(run, { entries: dynamicEntries.length });
      return undefined;
    }
    return run();
  }
