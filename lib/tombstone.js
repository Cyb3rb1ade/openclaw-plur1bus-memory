...
import { closeSync, existsSync, fsyncSync, mkdirSync, openSync, readFileSync, appendFileSync, statSync, truncateSync, writeSync } from "node:fs";
...
/**
 * Geparste Registry je Datei. Validiert über `mtimeMs` + Größe — beide
 * Schreibpfade (Append, Torn-Tail-Kürzung) ändern beides, eine zusätzliche
 * Invalidierung ist deshalb nicht nötig.
 *
 * Grund: `findBlockingTombstoneForCapture` läuft synchron auf dem
 * memory_store-Hotpath und las zuvor bei JEDER Erfassung die vollständige,
 * append-only wachsende Datei neu ein und parste sie zeilenweise.
 */
const registryCache = makeBoundedCache(50);
...
export function readTombstoneRegistry(baseDbPath, agentId, opts = {}) {
  ...
  let classified;
  try {
    classified = classifyRegistryCached(file, agentId);
  } catch (err) {
  ...
  if (opts.repairTornTail === false) {
    return {
      ok: true,
      tombstones: classified.tombstones.slice(),
  ...
  let revalidated;
  try {
    revalidated = classifyRegistryCached(file, agentId);
  } catch (err) {
    return { ok: false, tombstones: [], corruptLines: 0, readError: err?.message || String(err) };
  }
...
function toRegistryResult(classified) {
  return {
    ok: true,
    // Kopie: der Aufrufer darf den Cache-Inhalt nicht mutieren können.
    tombstones: classified.tombstones.slice(),
...
/**
 * Liest und klassifiziert die Registry, solange sich `mtimeMs` und Größe seit
 * dem letzten Mal nicht geändert haben, aus dem Cache.
 *
 * Nicht fail-open: kann die Datei nicht gestattet oder gelesen werden, wirft
 * diese Funktion und der Aufrufer meldet einen Lesefehler (= blockiert).
 *
 * @throws {Error} bei stat-/Lesefehler.
 */
function classifyRegistryCached(file, agentId) {
  const stat = statSync(file);
  const cached = registryCache.get(file);
  if (cached && cached.mtimeMs === stat.mtimeMs && cached.size === stat.size && cached.agentId === agentId) {
    return cached.classified;
  }
  const classified = classifyRegistryText(readFileSync(file, "utf8"), agentId);
  registryCache.set(file, { mtimeMs: stat.mtimeMs, size: stat.size, agentId, classified });
  return classified;
}
