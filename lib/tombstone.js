...
export function readTombstoneRegistry(baseDbPath, agentId) {
  ...
  for (const line of text.split("\n")) {
    ...
    try {
      ...
      if (isValidTombstone(parsed)) tombstones.push(parsed);
      else corruptLines += 1;
    } catch {
      corruptLines += 1;
    }
...
/**
 * Vollständige Schema-/Enum-/UUID-Validierung einer Tombstone-Zeile. Eine
 * Zeile mit nur `schemaVersion` (ohne memoryId/agentId/scope/status) gilt als
 * beschädigt und wird nicht akzeptiert.
 *
 * @param {object} parsed
 * @returns {boolean}
 */
export function isValidTombstone(parsed) {
