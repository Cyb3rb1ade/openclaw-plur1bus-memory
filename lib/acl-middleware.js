...
function canonicalStoredWorkspace(value, workspaceAliases) {
  if (!value) return "";
  if (value.startsWith("workspace:v1:") || value.startsWith("workspace-dir:v1:")) {
    if (!/^workspace(?:-dir)?:v1:[^\0\r\n]+$/.test(value) || value.length > INPUT_LIMITS.SESSION_KEY) {
      throw new Error("invalid canonical workspace binding");
    }
    return value;
  }
...
export function resolveOwnershipBindings(memory, workspaceAliases) {
  ...
  const rawWorkspaceId = validatedIdentity(memory.workspaceId, INPUT_LIMITS.SESSION_KEY, "memory.workspaceId");
  const rawWorkspaceKey = validatedIdentity(memory.workspaceKey, INPUT_LIMITS.SESSION_KEY, "memory.workspaceKey");
  let workspaceId = "";
  let workspaceKey = "";
  if (rawWorkspaceId || rawWorkspaceKey) {
...
export function checkAccess(ctx, memory) {
  ...
  if (scope === "agent-private") {
    ...
    let requester = "";
    try {
      requester = safeAgentId(validatedIdentity(ctx.agentId, INPUT_LIMITS.AGENT_ID, "agentId", { required: true }));
    } catch {
      return { allowed: false, reason: "acl.agent_private.missing_owner" };
    }
    if (requester === memoryAgentId) {
      return { allowed: true };
    }
