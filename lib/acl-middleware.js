...
function resolveMemoryAgentId(memory) {
  return String(memory?.agentId || memory?.storedBy || "").trim();
}

function resolveMemoryWorkspaceId(memory) {
  return String(memory?.workspaceId || memory?.workspaceKey || "").trim();
}

function resolveMemoryOwnerUserId(memory) {
  return String(memory?.ownerUserId || "").trim();
}
...
export function checkAccess(ctx, memory) {
  ...
  const memoryAgentId = resolveMemoryAgentId(memory);
  const memoryWorkspaceId = resolveMemoryWorkspaceId(memory);
  ...
  if (scope === "agent-private") {
    ...
    if (ctx.agentId && ctx.agentId === memoryAgentId) {
      return { allowed: true };
    }
  ...
  if (scope === "workspace") {
    ...
    if (ctx.workspaceId && ctx.workspaceId === memoryWorkspaceId) {
      return { allowed: true };
    }
  ...
  if (scope === "user") {
    const requestUserId = String(ctx.userId || "").trim();
    if (!requestUserId) {
      return { allowed: false, reason: "acl.user.not_authenticated" };
    }
    const ownerUserId = resolveMemoryOwnerUserId(memory);
    if (!ownerUserId) {
