...
export class EmotionScore {
  ...
  _validate() {
    // Reject non-finite numbers first: NaN/Infinity slip past range checks
    // (NaN < -1 and NaN > 1 are both false), which previously let NaN emotional
    // signal propagate silently.
    for (const field of ["valence", "arousal", "dominance", "intensity", "confidence"]) {
      if (!Number.isFinite(this[field])) {
        throw new Error(`${field} must be a finite number, got ${this[field]}`);
      }
    }
