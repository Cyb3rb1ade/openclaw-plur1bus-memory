...
import { findBestPattern } from "./pattern-surface.js";
 ...
 */
export async function hydrateGraphResults(dbTable, results, logger) {
...
export async function runRecallPipeline({
  ...
  if (results.length > 0) {
    ...
    boosted = results.map((r) => {
      ...
      // 4a. Importance-Boost
      if (applyImportance) {
        score *= 1 + (r.entry.importance ?? 0.5) * importanceBoost;
      }

      // 4b. Emotional Boost (stimmungsabhängiger Recall)
      if (emotionalState) {
        ...
        score *= emotionalState.computeRecallBoost(valence, r.entry.importance);
      }

      // 4c. Memory Strength Boost — softened factor (0.65 + 0.35 * strength)
      score *= 0.65 + 0.35 * (r.entry.memoryStrength ?? 1.0);
  ...
  // 5.5 Pattern Surfacing — collect top-15 candidates before dedup/slice
  let matchedPattern = null;
  if (recallConfig.patternSurfacing?.enabled && neoStore) {
    try {
      const patternCandidates = ordered.slice(0, 15).map(r => r.entry?.id ?? r.id).filter(Boolean);
      const allPatterns = neoStore.readPatterns(500);
      if (Array.isArray(allPatterns) && allPatterns.length > 0 && patternCandidates.length > 0) {
        const gate = new ContinuityGate();
        const sessionState = recallConfig.patternSurfacing?.sessionState ?? {
          associativeSurfacedCount: 0,
          patternSurfacedCount: 0,
          surfacedIds: new Set(),
        };
        const bestMatch = await findBestPattern(patternCandidates, allPatterns, gate, sessionState);
        matchedPattern = bestMatch ?? null;
      }
    } catch (e) {
      safeWarn(logger, "recall-pipeline pattern surfacing", e);
    }
  }
  ...
  return { queryVector: vector, canonical: canonicalHits, memories: ordered, matchedPattern };
}
