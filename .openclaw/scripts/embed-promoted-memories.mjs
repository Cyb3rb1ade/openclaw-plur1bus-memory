...
async function main() {
  ...
  // Pad a record with type-safe defaults for any schema fields not present.
  // Prevents "Append with different schema" errors when PLUR1BUS has added new
  // fields to the LanceDB table that this script doesn't know about.
  // Non-nullable fields get zero-value defaults; nullable fields get null.
  async function padToSchema(table, record) {
    try {
      const schema = await table.schema();
      const padded = { ...record };
      for (const field of schema.fields) {
        if (field.name in padded) continue;
        const t = field.type.toString();
        if (t === "Bool") padded[field.name] = false;  // Arrow rejects null Bool in single-row appends
        else if (field.nullable) padded[field.name] = null;
        else if (t.startsWith("Int") || t.startsWith("Float") || t.startsWith("Double") || t === "Float64" || t === "Int64") padded[field.name] = 0;
        else padded[field.name] = "";
      }
      return padded;
    } catch {
      return record;
  ...
  for (const agent of agents) {
    // ROOT ist kanonisch: OpenClaw-Core liest/seedet/dreamt in {workspace}/MEMORY.md.
    // sys/ ist nur noch Backup-Spiegel, memory/ ist Legacy. (Stand 2026-05-29: Root-canonical
    // wiederhergestellt, nachdem die 05-28 sys/-Migration die Root↔Core-Diskrepanz erzeugt hatte.)
    const memoryMdRoot   = join(agent.workspace, "MEMORY.md");
    const memoryMdLegacy = join(agent.workspace, "memory", "MEMORY.md");
    const memoryMdSys    = join(agent.workspace, "sys", "MEMORY.md");
    const memoryMd = existsSync(memoryMdRoot) ? memoryMdRoot
                   : existsSync(memoryMdLegacy) ? memoryMdLegacy
                   : memoryMdSys;
    if (!existsSync(memoryMd)) { console.log(`[${agent.id}] No MEMORY.md at ${memoryMdSys}, ${memoryMdRoot} or ${memoryMdLegacy}, skipping`); continue; }
    ...
    for (const p of toEmbed) {
      try {
        ...
        const record = await padToSchema(table, {
          id: randomUUID(),
          text: p.text,
