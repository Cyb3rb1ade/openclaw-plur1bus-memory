...
This release commit aligns Hermes with PLUR1BUS 7.3.1: the release process
will create the immutable `7.3.1-hermes` tag. Once that tag exists, install
both Python packages from it with the Hermes runtime's Python:

```bash
export HERMES_PYTHON="${HERMES_PYTHON:-python3}"
"$HERMES_PYTHON" -m pip install "git+https://github.com/Cyb3rb1ade/openclaw-plur1bus-memory.git@7.3.1-hermes#subdirectory=plur1bus-hermes"
"$HERMES_PYTHON" -m pip install "git+https://github.com/Cyb3rb1ade/openclaw-plur1bus-memory.git@7.3.1-hermes#subdirectory=plur1bus-controls"
```

The Hermes npm/OpenClaw package is a separate channel. The release process
will publish `7.3.1-hermes` to GitHub Packages with the `hermes` dist-tag,
...
```bash
openclaw plugins install @cyb3rb1ade/plur1bus-memory@7.3.1-hermes --pin
...
```bash
git clone --branch 7.3.1-hermes --depth 1 \
