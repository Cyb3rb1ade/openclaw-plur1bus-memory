...
- **Gateway startup (deferred bootstrap)** — a `gateway_start` handler in `index.js` schedules a one-off, non-blocking run 90 seconds after every healthy gateway start (long enough for the `openclaw` CLI to be able to talk to the now-running gateway). If host-patch registration fails, the first safety attempt is scheduled immediately, even when automatic provisioning is disabled. CLI/config/list/edit failures are retried in the same gateway process with bounded backoff (1s, 5s, 30s, 2m, 10m). This channel covers the *documented* install path (`git clone` + `rsync` into `~/.openclaw/extensions/...`, which never runs `npm install`) as well as ClawHub installs. Healthy runs are throttled to at most once per ~20h; pending work and plugin version changes retry earlier. Disable normal provisioning with `"featureCronSetup": { "auto": false }`; the fail-closed safety run remains active.
