...
- `skill-miner`: `skillMiner.enabled`; raw Croner-compatible cron/timezone after conservative syntax validation, defaulting to Sunday 03:00 in `Europe/Berlin` (`timezone: null` means local time). Invalid literals, ranges, names, modifiers, or literal-step forms are ineligible.
