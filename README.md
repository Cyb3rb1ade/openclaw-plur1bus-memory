...
installieren. Ab `4.2.8` fuehrt PLUR1BUS solche Cron-Slash-Commands vor der
...
standardmaessig nur Low-Risk-Items und ruft danach den normalen
Apply-Revalidation-Pfad auf; mittlere/hohe Risiken und blockierte Items bleiben
offen.
...
`prepare` ist nicht `apply`. `quickapply` ist die explizite Kurzform fuer
`approve low-risk` plus `apply`; es schreibt nur nach derselben
...
Cron-, Background- und Heartbeat-Turns behalten AutoRecall, Embeddings,
Canonical-First Recall, Deduplication und Reranking. Der Runtime-Scheduler aus
`4.2.18` begrenzt und priorisiert diese Arbeit, schaltet sie aber nicht ab.
