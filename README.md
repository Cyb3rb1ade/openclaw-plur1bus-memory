...
- **No vault found — bridge stays enabled but inert** — If no `obsidianBridge.workspaceRoot` / `vaultPath` is configured and no vault is auto-detected, the bridge features remain enabled and activate automatically once a workspace path is configured. No path is invented and no directory is created without a configured or detected path. This is intentional: the bridge cannot bootstrap without knowing where the vault lives.
