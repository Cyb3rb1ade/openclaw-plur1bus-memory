...
describe("job-rate-limit", () => {
  ...
  it("blocks second run before interval expires", () => {
    const statePath = makeStatePath();
    const intervalMs = 100000;
    recordJobRun("daily-consolidation", "agent-1", "ws-1", statePath);
  ...
  it("isolates different job keys", () => {
    const statePath = makeStatePath();
    recordJobRun("daily-consolidation", "agent-1", "ws-1", statePath);
  ...
  it("read-only check does not modify state", () => {
    const statePath = makeStatePath();
    recordJobRun("daily-consolidation", "agent-1", "ws-1", statePath);
  ...
  it("increments runCount on repeated runs", () => {
    const statePath = makeStatePath();
    recordJobRun("daily-consolidation", "agent-1", "ws-1", statePath);
    recordJobRun("daily-consolidation", "agent-1", "ws-1", statePath);
