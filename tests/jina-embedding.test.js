...
function fakeTransformersRuntime(calls, { architecture = "XLMRobertaModel" } = {}) {
  class FakeTensor {
  ...
  return {
    ...
    AutoModel: {
      async from_pretrained(model, options) {
        ...
        loaded.sessions = { model: { inputNames: ["input_ids", "attention_mask", "task_id"] } };
...
describe("downloadable multilingual JinaAI embedding", () => {
  it("pins the real Jina v3 fp16 ONNX export and its non-commercial license", () => {
    assert.equal(JINA_EMBEDDING_PROFILE.model, "jinaai/jina-embeddings-v3");
    assert.equal(JINA_EMBEDDING_PROFILE.revision, "ab036b023d30b4d1138c4c3bfa9f0c445ab455d6");
    ...
    assert.deepStrictEqual(
      JINA_EMBEDDING_PROFILE.artifacts.find((entry) => entry.path === "onnx/model_fp16.onnx"),
      {
        path: "onnx/model_fp16.onnx",
        size: 1_147_151_362,
        sha256: "329c3ea03a1815cc98f6b97efcafb9000c6c780c2d89d40d4f541b9a88434c38",
      },
    );
  });
  ...
  it("loads the pinned model offline, selects query/passage adapters, truncates, then normalizes", async () => {
    ...
    assert.equal(modelLoad[2].dtype, "fp16");
